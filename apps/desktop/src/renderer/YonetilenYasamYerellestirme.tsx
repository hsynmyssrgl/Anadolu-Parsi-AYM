...
const exactCopy: Readonly<Record<string, string>> = {
  'Arşiv belge bağlantısı:':'Archive document link:','Arşiv belge bağlantısı':'Archive document link','Arşiv belge bağlantısı kaydedildi; belge içeriği okunmadı.':'The archive document link was saved; the document content was not read.','Bağlı finans kaydı (isteğe bağlı)':'Linked finance record (optional)','Arşivden seçilen belge':'Document selected from the archive','Yüksek hassasiyetli arşiv belge bağlantısı':'Highly sensitive archive document link','Arşiv belge bağlantısı ·':'Archive document link ·',
  'Akıllı sayaç, hizmet sağlayıcı veya garanti sicili sorgulanmaz; metin tanıma, servis rezervasyonu, ödeme ve ağ erişimi yapılmaz.':'Smart-meter, service-provider, and warranty registries are not queried; text recognition, service booking, payment, and network access are not performed.','Belge içeriği okunmaz. Dosya yolu, ham belge, kart güvenlik bilgileri, parola, erişim anahtarı veya gizli anahtar kabul edilmez.':'Document content is not read. File paths, raw documents, card security details, passwords, access keys, and private keys are not accepted.','Yalnız arşivdeki belgeyle bağlantı kurulur; dosya seçilmez ve dosya ayrıntıları bu forma taşınmaz.':'Only a link to the archived document is created; no file is selected and file details are not brought into this form.','Yalnız arşivdeki belgeyle bağlantı kurulur; dosya seçilmez ve içerik bu forma taşınmaz.':'Only a link to the archived document is created; no file is selected and content is not brought into this form.','Dosya yolu ekrana verilmez. Seçilen arşiv belgeleri ayrı izinle, dosya başına en çok 10 MiB ve toplam 25 MiB olacak biçimde geçici olarak okunur; düz metin dosyası oluşturulmaz.':'File paths are not shown on screen. Selected archive documents are read temporarily with separate authorization, up to 10 MiB per file and 25 MiB in total; no plain-text file is created.','Ağ, bulut, mesaj veya acil servis teslimi yapılmaz. Güç kaynağı uygulamanın korumalı bölümünde izlenir; pil yüzdesi ölçülmez ve kendiliğinden düşük pil uyarısı üretilmez.':'No network, cloud, messaging, or emergency-service delivery occurs. The power source is observed in the protected part of the app; battery percentage is not measured and no automatic low-battery warning is created.',
  ...
  'Acil durum merkezi':'Emergency center','Acil durum profili':'Emergency profile','Acil irtibat':'Emergency contact',
  'açık':'enabled','açıldı':'enabled','kapalı':'disabled','kapatıldı':'disabled',
  ...
  'Çevrimdışı acil durum kaydı':'Offline emergency record','Çevrimdışı acil kart çıktısı':'Offline emergency-card export','Bütünleşik aile yaşamı':'Integrated family life','Yerel ve çevrimdışı güvenlik':'Local and offline safety',
  ...
  'Evcil hayvan bakımı':'Pet care','Evcil hayvan kaydı':'Pet record','Fatura':'Invoice','Garaj':'Garage','Garanti':'Warranty','Garanti belgesi':'Warranty document',
  ...
  'Güç kipi kaydedilemedi.':'The power mode could not be saved.','güç kaynağı':'power source','Güvenli çıktı hazırlanıyor…':'Preparing secure output…',
  ...
  'Para birimi':'Currency','PDF dosyasının':'PDF file','Pil':'Battery','Pil-duyarlı görünüm':'Battery-aware view','pil seviyesi ölçülmedi':'battery level not measured','Plaka (isteğe bağlı)':'License plate (optional)',
  ...
  'Sigorta türü':'Insurance type','Son aile üyesi durumları':'Latest family-member statuses','Son kip:':'Last mode:','Şebeke gücü':'AC power',
  ...
  'Garanti sicili:':'Warranty registry:','· OCR:':'· OCR:','· Metin tanıma:':'· Text recognition:','· Ödeme:':'· Payment:',
