...
const exactCopy: Readonly<Record<string,string>> = {
  'Masaüstü güvenlik kapanışı':'Desktop security closure','Oturum kilidi ve uygulama sertleştirmesi':'Session lock and application hardening',
  'Güvenli retli masaüstü güvenlik sözleşmesi etkin':'Fail-closed desktop security contract active',
  'Görüntüleyici':'Renderer','korumalı alan ve bağlam ayrımı etkin':'sandbox and context isolation enabled',
  'Uygulama güvenlik ayarları zorunlu · komut satırı düğüm seçenekleri kapalı · paket bütünlüğü ve yalnız paket içinden yükleme açık':'Application security settings are mandatory · command-line Node options are disabled · package integrity and package-only loading are enabled',
  'Ürün yüzeyi gerçeklik kapısı':'Product-surface reality gate','Belge, rota, menü, ekran ve uygulama arayüzü envanteri tek sözleşmede':'Document, route, menu, screen, and application-interface inventory in one contract',
  'sınıflandırılmış kullanılmayan görüntüleyici arayüzü':'classified unused renderer interfaces','Çözümlenmemiş arayüz':'Unresolved interfaces','eksik zincir derleme kapanışını güvenli biçimde durdurur':'an incomplete chain safely stops build closure',
  'Ağ çıkış güvenliği':'Network-egress security','İzinli bağlantılar ve sertifika yenileme':'Allowed connections and certificate rotation','Güvenli retli ağ çıkış politikası etkin':'Fail-closed network-egress policy active',
  'Yalnız kayıtlı iptal listesi, kimlik belirteci ve doğrulama anahtarı uç noktaları':'Only registered revocation-list, identity-token, and verification-key endpoints','Şifreli bağlantı sürümü doğrulanamadı':'Encrypted connection version not verified','çift sertifika sabitleme yenilemesi':'dual certificate-pin rotation','Karşılıklı kimlik doğrulama':'Mutual authentication','yetkili bağdaştırıcı':'authorized adapters',
  'Türetilmiş veri güvenliği':'Derived-data security','Güvenli retli politika mirası etkin':'Fail-closed policy inheritance active',
  'Hassas kayıt güvenliği':'Sensitive-record security','İçeriksiz kayıt ve tanı sınırı':'Content-free logging and diagnostics boundary','Güvenli retli hassas kayıt politikası etkin':'Fail-closed sensitive-record policy active','Metin tanıma sonucu ve veri içeriği':'OCR text and payload','keyfi mesaj ve hata yığını':'arbitrary messages and error stacks','masaüstü kayıt hedefi':'desktop logging sink','İzinli üstveri: kimlik · özet · sonuç · ilişkilendirme · sayaç · zaman · sürüm':'Allowed metadata: identity · hash · result · correlation · counter · time · version',
  'Değişmez karar denetimi':'Immutable decision audit','Politika kararlarının denetim zinciri':'Policy-decision audit chain','Karar denetim zinciri doğrulanamadı':'Decision-audit chain could not be verified','Politika sürümü ve yükümlülükler':'Policy version and obligations','birebir bağlı':'bound exactly','yalnız eklemeli HMAC-SHA-256':'append-only HMAC-SHA-256','yeni denetim kaydı':'new audit entries','tarihsel makbuz · istemciye veri içeriği verilmez':'historical receipts · no payload is exposed to the client',
  'Silme ve saklama yayılımı':'Deletion and retention propagation','Kaynak, türev, önbellek, kopya ve yedek yaşam döngüsü':'Lifecycle of source, derived data, cache, copies, and backups','Güvenli retli silme yayılımı doğrulandı':'Fail-closed deletion propagation verified','Metin tanıma · indeks · küçük resim · yapay zekâ hafızası üretim sahibi':'OCR · index · thumbnail · AI-memory production owner','açık metin kopya':'plaintext copy','Çalışma anı önbelleği silme öncesi temizlenir · yönetilen yedek doğrulanmış yeniden yazım ve karantina tamamlanana kadar silme işaretini korur':'Runtime cache is cleared before deletion · the deletion marker remains until verified rewrite and quarantine of managed backups complete','Yönetilmeyen ve harici kopyalar ayrı dikkat ve kanıt ister · karantina fiziksel imha sayılmaz · istemciye veri içeriği verilmez':'Unmanaged and external copies require separate attention and evidence · quarantine is not physical destruction · no payload is exposed to the client',
  'Ortak politika uyumu':'Common policy conformance','Windows, Apple profilleri ve hizmetler için tek doğrulama matrisi':'One verification matrix for Windows, Apple profiles, and services','Ortak politika sözleşmesi derleme aşamasında doğrulandı':'Common policy contract verified during the build','Politika uyumu doğrulanamadı':'Policy conformance could not be verified','Etkin çalışma hedefi':'Active runtime targets','yalnız profilli ve yayımlanmamış hedef':'profile-only and undeployed targets','Yerel Apple çalıştırması tamamlandı iddiası yoktur; ilgili uygulama yayımlanmadan önce yerel doğrulama zorunludur · istemciye test içeriği verilmez':'No claim is made that native Apple execution is complete; native verification is required before release · no test payload is exposed to the client',
  'Kaynak kod güvenlik kapısı':'Source-code security gate','Ayrıcalıklı kod yüzeylerinde varsayılan ret denetimi':'Default-deny ratchet for privileged code surfaces','TypeScript kaynak ağacı kapısı derleme aşamasında doğrulandı':'TypeScript AST gate verified during the build','Kaynak ağacı kapısı doğrulanamadı':'AST-gate status could not be verified','birebir izinli yüzey':'exact allowlisted surfaces','Yeni veya eski izin: güvenli ret · genel eşleme:':'New or legacy permission: fail-closed · wildcard:','Takma adlar, dinamik içe aktarma, modül yükleme ve hesaplanmış özellikler kaynak ağacında incelenir; bu kapı çalışma anı politikasının yerine geçmez · istemciye kaynak yolu veya izinli liste özeti verilmez':'Aliases, dynamic imports, require calls, and computed properties are inspected in the AST; this gate does not replace runtime policy · source paths and allowlist hashes are not exposed to the client',
  'İzin bildirimi güvenlik kapısı':'Capability-manifest security gate','Kamera, mikrofon, dosya, metin tanıma, yapay zekâ, konum ve ağ için çift katmanlı ret':'Dual-layer denial for camera, microphone, files, OCR, AI, location, and network','Derleme ve çalışma anı izin kapısı doğrulandı':'Build and runtime capability gate verified','İzin bildirimi doğrulanamadı':'Capability manifest could not be verified','birebir kaynak ağacı yüzeyi':'exact AST surfaces','Eksik veya beklenmeyen izin: güvenli ret · imzalı bildirim özeti ve kimliği doğrulanmış çekirdek hizmet çalışma anı kontrolü zorunlu':'Missing or unexpected capability: fail-closed · signed-manifest hash binding and authenticated Core Service runtime verification are required','Derleme bildirimi tek başına çalışma anı yetkisi vermez · istemciye kaynak yolu veya bildirim özeti verilmez · mevcut masaüstü kasası sahipliği korunur':'The build manifest alone grants no runtime authority · source paths and manifest hashes are not exposed to the client · existing Desktop vault ownership is preserved',
  'Uygulama güvenlik profili derleme aşamasında doğrulandı':'Application security profile verified during the build','yeni veya eksik profil derleme aşamasında reddedilir':'new or missing profiles are rejected during the build','Eşleme uygunluk sertifikası veya çalışma anı yetkisi değildir · yalnız profilli hedefler yerel olarak doğrulanmış sayılmaz · istemciye kaynak yolu ya da tehdit modeli özeti verilmez':'Mapping is not a compliance certificate or runtime authority · profile-only targets are not considered natively verified · source paths and threat-model hashes are not exposed to the client',
