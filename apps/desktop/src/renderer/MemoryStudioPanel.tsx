...
export function MemoryStudioPanel(){
  ...
  return <Surface className="memory-studio" aria-labelledby="memory-studio-title">
    <div className="panel-heading"><div><span className="eyebrow">{text('33‑X · yerel anı ve zaman kapsülü','33-X · local memories and time capsules')}</span><h2 id="memory-studio-title">{text('Hafıza stüdyosu','Memory studio')}</h2><p>{text('Korunan arşiv kayıtlarını kopyalamadan aile anlatıları, albüm taslakları ve iki onaylı zaman kapsülleri düzenleyin.','Organize family stories, album drafts and two-approval time capsules without copying protected archive records.')}</p></div><Button disabled={!!busy} onClick={()=>void refresh()}>{text('Yenile','Refresh')}</Button></div>
