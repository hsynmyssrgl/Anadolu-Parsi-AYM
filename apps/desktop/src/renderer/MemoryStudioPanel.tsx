...
import { MEMORY_STUDIO_RECORD_KINDS, type MemoryStudioCenterView, type MemoryStudioRecordKind, type MemoryTimeCapsuleView } from '@ppt/domain';
...
const capsuleStatus:Readonly<Record<MemoryTimeCapsuleView['status'],string>>=Object.freeze({
...
export function MemoryStudioPanel(){
  ...
  const run=async(key:string,task:()=>Promise<unknown>)=>{setBusy(key);setError('');try{await task();pending.current.delete(key);await reload();}
    catch(value){setError(value instanceof Error?value.message:'İşlem kaydedilemedi; aynı işlem kimliğiyle yeniden deneyebilirsiniz.');}finally{setBusy('');}};
  const createRecord=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();if(!window.pardus)return;
    ...
    await run(key,()=>window.pardus!.createMemoryStudioRecord({clientOperationId:op.clientOperationId,recordId:op.resourceId,kind,title:payload.title,
    ...
    setTitle('');setSummary('');setArchiveIds('');setPersonIds('');setOcrJobId('');setFaceApproved(false);
  };
  const createCapsule=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();if(!window.pardus)return;const parsed=new Date(unlockAt);
    ...
    await run(key,()=>window.pardus!.createMemoryTimeCapsule({clientOperationId:op.clientOperationId,capsuleId:op.resourceId,...payload}));
    setCapsuleTitle('');setCapsuleArchiveIds('');setSelectedRecords([]);setUnlockAt(futureDefault());
  };
  ...
  const reviewCapsule=(capsule:MemoryTimeCapsuleView,decision:'approve'|'revoke_approval')=>{if(!window.pardus)return;const key=`${decision}:${capsule.id}`;
  ...
  const transitionCapsule=(capsule:MemoryTimeCapsuleView,transition:'seal'|'release'|'cancel'|'rollback')=>{if(!window.pardus)return;const key=`${transition}:${capsule.id}`;
  ...
  return <Surface className="memory-studio" aria-labelledby="memory-studio-title">
    <div className="panel-heading"><div><span className="eyebrow">33‑X · yerel anı ve zaman kapsülü</span><h2 id="memory-studio-title">Hafıza stüdyosu</h2><p>Korunan arşiv kayıtlarını kopyalamadan aile anlatıları, albüm taslakları ve iki onaylı zaman kapsülleri düzenleyin.</p></div><Button disabled={!!busy} onClick={()=>void reload()}>Yenile</Button></div>
    ...
    <div className="memory-studio-grid"><form className="memory-studio-form" onSubmit={event=>void createRecord(event)}><span className="eyebrow">Yeni içeriksiz bağlantı</span><h3>Anı kaydı oluştur</h3>
      ...
      <Button tone="primary" type="submit" disabled={!!busy}>{busy==='create-record'?'Kaydediliyor…':'Anı kaydını ekle'}</Button></form>
      ...
      <Button tone="primary" type="submit" disabled={!!busy||(selectedRecords.length===0&&splitIds(capsuleArchiveIds).length===0)}>{busy==='create-capsule'?'Kaydediliyor…':'Kapsülü oluştur'}</Button></form></div>
    ...
    <section className="memory-studio-list" aria-labelledby="memory-capsules-title"><div className="panel-heading"><div><span className="eyebrow">{center?.capsules.length??0} kapsül</span><h3 id="memory-capsules-title">Zaman kapsülleri</h3></div></div>
       {!center?<p>Yerel merkez yükleniyor…</p>:center.capsules.length===0?<EmptyState title="Zaman kapsülü yok" body="En az bir korunan kaynakla, yedi günden ileri bir açılma tarihi belirleyin."/>:center.capsules.map(capsule=><article className="memory-studio-row memory-capsule-row" key={capsule.id}><div><strong>{capsule.title}</strong><span>{capsuleStatus[capsule.status]} · {capsule.approvals.length}/{capsule.minimumApprovals} onay · {new Date(capsule.unlockAt).toLocaleString('tr-TR')}</span></div><div className="button-row">{capsule.status==='awaiting_approvals'&&<><Button disabled={!!busy} onClick={()=>reviewCapsule(capsule,'approve')}>Onayla</Button><Button disabled={!!busy||capsule.approvals.length<capsule.minimumApprovals} onClick={()=>transitionCapsule(capsule,'seal')}>Mühürle</Button><Button tone="danger" disabled={!!busy} onClick={()=>transitionCapsule(capsule,'cancel')}>İptal et</Button></>}{capsule.status==='sealed'&&<><Button tone="primary" disabled={!!busy||Date.now()<Date.parse(capsule.unlockAt)} onClick={()=>transitionCapsule(capsule,'release')}>Yerel olarak aç</Button><Button tone="danger" disabled={!!busy} onClick={()=>transitionCapsule(capsule,'cancel')}>İptal et</Button></>}{capsule.status==='released'&&<Button tone="danger" disabled={!!busy||!capsule.releasedAt||Date.now()>Date.parse(capsule.releasedAt)+86_400_000} onClick={()=>transitionCapsule(capsule,'rollback')}>Açılışı geri al</Button>}</div></article>)}</section>
  </Surface>;
}
