...
const exactCopy: Readonly<Record<string,string>> = {
  ...
  'Bu sürümde ne değişti?':'What changed in this version?','Büyük arşivler indeksli filtreler ve anahtar tabanlı sayfalama ile açılır; yalnız yüklenen belgeler renderer belleğinde tutulur.':'Large archives open with indexed filters and keyset pagination; only loaded documents are retained in renderer memory.',
  ...
  'Dosya yolu yalnız ana süreç seçicisinde kalır. Belirsiz sonuçta aynı dosyayla yeniden deneyin; onay alınmamış işlem için yeniden başlatma kurtarma garantisi verilmez.':'The file path remains only in the main-process picker. If the result is uncertain, retry with the same file; restart recovery is not guaranteed for an unconfirmed operation.',
  ...
  'Yükleniyor…':'Loading…','Yüksek':'High',
