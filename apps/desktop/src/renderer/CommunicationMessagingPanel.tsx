...
export function CommunicationMessagingPanel(){
  ...
  const messageStateLabels:Readonly<Record<CommunicationMessageState,string>>={
    draft:text('Taslak','Draft'),queued:text('Sırada','Queued'),scheduled:text('Zamanlandı','Scheduled'),
    sealed_local:text('Yerel kasada mühürlü','Sealed in local vault'),deleted:text('Yerel olarak silindi','Deleted locally')
  };
  const deliveryStateLabels:Readonly<Record<CommunicationMessageDeliveryState,string>>={
    not_requested:text('İstenmedi','Not requested'),queued_offline:text('Çevrimdışı sırada','Queued offline'),
    retry_wait:text('Yeniden deneme bekliyor','Waiting to retry'),ready_local:text('Yerel olarak hazır','Ready locally'),
    transport_not_configured:text('Aktarım yapılandırılmadı','Transport not configured'),cancelled:text('İptal edildi','Canceled')
  };
  ...
  }catch(caught){setError(toUserFacingErrorMessage(caught,text('Mesajlaşma merkezi yüklenemedi.','Messaging center could not be loaded.')));}};
  ...
  }catch(caught){setError(`${toUserFacingErrorMessage(caught,text('Mesajlaşma değişikliği tamamlanamadı.','The messaging change could not be completed.'))} ${text('Aynı işlem kimliğiyle yeniden deneyebilirsiniz.','You can retry with the same operation identifier.')}`);}finally{setBusy('');}};
  ...
  const selectAttachment=async()=>{if(!window.pardus||!selectedRoomId)return;setBusy('attachment');setError('');try{
    ...
    if(candidates.length!==1)throw new Error(text('Hazırlanan yerel dosya kaydı güvenle doğrulanamadı.','The prepared local file record could not be verified safely.'));
  ...
  }catch(caught){setPreparedAttachment(undefined);setError(toUserFacingErrorMessage(caught,text('Dosya seçimi tamamlanamadı.','File selection could not be completed.')));}
  finally{setBusy('');}};
  ...
  }catch(caught){setError(toUserFacingErrorMessage(caught,text('Mesaj içeriği açılamadı.','Message content could not be opened.')));}finally{setBusy('');}};
  ...
  }catch(caught){setError(toUserFacingErrorMessage(caught,text('Mesaj araması tamamlanamadı.','Message search could not be completed.')));}finally{setBusy('');}};
  return <section className="communication-messaging panel" aria-labelledby="communication-messaging-title">
    ...
    <div className="communication-messaging-truth" role="note"><strong>{text('Bu sürüm yalnız yerel ve ağsız çalışır.','This release works locally and without network access only.')}</strong>
      ...
      <span>{text('Uzak teslim, karşı taraftan alındı bilgisi, mesaj doğrulaması ve gerçek mesaj alışverişi henüz kullanıma hazır değildir.','Remote delivery, recipient confirmation, message verification, and real message exchange are not ready for use yet.')}</span>
      <span>{text('Dosya ekleri güvenli dosya seçicisinden alınır, temiz tarama kanıtıyla aynı odaya bağlanır ve korumalı yerel alanda kalır.','File attachments are chosen through the secure file picker, linked to the same room after a clean scan, and kept in protected local storage.')}</span>
      <span>{text('Zamanlanmış mesajlar bu bilgisayarda yürütülür. Süresi dolan kayıt listeden kaldırılır; fiziksel güvenli silme ve yedeklerden kaldırma garantisi yoktur, birden çok cihazdaki çevrim içi durum da henüz birleştirilmez.','Scheduled messages run on this computer. Expired records are removed from the list; physical secure erasure and removal from backups are not guaranteed, and online status from multiple devices is not combined yet.')}</span></div>
    {error&&<p className="status-message danger">{error}</p>}
    {!center?<p>{text('Mesajlaşma merkezi yükleniyor…','Loading messaging center…')}</p>:<>
      <div className="communication-messaging-summary"><span><strong>{center.messages.length}</strong> {text('yerel mesaj kaydı','local message records')}</span>
      ...
      <div className="communication-messaging-compose">
        ...
        {(contentKind==='text'||contentKind==='location')?<label>{contentKind==='location'?text('Konum açıklaması veya koordinat','Location description or coordinates'):text('Mesaj','Message')}
          ...
          <div className="communication-messaging-attachment"><button type="button" disabled={Boolean(busy)||!selectedRoomId}
            onClick={()=>void selectAttachment()}>{text('Dosya seç ve korumalı alana ekle','Choose a file and add it to protected storage')}</button>
            {preparedAttachment?<small>{preparedAttachment.displayName} · {text('temiz yerel kayıt','clean local record')}</small>:
        ...
        {relationMode!=='none'&&<div className="communication-messaging-relation" role="status">
          <span>{relationMode==='reply'?text('Yanıt','Reply'):relationMode==='quote'?text('Alıntı','Quote'):text('Konu dizisi','Thread')} {text('için seçilen mesaj','selected message')}</span>
          <button type="button" onClick={()=>{setRelationMode('none');setRelatedMessageId('');}}>{text('İlişkiyi kaldır','Remove relation')}</button></div>}
        <label className="communication-messaging-check"><input type="checkbox" checked={silent} onChange={event=>setSilent(event.target.checked)} />{text('Sessiz gönderim bilgisi','Silent delivery setting')}</label>
      ...
      <div className="communication-messaging-controls">
        ...
        <label>{text('Çevrim içi durumu','Online status')}<select value={presenceStatus} onChange={event=>setPresenceStatus(event.target.value as CommunicationPresenceStatus)}>
          {Object.entries(presenceLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label>
        <button type="button" disabled={Boolean(busy)} onClick={()=>void setPresence()}>{text('Çevrim içi durumunu kaydet','Save online status')}</button>
        <label>{text('Saklama modu','Retention mode')}<select value={retentionMode} onChange={event=>setRetentionMode(event.target.value as CommunicationMessageRetentionMode)}>
          <option value="permanent">{text('Kalıcı','Permanent')}</option><option value="duration">{text('Süreli','Duration')}</option><option value="auto_delete">{text('Süre dolunca listeden kaldır','Remove from the list after expiry')}</option>
      ...
      <div className="communication-messaging-list">{messages.length===0?<p>{text('Bu oda için yerel mesaj kaydı yok.','There are no local message records for this room.')}</p>:messages.map(message=>
        <article key={message.id} className={message.deleted?'is-deleted':''}>
          <header><strong>{contentKindLabels[message.contentKind]}</strong><small>{message.updatedAt} · {text('değişiklik no','change no.')} {message.revision}</small></header>
          <p>{text('Durum','State')}: {messageStateLabels[message.state]} · {text('teslim durumu','delivery status')}: {deliveryStateLabels[message.deliveryState]}{message.edited?text(' · düzenlendi',' · edited'):''}</p>
          {(message.replyToMessageId||message.quotedMessageId||message.threadRootMessageId)&&<p className="communication-messaging-relation-summary">
            {message.replyToMessageId&&text('Bir mesaja yanıt','Reply to a message')}{message.quotedMessageId&&text('Bir mesajdan alıntı','Quote from a message')}
            {message.threadRootMessageId&&text('Bir konu dizisine bağlı','Linked to a thread')}</p>}
          ...
          {revealed[message.id]?.opaqueAttachmentHandle&&<div className="communication-messaging-content" role="status">
            <span>{contentKindLabels[revealed[message.id]!.contentKind]} {text('eki, aynı oda içindeki temiz korumalı dosya kaydına bağlıdır.','attachment is bound to the clean protected file record in the same room.')}</span>
          ...
          <div className="communication-messaging-message-actions">
            ...
            <button type="button" disabled={Boolean(busy)||message.deleted} onClick={()=>void delivery(message)}>{message.deliveryState==='queued_offline'||message.deliveryState==='retry_wait'?text('Yerel yeniden dene','Local retry'):text('Çevrimdışı kuyruğa al','Queue offline')}</button>
