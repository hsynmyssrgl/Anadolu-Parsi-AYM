...
export function FamilyAiAssistantPanel(){
  ...
  return <Surface className="family-ai-assistant" aria-labelledby="family-ai-assistant-title">
    <div className="panel-heading"><div><span className="eyebrow">{text('33‑W · onaya bağlı yerel yardımcı','33-W · approval-bound local assistant')}</span><h2 id="family-ai-assistant-title">{text('Aile asistanı','Family assistant')}</h2><p>{text('Yalnız açık izinli ve politika süzgecinden geçen yerel kaynaklardan, otomatik işlem yapmayan inceleme önerileri üretir.','Creates non-automating review suggestions only from explicitly permitted local sources that pass the policy filter.')}</p></div><Button disabled={!!busy} onClick={()=>void refresh()}>{text('Yenile','Refresh')}</Button></div>
