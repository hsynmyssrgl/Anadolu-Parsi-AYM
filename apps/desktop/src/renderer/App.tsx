...
export function SystemManagementScreen({draftCenterVisible=false,onDraftCenterVisibilityChange=()=>undefined}:{readonly draftCenterVisible?:boolean;readonly onDraftCenterVisibilityChange?:(visible:boolean)=>void}={}){
  ...
  const refresh=async()=>{
    ...
    })().catch(error=>setOperationsLoadError(error instanceof Error?error.message:'Sistem verileri yüklenemedi.')).finally(()=>{if(operationsRefreshRef.current===task)operationsRefreshRef.current=null;setOperationsLoading(false);});
...
export function PrivacyOwnershipCenter() {
  ...
  const panel=<section className="privacy-ownership-center" aria-labelledby="privacy-ownership-title">
    ...
    <div className="privacy-ownership-grid">
      ...
      <section><h3>Olay ve yerel sınırlama</h3><Button disabled={Boolean(busy)} onClick={()=>void createIncident()}>Yerel inceleme olayı aç</Button>{center.incidents.slice(0,5).map(item=><div className="list-row privacy-action-row" key={item.id}><div><strong>{item.title} · {item.severity} · {item.status}</strong><small>Uzaktan silme, cihaz yönetimi veya ağ teslimi yapılmadı</small>{item.resolutionNote&&<small>{item.resolutionNote}</small>}</div><div className="button-row">{item.status==='open'&&<Button disabled={Boolean(busy)} onClick={()=>void updateIncident(item,'contained_locally')}>Yerel sınırlamaya al</Button>}{['open','contained_locally'].includes(item.status)&&<Button disabled={Boolean(busy)} onClick={()=>void updateIncident(item,'resolved')}>Yerel çözümü kaydet</Button>}{['open','contained_locally'].includes(item.status)&&<Button disabled={Boolean(busy)} onClick={()=>void updateIncident(item,'cancelled')}>Yerel olayı iptal et</Button>}</div></div>)}</section>
...
export function IdentityAccessCredentialCenter({trustedDevices}:{trustedDevices:readonly TrustedDeviceView[]}) {
  ...
  const panel=<section className="identity-access-center" aria-label="Kimlik, passkey ve geçici yetki merkezi" aria-describedby="identity-access-description">
    ...
    <div className="identity-access-grid">
      ...
      <section className="identity-access-card"><h3>Salt okunur eşlikçi cihaz kopyası</h3><p>Windows tek yazardır. Eşlikçi cihaz yalnız şifreli, süreli ve salt okunur kopya alabilir; uzak yazma, çatışma birleştirme veya ağ teslimi yoktur.</p>{companionEligibleDevices.length===0?<AsyncStatePanel state="empty" title="Eşlikçi cihaz anahtarı yapılandırılmadı" message="Şifreli kopya oluşturma, doğrulanmış X25519 alıcı anahtarı bir güvenilir cihaza bağlanana kadar kapalıdır."/>:<><label>Hedef güvenilir cihaz<select value={companionDeviceId} onChange={event=>setCompanionDeviceId(event.target.value)}>{companionEligibleDevices.map(item=><option value={item.id} key={item.id}>{item.displayName}{item.current?' · bu cihaz':''}</option>)}</select></label><label>Bilinen kaynak sürümü <small>(isteğe bağlı)</small><input type="number" min={0} step={1} value={knownSourceVersion} onChange={event=>setKnownSourceVersion(event.target.value)}/></label><div className="button-row"><Button tone="primary" disabled={Boolean(busy)||!companionDeviceId} onClick={()=>void createCompanion('read_only')}>Şifreli salt okunur kopya oluştur</Button><Button disabled={Boolean(busy)||!companionDeviceId} onClick={()=>void createCompanion('write')}>Yazma reddini doğrula</Button></div></>}{companionResult&&<StatusMessage tone={companionResult.status==='snapshot_ready'?'success':'warning'}>{companionResult.status==='snapshot_ready'?`Hazır · kaynak v${companionResult.sourceVersion} · ${companionResult.envelopeBytes} bayt · ağ teslimi yok`:`Reddedildi: ${companionResult.status} · güncel kaynak v${companionResult.currentSourceVersion} · uzak yazma kabul edilmez`}</StatusMessage>}{companionResult?.status==='snapshot_ready'&&<Button onClick={()=>void navigator.clipboard.writeText(companionResult.encryptedEnvelopeBase64Url)}>Şifreli zarfı kopyala</Button>}{center.companionSnapshots.length===0?<EmptyState title="Eşlikçi cihaz kopyası yok" body="Üretilen kayıt bilgisi yalnız yerel merkezde listelenir; teslim sonucu iddia edilmez."/>:center.companionSnapshots.slice(0,6).map(item=><div className="identity-record" key={item.id}><div><strong>Kaynak v{item.sourceVersion} · Şema v{item.schemaVersion}</strong><small>{formatDate(item.generatedAt,{dateStyle:'short',timeStyle:'short'})} · {item.envelopeBytes} bayt · salt okunur</small></div></div>)}</section>
...
function SettingsSecurity({auth,accessibility,onAccessibilityChange,onFamilyDataChanged}:{auth:AuthStateView;accessibility:AccessibilityPreferences;onAccessibilityChange:(next:AccessibilityPreferences)=>void;onFamilyDataChanged:()=>Promise<void>}) {
  ...
  const begin2fa=async()=>{try{if(window.pardus)setTwoFactor(await window.pardus.beginTwoFactorSetup());}catch(err){setMessage(err instanceof Error?err.message:'İki aşamalı doğrulama başlatılamadı.');}};
  const enable2fa=async()=>{try{if(!window.pardus)return;await window.pardus.enableTwoFactor({code:twoFactorCode});setTwoFactor(null);setTwoFactorCode('');setMessage('İki aşamalı doğrulama etkinleştirildi. Kurtarma kodlarını güvenli yerde saklayın.');}catch(err){setMessage(err instanceof Error?err.message:'İki aşamalı doğrulama etkinleştirilemedi.');}};
  const disable2fa=async()=>{try{if(!window.pardus)return;await window.pardus.disableTwoFactor({password:currentPassword,code:twoFactorCode});setTwoFactorCode('');setMessage('İki aşamalı doğrulama kapatıldı.');}catch(err){setMessage(err instanceof Error?err.message:'İki aşamalı doğrulama kapatılamadı.');}};
