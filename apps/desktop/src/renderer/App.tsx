...
import type { BankInstitutionView, BankAccountView, CreateBankAccountInput, IbanStructuralValidationView, ValidateIbanInput } from '@ppt/domain';
...
function FinanceScreen({people,records,valuations,institutions,bankAccounts,onCreate,onCreateValuation,onValidateIban,onCreateBankAccount}:{people:FamilyMemberView[];records:FinanceRecordView[];valuations:FinanceValuationView[];institutions:BankInstitutionView[];bankAccounts:BankAccountView[];onCreate:(input:CreateFinanceRecordInput)=>Promise<void>;onCreateValuation:(input:CreateFinanceValuationInput)=>Promise<void>;onValidateIban:(input:ValidateIbanInput)=>Promise<IbanStructuralValidationView>;onCreateBankAccount:(input:CreateBankAccountInput)=>Promise<void>}){
...
export function App() {
  ...
  const ensureAuxiliaryScreen=async(screen:ScreenId):Promise<void>=>{
    ...
    task=(async()=>{
      if(screen==='finance'){
        const [records,valuations,institutions,accounts]=await Promise.all([window.pardus!.listFinance(),window.pardus!.listFinanceValuations(),window.pardus!.listBankInstitutions(),window.pardus!.listBankAccounts()]);
        asyncWriteGuardRef.current.commit(ticket,()=>{setFinanceRecords(records);setFinanceValuations(valuations);setBankInstitutions(institutions);setBankAccounts(accounts);});
  ...
  else if (active === 'finance') screen = <FinanceScreen people={snapshot.people} records={financeRecords} valuations={financeValuations} institutions={bankInstitutions} bankAccounts={bankAccounts} onCreate={createFinance} onCreateValuation={createFinanceValuation} onValidateIban={validateIban} onCreateBankAccount={createBankAccount} />;
