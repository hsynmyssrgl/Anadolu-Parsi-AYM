...
type EnumUiLanguage = 'tr' | 'en';
type EnumLabelPair = readonly [tr: string, en: string];
const enumLabel = <T extends string>(value:T, labels:Readonly<Record<T,EnumLabelPair>>, language:EnumUiLanguage):string => labels[value][language==='tr'?0:1];
const looseEnumLabel = (value:string, labels:Readonly<Record<string,EnumLabelPair>>, language:EnumUiLanguage, fallback:EnumLabelPair):string => (labels[value]??fallback)[language==='tr'?0:1];

const ARCHIVE_SENSITIVITY_LABELS:Readonly<Record<ArchivePageItemView['sensitivity'],EnumLabelPair>>={standard:['Standart','Standard'],personal:['Kişisel','Personal'],high:['Yüksek','High']};
const FAMILY_RELATION_TYPE_LABELS:Readonly<Record<FamilyAppSnapshot['relations'][number]['relationType'],EnumLabelPair>>={parent:['Ebeveyn','Parent'],spouse:['Eş','Spouse'],child:['Çocuk','Child'],sibling:['Kardeş','Sibling'],guardian:['Vasi','Guardian'],other:['Diğer','Other']};
export const archiveSensitivityLabel=(value:ArchivePageItemView['sensitivity'],language:EnumUiLanguage):string=>enumLabel(value,ARCHIVE_SENSITIVITY_LABELS,language);
export const familyRelationTypeLabel=(value:string,language:EnumUiLanguage):string=>looseEnumLabel(value,FAMILY_RELATION_TYPE_LABELS,language,['Aile bağı','Family relationship']);
const parseArchiveSensitivity=(value:string,language:EnumUiLanguage):ArchiveClassificationView['sensitivity']|undefined=>{
  const normalized=value.normalize('NFKC').trim().toLocaleLowerCase(language==='tr'?'tr-TR':'en-US');
  const values:Readonly<Record<string,ArchiveClassificationView['sensitivity']>>={standard:'standard',standart:'standard',personal:'personal','kişisel':'personal',kisisel:'personal',high:'high','yüksek':'high',yuksek:'high'};
  return values[normalized];
};
export const familyImportIssueLabel=(code:string,severity:FamilyDataImportPreviewView['issues'][number]['severity'],language:EnumUiLanguage):string=>{
  const labels:EnumLabelPair=code.includes('location_policy_batch')
    ?['Konum kayıtları şu anda güvenle içe aktarılamıyor.','Location records cannot be imported safely at this time.']
    :code.startsWith('import.person_')
      ?['Bir kişi kaydı düzeltilmelidir.','A person record needs correction.']
      :code.startsWith('import.relation_')
        ?['Bir aile bağı kaydı düzeltilmelidir.','A family-relationship record needs correction.']
        :code.startsWith('import.location_')
          ?['Bir konum kaydı düzeltilmelidir.','A location record needs correction.']
          :code.startsWith('import.event_')
            ?['Bir etkinlik kaydı düzeltilmelidir.','An event record needs correction.']
            :severity==='warning'
              ?['Mevcut kayıtlarla eşleştirilecek bir bilgi bulundu.','A record will be matched with existing information.']
              :['Dosya biçimi veya içeriği düzeltilmelidir.','The file format or content needs correction.'];
  return labels[language==='tr'?0:1];
};

const TIMELINE_EVENT_KIND_LABELS:Readonly<Record<string,EnumLabelPair>>={important_day:['Önemli gün','Important date'],health:['Sağlık','Health']};
export const timelineEventKindLabel=(value:string,language:EnumUiLanguage):string=>looseEnumLabel(value,TIMELINE_EVENT_KIND_LABELS,language,['Diğer olay','Other event']);
const AI_CONSENT_PURPOSE_LABELS:Readonly<Record<AiConsentView['purpose'],EnumLabelPair>>={search:['Doğal dil arama','Natural-language search'],summary:['Özetleme','Summarization'],recommendation:['Öneri','Recommendation'],classification:['Sınıflandırma','Classification'],sensitive_processing:['Hassas veri işleme','Sensitive-data processing'],external_export:['Haricî dışa aktarım','External export'],live_location_sharing:['Canlı konum paylaşımı','Live location sharing']};
const AI_RESOURCE_TYPE_LABELS:Readonly<Record<string,EnumLabelPair>>={person:['Aile üyesi','Family member'],event:['Zaman tüneli olayı','Timeline event'],archive_item:['Arşiv belgesi','Archive document'],finance_record:['Finans kaydı','Finance record'],health_record:['Sağlık kaydı','Health record'],life_record:['Yaşam kaydı','Life record'],local_ocr_job:['Yerel metin tanıma işi','Local text-recognition job'],household_operation_item:['Ev operasyon kaydı','Household operation record'],places_travel_item:['Yer ve seyahat kaydı','Place and travel record'],sensitive_data_profile:['Hassas veri profili','Sensitive-data profile']};
const AI_CONSENT_STATUS_LABELS:Readonly<Record<AiConsentView['status'],EnumLabelPair>>={granted:['Onay verildi','Granted'],revoked:['Onay geri çekildi','Revoked']};
export const aiConsentPurposeLabel=(value:AiConsentView['purpose'],language:EnumUiLanguage):string=>enumLabel(value,AI_CONSENT_PURPOSE_LABELS,language);
export const aiResourceTypeLabel=(value:string,language:EnumUiLanguage):string=>looseEnumLabel(value,AI_RESOURCE_TYPE_LABELS,language,['Diğer kaynak','Other resource']);
export const aiConsentStatusLabel=(value:AiConsentView['status'],language:EnumUiLanguage):string=>enumLabel(value,AI_CONSENT_STATUS_LABELS,language);
const FINANCE_RECORD_KIND_LABELS:Readonly<Record<FinanceRecordView['kind'],EnumLabelPair>>={asset:['Varlık','Asset'],debt:['Borç','Debt'],income:['Gelir','Income'],expense:['Gider','Expense']};
const LIFE_RECORD_STATUS_LABELS:Readonly<Record<LifeRecordView['status'],EnumLabelPair>>={planned:['Planlandı','Planned'],active:['Aktif','Active'],completed:['Tamamlandı','Completed'],expired:['Süresi doldu','Expired'],cancelled:['İptal edildi','Cancelled']};
const LOAN_STATUS_LABELS:Readonly<Record<LoanAccountView['status'],EnumLabelPair>>={active:['Aktif','Active'],overdue:['Gecikmede','Overdue'],restructured:['Yapılandırıldı','Restructured'],closed:['Kapalı','Closed']};
const LOAN_INSURANCE_STATUS_LABELS:Readonly<Record<LoanAccountView['insuranceStatus'],EnumLabelPair>>={none:['Yok','None'],active:['Aktif','Active'],expired:['Süresi doldu','Expired'],cancelled:['İptal edildi','Cancelled']};
const LOAN_COLLATERAL_TYPE_LABELS:Readonly<Record<LoanAccountView['collateralType'],EnumLabelPair>>={none:['Yok','None'],vehicle:['Araç','Vehicle'],real_estate:['Gayrimenkul','Real estate'],deposit:['Mevduat','Deposit'],guarantee:['Kefalet','Guarantee'],other:['Diğer','Other']};
export const financeRecordKindLabel=(value:FinanceRecordView['kind'],language:EnumUiLanguage):string=>enumLabel(value,FINANCE_RECORD_KIND_LABELS,language);
export const lifeRecordStatusLabel=(value:LifeRecordView['status'],language:EnumUiLanguage):string=>enumLabel(value,LIFE_RECORD_STATUS_LABELS,language);
export const loanStatusLabel=(value:LoanAccountView['status'],language:EnumUiLanguage):string=>enumLabel(value,LOAN_STATUS_LABELS,language);
export const loanInsuranceStatusLabel=(value:LoanAccountView['insuranceStatus'],language:EnumUiLanguage):string=>enumLabel(value,LOAN_INSURANCE_STATUS_LABELS,language);
export const loanCollateralTypeLabel=(value:LoanAccountView['collateralType'],language:EnumUiLanguage):string=>enumLabel(value,LOAN_COLLATERAL_TYPE_LABELS,language);
const BACKUP_TARGET_KIND_LABELS:Readonly<Record<BackupTargetView['kind'],EnumLabelPair>>={local:['Yerel','Local'],external:['Haricî disk','External disk'],cloud:['Bulut klasörü','Cloud folder']};
const BACKUP_SCHEDULE_LABELS:Readonly<Record<BackupSchedule,EnumLabelPair>>={manual:['Manuel','Manual'],hourly:['Saatlik','Hourly'],daily:['Günlük','Daily'],weekly:['Haftalık','Weekly'],monthly:['Aylık','Monthly']};
const BACKGROUND_TASK_STATUS_LABELS:Readonly<Record<BackgroundTaskView['status'],EnumLabelPair>>={running:['Çalışıyor','Running'],success:['Başarılı','Successful'],failed:['Başarısız','Failed'],deferred:['Ertelendi','Deferred']};
const QUEUED_TASK_PRIORITY_LABELS:Readonly<Record<QueuedTaskView['priority'],EnumLabelPair>>={critical:['Kritik','Critical'],high:['Yüksek','High'],normal:['Normal','Normal'],low:['Düşük','Low']};
const QUEUED_TASK_STATUS_LABELS:Readonly<Record<QueuedTaskView['status'],EnumLabelPair>>={queued:['Kuyrukta','Queued'],running:['Çalışıyor','Running'],completed:['Tamamlandı','Completed'],failed:['Başarısız','Failed'],deferred:['Ertelendi','Deferred']};
const SYSTEM_TASK_TYPE_LABELS:Readonly<Record<string,EnumLabelPair>>={backup:['Yedekleme','Backup'],['backup.due']:['Zamanı gelen yedek','Backup due'],['backup.propagation']:['Yedek yayılımı','Backup propagation'],['performance.sample']:['Performans örnekleme','Performance sampling'],['maintenance.integrity_check']:['Bütünlük kontrolü','Integrity check'],['maintenance.wal_checkpoint']:['Yazma günlüğü temizliği','Write-ahead-log cleanup'],['maintenance.analyze']:['Veritabanı analizi','Database analysis'],['maintenance.vacuum']:['Veritabanı sıkıştırma','Database compaction'],['maintenance.review']:['Bakım incelemesi','Maintenance review']};
export const backupTargetKindLabel=(value:BackupTargetView['kind'],language:EnumUiLanguage):string=>enumLabel(value,BACKUP_TARGET_KIND_LABELS,language);
export const backupScheduleLabel=(value:BackupSchedule,language:EnumUiLanguage):string=>enumLabel(value,BACKUP_SCHEDULE_LABELS,language);
export const backgroundTaskStatusLabel=(value:BackgroundTaskView['status'],language:EnumUiLanguage):string=>enumLabel(value,BACKGROUND_TASK_STATUS_LABELS,language);
export const queuedTaskPriorityLabel=(value:QueuedTaskView['priority'],language:EnumUiLanguage):string=>enumLabel(value,QUEUED_TASK_PRIORITY_LABELS,language);
export const queuedTaskStatusLabel=(value:QueuedTaskView['status'],language:EnumUiLanguage):string=>enumLabel(value,QUEUED_TASK_STATUS_LABELS,language);
export const systemTaskTypeLabel=(value:string,language:EnumUiLanguage):string=>looseEnumLabel(value,SYSTEM_TASK_TYPE_LABELS,language,['Sistem görevi','System task']);
const EXPORT_ARTIFACT_KIND_LABELS:Readonly<Record<ExportArtifactView['kind'],EnumLabelPair>>={diagnostic_report:['Tanılama raporu','Diagnostic report'],diagnostic_archive:['Tanılama arşivi','Diagnostic archive'],maintenance_history:['Bakım geçmişi','Maintenance history'],system_pdf:['Sistem raporu','System report']};
const DIAGNOSTIC_COMPARISON_KIND_LABELS:Readonly<Record<DiagnosticReportComparisonView['sectionChanges'][number]['kind'],EnumLabelPair>>={added:['Eklendi','Added'],removed:['Kaldırıldı','Removed'],changed:['Değiştirildi','Changed']};
const MAINTENANCE_OPERATION_LABELS:Readonly<Record<MaintenanceResultView['operation'],EnumLabelPair>>={integrity_check:['Bütünlük kontrolü','Integrity check'],wal_checkpoint:['Yazma günlüğü temizliği','Write-ahead-log cleanup'],analyze:['Veritabanı analizi','Database analysis'],vacuum:['Veritabanı sıkıştırma','Database compaction']};
const MAINTENANCE_SOURCE_LABELS:Readonly<Record<MaintenanceHistoryView['source'],EnumLabelPair>>={manual:['Manuel','Manual'],queue:['Görev kuyruğu','Task queue'],automatic:['Otomatik','Automatic']};
export const exportArtifactKindLabel=(value:ExportArtifactView['kind'],language:EnumUiLanguage):string=>enumLabel(value,EXPORT_ARTIFACT_KIND_LABELS,language);
export const diagnosticComparisonKindLabel=(value:DiagnosticReportComparisonView['sectionChanges'][number]['kind'],language:EnumUiLanguage):string=>enumLabel(value,DIAGNOSTIC_COMPARISON_KIND_LABELS,language);
export const maintenanceOperationLabel=(value:MaintenanceResultView['operation'],language:EnumUiLanguage):string=>enumLabel(value,MAINTENANCE_OPERATION_LABELS,language);
export const maintenanceSourceLabel=(value:MaintenanceHistoryView['source'],language:EnumUiLanguage):string=>enumLabel(value,MAINTENANCE_SOURCE_LABELS,language);
type BackupWorkflowStatus=BackupCleanRewriteStatusView['policy']['state']|BackupCleanRewriteRunView['status']|BackupPropagationRunView['status']|'skipped';
const BACKUP_WORKFLOW_STATUS_LABELS:Readonly<Record<BackupWorkflowStatus,EnumLabelPair>>={idle:['Hazır','Ready'],running:['Çalışıyor','Running'],backoff:['Yeniden deneme bekliyor','Waiting to retry'],deferred:['Ertelendi','Deferred'],attention:['İnceleme gerekiyor','Needs attention'],success:['Başarılı','Successful'],partial:['Kısmen tamamlandı','Partially completed'],failed:['Başarısız','Failed'],interrupted:['Kesintiye uğradı','Interrupted'],skipped:['Atlandı','Skipped']};
const EXTERNAL_BACKUP_COPY_KIND_LABELS:Readonly<Record<ExternalBackupCopyView['kind'],EnumLabelPair>>={offline_disk:['Çevrimdışı disk','Offline disk'],manual_file:['Manuel dosya','Manual file'],cloud_history:['Bulut sürüm geçmişi','Cloud version history'],other:['Diğer','Other']};
const EXTERNAL_BACKUP_COPY_STATUS_LABELS:Readonly<Record<ExternalBackupCopyView['status'],EnumLabelPair>>={active:['Etkin','Active'],unreachable:['Erişilemiyor','Unreachable'],retired:['Kullanımdan kaldırıldı','Retired'],destroyed:['İmha edildi','Destroyed']};
const REVOCATION_LIST_STATUS_LABELS:Readonly<Record<ExternalBackupEvidenceRevocationListView['status'],EnumLabelPair>>={current:['Güncel','Current'],superseded:['Yerine yeni liste geldi','Superseded'],expired:['Süresi doldu','Expired']};
export const backupWorkflowStatusLabel=(value:BackupWorkflowStatus,language:EnumUiLanguage):string=>enumLabel(value,BACKUP_WORKFLOW_STATUS_LABELS,language);
export const externalBackupCopyKindLabel=(value:ExternalBackupCopyView['kind'],language:EnumUiLanguage):string=>enumLabel(value,EXTERNAL_BACKUP_COPY_KIND_LABELS,language);
export const externalBackupCopyStatusLabel=(value:ExternalBackupCopyView['status'],language:EnumUiLanguage):string=>enumLabel(value,EXTERNAL_BACKUP_COPY_STATUS_LABELS,language);
export const revocationListStatusLabel=(value:ExternalBackupEvidenceRevocationListView['status'],language:EnumUiLanguage):string=>enumLabel(value,REVOCATION_LIST_STATUS_LABELS,language);
const IPC_MAINTENANCE_AUTHORITY_REASON_LABELS:Readonly<Record<IpcAdaptiveBudgetMaintenanceAuthorityView['reason'],EnumLabelPair>>={ALLOWED:['Açık','Open'],AUTHENTICATION_REQUIRED:['Kimlik doğrulaması gerekli','Authentication required'],SESSION_EXPIRED:['Oturum süresi doldu','Session expired'],ROLE_NOT_ALLOWED:['Bu işlem için yetki yok','Not authorized for this operation'],DEVICE_CONTEXT_REQUIRED:['Cihaz doğrulaması gerekli','Device verification required'],TRUSTED_DEVICE_REQUIRED:['Güvenilir cihaz gerekli','Trusted device required'],REAUTHENTICATION_LOCKED:['Güçlü doğrulama geçici olarak kilitli','Strong authentication temporarily locked']};
const IPC_MAINTENANCE_RECOVERY_REASON_LABELS:Readonly<Record<IpcAdaptiveBudgetMaintenanceRecoveryAuthorityView['reason'],EnumLabelPair>>={ALLOWED:['Açık','Open'],RECOVERY_NOT_REQUIRED:['Kurtarma gerekmiyor','Recovery not required'],AUTHENTICATION_REQUIRED:['Kimlik doğrulaması gerekli','Authentication required'],SESSION_EXPIRED:['Oturum süresi doldu','Session expired'],ROLE_NOT_ALLOWED:['Bu işlem için yetki yok','Not authorized for this operation'],DEVICE_CONTEXT_REQUIRED:['Cihaz doğrulaması gerekli','Device verification required'],TRUSTED_DEVICE_REQUIRED:['Güvenilir cihaz gerekli','Trusted device required'],RECOVERY_RATE_LIMITED:['Kurtarma denemeleri geçici olarak sınırlandı','Recovery attempts temporarily limited'],RECOVERY_COOLDOWN_ACTIVE:['Kurtarma bekleme süresi etkin','Recovery cooldown active']};
const IPC_ADAPTIVE_REASON_LABELS:Readonly<Record<IpcPerformanceTelemetryView['adaptiveBudget']['reason'],EnumLabelPair>>={['startup-baseline']:['Başlangıç ölçütleri','Startup baseline'],['insufficient-samples']:['Örnek sayısı yetersiz','Insufficient samples'],healthy:['Sağlıklı yük','Healthy load'],['warning-pressure']:['Yük baskısı uyarısı','Load pressure warning'],['critical-pressure']:['Kritik yük baskısı','Critical load pressure'],['invalid-telemetry']:['Ölçüm doğrulanamadı','Metrics could not be verified'],['recovery-hold']:['Kurtarma beklemesinde','Recovery hold'],restored:['Önceki bütçe geri yüklendi','Previous budget restored'],['restore-rejected']:['Önceki bütçe reddedildi','Previous budget rejected'],['persistence-failure']:['Kalıcılık işlemi başarısız','Persistence failed'],['manual-reset']:['Manuel olarak sıfırlandı','Manually reset']};
const IPC_PERSISTENCE_STATUS_LABELS:Readonly<Record<IpcPerformanceTelemetryView['adaptiveBudget']['persistence']['status'],EnumLabelPair>>={disabled:['Kapalı','Disabled'],initialized:['Başlatıldı','Initialized'],verified:['Doğrulandı','Verified'],recovered:['Kurtarıldı','Recovered'],rejected:['Reddedildi','Rejected'],['write-failed']:['Yazma başarısız','Write failed']};
const IPC_ADAPTIVE_MODE_LABELS:Readonly<Record<IpcPerformanceTelemetryView['adaptiveBudget']['mode'],EnumLabelPair>>={baseline:['Standart','Standard'],guarded:['Korumalı','Guarded'],restricted:['Kısıtlı','Restricted']};
const IPC_ALERT_FALLBACKS:Readonly<Record<IpcPerformanceTelemetryView['alerts'][number]['code'],EnumLabelPair>>={['duration-p95']:['Yanıt süresi uyarısı algılandı.','A response-time warning was detected.'],['queue-wait-p95']:['Kuyruk bekleme süresi uyarısı algılandı.','A queue-wait warning was detected.'],['timeout-rate']:['Süre aşımı oranı uyarısı algılandı.','A timeout-rate warning was detected.'],['queue-rejection-rate']:['Kuyruk reddi oranı uyarısı algılandı.','A queue-rejection-rate warning was detected.'],['global-pressure']:['Genel sistem yükü uyarısı algılandı.','An overall system-load warning was detected.']};
export const ipcMaintenanceAuthorityReasonLabel=(value:IpcAdaptiveBudgetMaintenanceAuthorityView['reason'],language:EnumUiLanguage):string=>enumLabel(value,IPC_MAINTENANCE_AUTHORITY_REASON_LABELS,language);
export const ipcMaintenanceRecoveryReasonLabel=(value:IpcAdaptiveBudgetMaintenanceRecoveryAuthorityView['reason'],language:EnumUiLanguage):string=>enumLabel(value,IPC_MAINTENANCE_RECOVERY_REASON_LABELS,language);
export const ipcAdaptiveReasonLabel=(value:IpcPerformanceTelemetryView['adaptiveBudget']['reason'],language:EnumUiLanguage):string=>enumLabel(value,IPC_ADAPTIVE_REASON_LABELS,language);
export const ipcPersistenceStatusLabel=(value:IpcPerformanceTelemetryView['adaptiveBudget']['persistence']['status'],language:EnumUiLanguage):string=>enumLabel(value,IPC_PERSISTENCE_STATUS_LABELS,language);
export const ipcAdaptiveModeLabel=(value:IpcPerformanceTelemetryView['adaptiveBudget']['mode'],language:EnumUiLanguage):string=>enumLabel(value,IPC_ADAPTIVE_MODE_LABELS,language);
export const ipcChannelLabel=(_value:string,language:EnumUiLanguage):string=>language==='tr'?'Uygulama işlemi':'Application operation';
export const ipcAlertMessage=(alert:IpcPerformanceTelemetryView['alerts'][number],language:EnumUiLanguage):string=>toUserFacingErrorMessage(alert.message,enumLabel(alert.code,IPC_ALERT_FALLBACKS,language));
const PRIVACY_CATEGORY_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['dataInventory'][number]['category'],EnumLabelPair>>={identity:['Kimlik','Identity'],family:['Aile','Family'],health:['Sağlık','Health'],finance:['Finans','Finance'],location:['Konum','Location'],communication:['İletişim','Communication'],archive:['Arşiv','Archive'],ai_memory:['Yapay zekâ hafızası','AI memory'],ocr:['Metin tanıma','Text recognition'],translation:['Çeviri','Translation'],legacy:['Dijital miras','Digital legacy'],security:['Güvenlik','Security'],other:['Diğer','Other']};
const PRIVACY_STORAGE_SCOPE_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['dataInventory'][number]['storageScope'],EnumLabelPair>>={local_encrypted:['Şifreli yerel alan','Encrypted local area'],managed_backup:['Yönetilen yedek','Managed backup'],external_copy_declared:['Beyan edilen haricî kopya','Declared external copy']};
const AI_MEMORY_STATUS_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['aiMemoryRecords'][number]['status'],EnumLabelPair>>={active:['Etkin','Active'],restricted:['Sınırlandı','Restricted'],expired:['Süresi doldu','Expired'],pending_deletion:['Silme bekliyor','Pending deletion'],deleted:['Silindi','Deleted']};
const DEVICE_TRUST_STATUS_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['localDeviceActivity'][number]['trustStatus'],EnumLabelPair>>={trusted:['Güvenilir','Trusted'],revoked:['Güveni kaldırıldı','Trust revoked']};
const DEVICE_SESSION_STATUS_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['localDeviceActivity'][number]['locallyObservedSession'],EnumLabelPair>>={current_session:['Geçerli oturum','Current session'],recently_seen:['Yakın zamanda görüldü','Recently seen'],not_observed:['Oturum gözlenmedi','Session not observed']};
const APPLE_SYNC_STATUS_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['localDeviceActivity'][number]['appleSyncStatus'],EnumLabelPair>>={not_configured:['Yapılandırılmadı','Not configured'],unsupported:['Desteklenmiyor','Unsupported'],locally_observed:['Yerel olarak gözlendi','Observed locally']};
const LOCAL_PROCESSING_KIND_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['localProcessingObservations'][number]['kind'],EnumLabelPair>>={ai:['Yapay zekâ','AI'],ocr:['Metin tanıma','Text recognition'],translation:['Çeviri','Translation']};
const LOCAL_PROCESSING_STATUS_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['localProcessingObservations'][number]['status'],EnumLabelPair>>={started:['Başladı','Started'],completed:['Tamamlandı','Completed'],failed:['Başarısız','Failed'],cancelled:['İptal edildi','Cancelled']};
const DATA_RIGHTS_KIND_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['rightsRequests'][number]['kind'],EnumLabelPair>>={encrypted_export:['Şifreli dışa aktarım','Encrypted export'],retention_change:['Saklama değişikliği','Retention change'],erasure:['Yerel silme','Local erasure'],legacy_export:['Dijital miras dışa aktarımı','Digital legacy export']};
const DATA_RIGHTS_STATUS_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['rightsRequests'][number]['status'],EnumLabelPair>>={requested:['Talep edildi','Requested'],in_review:['İnceleniyor','In review'],locally_completed:['Yerel olarak tamamlandı','Completed locally'],rejected:['Reddedildi','Rejected'],cancelled:['İptal edildi','Cancelled']};
const PRIVACY_INCIDENT_SEVERITY_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['incidents'][number]['severity'],EnumLabelPair>>={low:['Düşük','Low'],medium:['Orta','Medium'],high:['Yüksek','High'],critical:['Kritik','Critical']};
const PRIVACY_INCIDENT_STATUS_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['incidents'][number]['status'],EnumLabelPair>>={open:['Açık','Open'],contained_locally:['Yerel olarak sınırlandı','Contained locally'],resolved:['Çözüldü','Resolved'],cancelled:['İptal edildi','Cancelled']};
const ACCESS_DECISION_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['accessHistory'][number]['decision'],EnumLabelPair>>={allowed:['İzin verildi','Allowed'],denied:['Reddedildi','Denied']};
const ACCESS_PURPOSE_LABELS:Readonly<Record<string,EnumLabelPair>>={general:['Genel kullanım','General use'],care:['Bakım','Care'],finance:['Finans','Finance'],health:['Sağlık','Health'],archive:['Arşiv','Archive'],digital_legacy:['Dijital miras','Digital legacy'],ai_processing:['Yapay zekâ işleme','AI processing'],administration:['Yönetim','Administration']};
const DERIVED_KIND_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['derivedDataLineage'][number]['derivedKind'],EnumLabelPair>>={OCR_TEXT:['Tanınan metin','Recognized text'],SEARCH_INDEX:['Arama dizini','Search index'],THUMBNAIL:['Küçük görsel','Thumbnail'],AI_MEMORY:['Yapay zekâ hafızası','AI memory'],SUMMARY:['Özet','Summary'],EMBEDDING:['Anlamsal gösterim','Semantic representation'],TRANSLATION:['Çeviri','Translation'],TRANSCRIPT:['Döküm','Transcript'],CACHE:['Geçici kopya','Temporary copy'],REPLICA:['Yerel kopya','Local copy']};
const DELETION_PROPAGATION_LABELS:Readonly<Record<PrivacyOwnershipControlCenterView['derivedDataLineage'][number]['deletionPropagation'],EnumLabelPair>>={not_requested:['Silme istenmedi','Deletion not requested'],pending:['Silme bekliyor','Deletion pending'],locally_completed:['Yerel silme tamamlandı','Local deletion completed'],attention_required:['İnceleme gerekiyor','Review required']};
const COMPANION_STATUS_LABELS:Readonly<Record<ReadOnlyCompanionSnapshotView['status']|CompanionSyncDenialView['status'],EnumLabelPair>>={snapshot_ready:['Hazır','Ready'],write_forbidden:['Yazma kapalı','Writes disabled'],version_conflict:['Sürüm uyuşmazlığı','Version conflict'],device_revoked:['Cihaz güveni kaldırıldı','Device trust revoked'],security_epoch_stale:['Güvenlik dönemi güncel değil','Security epoch is not current']};
const TEMPORARY_PURPOSE_LABELS:Readonly<Record<(typeof TEMPORARY_CREDENTIAL_PURPOSE_BY_KIND)[TemporaryCredentialKind],EnumLabelPair>>={school_pickup_authorization:['Okuldan teslim alma yetkisi','School pickup authorization'],temporary_care_authorization:['Geçici bakım yetkisi','Temporary care authorization'],pet_care_authorization:['Evcil hayvan bakım yetkisi','Pet care authorization'],emergency_contact_health_access:['Acil durumda sağlık bilgisine erişim','Emergency health-information access'],event_invitation_access:['Etkinlik daveti erişimi','Event invitation access'],temporary_home_access:['Geçici ev erişimi','Temporary home access']};
const TEMPORARY_VERIFICATION_DECISION_LABELS:Readonly<Record<TemporaryCredentialVerificationView['decision'],EnumLabelPair>>={accepted_locally:['Yerel olarak kabul edildi','Accepted locally'],rejected:['Reddedildi','Rejected'],indeterminate_revocation:['İptal durumu doğrulanamadı','Revocation status could not be verified'],indeterminate_issuer:['Düzenleyen doğrulanamadı','Issuer could not be verified']};
const TEMPORARY_REVOCATION_STATUS_LABELS:Readonly<Record<TemporaryCredentialVerificationView['revocationStatus'],EnumLabelPair>>={not_revoked_locally:['Yerel iptal kaydı yok','No local revocation record'],revoked_locally:['Yerel olarak iptal edildi','Revoked locally'],unknown_offline:['Çevrimdışı doğrulanamadı','Could not be verified offline']};
const SECURITY_RECEIPT_STATUS_LABELS:Readonly<Record<SecurityEventReceiptVerificationView['status'],EnumLabelPair>>={VALID:['Doğrulandı','Verified'],INVALID:['Geçersiz','Invalid'],MALFORMED:['Biçimi geçersiz','Malformed']};
const MEMBERSHIP_ROLE_LABELS:Readonly<Record<PersonMembershipRole,EnumLabelPair>>={resident:['İkamet eden','Resident'],member:['Üye','Member'],guardian:['Vasi','Guardian'],dependent:['Bağımlı','Dependent'],other:['Diğer','Other']};
const MEMBERSHIP_STATUS_LABELS:Readonly<Record<string,EnumLabelPair>>={active:['Etkin','Active'],suspended:['Askıda','Suspended'],ended:['Sona erdi','Ended']};
const OFFLINE_CAPABILITY_LABELS:Readonly<Record<OfflineCapability,EnumLabelPair>>={['family.read']:['Aile verisini oku','Read family data'],['family.write']:['Aile verisini değiştir','Change family data'],['health.read']:['Sağlık verisini oku','Read health data'],['health.write']:['Sağlık verisini değiştir','Change health data'],['finance.read']:['Finans verisini oku','Read finance data'],['finance.write']:['Finans verisini değiştir','Change finance data'],['location.read']:['Konumu oku','Read location'],['location.share']:['Konumu paylaş','Share location'],['archive.read']:['Arşivi oku','Read archive'],['archive.write']:['Arşivi değiştir','Change archive'],['archive.ocr']:['Arşivde metin tanı','Recognize archive text'],['ai.process']:['Yapay zekâ ile işle','Process with AI'],['translation.process']:['Çeviri yap','Translate'],['communication.message']:['Mesajlaş','Send messages'],['communication.call']:['Arama yap','Make calls'],['communication.record']:['Kayıt rızasını yönet','Manage recording consent'],['file.share']:['Dosya paylaş','Share files'],['backup.create']:['Yedek oluştur','Create backup'],['backup.restore']:['Yedeği geri yükle','Restore backup'],['cluster.admin']:['Dağıtık sistemi yönet','Manage distributed system'],['plugin.execute']:['Eklenti çalıştır','Run plugin']};
const OFFLINE_LEASE_STATE_LABELS:Readonly<Record<OfflineCapabilityLeaseWorkspaceView['leases'][number]['state'],EnumLabelPair>>={pending:['Bekliyor','Pending'],active:['Etkin','Active'],expired:['Süresi doldu','Expired'],revoked:['İptal edildi','Revoked']};
const OFFLINE_CACHE_REASON_LABELS:Readonly<Record<OfflineCapabilityLeaseWorkspaceView['cache']['reason'],EnumLabelPair>>={NO_LEASE:['Etkin kira yok','No active lease'],ACTIVE:['Etkin kira var','Active lease'],NOT_YET_VALID:['Kira henüz başlamadı','Lease has not started'],EXPIRED:['Kiranın süresi doldu','Lease expired'],REVOKED:['Kira iptal edildi','Lease revoked'],INVALID_LEASE:['Kira doğrulanamadı','Lease could not be verified'],CONTEXT_MISMATCH:['Cihaz veya hesap eşleşmedi','Device or account did not match']};
const RESOURCE_TYPE_LABELS:Readonly<Record<string,EnumLabelPair>>={privacy_inventory:['Gizlilik envanteri','Privacy inventory'],person:['Aile üyesi','Family member'],event:['Aile olayı','Family event'],archive_item:['Arşiv belgesi','Archive document'],finance_record:['Finans kaydı','Finance record'],health_record:['Sağlık kaydı','Health record'],life_record:['Yaşam kaydı','Life record']};
export const privacyCategoryLabel=(value:PrivacyOwnershipControlCenterView['dataInventory'][number]['category'],language:EnumUiLanguage):string=>enumLabel(value,PRIVACY_CATEGORY_LABELS,language);
export const privacyStorageScopeLabel=(value:PrivacyOwnershipControlCenterView['dataInventory'][number]['storageScope'],language:EnumUiLanguage):string=>enumLabel(value,PRIVACY_STORAGE_SCOPE_LABELS,language);
export const aiMemoryStatusLabel=(value:PrivacyOwnershipControlCenterView['aiMemoryRecords'][number]['status'],language:EnumUiLanguage):string=>enumLabel(value,AI_MEMORY_STATUS_LABELS,language);
export const deviceTrustStatusLabel=(value:PrivacyOwnershipControlCenterView['localDeviceActivity'][number]['trustStatus'],language:EnumUiLanguage):string=>enumLabel(value,DEVICE_TRUST_STATUS_LABELS,language);
export const deviceSessionStatusLabel=(value:PrivacyOwnershipControlCenterView['localDeviceActivity'][number]['locallyObservedSession'],language:EnumUiLanguage):string=>enumLabel(value,DEVICE_SESSION_STATUS_LABELS,language);
export const appleSyncStatusLabel=(value:PrivacyOwnershipControlCenterView['localDeviceActivity'][number]['appleSyncStatus'],language:EnumUiLanguage):string=>enumLabel(value,APPLE_SYNC_STATUS_LABELS,language);
export const localProcessingKindLabel=(value:PrivacyOwnershipControlCenterView['localProcessingObservations'][number]['kind'],language:EnumUiLanguage):string=>enumLabel(value,LOCAL_PROCESSING_KIND_LABELS,language);
export const localProcessingStatusLabel=(value:PrivacyOwnershipControlCenterView['localProcessingObservations'][number]['status'],language:EnumUiLanguage):string=>enumLabel(value,LOCAL_PROCESSING_STATUS_LABELS,language);
export const dataRightsKindLabel=(value:PrivacyOwnershipControlCenterView['rightsRequests'][number]['kind'],language:EnumUiLanguage):string=>enumLabel(value,DATA_RIGHTS_KIND_LABELS,language);
export const dataRightsStatusLabel=(value:PrivacyOwnershipControlCenterView['rightsRequests'][number]['status'],language:EnumUiLanguage):string=>enumLabel(value,DATA_RIGHTS_STATUS_LABELS,language);
export const privacyIncidentSeverityLabel=(value:PrivacyOwnershipControlCenterView['incidents'][number]['severity'],language:EnumUiLanguage):string=>enumLabel(value,PRIVACY_INCIDENT_SEVERITY_LABELS,language);
export const privacyIncidentStatusLabel=(value:PrivacyOwnershipControlCenterView['incidents'][number]['status'],language:EnumUiLanguage):string=>enumLabel(value,PRIVACY_INCIDENT_STATUS_LABELS,language);
export const accessDecisionLabel=(value:PrivacyOwnershipControlCenterView['accessHistory'][number]['decision'],language:EnumUiLanguage):string=>enumLabel(value,ACCESS_DECISION_LABELS,language);
export const accessPurposeLabel=(value:string,language:EnumUiLanguage):string=>looseEnumLabel(value,ACCESS_PURPOSE_LABELS,language,['Belirtilen amaç','Specified purpose']);
export const derivedKindLabel=(value:PrivacyOwnershipControlCenterView['derivedDataLineage'][number]['derivedKind'],language:EnumUiLanguage):string=>enumLabel(value,DERIVED_KIND_LABELS,language);
export const deletionPropagationLabel=(value:PrivacyOwnershipControlCenterView['derivedDataLineage'][number]['deletionPropagation'],language:EnumUiLanguage):string=>enumLabel(value,DELETION_PROPAGATION_LABELS,language);
export const companionStatusLabel=(value:ReadOnlyCompanionSnapshotView['status']|CompanionSyncDenialView['status'],language:EnumUiLanguage):string=>enumLabel(value,COMPANION_STATUS_LABELS,language);
export const temporaryPurposeLabel=(value:(typeof TEMPORARY_CREDENTIAL_PURPOSE_BY_KIND)[TemporaryCredentialKind],language:EnumUiLanguage):string=>enumLabel(value,TEMPORARY_PURPOSE_LABELS,language);
export const temporaryVerificationDecisionLabel=(value:TemporaryCredentialVerificationView['decision'],language:EnumUiLanguage):string=>enumLabel(value,TEMPORARY_VERIFICATION_DECISION_LABELS,language);
export const temporaryRevocationStatusLabel=(value:TemporaryCredentialVerificationView['revocationStatus'],language:EnumUiLanguage):string=>enumLabel(value,TEMPORARY_REVOCATION_STATUS_LABELS,language);
export const securityReceiptStatusLabel=(value:SecurityEventReceiptVerificationView['status'],language:EnumUiLanguage):string=>enumLabel(value,SECURITY_RECEIPT_STATUS_LABELS,language);
export const membershipRoleLabel=(value:PersonMembershipRole,language:EnumUiLanguage):string=>enumLabel(value,MEMBERSHIP_ROLE_LABELS,language);
export const dataRepairMembershipRoleLabel=(value:string,language:EnumUiLanguage):string=>looseEnumLabel(value,MEMBERSHIP_ROLE_LABELS,language,['Diğer üyelik rolü','Other membership role']);
export const dataRepairMembershipStatusLabel=(value:string,language:EnumUiLanguage):string=>looseEnumLabel(value,MEMBERSHIP_STATUS_LABELS,language,['Bilinmeyen durum','Unknown status']);
export const offlineCapabilityLabel=(value:OfflineCapability,language:EnumUiLanguage):string=>enumLabel(value,OFFLINE_CAPABILITY_LABELS,language);
export const offlineLeaseStateLabel=(value:OfflineCapabilityLeaseWorkspaceView['leases'][number]['state'],language:EnumUiLanguage):string=>enumLabel(value,OFFLINE_LEASE_STATE_LABELS,language);
export const offlineCacheReasonLabel=(value:OfflineCapabilityLeaseWorkspaceView['cache']['reason'],language:EnumUiLanguage):string=>enumLabel(value,OFFLINE_CACHE_REASON_LABELS,language);
export const resourceTypeLabel=(value:string,language:EnumUiLanguage):string=>looseEnumLabel(value,RESOURCE_TYPE_LABELS,language,['Yerel kayıt','Local record']);
...
function usePersonCatalogData(query: string, selectedIds: readonly string[] = [], fallbackPeople: readonly FamilyMemberView[] = [], revision = 0) {
  ...
  const load = async (reset: boolean, cursor?: string) => {
    ...
    try {
      ...
      guardRef.current.commit(ticket, () => setError(toUserFacingErrorMessage(caught,getActiveUiLocale()==='tr-TR'?'Kişi kataloğu yüklenemedi.':'The person catalog could not be loaded.')));
    } finally {
  ...
  useEffect(() => {
    ...
    void window.pardus.lookupEntityCatalog({ personIds: missing }).then((result) => {
      ...
      guardRef.current.commit(ticket, () => setError(toUserFacingErrorMessage(caught,getActiveUiLocale()==='tr-TR'?'Seçili kişiler çözümlenemedi.':'The selected people could not be resolved.')));
    });
...
export function TreeScreen({ snapshot, onAddRelation }: { snapshot: FamilyAppSnapshot; onAddRelation: () => void }) {
  ...
  const loadPage=async(reset:boolean,cursor?:string)=>{
    ...
    }catch(error){guardRef.current.commit(ticket,()=>setPageMessage(toUserFacingErrorMessage(error,text('Soy ağacı sayfası yüklenemedi.','The family-tree page could not be loaded.'))));}
...
export function TimelineScreen({ snapshot, onEdit, onArchive, onOpenArchive }: { snapshot: FamilyAppSnapshot; onEdit: (event: FamilyEventView) => void; onArchive: (eventId: string) => Promise<void>; onOpenArchive: (eventId: string) => void }) {
  ...
  const loadPage=async(reset:boolean,cursor?:string)=>{
    ...
    }catch(caught){guardRef.current.commit(ticket,()=>setError(toUserFacingErrorMessage(caught,text('Zaman tüneli yüklenemedi.','The timeline could not be loaded.'))));}
    finally{guardRef.current.commit(ticket,()=>setLoadingPage(false));}
  };
  ...
  useEffect(()=>()=>guardRef.current.invalidateAll(),[]);
  const archive=async(event:FamilyEventView)=>{if(!confirm(language==='tr'?`“${event.title}” arşivlensin mi? Kayıt silinmez ve arşivden geri alınabilir.`:`Archive “${event.title}”? The record is not deleted and can be restored from the archive.`))return;setBusyId(event.id);setError('');try{await onArchive(event.id);setEvents(current=>current.filter(item=>item.id!==event.id));}catch(caught){setError(toUserFacingErrorMessage(caught,text('Olay arşivlenemedi.','The event could not be archived.')));}finally{setBusyId('');}};
  const clearFilters=()=>{setQuery('');setKind('all');setYear('all');};
  return <>
    <PageHeader eyebrow={text('Dijital aile hafızası','Digital family memory')} title={text('Zaman tüneli','Timeline')} description={text('Kalabalık aile geçmişinde bile kayıtlar hızlı yüklenir; arama ve filtrelerle aradığınız ana kolayca ulaşabilirsiniz.','Records load quickly even in a large family history, and search and filters help you find the moment you need.')}/>
    ...
    <section className="timeline-toolbar panel" aria-label={text('Zaman tüneli filtreleri','Timeline filters')}>
      ...
      <label>{text('Olay türü','Event type')}<select value={kind} onChange={event=>setKind(event.target.value)}><option value="all">{text('Tüm türler','All types')}</option>{kinds.map(value=><option key={value} value={value}>{timelineEventKindLabel(value,language)}</option>)}</select></label>
...
export function ImportantDaysScreen({ snapshot, archivedEvents, onAdd, onEdit, onArchive, onRestore, onOpenArchive }: { snapshot: FamilyAppSnapshot; archivedEvents: FamilyEventView[]; onAdd: () => void; onEdit: (event: FamilyEventView) => void; onArchive: (eventId: string) => Promise<void>; onRestore: (eventId: string) => Promise<void>; onOpenArchive:(eventId:string)=>void }) {
  ...
  const archive=async()=>{if(!selected||!confirm(language==='tr'?`“${selected.title}” arşivlensin mi?`:`Archive “${selected.title}”?`))return;setBusyId(selected.id);setError('');try{await onArchive(selected.id);setSelectedId('');}catch(caught){setError(toUserFacingErrorMessage(caught,text('Önemli gün arşivlenemedi.','The important date could not be archived.')));}finally{setBusyId('');}};
  const restore=async(eventId:string)=>{setBusyId(eventId);setError('');try{await onRestore(eventId);}catch(caught){setError(toUserFacingErrorMessage(caught,text('Kayıt geri alınamadı.','The record could not be restored.')));}finally{setBusyId('');}};
...
export function UnifiedAuthorizedSearchPanel({onOpenArchive}:{onOpenArchive:(id:string)=>Promise<void>}) {
  ...
  return <section className="panel unified-authorized-search" aria-labelledby="unified-authorized-search-title">
    ...
    <small>{text('Yalnız erişim izniniz olan kayıtlar aranır. Aramanız kaydedilmez; sonuçlar eksik ya da doğrulanmamışsa gösterilmez.','Only records you are allowed to access are searched. Your query is not saved, and incomplete or unverified results are not shown.')}</small>
    {error&&<StatusMessage tone="danger">{error}</StatusMessage>}
...
export function ArchiveScreen({ revision, snapshot, eventFilter, onEventFilterChange, onImport, onOpen }: { revision:number; snapshot: FamilyAppSnapshot; eventFilter:string; onEventFilterChange:(eventId:string)=>void; onImport: (input: { title: string; linkedEventId?: string }) => Promise<void>; onOpen:(id:string)=>Promise<void> }) {
  ...
  useEffect(()=>{guardRef.current.invalidate('archive-versions');if(!selectedItemId||!window.pardus){setVersions([]);return;}const ticket=guardRef.current.start('archive-versions');void window.pardus.listArchiveVersions(selectedItemId).then(value=>{guardRef.current.commit(ticket,()=>setVersions(value));}).catch(caught=>{guardRef.current.commit(ticket,()=>setError(toUserFacingErrorMessage(caught,'Arşiv sürümleri yüklenemedi.')));});return()=>guardRef.current.invalidate('archive-versions');},[selectedItemId]);
  useEffect(()=>{guardRef.current.invalidate('archive-evidence');if(!selectedItemId||!window.pardus){setRelationEvidence([]);setRelationEvidenceHistory([]);return;}const ticket=guardRef.current.start('archive-evidence');void Promise.all([window.pardus.listArchiveRelationEvidence(selectedItemId),window.pardus.listArchiveRelationEvidenceHistory(selectedItemId)]).then(([current,history])=>{guardRef.current.commit(ticket,()=>{setRelationEvidence(current);setRelationEvidenceHistory(history);});}).catch(caught=>{guardRef.current.commit(ticket,()=>setError(toUserFacingErrorMessage(caught,'İlişki kanıtları yüklenemedi.')));});return()=>guardRef.current.invalidate('archive-evidence');},[selectedItemId]);
  useEffect(()=>()=>guardRef.current.invalidateAll(),[]);
  const submit=async()=>{try{setBusy(true);setError('');await onImport({title,...(linkedEventId?{linkedEventId}:{})});setTitle('');await reload(true);}catch(caught){setError(toUserFacingErrorMessage(caught,'Dosya eklenemedi.'));}finally{setBusy(false);}};
  const toggle=(id:string)=>setSelectedIds(value=>value.includes(id)?value.filter(item=>item!==id):[...value,id]);
  const bulkClassify=async()=>{if(!window.pardus||!selectedIds.length)return;const category=prompt(language==='tr'?'Kategori adı veya kimliği (boş = mevcut kategori):':'Category name or identifier (blank = current category):',categoryId)??'';const tags=(prompt(language==='tr'?'Etiketler (virgülle):':'Tags (comma separated):',tag)??'').split(',').map(value=>value.trim()).filter(Boolean);const requestedLevel=prompt(language==='tr'?'Hassasiyet: Standart / Kişisel / Yüksek':'Sensitivity: Standard / Personal / High',language==='tr'?'Standart':'Standard');if(requestedLevel===null)return;const level=parseArchiveSensitivity(requestedLevel,language);if(!level){setError(language==='tr'?'Hassasiyet için Standart, Kişisel veya Yüksek yazın.':'Enter Standard, Personal, or High for sensitivity.');return;}setBusy(true);setError('');try{for(const itemId of selectedIds){const existing=results.find(item=>item.id===itemId);await window.pardus.updateArchiveClassification({itemId,...(category?{categoryId:category}:existing?.categoryId?{categoryId:existing.categoryId}:{}),tagNames:tags.length?tags:(existing?.tagNames??[]),sensitivity:level,aiProcessingAllowed:false});}setSelectedIds([]);await reload(true);}catch(caught){setError(toUserFacingErrorMessage(caught,language==='tr'?'Toplu sınıflandırma başarısız.':'Bulk classification failed.'));}finally{setBusy(false);}};
  const assignPolicy=async(itemId:string,policyId:string)=>{if(!window.pardus)return;setBusy(true);try{await window.pardus.assignArchiveRetentionPolicy({itemId,...(policyId?{policyId}:{})});await reload(true);}catch(caught){setError(toUserFacingErrorMessage(caught,'Saklama politikası atanamadı.'));}finally{setBusy(false);}};
  ...
  const destroy=async(id:string)=>{if(!window.pardus||!confirm(translateArchiveCenterCopy('Bu belge güvenli biçimde imha edilecek. Devam edilsin mi?',language)))return;setBusy(true);try{await window.pardus.securelyDestroyArchiveItem(id);await reload(true);}catch(caught){setError(toUserFacingErrorMessage(caught,translateArchiveCenterCopy('İmha işlemi başarısız.',language)));}finally{setBusy(false);}};
  const reattestLegacyOwnership=async()=>{if(!window.pardus||!selected||selected.ownershipBinding!=='legacy_unverified')return;setBusy(true);setError('');try{await window.pardus.reattestLegacyArchiveOwnership({itemId:selected.id,password:reattestPassword,...(reattestCode.trim()?{code:reattestCode.trim()}:{}),confirmation:reattestConfirmation});await reload(true);}catch(caught){setError(toUserFacingErrorMessage(caught,'Eski arşiv sahipliği yeniden doğrulanamadı.'));}finally{setReattestPassword('');setReattestCode('');setReattestConfirmation('');setBusy(false);}};
  ...
  const relationLabel=(relationId:string)=>{const relation=snapshot.relations.find(item=>item.id===relationId);if(!relation)return language==='tr'?'Aile bağı':'Family relationship';const from=snapshot.people.find(person=>person.id===relation.fromPersonId)?.displayName??(language==='tr'?'Aile üyesi':'Family member');const to=snapshot.people.find(person=>person.id===relation.toPersonId)?.displayName??(language==='tr'?'Aile üyesi':'Family member');return `${from} — ${to} · ${familyRelationTypeLabel(relation.relationType,language)}`;};
  const evidenceConfidenceLabel=(value:ArchiveRelationEvidenceConfidence)=>translateArchiveCenterCopy(({low:'Düşük',medium:'Orta',high:'Yüksek'} as const)[value],language);
  const panel = <><PageHeader eyebrow="Gelişmiş belge yaşam döngüsü" title="Doküman Merkezi" description="Büyük arşivlerde yalnız ihtiyaç duyduğunuz kayıtlar aşamalı olarak yüklenir; filtreler aradığınız belgeyi hızlıca bulmanıza yardımcı olur." actions={<Button tone="primary" onClick={()=>void submit()} disabled={busy||title.trim().length<2}>＋ Dosya ekle</Button>}/>
    ...
    <div className="document-import"><input value={title} onChange={event=>setTitle(event.target.value)} placeholder="Yeni belge başlığı" aria-label="Yeni belge başlığı"/><select value={linkedEventId} onChange={event=>setLinkedEventId(event.target.value)}><option value="">Etkinlik bağlantısı yok</option>{snapshot.events.map(event=><option key={event.id} value={event.id}>{event.title}</option>)}</select></div>
    {results.length?results.map(item=><div className={`document-row ${selectedItemId===item.id?'selected':''}`} key={item.id} role="button" tabIndex={0} aria-pressed={selectedItemId===item.id} aria-label={`${item.title} belgesini seç`} onClick={()=>setSelectedItemId(item.id)} onKeyDown={event=>{if(event.target!==event.currentTarget)return;if(event.key==='Enter'||event.key===' '){event.preventDefault();setSelectedItemId(item.id);}}}><input type="checkbox" aria-label={`${item.title} belgesini toplu işleme dahil et`} checked={selectedIds.includes(item.id)} onChange={()=>toggle(item.id)} onClick={event=>event.stopPropagation()}/><div><strong>{item.title}</strong><span>{item.originalName} · {(item.sizeBytes/1024).toFixed(1)} KB</span><small>{item.categoryName??translateArchiveCenterCopy('Kategorisiz',language)} · {archiveSensitivityLabel(item.sensitivity,language)} · {item.tagNames.join(', ')||translateArchiveCenterCopy('etiket yok',language)}</small></div><div className="document-row-actions"><span className={`tag ${item.eligibleForDestruction?'red':'blue'}`}>{item.retentionPolicyName??translateArchiveCenterCopy('Politika yok',language)}</span><Button onClick={event=>{event.stopPropagation();void onOpen(item.id);}}>Aç</Button></div></div>):<EmptyState title="Belge bulunamadı" body="Filtreleri değiştirin veya yeni belge ekleyin."/>}
...
function FirstRunSecuritySetup({twoFactorAlreadyEnabled,onComplete}:{twoFactorAlreadyEnabled:boolean;onComplete:(input:{code:string;password:string})=>Promise<void>}){
  const {language,t}=useLocalization();
  const [setup,setSetup]=useState<TwoFactorSetupView|null>(null);
  const [twoFactorEnabled,setTwoFactorEnabled]=useState(twoFactorAlreadyEnabled);
  ...
  const [busy,setBusy]=useState(false);
  const begin=async()=>{try{if(!window.pardus)return;setSetup(await window.pardus.beginTwoFactorSetup());setMessageTone('info');setMessage(language==='tr'?'Kimlik doğrulayıcı uygulamanıza anahtarı ekleyin ve kurtarma kodlarını güvenli yerde saklayın.':'Add the key to your authenticator application and keep the recovery codes in a safe place.');}catch(error){setMessageTone('danger');setMessage(toUserFacingErrorMessage(error,language==='tr'?'Güvenlik kurulumu başlatılamadı.':'Security setup could not be started.'));}};
  const finish=async()=>{try{if(!window.pardus||(!twoFactorEnabled&&!setup)||(setup&&!saved)||code.trim().length<6||password.length<12)return;setBusy(true);setMessageTone('info');setMessage('');if(!twoFactorEnabled){await window.pardus.enableTwoFactor({code:code.trim()});setTwoFactorEnabled(true);}await onComplete({code:code.trim(),password});playParsBrandSound();}catch(error){setMessageTone('danger');setMessage(toUserFacingErrorMessage(error,language==='tr'?'Güvenilir cihaz doğrulaması tamamlanamadı.':'Trusted-device verification could not be completed.'));}finally{setBusy(false);}};
  return <main className="first-run-security-shell"><section className="first-run-card panel"><img src={brandMarkUrl} alt="ParsYuva Aile Yaşam Merkezi"/><span className="eyebrow">{t('security.eyebrow')}</span><h1>{t('security.title')}</h1><p>{twoFactorEnabled?(language==='tr'?'Uygulamayı açmadan önce bu bilgisayarı yerel parolanız ve iki aşamalı doğrulama kodunuzla güvenilir cihaz olarak doğrulayın.':'Before opening the application, verify this computer as a trusted device with your local password and two-factor code.'):t('security.body')}</p>{!twoFactorEnabled&&!setup?<Button tone="primary" onClick={()=>void begin()}>{t('security.start')}</Button>:<div className="first-run-security-form">{setup&&<div className="notes-card"><strong>{t('security.authenticator')}</strong><small>{t('security.key')}: {setup.secret}</small><small>{t('security.uri')}: {setup.otpauthUri}</small><strong>{t('security.recoveryCodes')}</strong><small>{setup.recoveryCodes.join(' · ')}</small><Button onClick={()=>void navigator.clipboard.writeText(setup.recoveryCodes.join('\n'))}>{t('security.copy')}</Button></div>}<label className="first-run-security-field">{t('security.code')}<input inputMode="numeric" autoComplete="one-time-code" value={code} onChange={event=>setCode(event.target.value.replace(/\s+/g,''))}/></label><label className="first-run-security-field">{language==='tr'?'Yerel parolanız':'Your local password'}<input type="password" autoComplete="current-password" minLength={12} value={password} onChange={event=>setPassword(event.target.value)}/></label>{setup&&<label className="check-label"><input type="checkbox" checked={saved} onChange={event=>setSaved(event.target.checked)}/>{t('security.saved')}</label>}<Button tone="primary" disabled={busy||(Boolean(setup)&&!saved)||code.trim().length<6||password.length<12} onClick={()=>void finish()}>{busy?(language==='tr'?'Güvenilir cihaz doğrulanıyor…':'Verifying trusted device…'):t('security.finish')}</Button></div>}{message&&<StatusMessage tone={messageTone}>{message}</StatusMessage>}</section></main>;
}
...
export function InvitationAcceptancePanel({onAccepted,initiallyExpanded=false}:{onAccepted:(state:AuthStateView)=>Promise<void>;initiallyExpanded?:boolean}){
  ...
  const inspect=async()=>{
    ...
    catch(error){setMessage(toUserFacingErrorMessage(error,text('Davet kodu doğrulanamadı.','The invitation code could not be verified.')));}
  ...
  const accept=async(event:FormEvent<HTMLFormElement>)=>{
    ...
    catch(error){setMessage(toUserFacingErrorMessage(error,text('Davet kabul edilemedi.','The invitation could not be accepted.')));}
    finally{setBusy(false);}
  };
...
const windowsHelloOutcomeMessage = (
  ...
  security_epoch_changed: 'Hesap güvenlik dönemi değişti; parola ve iki aşamalı doğrulama ile yeniden kayıt olun.',
...
export function AuthScreen({ auth, onSetup, onLogin, onWindowsHelloLogin, onInvitationAccepted }: { auth: AuthStateView; onSetup:(input:SetupAdminInput)=>Promise<void>; onLogin:(input:LoginInput)=>Promise<void>; onWindowsHelloLogin:(input:LoginWithWindowsHelloInput)=>Promise<void>; onInvitationAccepted:(state:AuthStateView)=>Promise<void> }) {
  ...
  const loginWithHello=async()=>{
    ...
    catch(caught){setError(toUserFacingErrorMessage(caught,language==='tr'?'Windows Hello ile giriş tamamlanamadı.':'Windows Hello sign-in could not be completed.'));}
  ...
  const submit=async()=>{
    ...
    }catch(x){setError(toUserFacingErrorMessage(x,language==='tr'?'İşlem başarısız.':'The operation failed.'));}
    finally{setBusy(false)}
  };
...
export function DigitalLegacyScreen({ snapshot }: { snapshot: FamilyAppSnapshot }) {
  ...
  const run=async(action:()=>Promise<DigitalLegacyPlanView[]>)=>{setBusy(true);setError('');try{const p=await action();setPlans(p);await reload(selectedId);}catch(e){setError(toUserFacingErrorMessage(e,translateDigitalLegacyCopy('İşlem başarısız.',language)));}finally{setBusy(false)}};
  ...
  const submitGrant=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();if(!window.pardus||!selected)return;const d=new FormData(e.currentTarget);const actions=LEGACY_PERMISSION_ACTIONS.filter(x=>d.get(x)==='on');setBusy(true);setError('');try{setGrants(await window.pardus.upsertLegacyGrant({planId:selected.id,resourceType:String(d.get('resourceType')),resourceId:String(d.get('resourceId'))||'*',actions}));e.currentTarget.reset();}catch(x){setError(toUserFacingErrorMessage(x,translateDigitalLegacyCopy('Yetki paketi eklenemedi.',language)));}finally{setBusy(false)}};
  ...
  <section className="legacy-grants"><div className="panel-heading"><div><span className="eyebrow">{grants.length} paket</span><h3>Aktarılacak yetkiler</h3></div></div>{grants.map(g=><div className="grant-row" key={g.id}><strong>{resourceTypeLabel(g.resourceType,language)}</strong><span>{g.actions.map(action=>legacyPermissionActionLabel(action,language)).join(', ')}</span></div>)}<form className="grant-form" onSubmit={e=>void submitGrant(e)}><input name="resourceType" placeholder="Kaynak türü" required/><input name="resourceId" placeholder="Kayıt kimliği veya *" defaultValue="*"/><div className="check-row">{LEGACY_PERMISSION_ACTIONS.map(a=><label key={a}><input type="checkbox" name={a} defaultChecked={a==='read'}/>{legacyPermissionActionLabel(a,language)}</label>)}</div><Button type="submit" disabled={busy}>Yetki paketi ekle</Button></form></section></>:<EmptyState title="Plan seçilmedi" body="Ayrıntıları görmek için soldan bir plan seçin."/>}</article>
...
export function AiGovernanceScreen() {
  ...
  useEffect(()=>{void reload().catch(e=>setError(toUserFacingErrorMessage(e,translateAiGovernanceCopy('İzin merkezi yüklenemedi.',language))));},[]);

  const run=async(action:()=>Promise<void>)=>{setBusy(true);setError('');try{await action();}catch(e){setError(toUserFacingErrorMessage(e,translateAiGovernanceCopy('İşlem tamamlanamadı.',language)));}finally{setBusy(false)}};
  ...
  const panel=<>
    ...
    <section className="workspace-grid">
      ...
      <article className="panel workspace-summary">
        ...
        {preview?.allowedResources.map(r=><div className="context-stat" key={`${r.resourceType}-${r.resourceId}`}><strong>{r.title}</strong><span>{aiResourceTypeLabel(r.resourceType,language)}</span></div>)}
        <h3>Onay geçmişi</h3>{consents.filter(c=>c.resourceType!=='sensitive_data_profile').slice(0,8).map(c=><small key={c.id}>{aiConsentPurposeLabel(c.purpose,language)} · {aiResourceTypeLabel(c.resourceType,language)} · {aiConsentStatusLabel(c.status,language)}</small>)}
      </article>
      <article className="panel workspace-form">
        <span className="eyebrow">Hassasiyet profili</span><h2>Süreli ve açık rıza</h2>
      ...
      <article className="panel workspace-form">
        <span className="eyebrow">Güvenli dışa gönderim</span><h2>Göndermeden önce önizle</h2>
...
export function GovernedFormDraftCenter({visible}:{readonly visible:boolean}){
  ...
  const panel=<Surface className="workspace-summary governed-draft-center"><SectionHeader eyebrow="Güvenli taslak yönetimi" title="Taslak, otomatik kayıt ve geri alma merkezi"/>
...
export function SystemManagementScreen({draftCenterVisible=false,onDraftCenterVisibilityChange=()=>undefined}:{readonly draftCenterVisible?:boolean;readonly onDraftCenterVisibilityChange?:(visible:boolean)=>void}={}){
  const {language}=useLocalization();
  const coreRoleLabel=(value:CoreServiceHealthContract['role'])=>({
    standalone:language==='tr'?'Bağımsız':'Standalone',leader:language==='tr'?'Lider':'Leader',follower:language==='tr'?'Takipçi':'Follower',witness:language==='tr'?'Tanık':'Witness',backup_only:language==='tr'?'Yalnız yedek':'Backup only',maintenance:language==='tr'?'Bakım':'Maintenance'
  } as const)[value];
  const coreLifecycleLabel=(value:CoreServiceHealthContract['lifecycle'])=>({
    starting:language==='tr'?'Başlatılıyor':'Starting',ready:language==='tr'?'Hazır':'Ready',degraded:language==='tr'?'Kısıtlı':'Degraded',stopping:language==='tr'?'Durduruluyor':'Stopping',stopped:language==='tr'?'Durdu':'Stopped'
  } as const)[value];
  ...
  const operationsRefreshRef=useRef<Promise<void>|null>(null);
  const operationsGovernanceRefreshRef=useRef<Promise<void>|null>(null);
  const refreshGovernance=async(coreServiceReady:boolean)=>{
    if(!window.pardus||!coreServiceReady)return;
    if(operationsGovernanceRefreshRef.current){await operationsGovernanceRefreshRef.current;return;}
    const optional=async<T,>(operation:()=>Promise<T>):Promise<T|undefined>=>operation().catch(()=>undefined);
    let task:Promise<void>;
    task=(async()=>{
      const serviceAvailability=await optional(()=>window.pardus!.getPolicyServiceAvailabilityBoundary());
      if(!serviceAvailability)return;
      const network=await optional(()=>window.pardus!.getNetworkEgressBoundary());
      const derived=await optional(()=>window.pardus!.getDerivedDataPolicyBoundary());
      const logging=await optional(()=>window.pardus!.getSensitiveLoggingBoundary());
      const decisionAudit=await optional(()=>window.pardus!.getPolicyDecisionAuditBoundary());
      const deletion=await optional(()=>window.pardus!.getSourceDeletionPropagationBoundary());
      const conformance=await optional(()=>window.pardus!.getPolicyConformanceSuiteBoundary());
      const astGate=await optional(()=>window.pardus!.getPlatformPolicyAstGateBoundary());
      const capabilityGate=await optional(()=>window.pardus!.getPlatformCapabilityManifestGateBoundary());
      const securityProfile=await optional(()=>window.pardus!.getApplicationSecurityProfileGateBoundary());
      const surfaceGovernance=await optional(()=>window.pardus!.getProductSurfaceGovernance());
      const securityPosture=await optional(()=>window.pardus!.getDesktopSecurityPosture());
      setPolicyServiceAvailabilityBoundary(serviceAvailability);setNetworkEgressBoundary(network);setDerivedDataPolicyBoundary(derived);setSensitiveLoggingBoundary(logging);setPolicyDecisionAuditBoundary(decisionAudit);setSourceDeletionPropagationBoundary(deletion);setPolicyConformanceSuiteBoundary(conformance);setPlatformPolicyAstGateBoundary(astGate);setPlatformCapabilityManifestGateBoundary(capabilityGate);setApplicationSecurityProfileGateBoundary(securityProfile);setProductSurfaceGovernance(surfaceGovernance);setDesktopSecurityPosture(securityPosture);
    })().finally(()=>{if(operationsGovernanceRefreshRef.current===task)operationsGovernanceRefreshRef.current=null;});
    operationsGovernanceRefreshRef.current=task;await task;
  };
  const refresh=async()=>{
    if(!window.pardus)return;
    if(operationsRefreshRef.current){await operationsRefreshRef.current;return;}
    setOperationsLoading(true);setOperationsLoadError('');
    const optional=async<T,>(operation:()=>Promise<T>):Promise<T|undefined>=>operation().catch(()=>undefined);
    let task:Promise<void>;
    task=(async()=>{
      const h=await window.pardus!.getSystemHealth();
      const coreHealth=await optional(()=>window.pardus!.getCoreServiceHealth());
      const apiBoundary=await optional(()=>window.pardus!.getCoreServiceApiBoundary());
      const t=await window.pardus!.listBackupTargets();
      const r=await window.pardus!.listBackupRuns(30);
      const p=await window.pardus!.listPerformance(30);
      const tr=await window.pardus!.getPerformanceTrend(performanceHours);
      const bg=await window.pardus!.listBackgroundTasks(30);
      const sc=await window.pardus!.getSchedulerStatus();
      const d=await window.pardus!.listDiagnostics(50);
      const q=await window.pardus!.listQueuedTasks(40);
      const mp=await window.pardus!.getMaintenancePolicy();
      const n=await window.pardus!.listHealthNotifications(40);
      const hs=await window.pardus!.getHealthScore();
      const rh=await window.pardus!.listDiagnosticReports(20);
      const hh=await window.pardus!.listHealthHistory(Math.max(30,healthDays*4));
      const ht=await window.pardus!.getHealthTrend(healthDays);
      const ar=await window.pardus!.listDiagnosticArchives(20);
      const an=await window.pardus!.getPerformanceAnomalies(performanceHours);
      const mr=await window.pardus!.getMaintenanceRecommendations();
      const mhist=await window.pardus!.listMaintenanceHistory(30);
      const exports=await window.pardus!.listExportArtifacts(30);
      const ipc=await window.pardus!.getIpcPerformanceTelemetry();
      const ipcAuthority=await window.pardus!.getIpcAdaptiveBudgetMaintenanceAuthority();
      const ipcRecoveryAuthority=await window.pardus!.getIpcAdaptiveBudgetMaintenanceRecoveryAuthority();
      setHealth(h);setCoreServiceHealth(coreHealth);setCoreServiceApiBoundary(apiBoundary);setTargets(t);setRuns(r);setPerformance(p);setTrend(tr);setTasks(bg);setScheduler(sc);setDiagnostics(d);setQueue(q);setPolicy(mp);setNotifications(n);setHealthScore(hs);setReportHistory(rh);setHealthHistory(hh);setHealthTrend(ht);setArchives(ar);setAnomalies(an);setRecommendations(mr);setMaintenanceHistory(mhist);setExportHistory(exports);setIpcTelemetry(ipc);setIpcMaintenanceAuthority(ipcAuthority);setIpcMaintenanceRecoveryAuthority(ipcRecoveryAuthority);
      if(coreHealth?.lifecycle==='ready')void refreshGovernance(true);
    })().catch(error=>setOperationsLoadError(toUserFacingErrorMessage(error,'Sistem verileri yüklenemedi.'))).finally(()=>{if(operationsRefreshRef.current===task)operationsRefreshRef.current=null;setOperationsLoading(false);});
    operationsRefreshRef.current=task;await task;
  };
  useEffect(()=>{if(activeSystemModule!=='operations')return;void refresh();const timer=setInterval(()=>void refresh(),120_000);return()=>clearInterval(timer);},[activeSystemModule]);
  ...
  const openArchive=async(id:string)=>{if(!window.pardus)return;try{setArchiveId(id);setArchiveContent(await window.pardus.readDiagnosticArchive(id));setVerificationMessage('Arşiv doğrulandı ve geri yükleme yapılmadan açıldı.');}catch(error){setVerificationMessage(toUserFacingErrorMessage(error,'Arşiv açılamadı.'));}};
  ...
  const performFactoryReset=async()=>{if(!window.pardus||!factoryResetReady)return;setFactoryResetBusy(true);setSystemMessage('');try{await window.pardus.factoryResetToInitialState({password:factoryResetPassword,...(factoryResetCode.trim()?{code:factoryResetCode.trim()}:{}),confirmation:'ILK KURULUM ANINA DON'});setSystemMessage('Kişisel veriler siliniyor; uygulama ilk kurulum ekranıyla yeniden başlatılıyor.');}catch(error){setSystemMessage(toUserFacingErrorMessage(error,'İlk kurulum durumuna dönüş başlatılamadı.'));setFactoryResetBusy(false);setFactoryResetPassword('');setFactoryResetCode('');}};
  ...
  const systemModules=[
    ...
    {id:'communication-audit',label:language==='tr'?'İletişim denetimi':'Communication audit'},
    {id:'communication-files',label:language==='tr'?'Dosya paylaşımı':'File sharing'},
    {id:'communication-security',label:language==='tr'?'İletişim güvenliği':'Communication security'},
    {id:'communication-messaging',label:language==='tr'?'Mesajlaşma':'Messaging'},
    {id:'communication-calling',label:language==='tr'?'Arama ve görüşme':'Calling and meetings'},
    {id:'communication-recording',label:language==='tr'?'Kayıt ve saklama':'Recording and retention'},
  ...
  {activeSystemModule==='communication-files'&&<div id="system-module-communication-files"><CommunicationFileSharingPanel/></div>}
  {activeSystemModule==='communication-security'&&<div id="system-module-communication-security"><CommunicationSecurityPanel/></div>}
  {activeSystemModule==='communication-messaging'&&<div id="system-module-communication-messaging"><CommunicationMessagingPanel/></div>}
  {activeSystemModule==='communication-calling'&&<div id="system-module-communication-calling"><CommunicationRealtimeCallingPanel/></div>}
  {activeSystemModule==='communication-recording'&&<div id="system-module-communication-recording"><CommunicationRecordingRetentionPanel/></div>}
  {activeSystemModule==='translation'&&<div id="system-module-translation"><LocalTranslationLanguagePanel/></div>}
  {activeSystemModule==='signed-plugins'&&<div id="system-module-signed-plugins"><SignedPluginPlatformPanel/></div>}
  {activeSystemModule==='operations'&&<div id="system-module-operations" data-loading={operationsLoading?'true':'false'}>{operationsLoading&&<div className="system-operations-progress" aria-busy="true" aria-live="polite"><AsyncStatePanel state="loading" title="Sistem verileri hazırlanıyor" message="Ekran kullanıma açık; operasyon ve politika göstergeleri güvenli sırayla güncelleniyor."/></div>}{operationsLoadError&&<AsyncStatePanel state="error" title="Bazı sistem verileri yüklenemedi" message={operationsLoadError} retryLabel="Yeniden dene" onRetry={refresh}/>}<>
  ...
  <Surface className="workspace-summary"><SectionHeader eyebrow="Masaüstü güvenlik kapanışı" title="Oturum kilidi ve uygulama sertleştirmesi"/><div className="notes-card"><strong>{desktopSecurityPosture?.enforcement==='fail-closed'?'Güvenli retli masaüstü güvenlik sözleşmesi etkin':'Masaüstü güvenlik durumu doğrulanamadı'}</strong><small>Boşta kilit {desktopSecurityPosture?.session.idleTimeoutMinutes??15} dakika · erişilebilir uyarı {desktopSecurityPosture?.session.warningBeforeSeconds??60} saniye · açık form durumu kilitte korunur</small><small>Görüntüleyici {desktopSecurityPosture?.electron.primaryRendererProtocol??'doğrulanamadı'} · korumalı alan ve bağlam ayrımı etkin · gezinme, yeni pencere ve izinler varsayılan reddedilir</small><small>Uygulama güvenlik ayarları zorunlu · komut satırı düğüm seçenekleri kapalı · paket bütünlüğü ve yalnız paket içinden yükleme açık</small></div></Surface>
  <Surface className="workspace-summary"><SectionHeader eyebrow="Ürün yüzeyi gerçeklik kapısı" title="Belge, rota, menü, ekran ve uygulama arayüzü envanteri tek sözleşmede"/><div className="notes-card"><strong>{productSurfaceGovernance?.enforcement==='fail-closed'&&productSurfaceGovernance.unresolvedUnusedRendererApiCount===0?'Ürün yüzeyi zinciri doğrulandı':'Ürün yüzeyi zinciri doğrulanamadı'}</strong><small>{productSurfaceGovernance?.productModuleCount??0} ürün modülü + {productSurfaceGovernance?.governanceSurfaceCount??0} yönetişim yüzeyi = {productSurfaceGovernance?.navigationRouteCount??0} kanonik rota</small><small>Menü {productSurfaceGovernance?.menuEntryCount??0} · ekran {productSurfaceGovernance?.renderedScreenCount??0} · sınıflandırılmış kullanılmayan görüntüleyici arayüzü {productSurfaceGovernance?.classifiedUnusedRendererApiCount??0}</small><small>Çözümlenmemiş arayüz {productSurfaceGovernance?.unresolvedUnusedRendererApiCount??0} · eksik zincir derleme kapanışını güvenli biçimde durdurur · veritabanı göçü gerekmez</small></div></Surface>
  {policyServiceAvailabilityBoundary&&<article className={`panel health-alert ${policyServiceAvailabilityBoundary.mode==='read-write'?'info':policyServiceAvailabilityBoundary.mode==='read-only'?'warning':'critical'}`}><span className="eyebrow">Güvenli erişim sınırı</span><h2>Politika hizmeti çalışma modu</h2><strong>{policyServiceAvailabilityBoundary.mode==='read-write'?'Okuma ve yazma açık':policyServiceAvailabilityBoundary.mode==='read-only'?'Salt okunur — değişiklikler kapalı':'Erişim güvenli biçimde durduruldu'}</strong><p>{policyServiceAvailabilityBoundary.reason}</p><small>İmza {policyServiceAvailabilityBoundary.policyPackageVerified?'doğrulandı':'doğrulanamadı'} · canlılık {policyServiceAvailabilityBoundary.observationFresh?'güncel':'güncel değil'} · istemci bu göstergeden ek yetki türetemez.</small></article>}
  {health&&<div className="stats-grid"><article className="stat-card"><small>Sistem sağlık puanı</small><strong>{healthScore?.score??0}/100</strong><span>{healthScore?.grade==='excellent'?'Mükemmel':healthScore?.grade==='good'?'İyi':healthScore?.grade==='attention'?'Dikkat':'Kritik'}</span></article><article className="stat-card"><small>Genel durum</small><strong>{health.status==='healthy'?'Sağlıklı':health.status==='warning'?'Uyarı':'Kritik'}</strong><span>{health.integrityOk?'SQLite bütünlüğü doğrulandı':'Bütünlük sorunu'}</span></article><article className="stat-card"><small>{language==='tr'?'Temel Hizmet':'Core Service'}</small><strong>{coreServiceHealth?`${coreRoleLabel(coreServiceHealth.role)} · ${coreLifecycleLabel(coreServiceHealth.lifecycle)}`:(language==='tr'?'Bağlantı yok':'No connection')}</strong><span>{coreServiceHealth?`${coreServiceHealth.writable?(language==='tr'?'Yazılabilir':'Writable'):(language==='tr'?'Salt okunur':'Read only')} · ${coreServiceHealth.safeMode?(language==='tr'?'Güvenli mod':'Safe mode'):(language==='tr'?'Normal':'Normal')} · ${coreServiceHealth.policyVersion}`:(language==='tr'?'Sağlık yanıtı alınamadı':'Health response unavailable')}</span></article><article className="stat-card"><small>{language==='tr'?'Temel Hizmet API’si':'Core Service API'}</small><strong>{coreServiceApiBoundary?`${coreServiceApiBoundary.apiVersion} · ${language==='tr'?'Güvenli kapalı':'Fail closed'}`:(language==='tr'?'Doğrulanamadı':'Not verified')}</strong><span>{coreServiceApiBoundary?(language==='tr'?'Sürümlü zarf · uygulama bağı · güncellik · yinelenen istek koruması':'Versioned envelope · application binding · freshness · replay protection'):(language==='tr'?'API sınırı yanıtı alınamadı':'API boundary response unavailable')}</span></article><article className="stat-card"><small>Zamanlayıcı</small><strong>{scheduler?.active?'Etkin':'Kapalı'}</strong><span>{scheduler?.lastCycleAt?`Son tur ${formatDate(scheduler.lastCycleAt,{hour:'2-digit',minute:'2-digit'})}`:'Oturum açılınca çalışır'}</span></article><article className="stat-card"><small>24 saatlik eğilim</small><strong>{trendLabel}</strong><span>CPU %{trend?.averageCpuPercent??0} · RAM %{trend?.averageMemoryPercent??0}</span></article><article className="stat-card"><small>Veri büyümesi</small><strong>{bytes((trend?.databaseGrowthBytes??0)+(trend?.archiveGrowthBytes??0))}</strong><span>{trend?.sampleCount??0} örnek değerlendirildi</span></article></div>}
  <Surface className="workspace-summary"><SectionHeader eyebrow="Ağ çıkış güvenliği" title="İzinli bağlantılar ve sertifika yenileme"/><div className="notes-card"><strong>{networkEgressBoundary?.enforcement==='fail-closed'?'Güvenli retli ağ çıkış politikası etkin':'Ağ çıkış sınırı doğrulanamadı'}</strong><small>Yalnız kayıtlı iptal listesi, kimlik belirteci ve doğrulama anahtarı uç noktaları · {networkEgressBoundary?.minimumTlsVersion??'Şifreli bağlantı sürümü doğrulanamadı'} · çift sertifika sabitleme yenilemesi</small><small>Karşılıklı kimlik doğrulama {networkEgressBoundary?.mutualTlsSupported?'destekleniyor':'doğrulanamadı'} · yönlendirme {networkEgressBoundary?.redirectAllowed===false?'kapalı':'bilinmiyor'} · özel/yerel adresler {networkEgressBoundary?.privateAddressRejected?'reddediliyor':'bilinmiyor'}</small><small>{networkEgressBoundary?.authorizedAdapterCount??0} yetkili bağdaştırıcı · {networkEgressBoundary?.directPrimitiveExceptionCount??0} doğrudan ağ istisnası</small></div></Surface>
  <Surface className="workspace-summary"><SectionHeader eyebrow="Türetilmiş veri güvenliği" title="Kaynak politika mirası ve değişmez soy zinciri"/><div className="notes-card"><strong>{derivedDataPolicyBoundary?.enforcement==='fail-closed'?'Güvenli retli politika mirası etkin':'Türetilmiş veri sınırı doğrulanamadı'}</strong><small>En çok {derivedDataPolicyBoundary?.maximumSourceCount??0} kaynak · {derivedDataPolicyBoundary?.maximumLineageDepth??0} soy derinliği · kaynak erişim politikalarının zorunlu kesişimi</small><small>Hassasiyet düşürme {derivedDataPolicyBoundary?.sensitivityDowngradeAllowed===false?'yasak':'doğrulanamadı'} · erişim genişletme {derivedDataPolicyBoundary?.accessBroadeningAllowed===false?'yasak':'doğrulanamadı'}</small><small>{derivedDataPolicyBoundary?.authorizedRepositoryAdapterCount??0} yetkili bağdaştırıcı · {derivedDataPolicyBoundary?.directAccessExceptionCount??0} doğrudan erişim istisnası · içerik/yol {derivedDataPolicyBoundary?.payloadExposed===false&&derivedDataPolicyBoundary?.persistentPathExposed===false?'açığa çıkarılmıyor':'doğrulanamadı'}</small></div></Surface>
  <Surface className="workspace-summary"><SectionHeader eyebrow="Hassas kayıt güvenliği" title="İçeriksiz kayıt ve tanı sınırı"/><div className="notes-card"><strong>{sensitiveLoggingBoundary?.enforcement==='fail-closed'?'Güvenli retli hassas kayıt politikası etkin':'Hassas kayıt sınırı doğrulanamadı'}</strong><small>Metin tanıma sonucu ve veri içeriği {sensitiveLoggingBoundary?.ocrTextAllowed===false&&sensitiveLoggingBoundary?.payloadAllowed===false?'yasak':'doğrulanamadı'} · keyfi mesaj ve hata yığını {sensitiveLoggingBoundary?.arbitraryMessageAllowed===false&&sensitiveLoggingBoundary?.errorStackAllowed===false?'yasak':'doğrulanamadı'}</small><small>Tanı kaynak metni {sensitiveLoggingBoundary?.diagnosticTextStored===false&&sensitiveLoggingBoundary?.diagnosticSourceTextHashed?'saklanmıyor; yalnız SHA-256 özeti':'doğrulanamadı'} · masaüstü kayıt hedefi {sensitiveLoggingBoundary?.protectedDesktopSinkRequired?'cihaz korumalı':'doğrulanamadı'}</small><small>İzinli üstveri: kimlik · özet · sonuç · ilişkilendirme · sayaç · zaman · sürüm</small></div></Surface>
  <Surface className="workspace-summary"><SectionHeader eyebrow="Değişmez karar denetimi" title="Politika kararlarının denetim zinciri"/><div className="notes-card"><strong>{policyDecisionAuditBoundary?.status==='verified'&&policyDecisionAuditBoundary.enforcement==='fail-closed'?'Korumalı karar zinciri doğrulandı':'Karar denetim zinciri doğrulanamadı'}</strong><small>İzin ve ret kararları {policyDecisionAuditBoundary?.allowedDecisionsRecorded&&policyDecisionAuditBoundary?.deniedDecisionsRecorded?'birlikte kaydediliyor':'doğrulanamadı'} · ret nedeni {policyDecisionAuditBoundary?.denialReasonRequired?'zorunlu':'doğrulanamadı'}</small><small>Politika sürümü ve yükümlülükler {policyDecisionAuditBoundary?.obligationsRecordedExactly?'birebir bağlı':'doğrulanamadı'} · zincir {policyDecisionAuditBoundary?.appendOnly&&policyDecisionAuditBoundary?.hmacSha256Chained?'yalnız eklemeli HMAC-SHA-256':'doğrulanamadı'}</small><small>{policyDecisionAuditBoundary?.auditedEntryCount??0} yeni denetim kaydı · {policyDecisionAuditBoundary?.legacyReceiptEntryCount??0} tarihsel makbuz · istemciye veri içeriği verilmez</small></div></Surface>
  <Surface className="workspace-summary"><SectionHeader eyebrow="Silme ve saklama yayılımı" title="Kaynak, türev, önbellek, kopya ve yedek yaşam döngüsü"/><div className="notes-card"><strong>{sourceDeletionPropagationBoundary?.status==='verified'&&sourceDeletionPropagationBoundary.enforcement==='fail-closed'?'Güvenli retli silme yayılımı doğrulandı':'Silme yayılımı doğrulanamadı'}</strong><small>Metin tanıma · indeks · küçük resim · yapay zekâ hafızası üretim sahibi {sourceDeletionPropagationBoundary?.activeSemanticPersistentOwners===0?'yok ve statik çitle korunuyor':'doğrulanamadı'} · açık metin kopya {sourceDeletionPropagationBoundary?.plaintextReplicaAllowed===false?'yasak':'doğrulanamadı'}</small><small>Çalışma anı önbelleği silme öncesi temizlenir · yönetilen yedek doğrulanmış yeniden yazım ve karantina tamamlanana kadar silme işaretini korur</small><small>Yönetilmeyen ve harici kopyalar ayrı dikkat ve kanıt ister · karantina fiziksel imha sayılmaz · istemciye veri içeriği verilmez</small></div></Surface>
  <Surface className="workspace-summary"><SectionHeader eyebrow="Ortak politika uyumu" title="Windows, Apple profilleri ve hizmetler için tek doğrulama matrisi"/><div className="notes-card"><strong>{policyConformanceSuiteBoundary?.status==='build-verified'&&policyConformanceSuiteBoundary.enforcement==='fail-closed'?'Ortak politika sözleşmesi derleme aşamasında doğrulandı':'Politika uyumu doğrulanamadı'}</strong><small>{policyConformanceSuiteBoundary?.targetCount??0} hedef · {policyConformanceSuiteBoundary?.caseCount??0} aynı vaka · {policyConformanceSuiteBoundary?.totalMatrixAssertions??0} çekirdek değerlendirmesi</small><small>Etkin çalışma hedefi {policyConformanceSuiteBoundary?.deployedRuntimeTargets??0} · yalnız profilli ve yayımlanmamış hedef {policyConformanceSuiteBoundary?.profileOnlyTargets??0}</small><small>Yerel Apple çalıştırması tamamlandı iddiası yoktur; ilgili uygulama yayımlanmadan önce yerel doğrulama zorunludur · istemciye test içeriği verilmez</small></div></Surface>
  <Surface className="workspace-summary"><SectionHeader eyebrow="Kaynak kod güvenlik kapısı" title="Ayrıcalıklı kod yüzeylerinde varsayılan ret denetimi"/><div className="notes-card"><strong>{platformPolicyAstGateBoundary?.status==='build-verified'&&platformPolicyAstGateBoundary.enforcement==='fail-closed'?'TypeScript kaynak ağacı kapısı derleme aşamasında doğrulandı':'Kaynak ağacı kapısı doğrulanamadı'}</strong><small>{platformPolicyAstGateBoundary?.protectedRuleCount??0} kural · {platformPolicyAstGateBoundary?.productionSourceZones??0} üretim bölgesi · {platformPolicyAstGateBoundary?.exactAllowlistEntries??0} birebir izinli yüzey</small><small>Yeni veya eski izin: güvenli ret · genel eşleme: {platformPolicyAstGateBoundary?.wildcardsAllowed===false?'yasak':'doğrulanamadı'} · doğrudan rol yetkilendirmesi: {platformPolicyAstGateBoundary?.directRoleAuthorizationBypasses??'doğrulanamadı'}</small><small>Takma adlar, dinamik içe aktarma, modül yükleme ve hesaplanmış özellikler kaynak ağacında incelenir; bu kapı çalışma anı politikasının yerine geçmez · istemciye kaynak yolu veya izinli liste özeti verilmez</small></div></Surface>
  <Surface className="workspace-summary"><SectionHeader eyebrow="İzin bildirimi güvenlik kapısı" title="Kamera, mikrofon, dosya, metin tanıma, yapay zekâ, konum ve ağ için çift katmanlı ret"/><div className="notes-card"><strong>{platformCapabilityManifestGateBoundary?.status==='build-runtime-verified'&&platformCapabilityManifestGateBoundary.enforcement==='build-and-runtime-fail-closed'?'Derleme ve çalışma anı izin kapısı doğrulandı':'İzin bildirimi doğrulanamadı'}</strong><small>{platformCapabilityManifestGateBoundary?.protectedCapabilityCount??0} kaynak ailesi · {platformCapabilityManifestGateBoundary?.canonicalApplicationCount??0} uygulama · {platformCapabilityManifestGateBoundary?.exactAstSurfaceCount??0} birebir kaynak ağacı yüzeyi</small><small>Eksik veya beklenmeyen izin: güvenli ret · imzalı bildirim özeti ve kimliği doğrulanmış çekirdek hizmet çalışma anı kontrolü zorunlu</small><small>Derleme bildirimi tek başına çalışma anı yetkisi vermez · istemciye kaynak yolu veya bildirim özeti verilmez · mevcut masaüstü kasası sahipliği korunur</small></div></Surface>
  <Surface className="workspace-summary"><SectionHeader eyebrow="Uygulama güvenlik profili" title="ASVS, MASVS, SSDF eşlemesi ve uygulama başına tehdit modeli"/><div className="notes-card"><strong>{applicationSecurityProfileGateBoundary?.status==='build-mapping-verified'&&applicationSecurityProfileGateBoundary.enforcement==='fail-closed'?'Uygulama güvenlik profili derleme aşamasında doğrulandı':'Uygulama güvenlik profili doğrulanamadı'}</strong><small>{applicationSecurityProfileGateBoundary?.mappedApplicationCount??0}/{applicationSecurityProfileGateBoundary?.canonicalApplicationCount??0} uygulama · {applicationSecurityProfileGateBoundary?.threatModelCount??0} tehdit modeli · {applicationSecurityProfileGateBoundary?.mobileMasvsApplicationCount??0} mobil MASVS profili</small><small>ASVS {applicationSecurityProfileGateBoundary?.asvsVersion??'—'} · MASVS {applicationSecurityProfileGateBoundary?.masvsVersion??'—'} · SSDF {applicationSecurityProfileGateBoundary?.ssdfVersion??'—'} · yeni veya eksik profil derleme aşamasında reddedilir</small><small>Eşleme uygunluk sertifikası veya çalışma anı yetkisi değildir · yalnız profilli hedefler yerel olarak doğrulanmış sayılmaz · istemciye kaynak yolu ya da tehdit modeli özeti verilmez</small></div></Surface>
  <div className="content-grid two"><article className="panel"><h2>Yedekleme politikası</h2><form className="form-grid" onSubmit={e=>void saveTarget(e)}><label>Hedef adı<input name="name" required/></label><label>Tür<select name="kind"><option value="local">Yerel</option><option value="external">Harici disk</option><option value="cloud">Bulut klasörü</option></select></label><label className="span-2">Klasör yolu<input name="path" required placeholder="C:\\ParsYuva-Aile-Yasam-Merkezi-Yedek"/></label><label>Zamanlama<select name="schedule"><option value="manual">Manuel</option><option value="hourly">Saatlik</option><option value="daily">Günlük</option><option value="weekly">Haftalık</option><option value="monthly">Aylık</option></select></label><label>Saklanacak yedek<input name="retention" type="number" min="1" max="365" defaultValue="10"/></label><label>Yeniden deneme<input name="retry" type="number" min="0" max="5" defaultValue="2"/></label><Button tone="primary" type="submit">Politikayı kaydet</Button></form></article><article className="panel"><div className="section-heading"><div><h2>Yedek hedefleri</h2><p>Her hedef bağımsız çalışır ve SHA-256 ile doğrulanır.</p></div><Button onClick={()=>void runAllBackups()}>Tümünü çalıştır</Button></div>{backupMessage&&<p className="success-text">{backupMessage}</p>}{targets.length?targets.map(t=><div className="list-row" key={t.id}><div><strong>{t.name}</strong><small>{backupTargetKindLabel(t.kind,language)} · {backupScheduleLabel(t.schedule,language)} · son {t.retentionCount} yedek</small><small>{t.nextRunAt?`Sonraki ${formatDate(t.nextRunAt,{dateStyle:'short',timeStyle:'short'})}`:t.lastError??t.path}</small></div><Button onClick={()=>void runTarget(t.id)} disabled={!t.enabled}>Çalıştır</Button></div>):<EmptyState title="Yedek hedefi tanımlanmadı" body="Yerel, harici veya bulut hedefi ekleyin."/>}</article></div>
  <div className="content-grid two"><article className="panel"><h2>Performans eğilimi</h2><div className="stats-grid"><div className="context-stat"><strong>%{trend?.peakCpuPercent??0}</strong><span>tepe CPU</span></div><div className="context-stat"><strong>%{trend?.peakMemoryPercent??0}</strong><span>tepe RAM</span></div></div><div className="section-heading"><p>Beş dakikalık otomatik örnekleme geçmişi.</p><Button onClick={()=>void sample()}>Şimdi örnek al</Button></div>{performance.slice(0,8).map(x=><div className="list-row" key={x.id}><div><strong>CPU %{x.cpuLoadPercent} · RAM %{x.memoryUsagePercent}</strong><small>DB {bytes(x.databaseBytes)} · Arşiv {bytes(x.archiveBytes)}</small></div><span>{formatDate(x.sampledAt,{hour:'2-digit',minute:'2-digit'})}</span></div>)}</article><article className="panel"><h2>Arka plan görevleri</h2>{tasks.length?tasks.slice(0,10).map(t=><div className="list-row" key={t.id}><div><strong>{t.label}</strong><small>{systemTaskTypeLabel(t.taskType,language)} · {backgroundTaskStatusLabel(t.status,language)}{t.details?` · ${t.details}`:''}</small></div><span>{t.durationMs!=null?`${Math.round(t.durationMs/1000)} sn`:'çalışıyor'}</span></div>):<EmptyState title="Görev kaydı yok" body="Yedekleme ve bakım görevleri burada izlenecek."/>}</article></div>
...
export function DataLifecycleSettings({auth}:{auth:AuthStateView}){
  ...
  const [password,setPassword]=useState('');
  const [code,setCode]=useState('');
  const strongAuthenticationReady=password.length>0;
  ...
  const run=async(action:()=>Promise<DataLifecycleRecordView[]>,success:string)=>{try{setRecords(await action());setMessage(success);}catch(error){setMessage(toUserFacingErrorMessage(error,'Veri yaşam döngüsü işlemi tamamlanamadı.'));}};
  const createPolicy=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();if(!window.pardus)return;try{setPolicies(await window.pardus.createDataRetentionPolicy({name:policyName,resourceTypes:selectedTypes,retentionDays,graceDays,requiresStrongAuth:true}));setMessage('Saklama politikası oluşturuldu.');}catch(error){setMessage(toUserFacingErrorMessage(error,'Saklama politikası oluşturulamadı.'));}};
...
KANIT İPTAL LİSTESİ ${signerIssuerId} ${sequenceNumber}`,'');if(confirmation===null)return;try{const result=await window.pardus.applyExternalBackupEvidenceRevocationList({signerIssuerId,listId,sequenceNumber,thisUpdate,nextUpdate,entries,signatureBase64,...(sourceUrl?{sourceUrl}:{}),confirmation,password,...(code.trim()?{code:code.trim()}:{})});setExternalEvidenceRevocationLists(result.lists);setExternalEvidenceIssuers(result.issuers);const [evidence,copies]=await Promise.all([window.pardus.listExternalBackupDestructionEvidence(undefined,200),window.pardus.listExternalBackupCopies(200)]);setExternalEvidence(evidence);setExternalCopies(copies);setMessage('İmzalı sağlayıcı iptal listesi doğrulandı; eski sıra numarası ve süresi dolmuş liste korumaları uygulandı.');}catch(error){setMessage(toUserFacingErrorMessage(error,'İmzalı iptal listesi uygulanamadı.'));}};
  ...
  const panel=<section className="data-lifecycle-settings span-2"><h3>Veri saklama ve güvenli silme</h3><p>Varsayılan işlem geri alınabilir arşivlemedir. Kalıcı imha ancak saklama süresi ve geri alma penceresi tamamlandıktan sonra güçlü doğrulamayla çalışır.</p>
    ...
    {auth.role==='family_admin'&&<article className="panel"><div className="section-heading"><div><h4>Otomatik temiz yedek yeniden yazımı</h4><p>Yönetilen yedeklerde imha yayılımı, saklama süresi dolmuş kayıtlar için önce doğrulanmış temiz yedek oluşturur; eski kopyayı geri alınabilir karantinaya taşır. Kayıtlı hedefler dışındaki manuel veya yönetilmeyen kopyalara dokunulmaz.</p></div><Button onClick={()=>void propagateBackups()} disabled={propagationRunning||(cleanRewriteStatus?.dueRecords??0)===0}>{propagationRunning?'Yeniden yazılıyor…':'Şimdi çalıştır'}</Button></div>{cleanRewriteStatus&&<div className="stats-grid"><div className="context-stat"><strong>{cleanRewriteStatus.dueRecords}</strong><span>süresi dolan kayıt</span></div><div className="context-stat"><strong>{cleanRewriteStatus.enabledTargets}</strong><span>etkin hedef</span></div><div className="context-stat"><strong>{backupWorkflowStatusLabel(cleanRewriteStatus.policy.state,language)}</strong><span>kalıcı durum</span></div><div className="context-stat"><strong>{cleanRewriteStatus.policy.nextAttemptAt?formatDate(cleanRewriteStatus.policy.nextAttemptAt,{dateStyle:'short',timeStyle:'short'}):'Hazır'}</strong><span>sonraki deneme</span></div></div>}<form className="form-grid" onSubmit={event=>void saveCleanRewritePolicy(event)}><label><input type="checkbox" checked={cleanRewriteEnabled} onChange={event=>setCleanRewriteEnabled(event.target.checked)}/> Otomatik politika etkin</label><label>Saklama süresi (gün)<input type="number" min={1} max={3650} value={cleanRewriteRetentionDays} onChange={event=>setCleanRewriteRetentionDays(Number(event.target.value))}/></label><Button type="submit" disabled={!strongAuthenticationReady}>Politikayı kaydet</Button></form><small>Otomatik politika yalnız zamanlanmış çevrimi kontrol eder; “Şimdi çalıştır” manuel komutu politika kapalıyken de kullanılabilir. Manuel hata sonrası 1 saat, otomatik hata sonrası 6 saat geri çekilme uygulanır; yüksek sistem yükünde işlem güvenli biçimde ertelenir.</small>{cleanRewriteStatus?.policy.lastError&&<small className="warning-text">Son tanı: {cleanRewriteStatus.policy.lastError}</small>}{lastPropagation&&<small>Son yayılım: {backupWorkflowStatusLabel(lastPropagation.status,language)} · {lastPropagation.refreshedTargets}/{lastPropagation.targetCount} hedef · {lastPropagation.quarantinedArtifacts} karantina · {formatDate(lastPropagation.completedAt,{dateStyle:'short',timeStyle:'short'})}</small>}<div className="section-heading"><div><h4>Kalıcı çalışma geçmişi</h4><p>Her denemenin başlangıç ve sonuç durumu güvenli biçimde kaydedilir; böylece yarım kalan işlemler açıkça görülebilir.</p></div></div>{cleanRewriteRuns.length?cleanRewriteRuns.slice(0,8).map(run=><div className="list-row" key={run.id}><div><strong>{backupWorkflowStatusLabel(run.status,language)} · {run.trigger==='manual'?'manuel':'otomatik'}</strong><small>{run.dueRecords} kayıt · {run.enabledTargets} hedef · kesim {formatDate(run.retentionCutoff,{dateStyle:'short',timeStyle:'short'})}</small>{run.error&&<small className="warning-text">{run.error}</small>}</div><span>{formatDate(run.completedAt??run.startedAt,{dateStyle:'short',timeStyle:'short'})}</span></div>):<EmptyState title="Temiz yedek çalışma geçmişi yok" body="İlk sahiplenilmiş deneme başladığında kalıcı çalışma kaydı burada gösterilir."/>}</article>}
    ...
    <small>Güvenli silme mümkün olan en kapsamlı şekilde uygulanır. Depolama aygıtı, eşitlenen klasörler veya yönetilmeyen yedekler fiziksel kopyaları bir süre daha tutabilir.</small>{message&&<StatusMessage>{message}</StatusMessage>}
  </section>;
...
export function WindowsHelloScreen({auth}:{auth:AuthStateView}) {
  ...
  const refresh=async()=>{
    ...
    catch(error){setMessageTone('danger');setMessage(toUserFacingErrorMessage(error,translateWindowsHelloCopy('Windows Hello durumu okunamadı.',language)));}
    finally{setBusy('');}
  };
  ...
  const enroll=async()=>{
    ...
    }catch(error){setMessageTone('danger');setMessage(toUserFacingErrorMessage(error,translateWindowsHelloCopy('Windows Hello kaydı tamamlanamadı.',language)));}
    finally{setBusy('');}
  };
  const reauthenticate=async(useFallback:boolean)=>{
    ...
    try{
      ...
      await refresh();
    }catch(error){setMessageTone('danger');setMessage(toUserFacingErrorMessage(error,translateWindowsHelloCopy('Yeniden doğrulama tamamlanamadı.',language)));}
    finally{setBusy('');}
  };
  const panel=<>
    ...
    <section className="workspace-grid windows-hello-workspace">
      ...
      <Surface className="workspace-form"><SectionHeader eyebrow="Kayıt" title={state?.enrolled?'Windows Hello kaydını yenile':'Windows Hello’yu etkinleştir'}/><p>Kayıt için mevcut yerel parola ve hesabınızda etkinse iki aşamalı doğrulama kodu denetlenir. Ardından Windows Hello penceresi yalnız bu düğmeye bastığınızda açılır.</p><label>Cihaz adı<input value={displayName} maxLength={120} onChange={event=>setDisplayName(event.target.value)}/></label><label>Mevcut yerel parola<input type="password" autoComplete="current-password" maxLength={1024} value={enrollmentPassword} onChange={event=>setEnrollmentPassword(event.target.value)}/></label><label>İki aşamalı doğrulama / kurtarma kodu <small>{auth.twoFactorEnabled?'gerekli':'etkin değil'}</small><input autoComplete="one-time-code" maxLength={256} value={enrollmentCode} onChange={event=>setEnrollmentCode(event.target.value)}/></label><Button tone="primary" disabled={busy!==''||!enrollmentPassword||(auth.twoFactorEnabled&&!enrollmentCode.trim())||state?.availability!=='available'} onClick={()=>void enroll()}>{busy==='enroll'?'Windows Hello bekleniyor…':state?.enrolled?'Kaydı güvenli biçimde yenile':'Windows Hello’yu kaydet'}</Button></Surface>
      <Surface className="span-2 windows-hello-reauth"><SectionHeader eyebrow="Kritik işlem doğrulaması" title="Yeniden doğrula"/><p>Windows Hello iptal edilirse parola otomatik gönderilmez. Yedek doğrulama yalnız aşağıdaki ayrı düğmeyle ve açıkça yazdığınız bilgilerle çalışır.</p><div className="button-row"><Button tone="primary" disabled={busy!==''||!state?.enrolled||state.availability!=='available'} onClick={()=>void reauthenticate(false)}>{busy==='reauth'?'Windows Hello bekleniyor…':'Windows Hello ile yeniden doğrula'}</Button></div><div className="windows-hello-fallback"><label>Yerel parola<input type="password" autoComplete="current-password" maxLength={1024} value={fallbackPassword} onChange={event=>setFallbackPassword(event.target.value)}/></label><label>İki aşamalı doğrulama / kurtarma kodu<input autoComplete="one-time-code" maxLength={256} value={fallbackCode} onChange={event=>setFallbackCode(event.target.value)}/></label><Button disabled={busy!==''||!fallbackPassword||(auth.twoFactorEnabled&&!fallbackCode.trim())} onClick={()=>void reauthenticate(true)}>{busy==='fallback'?'Doğrulanıyor…':'Hello olmazsa parola ile devam et'}</Button></div></Surface>
    </section>
...
export function PrivacyOwnershipCenter() {
  ...
  const mutate=async(key:string,revision:number,run:(operation:{clientOperationId:string;expectedRevision:number})=>Promise<unknown>,success:string)=>{
    ...
    catch(error){setMessage(error instanceof Error?`${toUserFacingErrorMessage(error,'İşlem tamamlanamadı; yeniden deneme kimliği korundu.')} Aynı işlem kimliği ve özgün revizyonla yeniden deneyebilirsiniz.`:'İşlem tamamlanamadı; yeniden deneme kimliği korundu.');}
    finally{setBusy('');}
  };
  ...
  const createIncident=()=>{if(!window.pardus||!center)return;if(!confirm('Bu işlem yerel inceleme kaydını açar, mevcut oturumu kapatır ve yeniden giriş gerektirir. Devam edilsin mi?'))return;const targetId=center.key.accountId;return mutate('incident:create',0,operation=>window.pardus!.createPrivacyIncident({...operation,title:'Yerel erişim incelemesi',severity:'medium',suspectedAt:asIsoDateTime(new Date().toISOString()),actions:[{action:'revoke_local_session_authority',targetId}],evidenceReferenceIds:[]}),'Yerel inceleme kaydı açıldı; mevcut oturum kapatıldı.');};
  const updateIncident=(incident:PrivacyOwnershipControlCenterView['incidents'][number],status:'contained_locally'|'resolved'|'cancelled')=>window.pardus&&mutate(`incident:update:${incident.id}:${status}`,incident.revision,operation=>window.pardus!.updatePrivacyIncident({...operation,incidentId:incident.id,status,...(status==='resolved'?{resolutionNote:'Yerel inceleme kapatıldı; uzaktan silme, cihaz yönetimi veya ağ üzerinden teslim yapılmadı.'}:{})}),status==='contained_locally'?'Olay yalnız yerel sınırlama durumuna alındı.':status==='cancelled'?'Olay yalnız yerel kayıtta iptal edildi.':'Yerel olay çözüm notuyla kapatıldı; uzaktan silme, cihaz yönetimi veya ağ teslimi yapılmadı.');
  const simulate=async()=>{if(!window.pardus||!simulationAccountId)return;setBusy('simulate');try{const result=await window.pardus.simulatePrivacyPermission({targets:[{subjectAccountId:asUserId(simulationAccountId),resourceType:'privacy_inventory',resourceId:center?.key.ownerPersonId??'owner',action:'read',purpose:'general',occurredAt:asIsoDateTime(new Date().toISOString())}]});setSimulationResult(result.items.map(item=>`${item.visible?(language==='tr'?'Görünür':'Visible'):(language==='tr'?'Görünmez':'Hidden')}: ${language==='tr'?'Yetki kurallarına göre değerlendirildi':'Evaluated according to permission rules'}`).join(' · '));}catch(error){setSimulationResult(toUserFacingErrorMessage(error,language==='tr'?'Simülasyon tamamlanamadı.':'Simulation could not be completed.'));}finally{setBusy('');}};
  const exportEncrypted=async()=>{if(!window.pardus||!exportRequestId||!exportPassphrase)return;setBusy('export');setMessage('');try{const result=await window.pardus.exportEncryptedPrivacyData({requestId:exportRequestId,passphrase:exportPassphrase});await load();setMessage(`${result.fileName} doğrulandı · ${result.artifactSizeBytes} bayt · ağ teslimi yapılmadı.`);}catch(error){setMessage(toUserFacingErrorMessage(error,'Şifreli dışa aktarım tamamlanamadı.'));}finally{setExportPassphrase('');setBusy('');}};
  ...
  const panel=<section className="privacy-ownership-center" aria-labelledby="privacy-ownership-title">
    ...
    <div className="privacy-ownership-grid">
      <section><h3>Tutulan veri</h3><p className="privacy-inventory-summary" role="status">{center.dataInventory.length} kategori · toplam {inventoryRecordCount} yerel kayıt</p>{center.dataInventory.length===0?<AsyncStatePanel state="empty" title="Kayıt yok" message="Yerel envanterde gösterilecek veri bulunmuyor."/>:<div id="privacy-data-inventory-list" className="privacy-inventory-list">{(inventoryExpanded?center.dataInventory:center.dataInventory.slice(0,8)).map(item=><div className="list-row" key={item.id}><div><strong>{item.displayName}</strong><small>{privacyCategoryLabel(item.category,language)} · {item.recordCount} kayıt · {privacyStorageScopeLabel(item.storageScope,language)} · türetilmiş {item.derivedDataCount}</small></div></div>)}</div>}{center.dataInventory.length>8&&<Button aria-expanded={inventoryExpanded} aria-controls="privacy-data-inventory-list" onClick={()=>setInventoryExpanded(value=>!value)}>{inventoryExpanded?'Özeti göster':`Tüm ${center.dataInventory.length} kategoriyi göster`}</Button>}</section>
      <section><h3>AI hafıza denetimi</h3>{firstMemory?<><p><strong>{firstMemory.title}</strong> · revizyon {firstMemory.revision} · {aiMemoryStatusLabel(firstMemory.status,language)}</p><label>Başlık<input value={memoryTitle} onChange={event=>setMemoryTitle(event.target.value)}/></label><label>Düzeltme<textarea value={memoryStatement} onChange={event=>setMemoryStatement(event.target.value)}/></label><label>Saklama sonu<input type="datetime-local" value={retentionUntil} onChange={event=>setRetentionUntil(event.target.value)}/></label><div className="button-row"><Button disabled={Boolean(busy)} onClick={()=>void correctMemory()}>Düzelt</Button><Button disabled={Boolean(busy)} onClick={()=>void restrictMemory()}>Yalnız sahibine sınırla</Button><Button disabled={Boolean(busy)} onClick={()=>void expireMemory()}>Süre koy</Button><Button tone="danger" disabled={Boolean(busy)} onClick={()=>void deleteMemory()}>Yerel silme iste</Button></div></>:<AsyncStatePanel state="empty" title="AI hafıza kaydı yok" message="Yalnız yerel olarak gözlenen AI hafıza kayıtları burada gösterilir."/>}</section>
      <section><h3>Erişim geçmişi</h3>{center.accessHistory.slice(0,8).map(item=><div className="list-row" key={item.id}><div><strong>{item.actorDisplayName} · {accessDecisionLabel(item.decision,language)}</strong><small>{accessPurposeLabel(item.purpose,language)} · {formatDate(item.occurredAt,{dateStyle:'short',timeStyle:'short'})}</small></div></div>)}</section>
      <section><h3>Cihaz ve yerel işleme gözlemi</h3><p>Güvenilir cihaz, açık oturum anlamına gelmez. Apple eşzamanlama ile AI/OCR/çeviri yalnız yerelde gözlendiyse gösterilir.</p>{center.localDeviceActivity.map(item=><small key={item.id}>{item.displayName} · {deviceTrustStatusLabel(item.trustStatus,language)} · {deviceSessionStatusLabel(item.locallyObservedSession,language)} · Apple {appleSyncStatusLabel(item.appleSyncStatus,language)}</small>)}{center.localProcessingObservations.map(item=><small key={item.id}>{localProcessingKindLabel(item.kind,language)} · {localProcessingStatusLabel(item.status,language)} · ağ teslimi gözlenmedi</small>)}</section>
      <section><h3>Veri hakları, saklama ve şifreli dışa aktarım</h3><div className="button-row"><Button disabled={Boolean(busy)||!retentionUntil} title={!retentionUntil?'Önce saklama bitiş zamanını seçin.':undefined} onClick={()=>void createRightsRequest()}>Saklama talebi</Button><Button disabled={Boolean(busy)} onClick={()=>void createErasureRequest()}>Yerel silme talebi</Button><Button disabled={Boolean(busy)} onClick={()=>void createExportRequest('encrypted_export')}>Kendi verim için şifreli talep</Button><Button disabled={Boolean(busy)} onClick={()=>void createExportRequest('legacy_export')}>Dijital miras için şifreli talep</Button></div><small>Şifreli paket; sahip kapsamındaki yapılandırılmış kayıtları ve gizlilik merkezi verisini içerir. Arşiv ikili dosyaları, sahipliği kesin bağlanamayan aile etkinlikleri ve açıkça seçilmemiş form taslakları dahil edilmez.</small>{center.rightsRequests.slice(0,8).map(item=><div className="list-row privacy-action-row" key={item.id}><div><strong>{dataRightsKindLabel(item.kind,language)} · {dataRightsStatusLabel(item.status,language)}</strong><small>{resourceTypeLabel(item.scopeResourceType,language)} · dış kopya silme garantisi yok</small>{item.resolutionNote&&<small>{item.resolutionNote}</small>}</div><div className="button-row">{item.status==='requested'&&<Button disabled={Boolean(busy)} onClick={()=>void updateRightsRequest(item,'in_review')}>İncelemeye al</Button>}{item.status==='in_review'&&!['encrypted_export','legacy_export'].includes(item.kind)&&<Button disabled={Boolean(busy)} onClick={()=>void updateRightsRequest(item,'locally_completed')}>Yerel incelemeyi tamamla</Button>}{['requested','in_review'].includes(item.status)&&<Button disabled={Boolean(busy)} onClick={()=>void updateRightsRequest(item,'cancelled')}>Yerel talebi iptal et</Button>}</div></div>)}<label>Şifreli dışa aktarım talebi<select value={exportRequestId} onChange={event=>setExportRequestId(event.target.value)}><option value="">Aktif şifreli talep yok</option>{activeExportRequests.map(item=><option key={item.id} value={item.id}>{dataRightsKindLabel(item.kind,language)} · {dataRightsStatusLabel(item.status,language)}</option>)}</select></label><label>Şifreli dosya parolası<input type="password" autoComplete="new-password" value={exportPassphrase} onChange={event=>setExportPassphrase(event.target.value)}/></label><Button disabled={Boolean(busy)||exportPassphrase.length<12||!exportRequestId} onClick={()=>void exportEncrypted()}>Yerel şifreli dosya oluştur</Button></section>
      <section><h3>Türetilmiş veri zinciri</h3><p>İçerik gösterilmez; yalnız kaynak bağı, tür ve yerel silme yayılım durumu görünür.</p>{center.derivedDataLineage.slice(0,8).map(item=><small key={item.id}>{derivedKindLabel(item.derivedKind,language)} · derinlik {item.depth} · {deletionPropagationLabel(item.deletionPropagation,language)}</small>)}</section>
      <section><h3>Olay ve yerel sınırlama</h3><Button disabled={Boolean(busy)} onClick={()=>void createIncident()}>Yerel inceleme olayı aç</Button>{center.incidents.slice(0,5).map(item=><div className="list-row privacy-action-row" key={item.id}><div><strong>{item.title} · {privacyIncidentSeverityLabel(item.severity,language)} · {privacyIncidentStatusLabel(item.status,language)}</strong><small>Uzaktan silme, cihaz yönetimi veya ağ teslimi yapılmadı</small>{item.resolutionNote&&<small>{item.resolutionNote}</small>}</div><div className="button-row">{item.status==='open'&&<Button disabled={Boolean(busy)} onClick={()=>void updateIncident(item,'contained_locally')}>Yerel sınırlamaya al</Button>}{['open','contained_locally'].includes(item.status)&&<Button disabled={Boolean(busy)} onClick={()=>void updateIncident(item,'resolved')}>Yerel çözümü kaydet</Button>}{['open','contained_locally'].includes(item.status)&&<Button disabled={Boolean(busy)} onClick={()=>void updateIncident(item,'cancelled')}>Yerel olayı iptal et</Button>}</div></div>)}</section>
...
export function IdentityAccessCredentialCenter({trustedDevices}:{trustedDevices:readonly TrustedDeviceView[]}) {
  ...
  const load=async()=>{
    ...
    }catch(error){loadGuard.current.commit(ticket,()=>{setMessage(toUserFacingErrorMessage(error,'Kimlik ve yetki merkezi yüklenemedi.'));setPhase('error');});}
  };
  useEffect(()=>{void load();return()=>loadGuard.current.invalidate('identity-access-center');},[]);
  useEffect(()=>{const target=center?.companionRecipientTrustedDeviceId??'';const eligible=trustedDevices.some(item=>!item.revokedAt&&item.id===target);if(eligible&&companionDeviceId!==target)setCompanionDeviceId(target);else if(!eligible&&companionDeviceId)setCompanionDeviceId('');},[center?.companionRecipientTrustedDeviceId,trustedDevices,companionDeviceId]);
  ...
  const stableOperation=async(key:string,expectedRevision:number,operationKind:IdentityAccessOperationKind):Promise<PendingIdentityOperation>=>{
    ...
    const bridge=identityAccessBridge();if(!bridge)throw new Error(language==='tr'?'Güvenli işlem bağlantısı kullanılamıyor.':'The secure operation connection is unavailable.');
  ...
  const runMutation=async(key:string,expectedRevision:number,run:(operation:PendingIdentityOperation)=>Promise<unknown>,success:string)=>{
    ...
    catch(error){setMessage(error instanceof Error?`${toUserFacingErrorMessage(error,language==='tr'?'İşlem tamamlanamadı; güvenli yeniden deneme bilgisi korundu.':'The action could not be completed; secure retry information was preserved.')} ${language==='tr'?'Aynı işlemi yeniden deneyebilirsiniz.':'You can retry the same action.'}`:(language==='tr'?'İşlem tamamlanamadı; güvenli yeniden deneme bilgisi korundu.':'The action could not be completed; secure retry information was preserved.'));}
    finally{setBusy('');}
  };
  ...
  const registerPasskey=async()=>{
    ...
    try{
      ...
      if(!response)throw new Error(language==='tr'?'Geçiş anahtarı kaydı güvenli biçimde sürdürülemedi.':'The passkey registration could not continue safely.');
      ...
      pendingOperations.current.delete(key);setMessage(language==='tr'?'Geçiş anahtarı bu bilgisayarda doğrulandı; gizli anahtar ve biyometrik veri uygulamaya aktarılmadı.':'The passkey was verified on this computer; no private key or biometric data was shared with the app.');await load();
    }catch(error){setMessage(error instanceof Error?`${toUserFacingErrorMessage(error,language==='tr'?'Geçiş anahtarı kaydı tamamlanamadı.':'The passkey registration could not be completed.')} ${language==='tr'?'Güvenli yeniden deneme bilgileri korundu.':'Secure retry information was preserved.'}`:(language==='tr'?'Geçiş anahtarı kaydı tamamlanamadı.':'The passkey registration could not be completed.'));}
    finally{setBusy('');}
  };
  ...
  const authenticatePasskey=async(passkey:IdentityAccessCredentialCenterView['passkeys'][number])=>{
    ...
    try{
      ...
      const response=payload.response;
      if(!response)throw new Error(language==='tr'?'Geçiş anahtarı doğrulaması güvenli biçimde sürdürülemedi.':'Passkey verification could not continue safely.');
      ...
      pendingOperations.current.delete(key);setMessage(language==='tr'?'Geçiş anahtarı ve cihaz onayı bu bilgisayarda doğrulandı.':'The passkey and device approval were verified on this computer.');await load();
    }catch(error){setMessage(error instanceof Error?`${toUserFacingErrorMessage(error,language==='tr'?'Geçiş anahtarı doğrulaması tamamlanamadı.':'Passkey verification could not be completed.')} ${language==='tr'?'Güvenli yeniden deneme bilgileri korundu.':'Secure retry information was preserved.'}`:(language==='tr'?'Geçiş anahtarı doğrulaması tamamlanamadı.':'Passkey verification could not be completed.'));}
    finally{setBusy('');}
  };
  ...
  const recoverLostPasskey=async(credentialId:string,revision:number)=>{
    ...
    try{
      ...
      pendingOperations.current.delete(key);setMessage(language==='tr'?'Güçlü yerel doğrulama tamamlandı; kayıp geçiş anahtarının yetkisi ve eski yerel oturumlar kapatıldı.':'Strong local verification was completed; the lost passkey authorization and older local sessions were closed.');await load();
    }catch(error){setMessage(error instanceof Error?`${toUserFacingErrorMessage(error,language==='tr'?'Kayıp geçiş anahtarı kapatılamadı.':'The lost passkey could not be disabled.')} ${language==='tr'?'Güvenli yeniden deneme bilgisi korundu; parola kullanıyorsanız parolanızı yeniden girin.':'Secure retry information was preserved; enter your password again if you are using it.'}`:(language==='tr'?'Kayıp geçiş anahtarı kapatılamadı.':'The lost passkey could not be disabled.'));}
  ...
  const startFederated=async(provider:FederatedIdentityProvider)=>{
    ...
    try{
      ...
      setFederatedFlow({ceremony:payload.ceremony,clientOperationId:operation.clientOperationId});setMessage(language==='tr'?'Güvenli hesap bağlantısı hazır. Sağlayıcıdan uygulamaya dönüş korumalı bölümde doğrulanır; sağlayıcı kullanılabilirliği garanti edilmez.':'The secure account connection is ready. The return from the provider is verified in the protected part of the app; provider availability is not guaranteed.');
    }
    catch(error){setMessage(error instanceof Error?`${toUserFacingErrorMessage(error,language==='tr'?'Hesap bağlantısı başlatılamadı.':'The account connection could not be started.')} ${language==='tr'?'Güvenilir sağlayıcı doğrulaması yoksa bağlantı kapalı kalır.':'The connection remains disabled without trusted provider verification.'}`:(language==='tr'?'Hesap bağlantısı başlatılamadı.':'The account connection could not be started.'));}
    finally{setBusy('');}
  };
  ...
  const completeFederated=async()=>{
    ...
    try{
      ...
      await bridge.completeFederatedIdentityLink({expectedRevision:operation.expectedRevision,clientOperationId:operation.clientOperationId,provider:payload.ceremony.provider,flowId:payload.ceremony.flowId,confirmation:'FEDERATED KIMLIGI BAGLA'});pendingOperations.current.delete(key);setFederatedFlow(null);setMessage(language==='tr'?'Hesap bağlantısı ve sağlayıcıdan güvenli dönüş doğrulandı.':'The account connection and secure return from the provider were verified.');await load();
    }
    catch(error){const callbackPending=error instanceof Error&&String(error).includes('[OIDC_CALLBACK_NOT_CAPTURED]');const detail=toUserFacingErrorMessage(error,language==='tr'?'Sağlayıcıdan uygulamaya güvenli dönüş henüz doğrulanamadı.':'The secure return from the provider has not yet been verified.');if(!callbackPending){pendingOperations.current.delete(key);setFederatedFlow(null);}setMessage(callbackPending?`${detail} ${language==='tr'?'Sağlayıcıdan uygulamaya döndükten sonra yeniden deneyin.':'Return to the app from the provider and try again.'}`:`${detail} ${language==='tr'?'Yeni bir güvenli hesap bağlantısı başlatın.':'Start a new secure account connection.'}`);}
    finally{setBusy('');}
  };
  ...
  const issueTemporary=async()=>{
    ...
    try{
      ...
      const result=await bridge.issueTemporaryVerifiableCredential({expectedRevision:operation.expectedRevision,clientOperationId:operation.clientOperationId,kind:payload.kind,purpose:payload.purpose,audienceReference:payload.audienceReference,disclosedClaims:payload.disclosedClaims,notBefore:payload.notBefore,expiresAt:payload.expiresAt,confirmation:'GECICI YETKI BELGESI OLUSTUR'});pendingOperations.current.delete(key);setIssuedTemporary(result.issued??null);if(result.issued)setQrPayload(result.issued.qrPayload);setMessage(result.issued?(language==='tr'?'Yalnız gerekli bilgileri içeren, süreli ve çevrimdışı doğrulanabilir karekod oluşturuldu.':'A time-limited QR code containing only the necessary information was created for offline verification.'):(language==='tr'?'Aynı işlem daha önce kaydedildi; güvenlik nedeniyle karekod yeniden gösterilmedi.':'The same action was recorded before; the QR code was not shown again for security.'));await load();
    }
    catch(error){setMessage(error instanceof Error?`${toUserFacingErrorMessage(error,language==='tr'?'Geçici yetki belgesi oluşturulamadı; ilk seçimler korundu.':'The temporary authorization could not be created; the original selections were preserved.')} ${language==='tr'?'İlk hedef, paylaşılacak bilgiler, süre ve aynı işlem kimliği yeniden deneme için korundu.':'The original target, shared details, duration, and action identity were preserved for retry.'}`:(language==='tr'?'Geçici yetki belgesi oluşturulamadı; ilk seçimler korundu.':'The temporary authorization could not be created; the original selections were preserved.'));}
    finally{setBusy('');}
  };
  ...
  const verifyTemporary=async()=>{
    ...
    try{const result=await bridge.verifyTemporaryVerifiableCredential({qrPayload:qrPayload.trim(),expectedAudienceReference:temporaryAudience.trim()});setVerification(result);setMessage(language==='tr'?'Yerel imza, süre, hedef ve yalnız gerekli bilgiler çevrimdışı kontrol edildi; resmî kimlik ve uzaktaki iptal durumu garanti edilmez.':'The local signature, duration, target, and necessary details were checked offline; official identity and remote revocation status are not guaranteed.');}
    catch(error){setVerification(null);setMessage(toUserFacingErrorMessage(error,language==='tr'?'Karekod çevrimdışı doğrulanamadı.':'The QR code could not be verified offline.'));}
    finally{setBusy('');}
  };
  ...
  const createCompanion=async(requestedMode:'read_only'|'write')=>{
    ...
    try{const operation=await stableOperation(key,0,'companion_snapshot_create');const result=await bridge.createReadOnlyCompanionSnapshot({clientOperationId:operation.clientOperationId,trustedDeviceId:companionDeviceId,requestedMode,...(version===undefined?{}:{knownSourceVersion:version}),confirmation:'SALT OKUNUR ESLIKCI KOPYASI OLUSTUR'});pendingOperations.current.delete(key);setCompanionResult(result);setMessage(result.status==='snapshot_ready'?(language==='tr'?'Şifreli salt okunur kopya bu bilgisayarda hazırlandı; ağ üzerinden gönderilmedi.':'The encrypted read-only copy was prepared on this computer and was not sent over the network.'):(language==='tr'?'Yazma veya eski sürüm isteği güvenli biçimde reddedildi.':'The write or outdated-version request was safely rejected.'));if(result.status==='snapshot_ready')await load();}
    catch(error){setMessage(error instanceof Error?`${toUserFacingErrorMessage(error,language==='tr'?'Eşlikçi cihaz kopyası oluşturulamadı.':'The companion-device copy could not be created.')} ${language==='tr'?'Aynı işlemle yeniden deneyebilirsiniz.':'You can retry the same operation.'}`:(language==='tr'?'Eşlikçi cihaz kopyası oluşturulamadı.':'The companion-device copy could not be created.'));}
    finally{setBusy('');}
  };

  if(phase==='loading')return localizeIdentityAccessNode(<AsyncStatePanel state="loading" title="Kimlik ve geçici yetki merkezi yükleniyor" message="Geçiş anahtarları, bağlı hesaplar, süreli yetkiler ve eşlikçi cihaz kopyaları hazırlanıyor."/>,language);
  ...
  const companionEligibleDevices=activeDevices.filter(item=>item.id===center.companionRecipientTrustedDeviceId);
  const panel=<section className="identity-access-center" aria-label="Kimlik, geçiş anahtarı ve geçici yetki merkezi" aria-describedby="identity-access-description">
    <SectionHeader eyebrow="Güvenli kimlik ve erişim" title="Kimlik, geçiş anahtarı ve geçici yetki merkezi" action={<Button disabled={Boolean(busy)} onClick={()=>void load()}>Merkezi yenile</Button>}/>
    <p id="identity-access-description">Bu merkez yalnız bu bilgisayarda doğrulanan kimlik ve yetki kayıtlarını yönetir. Biyometrik veri veya geçiş anahtarının gizli anahtarı uygulamaya gelmez; resmî kimlik ya da hukuki yetki doğrulaması yapılmaz. Sağlayıcı kullanılabilirliği, uzaktaki iptal durumu ve ağ üzerinden teslim garanti edilmez.</p>
    <div className="identity-truth-strip" role="list" aria-label="Kimlik ve yetki sınırları"><span role="listitem">Gizli anahtar saklanmaz</span><span role="listitem">Biyometrik veri saklanmaz</span><span role="listitem">Erişim anahtarı içeriği gösterilmez</span><span role="listitem">Yazma işlemleri yalnız bu bilgisayarda yapılır</span></div>
    ...
    <div className="identity-access-grid">
      <section className="identity-access-card"><h3>Geçiş anahtarı ve cihaz doğrulaması</h3><p>Geçiş anahtarı yalnız sizin onayınızla kullanılır. Uygulama biyometrik örnek istemez, yakalamaz veya saklamaz.</p><label>Geçiş anahtarı adı<input value={passkeyName} maxLength={120} onChange={event=>setPasskeyName(event.target.value)}/></label><Button tone="primary" disabled={Boolean(busy)||passkeyName.trim().length<2} onClick={()=>void registerPasskey()}>{busy==='passkey:register'?'Cihaz onayı bekleniyor…':'Bu cihazda geçiş anahtarı oluştur'}</Button>{center.passkeys.length===0?<EmptyState title="Geçiş anahtarı yok" body="Bu hesapta kayıtlı geçiş anahtarı bulunmuyor."/>:center.passkeys.map(item=><div className="identity-record" key={item.id}><div><strong>{item.displayName} · {item.status==='active'?'Etkin':'İptal'}</strong><small>Kullanım sayısı {item.signCount} · güvenlik dönemi {item.securityEpoch}</small><small>Gizli anahtar saklanmaz · biyometrik veri saklanmaz · cihaz doğrulama ayrıntısı saklanmaz</small></div>{item.status==='active'&&<div className="button-row"><Button disabled={Boolean(busy)} onClick={()=>void authenticatePasskey(item)}>Geçiş anahtarıyla doğrula</Button><Button tone="danger" disabled={Boolean(busy)} onClick={()=>void runMutation(`passkey:revoke:${item.id}`,item.revision,operation=>identityAccessBridge()!.revokePasskey({expectedRevision:operation.expectedRevision,clientOperationId:operation.clientOperationId,credentialId:item.id,reason:'manual',confirmation:'PASSKEY YETKISINI IPTAL ET'}),'Geçiş anahtarının yerel yetkisi iptal edildi.')}>İptal et</Button></div>}</div>)}<fieldset><legend>Kayıp geçiş anahtarını kapatma</legend><label>Etkin geçiş anahtarı<select value={lostPasskeyId} onChange={event=>setLostPasskeyId(event.target.value)}><option value="">Geçiş anahtarı seçin</option>{activePasskeys.map(item=><option value={item.id} key={item.id}>{item.displayName}</option>)}</select></label><label>Güçlü yerel yeniden doğrulama<select value={recoveryMethod} onChange={event=>setRecoveryMethod(event.target.value as 'windows_hello'|'password_fallback')}><option value="windows_hello">Windows Hello</option><option value="password_fallback">Hesap parolası</option></select></label>{recoveryMethod==='password_fallback'&&<div className="identity-recovery-fallback"><label>Hesap parolası<input type="password" autoComplete="current-password" value={recoveryPassword} onChange={event=>setRecoveryPassword(event.target.value)}/></label><label>İkinci faktör kodu <small>(etkinse)</small><input inputMode="numeric" autoComplete="one-time-code" value={recoverySecondFactorCode} maxLength={256} onChange={event=>setRecoverySecondFactorCode(event.target.value)}/></label></div>}<Button tone="danger" disabled={Boolean(busy)||!lostPasskeyId||(recoveryMethod==='password_fallback'&&!recoveryPassword)} onClick={()=>{const item=activePasskeys.find(value=>value.id===lostPasskeyId);if(item)void recoverLostPasskey(item.id,item.revision);}}>{recoveryMethod==='windows_hello'?'Windows Hello ile doğrula ve yerel yetkiyi kapat':'Parola ile doğrula ve yerel yetkiyi kapat'}</Button><small>Güvenlik doğrulaması uygulamanın korumalı bölümünde üretilir. Bu işlem uzaktaki cihazı yönetmez veya silmez; yalnız bu bilgisayardaki geçiş anahtarı yetkisini ve eski oturumları kapatır.</small></fieldset></section>
      <section className="identity-access-card"><h3>Bağlı hesaplar</h3><p>Yalnız güvenilir biçimde yapılandırılmış hesap sağlayıcıları listelenir. Bağlantı, uygulamaya güvenli dönüş ve sağlayıcı imzası doğrulandıktan sonra etkin olur.</p>{configuredProviders.length===0?<AsyncStatePanel state="empty" title="Yapılandırılmış hesap sağlayıcısı yok" message="Apple, Google veya Microsoft için güvenilir tam yapılandırma bulunmadığından bağlantı başlatılamaz; sağlayıcı kullanılabilirliği iddia edilmez."/>:<div className="identity-provider-list">{configuredProviders.map(provider=>{const linked=center.federatedLinks.some(item=>item.provider===provider.id&&item.status==='linked');const flowActive=federatedFlow?.ceremony.provider===provider.id;return <div className="identity-provider" key={provider.id}><div><strong>{federatedProviderLabels[provider.id]}</strong><small>{provider.productionReady?'Yapılandırma bulundu · uygulamaya dönüşten sonra doğrulanacak':'Yapılandırma bulundu · canlı hesap henüz doğrulanmadı'}</small></div><Button disabled={Boolean(busy)||linked||flowActive} onClick={()=>void startFederated(provider.id)}>{linked?'Bağlı':flowActive?'Güvenli bağlantı açık':'Güvenli bağlantıyı başlat'}</Button></div>;})}</div>}{federatedFlow&&<div className="identity-ceremony"><strong>{federatedProviderLabels[federatedFlow.ceremony.provider]} güvenli bağlantısı</strong><small>Son: {formatDate(federatedFlow.ceremony.expiresAt,{dateStyle:'short',timeStyle:'short'})} · dönüş doğrulaması şifreli kasada korunuyor</small><a className="button default" href={federatedFlow.ceremony.authorizationUrl} target="_blank" rel="noreferrer">Sağlayıcı sayfasını aç</a><p>Sağlayıcıdaki işlem tamamlanınca uygulamaya dönüş yalnız korumalı bölümde doğrulanır; geçici bağlantı bilgileri ekranda gösterilmez.</p><Button tone="primary" disabled={Boolean(busy)} onClick={()=>void completeFederated()}>Uygulamaya dönüşü doğrula ve bağla</Button></div>}{center.federatedLinks.length===0?<EmptyState title="Bağlı hesap yok" body="Erişim anahtarı içeriği ekranda veya yerel işlem kayıtlarında gösterilmez."/>:center.federatedLinks.map(item=><div className="identity-record" key={item.id}><div><strong>{federatedProviderLabels[item.provider]} · {item.status==='linked'?'Canlı doğrulandı':'İptal'}</strong><small>{item.grantedScopes.length} izin · erişim bilgileri şifreli kasada · teslim garantisi yok</small></div>{item.status==='linked'&&<Button tone="danger" disabled={Boolean(busy)} onClick={()=>void runMutation(`federated:unlink:${item.id}`,item.revision,operation=>identityAccessBridge()!.unlinkFederatedIdentity({expectedRevision:operation.expectedRevision,clientOperationId:operation.clientOperationId,linkId:item.id,confirmation:'FEDERATED KIMLIK BAGINI KALDIR'}),'Bağlı hesap ve yerel erişim yetkisi iptal edildi.')}>Bağı kaldır</Button>}</div>)}</section>
      <section className="identity-access-card identity-temporary-card">
        <h3>Geçici doğrulanabilir yetkiler</h3>
        <p>{language==='tr'?'Tür, amaç ve paylaşılabilecek bilgiler önceden sınırlandırılmıştır. Yalnız zorunlu bilgiler ve sizin doldurduğunuz isteğe bağlı bilgiler imzalı karekoda eklenir. Bu karekod resmî kimlik veya hukuki yetki belgesi değildir.':'The type, purpose, and shareable details are limited in advance. Only required details and optional details you provide are added to the signed QR code. This QR code is not an official identity or legal authorization document.'}</p>
        <ValidationSummary issues={temporaryValidation}/>
        <div className="identity-form-grid"><label>Yetki türü<select value={temporaryKind} onChange={event=>{setTemporaryKind(event.target.value as TemporaryCredentialKind);setTemporaryValidation([]);}}>{temporaryKindOptions.map(item=><option key={item.kind} value={item.kind}>{item.label}</option>)}</select></label><label>Amaç<input readOnly value={temporaryPurposeLabel(TEMPORARY_CREDENTIAL_PURPOSE_BY_KIND[temporaryKind],language)}/></label><label id="temporary-audience-label">Hedef kişi / bağlam<input id="temporary-audience" value={temporaryAudience} maxLength={160} onChange={event=>setTemporaryAudience(event.target.value)}/></label><label id="temporary-expiry-label">Bitiş zamanı<input id="temporary-expiry" type="datetime-local" value={temporaryExpiresAt} onChange={event=>setTemporaryExpiresAt(event.target.value)}/></label></div>
        <fieldset><legend>{language==='tr'?'Paylaşılacak bilgiler':'Details to share'}</legend><div className="temporary-claim-grid">{currentRules.allowed.map(key=><label key={key}>{temporaryClaimLabels[key]} {(currentRules.required as readonly TemporaryCredentialClaimKey[]).includes(key)?<strong aria-label="zorunlu">*</strong>:<small>isteğe bağlı</small>}<input id={`temporary-claim-${key}`} maxLength={256} value={temporaryClaims[key]??''} onChange={event=>setTemporaryClaims(current=>({...current,[key]:event.target.value}))}/></label>)}</div></fieldset>
        <Button tone="primary" disabled={Boolean(busy)} onClick={()=>void issueTemporary()}>{language==='tr'?'Süreli, imzalı karekod oluştur':'Create a time-limited signed QR code'}</Button>
        {issuedTemporary&&<div className="issued-credential" role="status"><strong>{language==='tr'?'Karekod hazır':'QR code ready'} · {issuedTemporary.qrPayloadBytes} {language==='tr'?'bayt':'bytes'}</strong><small>{language==='tr'?'Yalnız seçilen bilgiler · özel imza anahtarı gösterilmez · ağ teslimi yapılmaz':'Selected details only · private signing key is not shown · no network delivery'}</small><code>{issuedTemporary.qrPayload}</code><Button onClick={()=>void navigator.clipboard.writeText(issuedTemporary.qrPayload)}>{language==='tr'?'Karekod verisini kopyala':'Copy QR code data'}</Button></div>}
        <h4>{language==='tr'?'Çevrimdışı karekod doğrulama':'Offline QR code verification'}</h4>
        <label>{language==='tr'?'Karekod verisi':'QR code data'}<textarea rows={5} maxLength={5462} value={qrPayload} onChange={event=>setQrPayload(event.target.value.replace(/\s+/gu,''))}/></label>
        <Button disabled={Boolean(busy)||!qrPayload.trim()} onClick={()=>void verifyTemporary()}>{language==='tr'?'İmza ve süreyi çevrimdışı doğrula':'Verify signature and time offline'}</Button>
        {verification&&<StatusMessage tone={verification.decision==='accepted_locally'?'success':verification.decision==='rejected'?'danger':'warning'}>{temporaryVerificationDecisionLabel(verification.decision,language)} · {language==='tr'?'imza':'signature'} {verification.signatureValid?(language==='tr'?'geçerli':'valid'):(language==='tr'?'geçersiz':'invalid')} · {language==='tr'?'süre':'time'} {verification.expired?(language==='tr'?'dolmuş':'expired'):(language==='tr'?'uygun':'valid')} · {temporaryRevocationStatusLabel(verification.revocationStatus,language)} · {language==='tr'?'uzak iptal güncelliği garanti edilmez':'remote revocation status is not guaranteed'}</StatusMessage>}
        {center.temporaryCredentials.length===0?<EmptyState title={language==='tr'?'Geçici yetki yok':'No temporary authorization'} body={language==='tr'?'Oluşturulan belgeler süre, amaç ve yalnız gerekli bilgilerle burada görünür.':'Created authorizations appear here with their duration, purpose, and necessary details.'}/>:center.temporaryCredentials.map(item=><div className="identity-record" key={item.id}><div><strong>{temporaryKindOptions.find(option=>option.kind===item.kind)?.label} · {item.status==='active'?(language==='tr'?'Etkin':'Active'):(language==='tr'?'İptal':'Revoked')}</strong><small>{temporaryPurposeLabel(item.purpose,language)} · {item.disclosedClaimKeys.length} {language==='tr'?'alan · son':'fields · until'} {formatDate(item.expiresAt,{dateStyle:'short',timeStyle:'short'})}</small><small>{language==='tr'?'Yerel imza ve süre doğrulaması · uzak iptal güncelliği garanti edilmez':'Local signature and time verification · remote revocation status is not guaranteed'}</small></div>{item.status==='active'&&<Button tone="danger" disabled={Boolean(busy)} onClick={()=>void runMutation(`temporary:revoke:${item.id}`,item.revision,operation=>identityAccessBridge()!.revokeTemporaryVerifiableCredential({expectedRevision:operation.expectedRevision,clientOperationId:operation.clientOperationId,credentialId:item.id,reason:'Kullanıcının yerel geçici yetki iptali',confirmation:'GECICI YETKI BELGESINI IPTAL ET'}),'Geçici yetki yerel olarak iptal edildi; dağıtılmış kopyalarda anlık iptal garantisi yoktur.')}>Yerel yetkiyi iptal et</Button>}</div>)}
      </section>
      <section className="identity-access-card"><h3>Salt okunur eşlikçi cihaz kopyası</h3><p>Bu bilgisayar ana kayıt yeridir. Eşlikçi cihaz yalnız şifreli, süreli ve salt okunur bir kopya alabilir; uzaktan değişiklik veya ağ üzerinden teslim yapılmaz.</p>{companionEligibleDevices.length===0?<AsyncStatePanel state="empty" title="Eşlikçi cihaz doğrulanmadı" message="Şifreli kopya oluşturma, güvenilir bir cihaz anahtarı doğrulanana kadar kapalıdır."/>:<><label>Hedef güvenilir cihaz<select value={companionDeviceId} onChange={event=>setCompanionDeviceId(event.target.value)}>{companionEligibleDevices.map(item=><option value={item.id} key={item.id}>{item.displayName}{item.current?' · bu cihaz':''}</option>)}</select></label><label>Bilinen kaynak sürümü <small>(isteğe bağlı)</small><input type="number" min={0} step={1} value={knownSourceVersion} onChange={event=>setKnownSourceVersion(event.target.value)}/></label><div className="button-row"><Button tone="primary" disabled={Boolean(busy)||!companionDeviceId} onClick={()=>void createCompanion('read_only')}>Şifreli salt okunur kopya oluştur</Button><Button disabled={Boolean(busy)||!companionDeviceId} onClick={()=>void createCompanion('write')}>Yazma isteğinin reddini denetle</Button></div></>}{companionResult&&<StatusMessage tone={companionResult.status==='snapshot_ready'?'success':'warning'}>{companionResult.status==='snapshot_ready'?`${companionStatusLabel(companionResult.status,language)} · kaynak sürümü ${companionResult.sourceVersion} · ${companionResult.envelopeBytes} bayt · ağ üzerinden gönderilmedi`:`${companionStatusLabel(companionResult.status,language)} · güncel kaynak sürümü ${companionResult.currentSourceVersion} · uzaktan değişiklik kabul edilmez`}</StatusMessage>}{companionResult?.status==='snapshot_ready'&&<Button onClick={()=>void navigator.clipboard.writeText(companionResult.encryptedEnvelopeBase64Url)}>Şifreli kopyayı kopyala</Button>}{center.companionSnapshots.length===0?<EmptyState title="Eşlikçi cihaz kopyası yok" body="Hazırlanan kopyalar yalnız bu bilgisayarda listelenir; teslim edildiği iddia edilmez."/>:center.companionSnapshots.slice(0,6).map(item=><div className="identity-record" key={item.id}><div><strong>Kaynak sürümü {item.sourceVersion} · belge düzeni {item.schemaVersion}</strong><small>{formatDate(item.generatedAt,{dateStyle:'short',timeStyle:'short'})} · {item.envelopeBytes} bayt · salt okunur</small></div></div>)}</section>
    </div>{busy&&<StatusMessage tone="info">İşlem sürüyor; ikinci gönderim kilitli ve güvenli yeniden deneme bilgisi korunuyor.</StatusMessage>}{message&&<StatusMessage>{message}</StatusMessage>}
  </section>;
...
function SettingsSecurity({auth,accessibility,onAccessibilityChange,onFamilyDataChanged}:{auth:AuthStateView;accessibility:AccessibilityPreferences;onAccessibilityChange:(next:AccessibilityPreferences)=>void;onFamilyDataChanged:()=>Promise<void>}) {
  ...
  const changeLanguagePreference=async(preference:UiLanguagePreference)=>{
    ...
    catch(error){setMessage(toUserFacingErrorMessage(error,language==='tr'?'Dil tercihi kaydedilemedi.':'The language preference could not be saved.'));setLanguagePreferenceBusy(false);}
  };
  useEffect(()=>{
    let cancelled=false;
    const load=async()=>{
    ...
    void load().catch(error=>{if(!cancelled)setMessage(toUserFacingErrorMessage(error,language==='tr'?'Güvenlik modülü yüklenemedi.':'The security module could not be loaded.'));});
  ...
  const exportFull=async()=>{
    ...
    try{
      ...
      setMessage(toUserFacingErrorMessage(err,'Tam yedek oluşturulamadı.'));
    }
  };
  const inspectFull=async()=>{
    ...
    try{
      ...
      setMessage(toUserFacingErrorMessage(err,'Yedek incelenemedi.'));
    }
  };
  const restoreFull=async()=>{
    ...
    try{
      ...
      setMessage(r.canceled?'Geri yükleme iptal edildi.':'Yedek geri yüklendi. Uygulama yeniden başlatılıyor; tüm cihaz güvenleri iptal edildiği için parola ve gerekiyorsa iki aşamalı doğrulama ile tekrar giriş istenecek.');
    }catch(err){
      setMessage(toUserFacingErrorMessage(err,'Geri yükleme başarısız.'));
    }
  };
  const changePassword=async(e:FormEvent)=>{e.preventDefault();try{if(!window.pardus)return;await window.pardus.changePassword({currentPassword,newPassword});setCurrentPassword('');setNewPassword('');setMessage('Parola başarıyla değiştirildi.');}catch(err){setMessage(toUserFacingErrorMessage(err,'Parola değiştirilemedi.'));}};
  const begin2fa=async()=>{try{if(window.pardus)setTwoFactor(await window.pardus.beginTwoFactorSetup());}catch(err){setMessage(toUserFacingErrorMessage(err,'İki aşamalı doğrulama başlatılamadı.'));}};
  const enable2fa=async()=>{try{if(!window.pardus)return;await window.pardus.enableTwoFactor({code:twoFactorCode});setTwoFactor(null);setTwoFactorCode('');setMessage('İki aşamalı doğrulama etkinleştirildi. Kurtarma kodlarını güvenli yerde saklayın.');}catch(err){setMessage(toUserFacingErrorMessage(err,'İki aşamalı doğrulama etkinleştirilemedi.'));}};
  const disable2fa=async()=>{try{if(!window.pardus)return;await window.pardus.disableTwoFactor({password:currentPassword,code:twoFactorCode});setTwoFactorCode('');setMessage('İki aşamalı doğrulama kapatıldı.');}catch(err){setMessage(toUserFacingErrorMessage(err,'İki aşamalı doğrulama kapatılamadı.'));}};
  const trustDevice=async()=>{try{if(!window.pardus)return;setDevices(await window.pardus.trustCurrentDevice({password:currentPassword,code:twoFactorCode,displayName:deviceName.trim()||'Bu bilgisayar'}));setMessage('Bu cihaz güvenilir cihaz olarak kaydedildi.');}catch(err){setMessage(toUserFacingErrorMessage(err,'Cihaz güvenilir olarak kaydedilemedi.'));}};
  const reauthorizeDeviceAfterRecovery=async()=>{try{if(!window.pardus)return;const result=await window.pardus.reauthorizeCurrentDeviceAfterRecovery({password:currentPassword,code:twoFactorCode,confirmation:DEVICE_REAUTHORIZATION_CONFIRMATION,displayName:deviceName.trim()||'Bu bilgisayar'});setDevices(result.devices);setSecurityReceipt(result.receipt);setDeviceReauthorized(true);setDeviceReauthorizationConfirmation('');setCurrentPassword('');setTwoFactorCode('');setSecurityReceiptHistory(await window.pardus.listSecurityEventReceipts(20));setMessage(`Cihaz güvenlik dönemi ${result.receipt.securityEpoch} için yeniden yetkilendirildi. İmzalı güvenlik makbuzu oluşturuldu${result.receiptArchived?' ve yerel geçmişe kaydedildi':' ancak yerel geçmişe kaydedilemedi'}.`);}catch(err){setMessage(toUserFacingErrorMessage(err,'Cihaz yeniden yetkilendirilemedi.'));}};
  const revokeDevice=async(id:string)=>{if(window.pardus)setDevices(await window.pardus.revokeTrustedDevice(id));};
  const setLiveLocationConsent=async(status:'granted'|'revoked')=>{try{if(!window.pardus)return;setPrivacyCenter(await window.pardus.setLiveLocationConsent({status,explicitConsent:true,...(status==='granted'?{durationMinutes:liveLocationDuration}:{})}));setMessage(status==='granted'?'Canlı konum paylaşım yetkisi süreli olarak açıldı; bu uygulama konum iletimi yapmaz.':'Canlı konum paylaşım rızası derhal iptal edildi.');}catch(err){setMessage(toUserFacingErrorMessage(err,'Rıza güncellenemedi.'));}};
  const shutdownLostDevice=async()=>{try{if(!window.pardus||!lostDeviceId)return;const result=await window.pardus.shutdownLostDevice({trustedDeviceId:lostDeviceId,password:lostDevicePassword,...(lostDeviceCode.trim()?{code:lostDeviceCode.trim()}:{}),confirmation:'KAYIP CİHAZ YETKİLERİNİ KAPAT'});setMessage(`Yerel yetki kapatıldı: ${result.revokedTrustedDeviceCount} cihaz, ${result.revokedOfflineLeaseCount} kira ve ${result.revokedConsentCount} rıza. Uzaktan silme/MDM/ağ teslimi yapılmadı.`);setLostDevicePassword('');setLostDeviceCode('');setLostDeviceConfirmation('');globalThis.location.reload();}catch(err){setMessage(toUserFacingErrorMessage(err,'Kayıp cihaz yetkileri kapatılamadı.'));}};
  ...
  const previewFamilyImport=async()=>{
    ...
    }catch(error){setImportPreview(null);setMessage(toUserFacingErrorMessage(error,'Aile verisi ön izlemesi oluşturulamadı.'));}
  ...
  const applyFamilyImport=async()=>{
    ...
    }catch(error){setMessage(toUserFacingErrorMessage(error,'Aile verisi uygulanamadı; işlem geri alındı.'));}
    finally{setImportBusy(false);}
  };
  const rollbackFamilyImport=async(batch:FamilyDataImportBatchView)=>{
    ...
    }catch(error){setMessage(toUserFacingErrorMessage(error,'İçe aktarma geri alınamadı.'));}
    finally{setImportBusy(false);}
  };
  ...
  return <Surface className="security-center">
    ...
    <section className="language-preference-card" aria-label={language==='tr'?'Uygulama dili tercihi':'Application language preference'}>
      ...
      <p>{language==='tr'?'İlk açılışta Windows sistem dili kullanılır. Buradan yaptığınız seçim bu cihazdaki sonraki açılışlarda korunur.':'Windows system language is used on first launch. A choice made here is preserved for future launches on this device.'}</p>
      <label>{language==='tr'?'Dil tercihi':'Language preference'}<select value={languagePreference} disabled={languagePreferenceBusy} onChange={event=>void changeLanguagePreference(event.target.value as UiLanguagePreference)}><option value="system">{language==='tr'?'Sistem dilini kullan':'Use system language'}</option><option value="tr">Türkçe</option><option value="en">English</option></select></label>
      <small>{language==='tr'?`Algılanan sistem dili: ${bootstrap.systemLocale} · etkin dil: ${bootstrap.locale}`:`Detected system language: ${bootstrap.systemLocale} · active language: ${bootstrap.locale}`}</small>
    </section>
    ...
    <p>Oturum: {auth.displayName??'Yönetici'} · İki aşamalı doğrulama {auth.twoFactorEnabled?'açık':'kapalı'}</p>
    <section className="privacy-control-center">
      ...
      <label>Yerel parola<input type="password" autoComplete="current-password" maxLength={1024} value={lostDevicePassword} onChange={e=>setLostDevicePassword(e.target.value)}/></label><label>İki aşamalı doğrulama / kurtarma kodu<input maxLength={256} value={lostDeviceCode} onChange={e=>setLostDeviceCode(e.target.value)}/></label><label>Onay ifadesi<input value={lostDeviceConfirmation} onChange={e=>setLostDeviceConfirmation(e.target.value)} placeholder="KAYIP CİHAZ YETKİLERİNİ KAPAT"/></label>
    ...
    <div className="security-grid">
      ...
      <section className="family-import-panel">
        ...
        <p>{language==='tr'?'Seçtiğiniz aile yedek dosyası önce yalnız okunur incelenir. Yapısı ve kayıt çakışmaları doğrulanmadan aile alanına eklenmez.':'The selected family backup is inspected in read-only mode first. It is not added to the family space until its structure and record conflicts are verified.'}</p>
        <div className="button-row"><Button onClick={()=>void previewFamilyImport()} disabled={importBusy}>{importBusy?(language==='tr'?'İşleniyor…':'Processing…'):(language==='tr'?'Yedek dosyası seç ve ön izle':'Choose a backup and preview')}</Button><Button onClick={()=>void refreshImports()} disabled={importBusy}>{language==='tr'?'Geçmişi yenile':'Refresh history'}</Button></div>
        {importPreview&&<div className={`family-import-preview ${importPreview.valid?'valid':'invalid'}`}>
          <div className="family-import-heading"><div><strong>{importPreview.fileName}</strong><small>{(importPreview.fileSizeBytes/1048576).toFixed(2)} MB · {language==='tr'?'dosya bütünlüğü doğrulandı':'file integrity verified'}</small></div><span>{importPreview.valid?(language==='tr'?'Doğrulandı':'Verified'):(language==='tr'?'Engellendi':'Blocked')}</span></div>
          ...
          {importPreview.issues.length>0&&<div className="family-import-issues">{importPreview.issues.slice(0,12).map((item,index)=><small className={item.severity} key={`${item.code}-${index}`}>{item.severity==='error'?'!':'△'} {familyImportIssueLabel(item.code,item.severity,language)}</small>)}</div>}
          {importPreview.valid&&<><label>Yönetici parolası<input type="password" autoComplete="current-password" value={importPassword} onChange={event=>setImportPassword(event.target.value)}/></label><label>İki aşamalı doğrulama / kurtarma kodu<input value={importCode} onChange={event=>setImportCode(event.target.value)} placeholder={auth.twoFactorEnabled?'Gerekli':'Etkin değil'}/></label><Button tone="primary" disabled={importBusy||!importPassword} onClick={()=>void applyFamilyImport()}>Doğrula ve atomik uygula</Button></>}
        </div>}
        <div className="family-import-history"><h4>Son içe aktarmalar</h4>{importBatches.length===0?<small>Henüz içe aktarma yapılmadı.</small>:importBatches.slice(0,8).map(batch=><div className="family-import-batch" key={batch.id}><div><strong>{batch.sourceFileName}</strong><small>{formatDate(batch.appliedAt,{dateStyle:'short',timeStyle:'short'})} · {batch.totalCreatedRecords} yeni · {batch.totalReusedRecords} eşleşen</small><small>Durum: {batch.status==='applied'?'Uygulandı':batch.status==='rolled_back'?'Geri alındı':'Geri alma engellendi'} · dosya bütünlüğü doğrulandı</small>{batch.rollbackBlockers.map((blocker,index)=><small className="error" key={`${batch.id}-${index}`}>{familyImportIssueLabel('rollback_blocked','error',language)}</small>)}</div>{batch.rollbackAvailable&&<Button tone="danger" disabled={importBusy||!importPassword} onClick={()=>void rollbackFamilyImport(batch)}>Geri al</Button>}</div>)}</div>
      </section>
      <section><h3>Parola ve iki aşamalı doğrulama</h3><form className="form-grid" onSubmit={(e)=>void changePassword(e)}><label>Mevcut parola<input type="password" value={currentPassword} onChange={e=>setCurrentPassword(e.target.value)} required /></label><label>Yeni parola<input type="password" minLength={12} value={newPassword} onChange={e=>setNewPassword(e.target.value)} required /></label><Button type="submit">Parolayı değiştir</Button></form><div className="button-row">{auth.twoFactorEnabled?<Button disabled={!currentPassword||!twoFactorCode} onClick={()=>void disable2fa()}>İki aşamalı doğrulamayı kapat</Button>:<Button onClick={()=>void begin2fa()}>İki aşamalı doğrulama kurulumunu başlat</Button>}</div>{twoFactor&&<div className="notes-card"><small>Kimlik doğrulayıcı anahtarı: {twoFactor.secret}</small><small>Kurulum URI: {twoFactor.otpauthUri}</small><small>Kurtarma kodları: {twoFactor.recoveryCodes.join(' · ')}</small></div>}<label>İki aşamalı doğrulama / kurtarma kodu<input value={twoFactorCode} onChange={e=>setTwoFactorCode(e.target.value)} /></label>{twoFactor&&<Button tone="primary" onClick={()=>void enable2fa()}>Doğrula ve etkinleştir</Button>}</section>
      <section><h3>Güvenilir cihazlar</h3><p>Hesap güvenlik sürümü: <strong>{auth.securityEpoch??0}</strong> · Oturum güvenlik sürümü: <strong>{auth.sessionSecurityEpoch??0}</strong></p><label>Cihaz adı<input value={deviceName} onChange={e=>setDeviceName(e.target.value)}/></label>{auth.deviceReauthorizationRequired&&!deviceReauthorized?<div className="notes-card"><strong>Kurtarma sonrası yeniden yetkilendirme gerekiyor</strong><small>Eski cihaz güveni yeni güvenlik sürümüne taşınmaz. Parola, iki aşamalı doğrulama ve bu cihazın korumalı anahtarı yeniden doğrulanır.</small><label>Onay metni<input value={deviceReauthorizationConfirmation} onChange={e=>setDeviceReauthorizationConfirmation(e.target.value)} placeholder={DEVICE_REAUTHORIZATION_CONFIRMATION}/></label><Button tone="primary" onClick={()=>void reauthorizeDeviceAfterRecovery()} disabled={!canSubmitDeviceReauthorization({twoFactorEnabled:auth.twoFactorEnabled===true,password:currentPassword,code:twoFactorCode,confirmation:deviceReauthorizationConfirmation})}>Cihazı yeniden yetkilendir</Button></div>:<Button onClick={()=>void trustDevice()} disabled={!auth.twoFactorEnabled||!currentPassword||!twoFactorCode}>Bu cihazı güvenilir yap</Button>}{securityReceipt&&<div className="notes-card"><strong>Güvenlik doğrulama kaydı</strong><small>Bu cihaz yeniden yetkilendirildi · güvenlik sürümü {securityReceipt.securityEpoch}</small><small>Bütünlük ve korumalı imza denetimi hazır.</small><Button onClick={()=>void navigator.clipboard.writeText(JSON.stringify(securityReceipt,null,2))}>Doğrulama kaydını kopyala</Button></div>}{devices.map(device=><div className="list-row" key={device.id}><div><strong>{device.displayName}{device.current?' · Bu cihaz':''}</strong><small>Güvenlik sürümü: {device.securityEpoch} · Son kullanım: {formatDate(device.lastSeenAt,{dateStyle:'short',timeStyle:'short'})}</small></div>{!device.revokedAt&&<Button tone="danger" onClick={()=>void revokeDevice(device.id)}>Kaldır</Button>}</div>)}</section>
      <section className="security-receipt-history"><h3>Güvenlik doğrulama kayıtları</h3><p>Kurtarma sonrası cihaz doğrulama kayıtları bu hesaba göre listelenir ve her açılışta bütünlükleri yeniden denetlenir.</p>{auth.securityEpoch!==auth.sessionSecurityEpoch&&<StatusMessage tone="danger">Bu oturum eski güvenlik sürümüne ait. Güvenlik geçmişine erişmeden önce yeniden giriş yapın.</StatusMessage>}<div className="button-row"><Button onClick={()=>void refreshSecurityReceipts()}>Geçmişi yenile</Button></div>{securityReceiptHistory.length===0?<small>Bu hesap için arşivlenmiş güvenlik doğrulama kaydı yok.</small>:securityReceiptHistory.map(item=><div className="list-row" key={item.receipt.receiptId}><div><strong>{item.verificationStatus==='valid'?'✓ Doğrulandı':'! Geçersiz'} · güvenlik sürümü {item.receipt.securityEpoch}</strong><small>{formatDate(item.receipt.occurredAt,{dateStyle:'short',timeStyle:'short'})} · bütünlük denetimi hazır</small></div><Button onClick={()=>void navigator.clipboard.writeText(JSON.stringify(item.receipt,null,2))}>Kopyala</Button></div>)}<label>Haricî güvenlik doğrulama kaydı<textarea rows={6} value={securityReceiptJson} onChange={event=>setSecurityReceiptJson(event.target.value)} placeholder="Doğrulanacak güvenlik kaydını buraya yapıştırın"/></label><Button onClick={()=>void verifySecurityReceiptJson()} disabled={!securityReceiptJson.trim()}>Güvenlik kaydını doğrula</Button>{securityReceiptVerification&&<StatusMessage tone={securityReceiptVerification.valid?'success':'danger'}>{securityReceiptStatusLabel(securityReceiptVerification.status,language)} · {toUserFacingErrorMessage(securityReceiptVerification.message,language==='tr'?'Güvenlik kaydı doğrulaması tamamlandı.':'Security record verification completed.')}</StatusMessage>}</section>
...
export function AddMemberModal({ fallbackPeople = [], onClose, onSave }: { fallbackPeople?: readonly FamilyMemberView[]; onClose: () => void; onSave: (input: CreateFamilyMemberInput) => Promise<void> }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      onClose();
    } catch (caught) { setError(toUserFacingErrorMessage(caught,'Kayıt oluşturulamadı.')); }
    finally { setBusy(false); }
  };
...
export function AddEventModal({ fallbackPeople = [], locations, onClose, onSave }: { fallbackPeople?: readonly FamilyMemberView[]; locations: FamilyAppSnapshot['locations']; onClose: () => void; onSave: (input: CreateFamilyEventInput) => Promise<void> }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    } catch (caught) { setError(toUserFacingErrorMessage(caught,'Etkinlik oluşturulamadı.')); }
    finally { setBusy(false); }
  };
...
export function EditEventModal({ event, fallbackPeople = [], locations, onClose, onSave }: { event: FamilyEventView; fallbackPeople?: readonly FamilyMemberView[]; locations: FamilyAppSnapshot['locations']; onClose: () => void; onSave: (input: UpdateFamilyEventInput) => Promise<void> }) {
  ...
  const submit=async(formEvent:FormEvent<HTMLFormElement>)=>{
    ...
    try{
      ...
      onClose();
    }catch(caught){setError(toUserFacingErrorMessage(caught,'Olay güncellenemedi.'));}
    finally{setBusy(false);}
  };
...
export function HouseholdMembershipScreen({people,workspace,onChanged}:{people:FamilyMemberView[];workspace:HouseholdMembershipWorkspaceView;onChanged:()=>Promise<void>}){
  ...
  const [busy,setBusy]=useState(false);
  const run=async(operation:()=>Promise<unknown>,success:string)=>{setBusy(true);setMessage('');try{await operation();await onChanged();setTone('success');setMessage(success);}catch(error){setTone('danger');setMessage(toUserFacingErrorMessage(error,'İşlem tamamlanamadı.'));}finally{setBusy(false);}};
...
export function PersonLifecycleScreen({people,onChanged}:{people:FamilyMemberView[];onChanged:()=>Promise<void>}){
  ...
  const loadWorkspace=async(personId:string)=>{
    ...
    catch(error){setWorkspace(undefined);setTone('danger');setMessage(toUserFacingErrorMessage(error,'Kişi profili yüklenemedi.'));}
  ...
  const run=async(operation:()=>Promise<unknown>,success:string)=>{
    ...
    catch(error){setTone('danger');setMessage(toUserFacingErrorMessage(error,'Kişi profili işlemi tamamlanamadı.'));}
    finally{setBusy(false);}
  };
  ...
  const panel=<><PageHeader eyebrow="Yönetişimli yaşam döngüsü" title="Kişi profilleri" description="Düzenleme, mantıksal birleştirme, geri alınabilir arşivleme ve referans güvenli silme işlemlerini sürüm denetimiyle yönetin."/>
    <section className="workspace-grid">
...
export function InvitationsScreen({snapshot}:{snapshot:FamilyAppSnapshot}){
  ...
  useEffect(()=>{void refresh().catch(error=>{setMessageTone('danger');setMessage(toUserFacingErrorMessage(error,'Davetler yüklenemedi.'));});},[]);
...
const dataRepairSnapshotLabel = (snapshot: DataRepairEntitySnapshot, language:EnumUiLanguage): string => {
  ...
  if (snapshot.entityType === 'relation') {
    return snapshot.row
      ? `${language==='tr'?'Aile bağı':'Family relationship'} · ${familyRelationTypeLabel(snapshot.row.relationType,language)}`
      : language==='tr'?'Bu aile bağı kaldırılmış olacak.':'This family relationship will be removed.';
  }
  return `${language==='tr'?'Aile üyesi':'Family member'} · ${dataRepairMembershipRoleLabel(snapshot.row.role,language)} · ${dataRepairMembershipStatusLabel(snapshot.row.status,language)}`;
};
...
export function DataRepairScreen() {
  ...
  const refresh=async()=>{
    ...
    try{
      ...
      setMessage(toUserFacingErrorMessage(error,'Veri Onarma Merkezi açılamadı.'));
    }finally{setLoading(false);}
  };
  useEffect(()=>{void refresh();},[]);
  ...
  const createPreview=async()=>{
    ...
    try{
      ...
      await refresh();
    }catch(error){setMessageTone('danger');setMessage(toUserFacingErrorMessage(error,'Onarma önizlemesi hazırlanamadı.'));}
    finally{setBusy('');}
  };
  const applyPreview=async()=>{
    ...
    try{
      ...
      await refresh();
    }catch(error){setMessageTone('danger');setMessage(toUserFacingErrorMessage(error,'Veri değiştiği için onarma uygulanamadı. Yeniden tarayın.'));}
    finally{setBusy('');}
  };
  const undoOperation=async(operationId:string)=>{
    ...
    try{
      ...
      await refresh();
    }catch(error){setMessageTone('danger');setMessage(toUserFacingErrorMessage(error,'Onarma güvenle geri alınamadı.'));}
    finally{setBusy('');}
  };
    ...
    <section className="workspace-grid data-repair-workspace">
      ...
      <Surface className="workspace-form">
        ...
        {!selectedIssue&&!preview?<EmptyState title="Bir bulgu seçin" body="Sol taraftaki bir bulguyu seçerek gerekçe ve değişiklik önizlemesini hazırlayın."/>:<>
          ...
          {preview&&<div className="data-repair-preview" aria-live="polite"><div className="preview-card before"><span className="eyebrow">Önceki durum</span><strong>{dataRepairSnapshotLabel(preview.beforeSnapshot,language)}</strong></div><div className="preview-arrow" aria-hidden="true">→</div><div className="preview-card after"><span className="eyebrow">Onarma sonrası</span><strong>{dataRepairSnapshotLabel(preview.afterSnapshot,language)}</strong></div><p><strong>Uygulanacak işlem:</strong> {dataRepairResolutionLabel(preview.resolution)}</p><p><strong>Gerekçe:</strong> {preview.reason}</p><label className="confirmation-row"><input type="checkbox" checked={confirmed} onChange={(event)=>setConfirmed(event.target.checked)}/> Önceki ve sonraki durumu karşılaştırdım; bu onarmayı uygulamak istiyorum.</label><div className="button-row"><Button onClick={()=>{setPreview(null);setConfirmed(false);}}>Önizlemeyi kapat</Button><Button tone="danger" disabled={!confirmed||busy!==''} onClick={()=>void applyPreview()}>{busy===preview.id?'Uygulanıyor…':'Onarmayı uygula'}</Button></div></div>}
        </>}
      </Surface>
...
export function PermissionsScreen({ auth }: { auth: AuthStateView }) {
  ...
  const finiteAllowRequired=effect==='allow'&&(resourceType==='event'||resourceType==='location');
  const savePermission=async()=>{try{setMessage(''); if(!window.pardus)return;const reason=denialReason.trim();const percent=ownershipPercent.trim()===''?undefined:Number(ownershipPercent);const ownershipBasisPoints=percent===undefined?undefined:Math.round(percent*100);if(percent!==undefined&&(!Number.isFinite(percent)||percent<=0||percent>100||ownershipBasisPoints!==(percent*100))){setMessage('Sahiplik oranı %0,01 ile %100 arasında ve en fazla iki ondalık basamaklı olmalıdır.');return;}if(effect==='deny'&&ownershipBasisPoints!==undefined){setMessage('Açık ret kaydı sahiplik oranı taşıyamaz.');return;}if(effect==='deny'&&reason.length<5){setMessage('Ret kararı için en az 5 karakterlik açık gerekçe yazın.');return;}if(resourceType==='location'&&familyBranchId){setMessage('Kayıtlı konum izni aile dalına bağlanamaz.');return;}if(finiteAllowRequired&&!endsOn){setMessage('Bu kaynak türünde izin veren kayıt için sonlu bir bitiş tarihi zorunludur.');return;}if(endsOn&&endsOn<startsOn){setMessage('Bitiş tarihi başlangıç tarihinden önce olamaz.');return;}await window.pardus.upsertPermission({subjectAccountId:subject,resourceType,resourceId,actions,effect,purpose,...(familyBranchId?{familyBranchId}:{}),...(ownershipBasisPoints===undefined?{}:{ownershipBasisPoints}),startsAt:`${startsOn}T00:00:00.000Z`,...(endsOn?{endsAt:`${endsOn}T23:59:59.999Z`}:{}),...(effect==='deny'?{denialReason:reason}:{})}); setMessage('Amaç, aile dalı, süre ve sahiplik oranıyla bağlamsal izin kaydedildi.'); await refresh();}catch(e){setMessage(toUserFacingErrorMessage(e,'İzin kaydedilemedi.'));}};
  const updateProfile=async(account:FamilyAccountView,changes:Partial<Pick<FamilyAccountView,'role'|'status'|'personId'>>)=>{try{if(!window.pardus)return;setAccounts(await window.pardus.updateAccount({accountId:account.id,role:changes.role??account.role,status:changes.status??account.status,startsAt:account.startsAt,...((changes.personId??account.personId)?{personId:changes.personId??account.personId}:{}),...(account.endsAt?{endsAt:account.endsAt}:{})}));setMessage('Profil yetkileri güncellendi.');}catch(e){setMessage(toUserFacingErrorMessage(e,'Profil güncellenemedi.'));}};
  ...
  const actionLabel=(value:ObjectPermissionAction)=>language==='tr'?({read:'Oku',create:'Oluştur',update:'Güncelle',delete:'Sil',share:'Paylaş',record:'Kaydet',ai_process:'Yapay zekâ ile işle',administer:'Yönet'} as const)[value]:({read:'Read',create:'Create',update:'Update',delete:'Delete',share:'Share',record:'Record',ai_process:'Process with AI',administer:'Administer'} as const)[value];
  const issueOfflineLease=async()=>{try{if(!window.pardus||!subject)return;setMessage('');setOfflineLeases(await window.pardus.issueOfflineCapabilityLease({subjectAccountId:subject,capability:offlineCapability,durationMinutes:offlineDurationMinutes}));setMessage('Çevrimdışı yetki kirası cihaz ve politika paketine bağlı olarak oluşturuldu.');}catch(error){setMessage(toUserFacingErrorMessage(error,'Çevrimdışı yetki kirası oluşturulamadı.'));}};
  const revokeOfflineLease=async(leaseId:string)=>{try{if(!window.pardus)return;setOfflineLeases(await window.pardus.revokeOfflineCapabilityLease(leaseId));setMessage('Çevrimdışı yetki kirası iptal edildi; hassas önbellek kilitlendi.');}catch(error){setMessage(toUserFacingErrorMessage(error,'Çevrimdışı yetki kirası iptal edilemedi.'));}};
  ...
  <Surface className="workspace-summary policy-client-data-access"><SectionHeader eyebrow="Güvenli veri erişimi" title="Uygulama verilerine güvenli erişim"/><div className="notes-card"><strong>Doğrudan erişim: {clientDataAccess?.enforcement==='fail-closed'?'Kapalı ve güvenli ret etkin':'Doğrulanamadı'}</strong><small>Aile kayıtları {clientDataAccess?.directAccess.repository===false?'korumalı':'doğrulanamadı'} · sorgular {clientDataAccess?.directAccess.sql===false&&clientDataAccess?.directAccess.sqlite===false?'korumalı':'doğrulanamadı'} · dosyalar {clientDataAccess?.directAccess.vaultFile===false?'korumalı':'doğrulanamadı'}</small><small>{clientDataAccess?.registeredApplicationServiceChannels??0} güvenli uygulama hizmeti · {clientDataAccess?.directAccessExceptionCount??0} doğrudan erişim istisnası</small><small>Mevcut yerel verileriniz korunur; veri sahipliği veya saklama alanı bu özellikle değiştirilmez.</small></div></Surface>
  <Surface className="workspace-summary"><SectionHeader eyebrow="Süreli çevrimdışı erişim" title="Çevrimdışı yetki kirası ve hassas önbellek kilidi"/><div className="form-grid"><label>Hesap<select value={subject} onChange={e=>setSubject(e.target.value)}>{accounts.map(account=><option key={account.id} value={account.id}>{account.displayName}</option>)}</select></label><label>Yetenek<select value={offlineCapability} onChange={e=>setOfflineCapability(e.target.value as OfflineCapability)}><option value="family.read">Aile verisini oku</option><option value="health.read">Sağlık verisini oku</option><option value="finance.read">Finans verisini oku</option><option value="location.read">Konum verisini oku</option><option value="archive.read">Arşivi oku</option></select></label><label>Süre (dakika)<input type="number" min={offlineLeases?.minimumDurationMinutes??1} max={offlineLeases?.maximumDurationMinutes??1440} value={offlineDurationMinutes} onChange={e=>setOfflineDurationMinutes(Number(e.target.value))}/></label><Button tone="primary" disabled={!subject||offlineDurationMinutes<1||offlineDurationMinutes>1440} onClick={()=>void issueOfflineLease()}>Sonlu kira oluştur</Button></div><div className="notes-card"><strong>Hassas önbellek: {offlineLeases?.cache.locked?'Kilitli':'Kira kapsamında açık'}</strong><small>{offlineLeases?offlineCacheReasonLabel(offlineLeases.cache.reason,language):(language==='tr'?'Etkin kira yok':'No active lease')} · {offlineLeases?.cache.entryCount??0} kayıt{offlineLeases?.cache.expiresAt?` · ${formatDate(offlineLeases.cache.expiresAt,{dateStyle:'short',timeStyle:'short'})} tarihinde kilitlenir`:''}</small></div><div className="stack-list">{!offlineLeases||offlineLeases.leases.length===0?<EmptyState title="Çevrimdışı kira yok" body="Hassas önbellek varsayılan olarak kilitlidir."/>:offlineLeases.leases.map(lease=><div className="context-stat" key={lease.leaseId}><strong>{offlineCapabilityLabel(lease.capability,language)} · {offlineLeaseStateLabel(lease.state,language)}</strong><span>{accounts.find(account=>account.id===lease.subjectAccountId)?.displayName??(language==='tr'?'Yerel hesap':'Local account')} · cihaz {lease.deviceId.slice(0,12)}…</span><small>{formatDate(lease.notBefore,{dateStyle:'short',timeStyle:'short'})} — {formatDate(lease.expiresAt,{dateStyle:'short',timeStyle:'short'})} · {lease.leaseSha256.slice(0,16)}…</small>{lease.state==='active'&&<Button tone="danger" onClick={()=>void revokeOfflineLease(lease.leaseId)}>Kirayı iptal et ve kilitle</Button>}</div>)}</div></Surface>
  <Surface className="workspace-summary"><SectionHeader eyebrow="Ortak kaynak bağlamı" title="Sahiplik oranı"/><label>İzin kaydının sahiplik oranı (%)<input type="number" min="0.01" max="100" step="0.01" disabled={effect==='deny'} value={ownershipPercent} onChange={e=>setOwnershipPercent(e.target.value)} placeholder="İsteğe bağlı: 0,01–100"/></label><small>{effect==='deny'?'Açık ret kayıtları sahiplik oranı taşımaz.':'Oran, izin kaydına 1–10.000 baz puan olarak bağlanır.'}</small><div className="stack-list">{permissions.filter(permission=>permission.ownershipBasisPoints!==undefined).map(permission=><small key={permission.id}>{resourceTypeLabel(permission.resourceType,language)} · %{((permission.ownershipBasisPoints??0)/100).toLocaleString('tr-TR',{maximumFractionDigits:2})}</small>)}</div></Surface>
  <section className="workspace-grid">
...
function FinanceOverviewScreen({people,records,valuations,institutions,bankAccounts,paymentCards,loanAccounts,planningWorkspace,onCreate,onCreateValuation,onValidateIban,onCreateBankAccount,onCreatePaymentCard,onCreateLoanAccount,onRecordLoanPayment,onRecordPlanning,onPlanningWorkspaceChange}:{people:FamilyMemberView[];records:FinanceRecordView[];valuations:FinanceValuationView[];institutions:BankInstitutionView[];bankAccounts:BankAccountView[];paymentCards:PaymentCardView[];loanAccounts:LoanAccountView[];planningWorkspace:FinancePlanningWorkspaceView|undefined;onCreate:(input:CreateFinanceRecordInput)=>Promise<void>;onCreateValuation:(input:CreateFinanceValuationInput)=>Promise<void>;onValidateIban:(input:ValidateIbanInput)=>Promise<IbanStructuralValidationView>;onCreateBankAccount:(input:CreateBankAccountInput)=>Promise<void>;onCreatePaymentCard:(input:CreatePaymentCardInput)=>Promise<void>;onCreateLoanAccount:(input:CreateLoanAccountInput)=>Promise<void>;onRecordLoanPayment:(input:RecordLoanPaymentInput)=>Promise<void>;onRecordPlanning:(input:RecordFinancePlanningItemInput)=>Promise<void>;onPlanningWorkspaceChange:(workspace:FinancePlanningWorkspaceView)=>void}){
  ...
  return <><PageHeader eyebrow="TCMB kataloglu · güvenli banka hesabı" title="Aile finansı" description="Banka hesapları merkezî finans politikasıyla korunur; IBAN yapısal kontrolü gerçek hesap veya sahiplik doğrulaması değildir."/>
    <section className="workspace-grid">
      <Surface className="workspace-form"><SectionHeader eyebrow="Güvenli finans verisi" title="Yeni banka hesabı"/>
        ...
        <div className="notes-card"><strong>Bu form kart veya internet bankacılığı sırrı kabul etmez.</strong><small>Tam kart numarası, güvenlik kodu, kart şifresi ve internet bankacılığı parolası bu alana kaydedilemez.</small></div>
      ...
      <Surface className="workspace-form"><SectionHeader eyebrow="Yalnız son dört hane saklanır" title="Yeni kart profili"/>
        ...
        <div className="notes-card"><strong>Tam kart numarası kesinlikle kaydedilmez.</strong><small>Otomatik ödeme alanı yalnız takip modudur; banka talimatı veya ödeme işlemi başlatmaz. Tam kart numarası, güvenlik kodu, kart şifresi ve parola kabul edilmez.</small></div>
      ...
      <Surface className="workspace-form"><SectionHeader eyebrow="Manuel ve doğrulanmamış kayıt" title="Yeni kredi profili"/>
      ...
      <Surface className="workspace-summary"><SectionHeader eyebrow="Anapara · vade · gecikme · teminat" title={`${loanAccounts.length} kredi profili`}/>
        {loanAccounts.length===0?<EmptyState title="Kredi profili yok" body="İlk krediyi manuel takip kaydı olarak ekleyin."/>:loanAccounts.map(loan=>{const totalPaid=loan.paymentHistory.reduce((sum,payment)=>sum+payment.amount,0);const first=loan.paymentSchedule[0];const last=loan.paymentSchedule.at(-1);return <div className="context-stat" key={loan.id}><strong>{loan.title} · {loanKindLabels[loan.kind]} · {loanStatusLabel(loan.status,language)}</strong><span>{loan.institutionOfficialName} · {loanRateLabels[loan.rateType]} %{(loan.annualRateBasisPoints/100).toLocaleString('tr-TR',{maximumFractionDigits:2})} · {loan.termMonths} ay</span><small>İlk anapara {loan.originalPrincipal.toLocaleString('tr-TR')} {loan.currency} · Kalan {loan.remainingPrincipal.toLocaleString('tr-TR')} · Taksit {loan.installmentAmount.toLocaleString('tr-TR')}</small><small>Plan {first?formatDate(first.dueAt):'—'} — {last?formatDate(last.dueAt):'—'} · {loan.paymentSchedule.length} taksit · Kayıtlı ödeme {totalPaid.toLocaleString('tr-TR')} {loan.currency}</small><small>Erken kapama {loan.earlySettlementAmount>0?`${loan.earlySettlementAmount.toLocaleString('tr-TR')} · ${formatDate(loan.earlySettlementQuotedAt!)}`:'teklifi yok'} · Gecikme {loan.overdueInstallmentCount} taksit / {loan.overdueAmount.toLocaleString('tr-TR')} · {loan.daysPastDue} gün</small><small>Sigorta {loanInsuranceStatusLabel(loan.insuranceStatus,language)}{loan.insuranceProvider?` · ${loan.insuranceProvider}`:''} · Teminat {loanCollateralTypeLabel(loan.collateralType,language)}{loan.collateralDescription?` · ${loan.collateralDescription}`:''}</small><small>Manuel veri · Banka doğrulaması yapılmadı · Ödeme icrası yapılmadı</small>{loan.paymentHistory.slice(0,3).map(payment=><span key={payment.id}>{formatDate(payment.paidAt)} · {payment.amount.toLocaleString('tr-TR')} {loan.currency}{payment.scheduledInstallmentSequence?` · ${payment.scheduledInstallmentSequence}. taksit`:''}</span>)}</div>;})}
      </Surface>
...
export function HealthScreen({people,records,medications,history,onCreate,onCreateMedication,onCreateHistory}:{people:FamilyMemberView[];records:HealthRecordView[];medications:MedicationPlanView[];history:FamilyHealthHistoryView[];onCreate:(input:CreateHealthRecordInput)=>Promise<void>;onCreateMedication:(input:CreateMedicationPlanInput)=>Promise<void>;onCreateHistory:(input:CreateFamilyHealthHistoryInput)=>Promise<void>}){
  const {language}=useLocalization();
  const kindLabels:Record<HealthRecordView['kind'],string>={appointment:language==='tr'?'Randevu':'Appointment',medication:language==='tr'?'İlaç':'Medication',diagnosis:language==='tr'?'Tanı':'Diagnosis',vaccine:language==='tr'?'Aşı':'Vaccine',note:language==='tr'?'Not':'Note'};
  const privacyLabels:Record<HealthRecordView['privacy'],string>={private:language==='tr'?'Özel':'Private',selected_members:language==='tr'?'Seçili üyeler':'Selected members',family:language==='tr'?'Aile':'Family'};
...
export function AutomationScreen({rules,onCreate,onToggle}:{rules:AutomationRuleView[];onCreate:(input:CreateAutomationRuleInput)=>Promise<void>;onToggle:(id:string,enabled:boolean)=>Promise<void>}){
  const {language}=useLocalization();
  const sourceTypeLabels:Record<AutomationRuleView['sourceType'],string>={important_day:language==='tr'?'Önemli gün':'Important date',life_record:language==='tr'?'Yaşam kaydı':'Life record',finance_record:language==='tr'?'Finans kaydı':'Finance record',medication_plan:language==='tr'?'İlaç planı':'Medication plan'};
  ...
  const runNow=async()=>{try{if(!window.pardus)return;setRuns(await window.pardus.runAutomationRules());setMessageTone('success');setMessage('Etkin kurallar şimdi değerlendirildi.');}catch(e){setMessageTone('danger');setMessage(toUserFacingErrorMessage(e,'Kurallar çalıştırılamadı.'));}};
  const submit=async()=>{try{await onCreate({title,sourceType,daysBefore,enabled:true});setMessageTone('success');setMessage('Otomasyon kuralı eklendi.');}catch(e){setMessageTone('danger');setMessage(toUserFacingErrorMessage(e,'Kural eklenemedi.'));}};
  const panel=<><PageHeader eyebrow="Akıllı takip" title="Bildirim ve otomasyon merkezi" description="Önemli gün, görev, finans ve ilaç planları için yerel hatırlatma kuralları oluşturun." actions={<Button tone="primary" onClick={()=>void runNow()}>Kuralları şimdi çalıştır</Button>}/><section className="workspace-grid"><Surface className="workspace-form"><SectionHeader eyebrow="Yeni kural" title="Otomasyon oluştur"/><label>Başlık<input value={title} onChange={e=>setTitle(e.target.value)}/></label><label>Kaynak<select value={sourceType} onChange={e=>setSourceType(e.target.value as AutomationRuleView['sourceType'])}><option value="important_day">Önemli gün</option><option value="life_record">Yaşam kaydı</option><option value="finance_record">Finans kaydı</option><option value="medication_plan">İlaç planı</option></select></label><label>Kaç gün önce<input type="number" min="0" max="365" value={daysBefore} onChange={e=>setDaysBefore(Number(e.target.value))}/></label><Button tone="primary" onClick={()=>void submit()} disabled={!title.trim()}>Kural ekle</Button>{message&&<StatusMessage tone={messageTone}>{message}</StatusMessage>}</Surface><Surface className="workspace-summary"><SectionHeader eyebrow="Kurallar" title={`${rules.length} otomasyon`}/>{rules.length?rules.map(r=><StatRow key={r.id} value={r.title} label={`${sourceTypeLabels[r.sourceType]} · ${r.daysBefore} gün önce · ${r.enabled?'Etkin':'Kapalı'}`} action={<Button onClick={()=>void onToggle(r.id,!r.enabled)}>{r.enabled?'Kapat':'Etkinleştir'}</Button>}/>):<EmptyState title="Kural yok" body="İlk otomasyon kuralınızı oluşturun."/>}</Surface><Surface className="span-2"><SectionHeader eyebrow="Çalışma geçmişi" title={`${runs.length} sonuç`}/>{runs.length?runs.slice(0,20).map(run=><StatRow key={run.id} value={run.title} label={`${run.status==='generated'?'Görev üretildi':run.status==='skipped'?'Atlandı':'Başarısız'} · ${formatDate(run.createdAt,{dateStyle:'short',timeStyle:'short'})}`}/>):<EmptyState title="Henüz çalışma yok" body="Kuralları şimdi çalıştırdığınızda sonuçlar burada görünür"/>}</Surface></section></>;
  return localizeOperationsCenterNode(panel,language);
}
...
export function SessionLockOverlay({state,twoFactorEnabled,onContinue,onLockNow,onUnlock}:{
  ...
  const submitUnlock=async(event:FormEvent)=>{
    ...
    }catch(caught){setError(toUserFacingErrorMessage(caught,'Oturum yeniden açılamadı.'));}
    finally{setBusy(false);}
  };
  ...
  const panel=<div ref={rootRef} className="session-lock-backdrop locked" role="dialog" aria-modal="true" aria-labelledby="session-lock-title" aria-describedby="session-lock-description">
    <form className="session-lock-card" onSubmit={event=>void submitUnlock(event)}>
      ...
      {twoFactorEnabled&&<label>İki aşamalı doğrulama / kurtarma kodu<input autoComplete="one-time-code" maxLength={256} value={secondFactorCode} onChange={event=>setSecondFactorCode(event.target.value)} required/></label>}
      {error&&<div className="form-error" role="alert">{error}</div>}
...
export function App() {
  ...
  useEffect(()=>{
    ...
    const run=async()=>{
      ...
      }catch{if(!cancelled)setScreenDataError(language==='tr'?'Ekran verileri güvenli biçimde yüklenemedi.':'Screen data could not be loaded safely.');}
    };
  ...
  useEffect(()=>{if(notificationOpen&&auth.authenticated&&!loadedSnapshotSectionsRef.current.has('timeline'))void ensureSnapshotSection('timeline').catch(()=>setScreenDataError(language==='tr'?'Bildirimler güvenli biçimde yüklenemedi.':'Notifications could not be loaded safely.'));},[notificationOpen,auth.authenticated,language]);
  ...
  const setupAdmin=async(input:SetupAdminInput)=>{if(!window.pardus)throw new Error(language==='tr'?'Güvenli kurulum bağlantısı başlatılamadı. Uygulamayı tamamen kapatıp yeniden açın.':'The secure setup connection could not start. Quit the application completely, then reopen it.');const ticket=asyncWriteGuardRef.current.start('auth-transition');const state=await window.pardus.setupAdmin(input);asyncWriteGuardRef.current.commit(ticket,()=>setAuth(state));};
  const login=async(input:LoginInput)=>{if(window.pardus){const ticket=asyncWriteGuardRef.current.start('auth-transition');const state=await window.pardus.login(input);await bootstrapAuthenticatedSession();asyncWriteGuardRef.current.commit(ticket,()=>setAuth(state));}};
  const loginWithWindowsHello=async(input:LoginWithWindowsHelloInput)=>{if(window.pardus){const ticket=asyncWriteGuardRef.current.start('auth-transition');const result=await window.pardus.loginWithWindowsHello(input);if(!result.authenticated)throw new Error(windowsHelloOutcomeMessage(result.outcome));const state=await window.pardus.getAuthState();await bootstrapAuthenticatedSession();asyncWriteGuardRef.current.commit(ticket,()=>setAuth(state));}};
  const completeInvitationAcceptance=async(state:AuthStateView)=>{const ticket=asyncWriteGuardRef.current.start('auth-transition');await bootstrapAuthenticatedSession();asyncWriteGuardRef.current.commit(ticket,()=>setAuth(state));};
  const completeFirstRunSecuritySetup=async({code,password}:{code:string;password:string})=>{if(!window.pardus)throw new Error(language==='tr'?'Güvenli cihaz doğrulama bağlantısı kullanılamıyor.':'The secure device-verification connection is unavailable.');await window.pardus.trustCurrentDevice({password,code,displayName:language==='tr'?'Bu bilgisayar':'This computer'});const trustedState=await window.pardus.getAuthState();if(trustedState.trustedDevice!==true)throw new Error(language==='tr'?'Bu bilgisayar güvenilir cihaz olarak doğrulanamadı.':'This computer could not be verified as a trusted device.');await bootstrapAuthenticatedSession();setAuth(trustedState);};
  ...
  if(auth.authenticated && (!auth.twoFactorEnabled||auth.trustedDevice!==true)) {
    const setup=<FirstRunSecuritySetup twoFactorAlreadyEnabled={auth.twoFactorEnabled===true} onComplete={completeFirstRunSecuritySetup}/>;
  ...
  const openImportantDayModal=()=>{void ensureSnapshotSection('timeline').then(()=>setEventModal(true)).catch(()=>setScreenDataError(language==='tr'?'Önemli gün verileri güvenli biçimde yüklenemedi.':'Important-day data could not be loaded safely.'));};
