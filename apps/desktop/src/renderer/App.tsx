...
export function SystemManagementScreen({draftCenterVisible=false,onDraftCenterVisibilityChange=()=>undefined}:{readonly draftCenterVisible?:boolean;readonly onDraftCenterVisibilityChange?:(visible:boolean)=>void}={}){
  ...
  const refresh=async()=>{
    ...
    task=(async()=>{
      ...
      setNetworkEgressBoundary(network);setDerivedDataPolicyBoundary(derived);setSensitiveLoggingBoundary(logging);setPolicyDecisionAuditBoundary(decisionAudit);setSourceDeletionPropagationBoundary(deletion);setPolicyConformanceSuiteBoundary(conformance);setPlatformPolicyAstGateBoundary(astGate);setPlatformCapabilityManifestGateBoundary(capabilityGate);setApplicationSecurityProfileGateBoundary(securityProfile);setPolicyServiceAvailabilityBoundary(serviceAvailability);setProductSurfaceGovernance(surfaceGovernance);setDesktopSecurityPosture(securityPosture);
  ...
  useEffect(()=>{if(activeSystemModule!=='operations')return;void refresh();const timer=setInterval(()=>void refresh(),30_000);return()=>clearInterval(timer);},[activeSystemModule]);
