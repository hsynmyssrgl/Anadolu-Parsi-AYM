...
function PermissionsScreen({ auth }: { auth: AuthStateView }) {
  ...
  const refresh=async()=>{ if(!window.pardus) return;try{const [workspace,leaseWorkspace]=await Promise.all([window.pardus.getAuthorizationContextWorkspace(),window.pardus.getOfflineCapabilityLeaseWorkspace()]); const a=[...workspace.accounts];setAccounts(a);setPermissions([...workspace.permissions]);setBranches(workspace.branches);setOfflineLeases(leaseWorkspace); if(!subject&&a[0]) setSubject(a[0].id);}catch{setAccounts([]);setPermissions([]);setBranches([]);setOfflineLeases(null);} };
