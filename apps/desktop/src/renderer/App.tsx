...
function UnifiedAuthorizedSearchPanel({onOpenArchive}:{onOpenArchive:(id:string)=>Promise<void>}) {
  const [query,setQuery]=useState('');
  ...
  const guardRef=useRef(new AsyncWriteGuard());
  useEffect(()=>()=>guardRef.current.invalidateAll(),[]);
  const search=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();if(!window.pardus||query.normalize('NFKC').trim().length<2)return;const ticket=guardRef.current.start('unified-authorized-search');setBusy(true);setError('');try{const next=await window.pardus.searchUnifiedAuthorizedRecords({query,limit:25});guardRef.current.commit(ticket,()=>setResult(next));}catch(caught){guardRef.current.commit(ticket,()=>{setResult(undefined);setError(caught instanceof Error?caught.message:'Birleşik arama tamamlanamadı.');});}finally{guardRef.current.commit(ticket,()=>setBusy(false));}};
  return <section className="panel unified-authorized-search" aria-labelledby="unified-authorized-search-title">
    <div className="section-heading"><div><span className="eyebrow">B3-05 · Yetki filtreli okuma</span><h2 id="unified-authorized-search-title">Tüm modüllerde ara</h2><p>Aile, olay, belge, finans, sağlık ve yaşam kayıtları yalnız çağrı anındaki kaynak yetkileriyle birlikte aranır.</p></div></div>
    <form className="unified-authorized-search-form" onSubmit={event=>void search(event)}><label htmlFor="unified-authorized-search-query">Arama sorgusu</label><input id="unified-authorized-search-query" value={query} maxLength={80} onChange={event=>setQuery(event.target.value)} placeholder="En az iki karakter yazın"/><Button type="submit" disabled={busy||query.normalize('NFKC').trim().length<2}>{busy?'Aranıyor…':'Birleşik ara'}</Button></form>
    <small>Bir kaynak yetkilendirilemez veya yüklenemezse kısmi sonuç gösterilmez. Sorgu IPC yanıtında yankılanmaz; sonuçlar kalıcı arama geçmişine yazılmaz.</small>
    {error&&<StatusMessage tone="danger">{error}</StatusMessage>}
    {result&&<div className="unified-authorized-search-results" aria-live="polite"><div className="section-heading"><strong>{result.items.length} yetkili sonuç{result.truncated?' · ilk 25 gösteriliyor':''}</strong><small>{result.searchedModules.length} modül · politika filtreli ve tam kaynak yanıtı</small></div>{result.items.length===0?<EmptyState title="Yetkili sonuç bulunamadı" body="Sorguyu değiştirip yeniden deneyin."/>:result.items.map(item=><article key={`${item.resourceType}:${item.resourceId}`}><div><span className="tag blue">{unifiedSearchModuleLabels[item.module]}</span><strong>{item.title}</strong>{item.occurredAt&&<small>{formatDate(item.occurredAt,{dateStyle:'medium'})}</small>}</div>{item.resourceType==='archive_item'&&<Button onClick={()=>void onOpenArchive(item.resourceId)}>Belgeyi aç</Button>}</article>)}</div>}
  </section>;
}
...
function DigitalLegacyScreen({ snapshot }: { snapshot: FamilyAppSnapshot }) {
  ...
  return <><PageHeader eyebrow="Kritik güvenlik alanı" title="Dijital Miras Yönetimi" description="Vefat sonrası erişim devrini çift yönetici onayı, zaman kilidi ve geri alma penceresiyle yönetin."/><MemoryStudioPanel/>
...
function SystemManagementScreen(){
  ...
  return <section><PageHeader eyebrow="Sistem yönetimi" title="Sistem, bakım ve operasyon" description="Yedek hedeflerini, performansı, bakım görevlerini ve tanılama işlemlerini yönetin." actions={<Button onClick={()=>void refresh()}>Yenile</Button>}/><CommunicationSecurityPanel/><SignedPluginPlatformPanel/>
...
function PermissionsScreen({ auth }: { auth: AuthStateView }) {
  ...
  return <><PageHeader eyebrow="Amaç ve aile dalı bağlamı" title="Bağlamsal Yetkiler" description="Her izni amacı, aile dalı, geçerlilik aralığı, sahiplik oranı ve açık ret gerekçesiyle yönetin."/>
