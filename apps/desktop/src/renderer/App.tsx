...
function FinanceScreen({people,records,valuations,institutions,bankAccounts,paymentCards,loanAccounts,onCreate,onCreateValuation,onValidateIban,onCreateBankAccount,onCreatePaymentCard,onCreateLoanAccount,onRecordLoanPayment}:{people:FamilyMemberView[];records:FinanceRecordView[];valuations:FinanceValuationView[];institutions:BankInstitutionView[];bankAccounts:BankAccountView[];paymentCards:PaymentCardView[];loanAccounts:LoanAccountView[];onCreate:(input:CreateFinanceRecordInput)=>Promise<void>;onCreateValuation:(input:CreateFinanceValuationInput)=>Promise<void>;onValidateIban:(input:ValidateIbanInput)=>Promise<IbanStructuralValidationView>;onCreateBankAccount:(input:CreateBankAccountInput)=>Promise<void>;onCreatePaymentCard:(input:CreatePaymentCardInput)=>Promise<void>;onCreateLoanAccount:(input:CreateLoanAccountInput)=>Promise<void>;onRecordLoanPayment:(input:RecordLoanPaymentInput)=>Promise<void>}){
...
export function App() {
  ...
  const ensureAuxiliaryScreen=async(screen:ScreenId):Promise<void>=>{
    ...
    task=(async()=>{
      if(screen==='finance'){
        const [records,valuations,institutions,accounts,cards,loans]=await Promise.all([window.pardus!.listFinance(),window.pardus!.listFinanceValuations(),window.pardus!.listBankInstitutions(),window.pardus!.listBankAccounts(),window.pardus!.listPaymentCards(),window.pardus!.listLoanAccounts()]);
        asyncWriteGuardRef.current.commit(ticket,()=>{setFinanceRecords(records);setFinanceValuations(valuations);setBankInstitutions(institutions);setBankAccounts(accounts);setPaymentCards(cards);setLoanAccounts(loans);});
