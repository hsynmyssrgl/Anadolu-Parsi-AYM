...
export function App() {
  ...
  const ensureAuxiliaryScreen=async(screen:ScreenId):Promise<void>=>{
    ...
    task=(async()=>{
      if(screen==='finance'){
        const [records,valuations,institutions,accounts,cards]=await Promise.all([window.pardus!.listFinance(),window.pardus!.listFinanceValuations(),window.pardus!.listBankInstitutions(),window.pardus!.listBankAccounts(),window.pardus!.listPaymentCards()]);
        asyncWriteGuardRef.current.commit(ticket,()=>{setFinanceRecords(records);setFinanceValuations(valuations);setBankInstitutions(institutions);setBankAccounts(accounts);setPaymentCards(cards);});
