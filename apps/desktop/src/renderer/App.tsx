...
export function SystemManagementScreen({draftCenterVisible=false,onDraftCenterVisibilityChange=()=>undefined}:{readonly draftCenterVisible?:boolean;readonly onDraftCenterVisibilityChange?:(visible:boolean)=>void}={}){
  ...
  useEffect(()=>{if(activeSystemModule!=='operations')return;void window.pardus?.getNetworkEgressBoundary().then(setNetworkEgressBoundary).catch(()=>setNetworkEgressBoundary(undefined));},[activeSystemModule]);
  useEffect(()=>{if(activeSystemModule!=='operations')return;void window.pardus?.getDerivedDataPolicyBoundary().then(setDerivedDataPolicyBoundary).catch(()=>setDerivedDataPolicyBoundary(undefined));},[activeSystemModule]);
  useEffect(()=>{if(activeSystemModule!=='operations')return;void window.pardus?.getSensitiveLoggingBoundary().then(setSensitiveLoggingBoundary).catch(()=>setSensitiveLoggingBoundary(undefined));},[activeSystemModule]);
  useEffect(()=>{if(activeSystemModule!=='operations')return;void window.pardus?.getPolicyDecisionAuditBoundary().then(setPolicyDecisionAuditBoundary).catch(()=>setPolicyDecisionAuditBoundary(undefined));},[activeSystemModule]);
  useEffect(()=>{if(activeSystemModule!=='operations')return;void window.pardus?.getSourceDeletionPropagationBoundary().then(setSourceDeletionPropagationBoundary).catch(()=>setSourceDeletionPropagationBoundary(undefined));},[activeSystemModule]);
  useEffect(()=>{if(activeSystemModule!=='operations')return;void window.pardus?.getPolicyConformanceSuiteBoundary().then(setPolicyConformanceSuiteBoundary).catch(()=>setPolicyConformanceSuiteBoundary(undefined));},[activeSystemModule]);
  useEffect(()=>{if(activeSystemModule!=='operations')return;void window.pardus?.getPlatformPolicyAstGateBoundary().then(setPlatformPolicyAstGateBoundary).catch(()=>setPlatformPolicyAstGateBoundary(undefined));},[activeSystemModule]);
  useEffect(()=>{if(activeSystemModule!=='operations')return;void window.pardus?.getPlatformCapabilityManifestGateBoundary().then(setPlatformCapabilityManifestGateBoundary).catch(()=>setPlatformCapabilityManifestGateBoundary(undefined));},[activeSystemModule]);
  useEffect(()=>{if(activeSystemModule!=='operations')return;void window.pardus?.getApplicationSecurityProfileGateBoundary().then(setApplicationSecurityProfileGateBoundary).catch(()=>setApplicationSecurityProfileGateBoundary(undefined));},[activeSystemModule]);
  useEffect(()=>{if(activeSystemModule!=='operations')return;const load=()=>void window.pardus?.getPolicyServiceAvailabilityBoundary().then(setPolicyServiceAvailabilityBoundary).catch(()=>setPolicyServiceAvailabilityBoundary(undefined));load();const timer=setInterval(load,15_000);return()=>clearInterval(timer);},[activeSystemModule]);
  useEffect(()=>{if(activeSystemModule!=='operations')return;void window.pardus?.getProductSurfaceGovernance().then(setProductSurfaceGovernance).catch(()=>setProductSurfaceGovernance(undefined));},[activeSystemModule]);
  useEffect(()=>{if(activeSystemModule!=='operations')return;void window.pardus?.getDesktopSecurityPosture().then(setDesktopSecurityPosture).catch(()=>setDesktopSecurityPosture(undefined));},[activeSystemModule]);
  ...
  {activeSystemModule==='operations'&&<div id="system-module-operations">
