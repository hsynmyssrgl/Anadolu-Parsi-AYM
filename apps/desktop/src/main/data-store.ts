...
import { RepositoryBackedAiConsentQueryPort, RepositoryBackedAiConsentUnitOfWork } from './ai-consent-application-adapter.js';
...
export class FamilyDataStore {
  ...
  public previewAiAccess(purpose:AiConsentView['purpose']): AiAccessPreviewView { const result=this.#previewAiAccessUseCase.execute(this.#aiConsentApplicationContext('ai-access-preview'),purpose); if(!result.ok) throw new Error(`[${result.error.code}] ${result.error.message}`); return result.value; }
