...
export const createVerifiedUninstallBackups = async (input: {
  ...
  for (const target of input.targets) {
    ...
    const backupDirectory = join(rootPath, `ParsYuva_Aile_Yasam_Merkezi_Kaldirma_Yedegi_${timestamp}`);
    ...
    const manifest = {
      schemaVersion: 1, product: 'ParsYuva Aile Yaşam Merkezi', applicationVersion: input.applicationVersion,
