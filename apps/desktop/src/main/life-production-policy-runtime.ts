...
const findLifeResourceForPolicyResolution = (
  ...
  if (resourceType === 'signed_plugin_installation') {
    if (!dependencies.signedPluginPlatformPolicyResourceRepository) throw new PlatformPolicyEnforcementError(
      'RESOURCE_RESOLUTION_FAILED', 'Signed plugin policy resource repository is not composed');
    const found = dependencies.signedPluginPlatformPolicyResourceRepository.resolvePolicyResource(
      execution,
      'signed_plugin_installation',
      resourceId
    );
  ...
  if (resourceType === 'communication_recording_request') {
    if (!dependencies.communicationRecordingPolicyResourceRepository) throw new PlatformPolicyEnforcementError(
      'RESOURCE_RESOLUTION_FAILED', 'Communication recording policy resource repository is not composed');
    const found = dependencies.communicationRecordingPolicyResourceRepository.resolvePolicyResource(
      execution, resourceType, resourceId);
    if (!found.ok) return found;
    return ok(found.value
      ? Object.freeze({ familyId: found.value.familyId, ownerPersonId: found.value.ownerPersonId,
  ...
  if (resourceType === 'family_meeting') {
    if (!dependencies.familyMeetingPolicyResourceRepository) throw new PlatformPolicyEnforcementError(
      'RESOURCE_RESOLUTION_FAILED', 'Family meeting policy resource repository is not composed');
    const found = dependencies.familyMeetingPolicyResourceRepository.resolvePolicyResource(
      execution, 'family_meeting', resourceId);
    if (!found.ok) return found;
    return ok(found.value
      ? Object.freeze({ familyId: found.value.familyId, ownerPersonId: found.value.ownerPersonId,
