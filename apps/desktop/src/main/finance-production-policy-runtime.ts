import type { FinanceApplicationContext, FinancePolicyIntent } from '@ppt/application';
import {
  ERROR_CODES,
  asIsoDateTime,
  asPersonId,
...
import type {
  FinancePolicyCommittedTransactionInput,
  FinancePolicyEnforcementPoint,
  FinancePolicyEnforcementPointResolver,
  FinancePolicyTransactionRevalidationInput
...
export interface FinanceProductionPolicyRuntimeDependencies {
  ...
  readonly financePolicyResourceRepository: FinancePolicyResourceRepositoryPort;
...
interface FinanceResourceSnapshot {
  readonly resource: PolicyResource;
  readonly stateFingerprint: string;
}

const FINANCE_POLICY_FENCE_NAME = 'finance-write';
...
const readLiveFence = (
  dependencies: FinanceProductionPolicyRuntimeDependencies
  ...
  try {
    ...
    throw new PlatformPolicyEnforcementError(
      ...
      'Finance production cluster fence could not be read',
      { cause: error }
    );
  }
  if (!fence || !Number.isSafeInteger(fence.epoch) || fence.epoch < 0 || typeof fence.writable !== 'boolean') {
    throw new PlatformPolicyEnforcementError(
      'CLUSTER_FENCE_CHANGED',
      'Finance production cluster fence is invalid'
    );
  }
...
const synchronizeDatabaseFence = (
  ...
  const result = dependencies.transactionExecutor.execute(
    ...
    (transaction) => dependencies.policyTransactionRepository.synchronizeFence(
      ...
      {
        fenceName: FINANCE_POLICY_FENCE_NAME,
  ...
  if (
    !result.ok
    || result.value.fenceName !== FINANCE_POLICY_FENCE_NAME
    || result.value.epoch !== liveFence.epoch
    || result.value.writable !== liveFence.writable
  ) {
    throw new PlatformPolicyEnforcementError(
      'CLUSTER_FENCE_CHANGED',
      'Finance production cluster fence could not be monotonically synchronized to SQLite',
  ...
  reserve(reservation: PlatformPolicyReplayReservation) {
    ...
    if (!result.ok) {
      throw new PlatformPolicyEnforcementError(
        ...
        'Finance production replay reservation could not be persisted',
        { cause: result.error }
      );
    }
    return result.value;
...
const recordAuthorizedProductionTransaction = (
  ...
  if (!input.authorization.fenceWritable) {
    return invalidAuthority(input.context, 'Finance policy operation requires a writable database fence');
  }
...
const drainPendingJournalProjections = async (
  ...
  if (typeof ensureReceipt !== 'function' || typeof verifyProjectionProof !== 'function') {
    return projectionFailure(
      context,
      'Finance policy receipt projection proof boundary is unavailable; the durable pending record was retained.',
      { businessTransactionCommitted: options.businessTransactionCommitted }
    );
  }
  ...
  if (!anchored.ok) {
    return projectionFailure(
      context,
      'Finance policy journal rollback anchor could not be loaded.',
  ...
  if (anchored.value) {
    ...
    try {
      ...
      return projectionFailure(
        context,
        'Finance policy journal rollback anchor verification failed.',
    ...
    if (!anchorValid) {
      return projectionFailure(
        context,
        'Finance policy journal no longer contains its SQLite-anchored complete head; authorization stopped.',
        { businessTransactionCommitted: options.businessTransactionCommitted }
      );
    }
  }
  ...
  for (let round = 0; round < MAX_PROJECTION_DRAIN_ROUNDS; round += 1) {
    ...
    if (!pending.ok) {
      return projectionFailure(
        context,
        'Finance policy pending receipt projections could not be loaded.',
    ...
    for (const projection of pending.value) {
      ...
      try {
        ...
        return projectionFailure(
          context,
          'Finance policy pending receipt could not be cryptographically verified.',
          { businessTransactionCommitted: options.businessTransactionCommitted, cause: error }
        );
      }
      if (!receiptVerified) {
        return projectionFailure(
          context,
          'Finance policy pending receipt failed cryptographic verification and remains durable.',
          { businessTransactionCommitted: options.businessTransactionCommitted }
        );
      }
      ...
      try {
        ...
        return projectionFailure(
          context,
          'Finance policy receipt journal projection failed; the durable projection remains pending.',
          { businessTransactionCommitted: options.businessTransactionCommitted, cause: error }
        );
      }
      ...
      try {
        ...
        return projectionFailure(
          context,
          'Finance policy receipt journal returned a proof that could not be verified.',
          { businessTransactionCommitted: options.businessTransactionCommitted, cause: error }
        );
      }
      if (!proofValid) {
        return projectionFailure(
          context,
          'Finance policy receipt journal returned an invalid projection proof.',
          { businessTransactionCommitted: options.businessTransactionCommitted }
        );
      }
      ...
      if (!acknowledged.ok) {
        return projectionFailure(
          context,
          'Finance policy receipt projection acknowledgement failed.',
  ...
  if (!remaining.ok || remaining.value.length > 0) {
    return projectionFailure(
      context,
      'Finance policy receipt projection backlog remains durable and pending.',
      {
        businessTransactionCommitted: options.businessTransactionCommitted,
  ...
  if (options.expectedAuthorization) {
    ...
    if (!receipt.ok || !receipt.value || stable(receipt.value.record) !== stable(expected.receiptRecord)) {
      return projectionFailure(
        context,
        'Finance policy committed receipt could not be confirmed after journal projection.',
...
const permissionIsValid = (
  ...
  return nonEmpty(row.id, 256)
    ...
    && row.resourceType === 'finance_record'
...
const securityFingerprint = (
  ...
  person: PersonRecord,
  device: TrustedDeviceRow,
  financePermissions: readonly ObjectPermissionRow[]
): string => stable({
  account: {
    id: account.id,
    role: account.role,
...
const loadAuthoritySnapshotInTransaction = (
  ...
  if (!account || !accountIsActive(account, context, execution.occurredAt)) {
    return invalidAuthority(context, 'Finance policy account, role or person membership is not active and exact');
  }
  ...
  if (!person || person.id !== account.personId || person.familyId !== context.familyId || person.status !== 'active') {
    return invalidAuthority(context, 'Finance policy person membership does not match the active family');
  }
  ...
  if (!device || !deviceIsTrusted(device, identity, account, execution.occurredAt)) {
    return invalidAuthority(context, 'Finance policy device identity is not trusted for the active security epoch');
  }
  ...
  const financePermissions = permissionsResult.value.filter((row) =>
    row.resourceType === 'finance_record'
    && (row.purpose === 'finance' || row.purpose === 'general')
  );
  if (
    financePermissions.length > 10_000
    || financePermissions.some((row) => !permissionIsValid(row, account, execution.occurredAt))
  ) return invalidAuthority(context, 'Finance policy permission snapshot contains invalid grants');

  const expiresAt = authorityExpiry(account, execution.occurredAt);
  if (!expiresAt) return invalidAuthority(context, 'Finance policy authority expiry could not be established');
  return ok({
    authority: Object.freeze({
      ...
      online: true,
      grants: Object.freeze(financePermissions.map(toPolicyGrant)),
      expiresAt
    }),
    securityFingerprint: securityFingerprint(account, person, device, financePermissions)
  });
};
...
const sensitivityFor = (privacy: FinanceRecordRow['privacy']): PolicyResource['sensitivity'] => {
  ...
  throw new PlatformPolicyEnforcementError(
    ...
    'Finance record privacy is not supported by the platform policy model'
  );
};

const loadFinanceResourceSnapshotInTransaction = (
  dependencies: FinanceProductionPolicyRuntimeDependencies,
  context: FinanceApplicationContext,
  requestedIntent: FinancePolicyIntent,
  transaction: TransactionContext
): Result<FinanceResourceSnapshot, AppError> => {
  const execution = repositoryContext(context, transaction);
  if (requestedIntent.action === 'read' && requestedIntent.resourceId === '*') {
    if (!context.actor.personId) return invalidAuthority(context, 'Finance collection read requires an exact person identity');
  ...
  if (requestedIntent.action === 'create') {
    if (!requestedIntent.ownerPersonId || !requestedIntent.privacy) {
      return invalidAuthority(context, 'Finance create policy metadata is incomplete');
    }
    const existing = dependencies.financePolicyResourceRepository.findRecordForPolicyResolution(
      execution,
    ...
    if (!owner.value || owner.value.familyId !== context.familyId || owner.value.status !== 'active') {
      return invalidAuthority(context, 'Finance policy owner does not exist in the active family');
    }
    const resource = Object.freeze({
      type: 'finance_record',
  ...
  if (requestedIntent.action === 'update') {
    const record = dependencies.financePolicyResourceRepository.findRecordForPolicyResolution(
      execution,
      requestedIntent.resourceId
    );
...
const loadFinanceResourceSnapshot = (
  dependencies: FinanceProductionPolicyRuntimeDependencies,
  context: FinanceApplicationContext,
  requestedIntent: FinancePolicyIntent
): FinanceResourceSnapshot => {
  const result = dependencies.transactionExecutor.execute<FinanceResourceSnapshot>(
    context.correlationId,
    (transaction) => loadFinanceResourceSnapshotInTransaction(
      dependencies,
      context,
      requestedIntent,
      transaction
  ...
  if (!result.ok) {
    throw new PlatformPolicyEnforcementError(
      'RESOURCE_RESOLUTION_FAILED',
      `Finance policy resource snapshot could not be loaded: ${result.error.code}`,
      { cause: result.error }
    );
  }
  return result.value;
...
const resolveResource = (
  ...
  if (
    ...
    throw new PlatformPolicyEnforcementError(
      'RESOURCE_RESOLUTION_FAILED',
      'Finance policy authority changed outside the resolved application context'
    );
  }
  if (
    ...
    throw new PlatformPolicyEnforcementError(
      'RESOURCE_RESOLUTION_FAILED',
      'Finance policy intent changed before resource resolution'
    );
  }
  return loadFinanceResourceSnapshot(dependencies, context, requestedIntent);
};
...
const revalidateProductionTransaction = (
  ...
  capturedResource: FinanceResourceSnapshot | undefined,
  input: FinancePolicyTransactionRevalidationInput
): Result<void, AppError> => {
  if (
  ...
  ) return invalidAuthority(context, 'Finance policy application context changed before the business transaction');
  if (!capturedResource) return invalidAuthority(context, 'Finance policy resource was not captured before receipt issuance');
  ...
  if (currentAuthority.value.securityFingerprint !== capturedAuthority.securityFingerprint) {
    return invalidAuthority(context, 'Finance policy authority changed after receipt issuance');
  }
  if (
  ...
  ) return invalidAuthority(context, 'Finance policy receipt subject no longer matches the live authority');

  const currentResource = loadFinanceResourceSnapshotInTransaction(
    dependencies,
    context,
    requestedIntent,
    input.transaction
  ...
  ) return invalidAuthority(context, 'Finance policy resource changed after receipt issuance');
  return ok(undefined);
};

const ensureRuntimeConfiguration = (dependencies: FinanceProductionPolicyRuntimeDependencies): void => {
  if (
    !dependencies
    ...
    throw new PlatformPolicyEnforcementError(
      'ENFORCEMENT_UNAVAILABLE',
      'Finance production policy runtime configuration is incomplete or invalid'
    );
  }
};

export const createFinanceProductionPolicyEnforcementPointResolver = (
  dependencies: FinanceProductionPolicyRuntimeDependencies
): FinancePolicyEnforcementPointResolver => {
  ...
  return Object.freeze({
    async resolve(
      context: FinanceApplicationContext,
      requestedIntent: FinancePolicyIntent
    ): Promise<FinancePolicyEnforcementPoint> {
      ...
      if (!recovered.ok) {
        throw new PlatformPolicyEnforcementError(
          ...
          'Finance production policy receipt recovery must succeed before authorization',
      ...
      try {
        ...
        throw new PlatformPolicyEnforcementError(
          ...
          'Finance policy device identity snapshot could not be loaded',
          { cause: error }
        );
      }
      if (
        ...
        throw new PlatformPolicyEnforcementError(
          'AUTHORITY_INVALID',
          'Finance policy device identity snapshot is invalid'
        );
      }
      ...
      if (!snapshot.ok) {
        throw new PlatformPolicyEnforcementError(
          'AUTHORITY_RESOLUTION_FAILED',
          `Finance production policy authority could not be loaded: ${snapshot.error.code}`,
          { cause: snapshot.error }
        );
      }

      let capturedResource: FinanceResourceSnapshot | undefined;
