...
import { ManagedLifePanel, managedLifeProfileStatusLabel } from '../src/renderer/ManagedLifePanel';
...
describe('feature-panel English localization wave eighteen', () => {
  ...
  it('localizes every managed-life profile status instead of exposing the stored enum', () => {
    const cases = [
      ['planned', 'Planlandı', 'Planned'], ['active', 'Aktif', 'Active'], ['completed', 'Tamamlandı', 'Completed'],
      ['expired', 'Süresi doldu', 'Expired'], ['cancelled', 'İptal', 'Canceled']
    ] as const;
    for (const [status, turkish, english] of cases) {
      expect(managedLifeProfileStatusLabel(status, 'tr')).toBe(turkish);
      expect(managedLifeProfileStatusLabel(status, 'en')).toBe(english);
      expect(managedLifeProfileStatusLabel(status, 'tr')).not.toBe(status);
    }
    const source = readFileSync(new URL('../src/renderer/ManagedLifePanel.tsx', import.meta.url), 'utf8');
    expect(source).toContain('managedLifeProfileStatusLabel(profile.status,language)');
    expect(source).not.toContain(' · {profile.status} · ');
  });
  ...
  it('does not leave capitalized Turkish UI literals outside the English copy table', () => {
    ...
    const allowedUnchanged = new Set([
      'A Rh+','A Rh−','B Rh+','B Rh−','AB Rh+','AB Rh−','Assistance','Card','Contact','DASK','Health','Wh',
      'Emergency profile','Health information','Emergency contact','Assistance instruction','Battery','Unknown'
    ]);
