...
describe('installer progress, narration and Silver help experience', () => {
  ...
  it('uses a transitional narrated welcome while reserving progress for real file installation', async () => {
    ...
    expect(narration).toContain("'SAME_LANGUAGE_FEMALE_PREFERRED'");
    expect(narration).toContain("'SAME_LANGUAGE_MALE_FALLBACK'");
    expect(narration).toContain("'SAME_LANGUAGE_FIRST_VOICE_FALLBACK'");
    expect(narration).toContain('$synthesizer.SetOutputToWaveFile($waveWorkingPath)');
    expect(narration).toContain('$synthesizer.SpeakAsyncCancel($prompt)');
    expect(narration).toContain("outputMode = if ([string]::IsNullOrWhiteSpace($WaveOutputPath)) { 'DEFAULT_AUDIO_DEVICE' } else { 'WAVE_CAPTURE' }");
    expect(narration).toContain('Get-NarrationFileSha256 -Path $waveTarget');
    expect(narration).not.toContain('Get-FileHash');
    expect(narration).toContain("Join-Path ([IO.Path]::GetTempPath()) 'ParsYuvaInstallerEvidence'");
    expect(narration).toContain('Narration diagnostic output must remain under the approved temporary root.');
    expect(narration).toContain('WaveOutputPath requires EvidencePath so every wave remains receipt-bound.');
    expect(narration).toContain('Assert-NarrationNoReparseAncestors');
    expect(narration).toContain('Narration diagnostic path contains a reparse ancestor');
    expect(narration).toContain('Register-ObjectEvent -InputObject $synthesizer -EventName SpeakCompleted');
    expect(narration).toContain("$evidence.failureCode = 'NARRATION_COMPLETION_EVENT_MISSING'");
    expect(narration).toContain("$evidence.failureCode = 'NARRATION_ASYNC_ERROR'");
    expect(narration).toContain("$evidence.engine = 'WINRT_ONECORE'");
    expect(narration).toContain('Windows.Media.SpeechSynthesis.SpeechSynthesizer');
    expect(narration).toContain('Await-WinRtNarrationOperation');
    expect(narration).toContain('New-Object System.Media.SoundPlayer');
    expect(narration).toContain('if ($useWinRtFallback) { Invoke-WinRtNarration -Text $text }');
    expect(narration).toContain('OFFLINE_WAVE_SYNTHESIS_ONLY_NOT_AUDIBLE_OUTPUT');
    expect(narration).toContain('[IO.FileMode]::CreateNew');
    expect(narration).toContain('$stream.Flush($true)');
    expect(narration).toContain('scriptSha256');
    expect(narration).toContain('applicationVersion');
    expect(narration).toContain('$evidence.failureMessage = $_.Exception.Message');
    expect(narration).toContain('Get-NarrationRelativePath');
    expect(narration).not.toContain('[IO.Path]::GetRelativePath');
    expect(narration).not.toContain('[Convert]::ToHexString');
    expect(narration).toContain('Convert-NarrationBytesToLowerHex');
    expect(narration).toContain("if ($chunkId -eq 'fmt ')");
    expect(narration).toContain("if ($offset -ne $bytes.Length)");
    expect(narration).toContain('Narration wave chunk padding is missing.');
    expect(narration).toContain("[Text.Encoding]::ASCII.GetString($bytes, 0, 4) -ne 'RIFF'");
    expect(narration).toContain("[Text.Encoding]::ASCII.GetString($bytes, 8, 4) -ne 'WAVE'");
    expect(narration).toContain("if ($chunkId -eq 'data') { $dataCount += 1; $dataBytes += $chunkSize }");
    expect(narration).toContain('Remove-PartialWave');
    expect(narration).toContain("$evidence.failureCode = 'NARRATION_CANCEL_TIMEOUT'");
    expect(narration).toContain('if ($diagnosticMode) { [Environment]::Exit(1) }');
    expect(narration.trimEnd().endsWith('[Environment]::Exit(0)')).toBe(true);
    ...
    expect(narration).not.toMatch(/Set-ItemProperty|New-ItemProperty|reg\.exe/iu);
    expect(narration).not.toContain('$synthesizer.SetOutputToWaveFile($waveTarget)');
