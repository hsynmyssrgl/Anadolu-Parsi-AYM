...
const options = (): SupplyChainReleasePolicyOptions => ({
  ...
  expectedCoverage: {
    workspaceCount: 18,
    sbomComponentCount: 442,
    dependencyNodeCount: 442,
    externalRegistryPackageCount: 402,
    licenseInventoryComponentCount: 382
  },
  ...
  it('derives expected release, materials, coverage and assets from trusted inputs instead of candidate evidence', () => {
    const independentlyDerived = createSupplyChainReleasePolicyOptions({
      policy: {
        ...
        requiredSbomComponentCount: 442,
        requiredDependencyNodeCount: 442,
        requiredRegistryPackageCount: 402,
        requiredLicenseComponentCount: 382,
        vulnerability: {
    ...
    expect(independentlyDerived.expectedCoverage.sbomComponentCount).toBe(442);
