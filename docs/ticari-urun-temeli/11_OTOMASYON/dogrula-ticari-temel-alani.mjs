...
check(commercialChange52?.durum === 'ACTIVE'
  ...
  && String(commercialChange52?.isListesiEtkisi ?? '').includes('source-integrity FAIL')
  && String(commercialChange52?.isListesiEtkisi ?? '').includes('77a87a87 ile korunur')
  && String(commercialChange52?.isListesiEtkisi ?? '').includes('yanlis --no-write preflight bayraginin writer moda sessiz dusmesi 0854a4ec ile korunur')
  && String(commercialChange52?.isListesiEtkisi ?? '').includes('yalniz argumansiz writer veya exact --read-only kabul eder')
