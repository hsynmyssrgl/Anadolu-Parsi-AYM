...
| IS-0211 | Mutasyon sonrasi exact commit kanit ve taze kurulu EXE UAT kapisi | Codex | DEVAM | Bootstrap producer pointer sourceCommit ve base-pointer-HEAD ancestry bagi duzeltildi; guncel exact commit etki analizi, hedefli test, tam regresyon, kaynak butunlugu ve paket sonrasi kurulu EXE UAT makbuzlari NOT_RUN |
...
| IS-0215 | En kucuk degisiklikte tum kayit ve test kapanisi | Codex | DEVAM | Tum bagimli kayit siniflari UPDATED veya kanitli NOT_AFFECTED; byte-exact tarihsel kayitlar degistirilmez, tetikleyici/değişmezlik kapisi olarak korunur; guncel uretilen makbuzlar Git teslimine girer. FAIL makbuzu repo-relative dosya/test kimligini guvenle gosterir. Hedefli ve filtresiz tam regresyon, typecheck, sozdizimi, kaynak butunlugu ve UI etkisinde tum etkilesim/gorsel UAT PASS; gercek FAIL wip(rejected), ara installer yok |
