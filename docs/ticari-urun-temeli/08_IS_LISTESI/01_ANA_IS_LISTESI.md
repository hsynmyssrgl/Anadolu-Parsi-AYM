...
| IS-0211 | Mutasyon sonrasi exact commit kanit ve taze kurulu EXE UAT kapisi | Codex | DEVAM | Aktif son kaynak reti `62d65314`; `e71bd0d2` exact assessment/analysis 121 dosya/23 hedef test PASS sonrasında canlılık denetiminin operation-rule makbuzunu değiştirmesi execution guard tarafından reddedildi. Ürün assertion hatası yoktur. `a5334c13` Bronze 52 release görsel retidir. Yeni exact hedefli/tam/bütünlük, paket, `.52→.53` kurulu EXE UAT ve final readback olmadan teslim yok |
| IS-0212 | Acik tek seferli surum tahsisi ve onceden tahsisli paket kimligi | Codex | DEVAM | Bronze 27.08.2026.53, `bronze-2026-08-27-r53` kimliğiyle tek kez tahsis edildi; allocator tekrar çalıştırılmaz. `.52` history parent `.51`, `.53` history parent `.52` exact kaydedilir. Aktif son kaynak reti `62d65314`; yeni exact kapılar pendingdir |
...
| IS-0215 | En kucuk degisiklikte tum kayit ve test kapanisi | Codex | DEVAM | Aktif son ret `62d65314`; `e71bd0d2` exact assessment/analysis 121 dosya/23 hedef test PASS sonrasında eşzamanlı operation-rule receipt yazımı execution guard tarafından fail-closed reddedildi. Ürün assertion hatası yoktur; korumalı testler paralel kural kontrolü olmadan seri retry edilir. Önceki retler tarihsel korunur; Bronze 53 yeni exact zincir ve tam UAT PASS olmadan kapanmaz |
