...
| IS-0211 | Mutasyon sonrasi exact commit kanit ve taze kurulu EXE UAT kapisi | Codex | DEVAM | Aktif son kaynak reti `bbea101b`; `9999b216` etki planı 120 dosya/23 test PASS, resmî hedefli çağrı boş liste aktarımı FAIL'dir. Düzeltme 2 dosya/13 test PASS. `a5334c13` Bronze 52 release görsel retidir. Yeni exact hedefli/tam/bütünlük, paket, `.52→.53` kurulu EXE UAT ve final readback olmadan teslim yok |
| IS-0212 | Acik tek seferli surum tahsisi ve onceden tahsisli paket kimligi | Codex | DEVAM | Bronze 27.08.2026.53, `bronze-2026-08-27-r53` kimliğiyle tek kez tahsis edildi; allocator tekrar çalıştırılmaz. `.52` history parent `.51`, `.53` history parent `.52` exact kaydedilir. Aktif son kaynak reti `bbea101b`; yeni exact kapılar pendingdir |
...
| IS-0215 | En kucuk degisiklikte tum kayit ve test kapanisi | Codex | DEVAM | Aktif son ret `bbea101b`; resmî hedefli kanıt çağrısında exact planın 23 test listesi varsayılan alınmamıştır. Kalıcı düzeltme ve odaklı 2 dosya/13 test PASS; explicit liste exact eşitlik kapısı korunur. `de63eb2b`, `cb7a5d4d` ve `453e8c59` tarihsel retleri korunur; final master 59/59 görsel QA PASS. Bronze 53 yeni exact zincir ve tam UAT PASS olmadan kapanmaz |
