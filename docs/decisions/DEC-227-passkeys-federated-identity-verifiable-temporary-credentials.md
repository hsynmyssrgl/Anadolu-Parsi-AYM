...
İntake self-testinde geçerli paket, byte tamperi, yabancı signer, kriptografik olarak geçerli olsa da trusted registry dışında kalan self-signed paket, yabancı source commit ve süre aşımı sınırları doğrulanır. Başarılı rapor; evidence source commit/tree, signer SPKI SHA-256, manifest SHA-256 ve sekiz dosyanın content-free tree SHA-256 bağını üretir; sonraki receipt bu kanıt commitini exact ata olarak bağlayabilir. Bununla birlikte `PASS` yalnız `READY_FOR_GOVERNED_REVIEW` üretir; requirement PASS vermez, registry mutasyonu yapmaz ve persistent receipt yazmaz. actual external evidence `NOT_RUN` durumundadır ve kullanıcıdan bağımsız gerçek cihaz/hesap/reviewer kanıtı olmadan değiştirilemez.

Pure completion-preparation state machine; exact aktif 33-P adımı, 33-O predecessor receipt bağı, temiz Git commit/tree, governed signer, sekiz kanıt bindingi, boundary/contract/runtime, migration 93, PPK-021/022, tam Vitest ve üç production build kanıtı olmadan fail-closed kalır. `prepare:33-p:completion` aynı state machine’i gerçek I/O sınırında kullanır; exact üç evidence argümanı zorunludur, `--dry-run` tüm intake/gate/test/build zincirini governance yazımı olmadan çalıştırır ve eksik argüman state byte’larını değiştirmeden reddedilir. Başarılı mutating hazırlık dahi yalnız iki content-free SHA-256 sidecar’lı kanıt kaydı ile `VALIDATED_RECEIPT_PENDING` taslağı üretir; accepted-scope registry byte-anlamında değişmeden kalır, requirement PASS false ve persistent receipt pending olur. Böylece hazırlık başarısı resmi kapanış veya 33-Q aktivasyonu gibi yorumlanamaz. State machine öz-testinde exact hazırlık, yabancı source, untrusted signer, başarısız build, aktif olmayan adım ve eksik-evidence CLI negatifleri doğrulanmıştır; actual preparation `NOT_RUN` durumundadır.

Finalization state machine yalnız exact hazırlık diffini, hâlâ aktif governed signerı, süresi dolmamış sekiz sınıflı evidence bağını, 33-O ancestry ve iki remote HEAD eşitliğini kabul eder. `finalize:33-p:external-receipt` aynı imzalı paketi yeniden doğrular; manifesti, sekiz evidence belgesini ve Ed25519 public keyi no-overwrite yerel/D checkpoint içine byte/size/SHA-256 ile kalıcılaştırır. Registry zinciri ve requirement PASS yalnız receipt üreten final state içinde atomik olarak açılır; 33-Q yalnız `READY_NEXT` olur. Completion verifier local proof ile external source-protection modlarını ayırır. Finalization negatif matrisi 10/10 PASS’tir; actual finalization, persistent receipt, source protection ve completion verification `NOT_RUN` kalır.
