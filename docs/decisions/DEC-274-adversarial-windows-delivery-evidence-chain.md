...
Bronze local-test Windows teslimi yalnız aynı temiz exact commit, governed fingerprint ve kanonik kural hash'ini canlı geri okuyan schema-2 package provenance ile başlar. Önceki sürüm güveni eski schema-1 final makbuzundan veya değişmiş legacy nested kurulumdan alınamaz. Current package `parentRelease` ile bağlı immutable önceki schema-2 package provenance arşivi ve `C:\Program Files\PPT\ParsYuva-Bronze\ParsYuva-Bronze.exe` yolundaki canlı N runtime, arşivdeki packaged runtime ile SHA-256, boyut ve FileVersion bakımından exact eşleşmelidir.

Yükseltme aynı Bronze aylık sıra döneminde sequence tam bir artış ve monotonik tarih taşır. Legacy nested program ve kullanıcı verisi yalnız değişmezlik snapshot sınırıdır; trusted predecessor değildir ve silinmez. UAT110 V2 gerçek N→N+1 ile ayrı N+1→N+1 maintenance fazlarında kullanıcı verisi içerik eşitliğini, sentetik marker cleanup/absence readback'ini, diğer kanal ve legacy program/registry sıfır yazımını ve installed==packaged canlı kimliğini kanıtlar.
