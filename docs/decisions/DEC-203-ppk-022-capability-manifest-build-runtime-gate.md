...
1. Üretim TypeScript/JSX ağacı `@babel/parser` AST ile taranır. Her kaynak yüzeyi exact `kind|path|symbol` anahtarı, source-prefix ile sabit kanonik uygulama sahipliği, runtime capability ve exact bootstrap/signed-startup enforcement aşamasıyla manifestte bulunmalıdır. Literal `executeJavaScript` içindeki korunan API'ler de taranır; çözülemeyen dinamik yürütme fail-closed reddedilir. Parse edilemeyen kaynak, çözülemeyen dinamik import, yeni/stale/duplicate yüzey, owner/stage drift ve wildcard build'i durdurur.
...
Yedi kanonik runtime capability şunlardır: `camera.access`, `microphone.access`, `file.access`, `ocr.process`, `ai.process`, `location.access`, `network.access`. On dört kanonik uygulama exact registry'de bulunur. Deployed `windows-desktop` profili yerel çağrı ön-kontrolü için `camera.access` ve `microphone.access`, ayrıca `file.access`, `network.access` ve yerel OCR için `ocr.process` taşır; `windows-core-service` profili yalnız `file.access` ile `network.access` taşır. Diğer on iki profil boş capability kümesiyle not-deployed/profile-only kalır; özellikle `ocr-worker` capability kümesi boştur ve `communication-service` capability kümesi boştur. Bu aggregate Desktop capability kaydı fiziksel cihaz işlevi sertifikası, ayrı iletişim servisi, arama teslimi veya ağ erişimi yetkisi değildir. Build kaydı veya profil tanımı yetki vermez. Mevcut Windows OCR child-process yürütümü için `lowPrivilegeSandboxVerified=false` gerçeği korunur; capability kaydı düşük ayrıcalık doğrulaması iddiası değildir.
...
Core Service companion, yerel ürün lisansı, fabrika ayarına dönüş, kaldırma yedeği, şifreli sürüm-geçiş kasası, 33-W yerel model ve çevrimdışı aile haritası bağları sonrasındaki güncel source ratchet, tarihsel kapanış sayılarını değiştirmeden, 18 zone / 568 dosya / 428 exact capability yüzeyini sıfır bulguyla bağlar. Yeni beş dosya yüzeyi yalnız sabit kullanıcı-verisi altındaki `haritalar/turkiye.pmtiles` paketinin parça parça, salt-okunur ve link/realpath doğrulamalı okunması içindir. Yeni network yüzeyi yoktur; sabit `127.0.0.1:11434` Ollama döngüsel taşıması dışında genel uzak uç, renderer ağ yetkisi veya `ai-worker` kimliği oluşturulmaz. Güncel manifest SHA-256 değeri `1bf21d23c862afbccb9611083c093f9ced703adadf7a170c29f53479d21397b1` olarak sabitlenmiştir ve 26 bootstrap yüzeyi değişmemiştir.
