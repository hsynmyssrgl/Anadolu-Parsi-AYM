...
- Çağıranın `authorized=true` işareti yetki kanıtı sayılmaz; aday listesi yalnız production doğrulanmış arama authority sağlayıcısından kabul edilir. Sağlayıcı yoksa veri araması fail-closed kapanır.
...
- Windows yaşam döngüsü ve soak kanıtı çağıran boolean'larından alınmaz. Production sağlayıcı kimliği, kanıt SHA-256'sı, kanonik gözlem zamanı, bounded sayılar ve gerçek Windows lifecycle alanları exact doğrulanır; gelecekte tarihli kanıt reddedilir.
- UX tercihleri exact JSON anahtarları, mod/saat/array sınırları, optimistic revision, sahibi bağlı result hash ve durable PEP receipt ile korunur.
