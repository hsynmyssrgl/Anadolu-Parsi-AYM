...
- `DEC-275` — En küçük değişiklik dahi etkilenen tüm kaynak, sicil, belge, iş listesi, manifest/indeks, ana DOCX/PDF ve kanıt sözleşmelerini aynı zincirde günceller; hedefli ve tam test ile UI etkisinde bütün etkileşim/görsel UAT tamamlanmadan ve gerçek hatalar `wip(rejected)` kaydedilmeden paket üretilemez. `65db62ad` 34-B–34-F no-write sızıntısını, `90b5ad40` 34-G–34-K alt komut no-write aktarım boşluğunu, `47af84bd` ve `b34e951b` DOCX render altyapı hatalarını, `10282bf4` çift `ADR-039` kimliğini, `9f2aaf86` ise tarihsel başlık ayraçlarını yanlış reddeden ilk kapıyı FAIL olarak korur. 34-B 13/13 + altı makbuz, 34-G 37/37 + üç makbuz değişmezliği PASS; nihai ana belge görsel QA 28/28 PASS; Build 169 kararı `ADR-107` olarak benzersizleştirilmiş ve düzeltilmiş kimlik kapısı 2 dosya/12 test PASS vermiştir. Yeni exact tam regresyon ve kurulu tam UI UAT zorunludur
