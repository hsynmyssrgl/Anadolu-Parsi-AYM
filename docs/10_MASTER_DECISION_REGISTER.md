...
22.08.2026 tarihinde `.46` NSIS derlemesinde callback adresi bağlanmadığı için oluşan warning 6010 sonucu tarihsel FAIL olarak korunmuştur. Callback, compiler-bound `${NSD_CreateTimer}` ve `${NSD_KillTimer}` makrolarına alınır. `.47` paketlemesi tamamlanmış olsa da eski dahili `@pptdesktop-*.nsis.7z` payload bulunduğu için teslim reddedilmiş ve paket silinmiştir; retention kuralı bu dahili arşivleri de kapsayacak şekilde kalıcı testle kapatılmıştır. `.48` kurulu UAT'de ilk kullanıcı ve 2FA sonrası mevcut cihaz güveni iptal kaldığından Gösterge Paneli `AUTHORITY_RESOLUTION_FAILED` vermiş ve paket reddedilmiştir. `.49`, ana uygulama öncesinde yerel parola ve TOTP/kurtarma koduyla mevcut bilgisayarı güçlü biçimde güvenilir cihaz yapar; `trustedDevice=true` doğrulanmadan politika korumalı ekranı açmaz. Paketli ve kurulu runtime açılışı, sentetik aile/yönetici kullanıcısının ön yüzden oluşturulması, tam otomatik regresyon, SHA-256/imza durumu ile GitHub, haricî Git ve geri-okumalı haricî kaynak arşivi gerçek sonuçlarına göre kaydedilir. Gerçek kullanıcı verisi kullanılmaz. DEC-268, DEC-267 ve PR-233 kabul zincirini uygular.
