...
Master makine doğrulaması PASS sonrasında Vitest 4.1.10'un desteklemediği `--minWorkers` seçeneği çağrıyı test başlamadan durdurmuştur. Bu gerçek araç çağrısı reddi `86602f7a` ile aktif son rejected checkpoint olarak korunur; ürün veya master belge kusuru ve PASS değildir. Doğru Vitest çağrısı desteklenen seçeneklerle yeniden çalıştırılacaktır.
