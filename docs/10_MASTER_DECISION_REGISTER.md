...
24.08.2026 tarihinde her küçük mutasyonun etkilenen ana/kanal kaynakları, kural ve karar sicilleri, aktif/ticari belgeler, iş listesi, kapsam-envanter-ratchet-manifest-indeks, güncel ana DOCX/PDF ve kanıt sözleşmelerini aynı zincirde kapatması bağlandı. Hedefli ve filtresiz tam regresyon, typecheck, sözdizimi, kaynak bütünlüğü ile UI etkisinde bütün etkileşim ve görsel UAT zorunludur. Gerçek hata `wip(rejected)` checkpoint olarak kaydedilir; tam kapanıştan önce ara installer üretilemez. `65db62ad`, 34-B–34-F; `90b5ad40`, 34-G–34-K; `4c6652e0` ise PPK-022 masaüstü başlangıç `--no-write` aktarım kusurunu korur. Son düzeltmeden sonra kaynak regresyonu 1 dosya/6 test, masaüstü çalışma 51/51, sözleşme 41/41 ve PPK-022 24/24 PASS; 1.571 doğrulama dosyasında sıfır değişiklik vermiştir. `47af84bd`, `b34e951b` ve `6ec632c8` ana DOCX render altyapısındaki LibreOffice/Poppler PATH hatalarını, `10282bf4` çift `ADR-039` kimliğini korur. Mutlak araç yollarıyla güncel ana belge 28/28 görsel QA PASS vermiştir; önceki onaylı renderla 2–6 ve 27–28. sayfalar SHA-256 özdeş, değişen 1 ve 7–26. sayfalar yeniden görsel incelenmiştir. Build 164 `ADR-039`, Build 169 `ADR-107` olur. `9f2aaf86` kimlik kapısı yanlış-pozitifini korur; düzeltilmiş kapı 2 dosya/12 test PASS vermiştir. `4c8b6b7d`, 577 değişen yol/94 test PASS analizinden sonra zorunlu açık test listesi verilmeden yapılan hedefli üretici çağrısını fail-closed ret olarak korur; yeni koşu aynı 94 dosyayı exact açık argümanla aktaracaktır. Yeni exact hedefli ve tam regresyon olmadan paket yoktur. Bağlayıcı kayıt DEC-275 ve PR-240'tır.

`47f441e1`, Git-index ile üretilen kanonik artefakt indeksinin varsayılan canlı-ağaç modunda doğrulanması nedeniyle oluşan kapsam uyuşmazlığını ret olarak korur. Üreticiyle aynı `--git-index --no-report` doğrulaması 13.146 kontrol/4.407 dosya/2.143 belge, kaynak bütünlüğü 4.868/4.868 dosya ve 4.869 SHA PASS vermiştir; yanlış-mod ret PASS değildir.
...
`c7a3c130`, `80cf2a39` exact Bronze kaynağında hedefli 94 dosya/598 test ve filtresiz 398 dosya/2.469 test PASS sonrasında `33-R` runtime alt sürecinin migration manifestinde yalnız `generatedAt` değiştirmesini fail-closed ret olarak korur. `33-R` migration, data-store smoke, PPK-021 ve PPK-022 alt doğrulayıcılarına `--no-write` aktaracak biçimde düzeltilmiştir. Odaklı 1 dosya/6 test ve gerçek 33-R matrisi 11/11, 8 dosya/30 test PASS; migration manifest SHA-256 byte-exact değişmezdir. Yeni temiz exact commit hedefli/tam regresyonu ve exact Bronze kaynak bütünlüğü olmadan paket yoktur.

`51316ac3`, `bfb6db9f` exact Bronze kaynağında hedefli 94 dosya/598 test ve filtresiz 398 dosya/2.469 test PASS iken tam regresyon üreticisinin doğrudan `node` çağrısıyla npm ortam bağını kaybetmesini korur. 171 ek komuttan ortak npm alt çağrısı kullanan 34-B/C/D/F runtime’ları FAIL olmuştur. Kanonik npm ortamında 34-B 13/13, 5 dosya/30 test PASS; final exact tur yalnız resmî npm scriptiyle yeniden çalıştırılır.

`c02744cd` exact Bronze resmî npm tam regresyonu 398 dosya/2.469 test ve 171/171 ek komut PASS vermiştir. Sonraki kaynak bütünlüğü 683 eksik Git-dışı manifest payloadı ile linked-worktree `.git` dosyasının yanlış kaynak sayılması nedeniyle 684 bulguyla FAIL olmuş ve `7d67fcff` ile korunmuştur. Kanal kurulumu manifest payloadını güvenli yol/normal dosya/byte/SHA-256/atomik readback ile üç ayrı kanala eşitler; kaynak toplayıcı `.git` dosya ve klasörünü dışlar. Odaklı 2 dosya/9 test PASS; yeni exact kaynak bütünlüğü zorunludur.

`96b9faac` kaynaklı ana DOCX'in ilk PNG render denemesinde yanlış bundled PATH nedeniyle Poppler `pdfinfo` çözülememiş ve gerçek FAIL `e5787764` ile korunmuştur. Doğru Poppler `native\poppler\Library\bin` ve LibreOffice yollarıyla retry 28/28 sayfa üretmiştir; bütün sayfalar temas sayfalarında, 6–7 ve 27–28. sayfalar ayrıca özgün çözünürlükte görsel QA PASS vermiştir. Yanlış-yol ret kaydı PASS sayılmaz; yeni exact test, kaynak bütünlüğü ve kurulu tam UI UAT zorunludur.

Kapanış kayıtlarını içeren `7f866e69` kaynağından üretilen final master DOCX doğru native Poppler/LibreOffice zinciriyle 29/29 sayfa görsel QA PASS vermiştir. Beş temas sayfasında tüm belge, ayrıca özgün çözünürlükte 6–7 ve 26–29. sayfalar taşma, örtüşme, kesilme, font ve bozuk karakter açısından kusursuzdur.

`d421c299` exact Bronze hedefli 95 dosya/600 test, filtresiz 399 dosya/2.471 test, 172/172 ek komut ve 4.869/4.869 kaynak bütünlüğü PASS sonrasında governed preflight iki ayrı fail-closed boşluğu göstermiştir. `607a9a53` temizlenen installer makbuzlarının yeniden üretilmeden çağrılmasını, `8b2b5ccc` ise tamamlanmış çalışma adımlarının 1.428 kanıt yolundan 803 Git-dışı checkpoint dosyasının Bronze kanalında bulunmamasını korur. Kalıcı düzeltme, kanal worktree kurulumunda manifest payloadına ek olarak tamamlanmış `localEvidence` ve persistent receipt yollarını tracked/manifest dışlamalı kanonik yol, normal dosya, SHA-256, atomik yazım ve readback ile üç kanala eşitler. Seçici 809 yolu, odaklı sözleşme 1 dosya/9 testi ve ticari temel 1.234 kontrolü PASS vermiştir; yeni exact preflight PASS olmadan paket yoktur.

Checkpoint payload kalıcı düzeltmesini içeren `65ffd15d` kaynak commitinden üretilen güncel master DOCX, doğru native LibreOffice/Poppler zinciriyle 29/29 PNG sayfa vermiştir. Sayfaların tamamı özgün çözünürlükte tek tek incelenmiş; taşma, örtüşme, kesilme, font veya bozuk karakter kusuru görülmemiştir. Belge QA PASS'i yeni exact kanal eşitliği, test, preflight ve kurulu uygulama UAT kapılarının yerine geçmez.

`0f0a4653` exact Bronze etki analizi 95 hedefli test dosyası hesaplamıştır. Hedefli turda 600 test PASS iken `operation-rule-check-policy.test.ts`, aktif ürün yaşam döngüsünden ayrı PR-240 teslim kapanışının current-mutation `preflightStatus` alanında tarihsel `607a9a53/8b2b5ccc` FAIL metnini taşımasını reddetmiş ve gerçek sonuç `50f4d9e5` ile korunmuştur. Tarihsel preflight retleri QA alanlarında kalır; güncel mutasyon durumu `NOT_RUN_CURRENT_MUTATION` olur. Bronze/Silver/Gold hidrasyonu her kanalda 1.428/1.428 checkpoint yolu ve 0 eksik PASS vermiştir; yeni exact hedefli/tam regresyon olmadan paket yoktur.

`af2d15fa` sessiz canlı paket doğrulaması sırasında erken kesilen installer deneyimi UAT çağrısını, `151384be` ise doğrulama tamamlandığında görülen 2.795 preflight / 2.793 Git kaynak kapsamı uyuşmazlığını korur. İki fazla yol ignored Python `scripts/__pycache__/*.pyc` dosyalarıdır. Yönetilen çalışma ağacı fingerprint taraması `__pycache__` dizinlerini Git otoritesiyle aynı exact kapsam için dışlar; odaklı sözleşme 1 dosya/6 test PASS'tir. Eski paket kabul edilmez ve bütün exact teslim zinciri yeni committe yeniden üretilir.
...
`5179a170` exact hedefli 95 dosya/601 test ve filtresiz 399 dosya/2.472 test PASS iken 172 ek komuttan 34-B/C/D/F runtime'ları Bronze kanalında bulunmayan geçici npm CLI yoluna düştüğü için toplam regresyon FAIL olmuş ve `f8946730` ile korunmuştur; ortak npm-alt-adım tanısı `1cb33525`tir. 34-B–34-F npm CLI'yi aktif npm ortamı, aktif Node kurulumu ve kontrollü geçici fallback sırasıyla varlık doğrulamalı seçer. Odaklı 1 dosya/6 test ve gerçek no-write runtime sonuçları 34-B 13/13, 34-C 14/14, 34-D 14/14, 34-E 15/15, 34-F 15/15 PASS'tir; yeni exact bütün kapılar zorunludur.

Son belge QA turunda `c5879c9a` LibreOffice, `194a8281` Poppler PATH eksikliğini, `62d0b1d8` temas sayfası yardımcı komutunun sözdizimi hatasını korur. Doğrulanmış mutlak araç yollarıyla 29/29 sayfa render edilmiş; tüm sayfalar temaslarda, 6–8 ve 27–29 özgün çözünürlükte kusursuz bulunmuştur. Bu test altyapısı retleri uygulama/belge içerik kusuru değildir ve yeni exact teslim zincirini kaldırmaz.

## DEC-276 — Bronze 51 rejected predecessor recovery bootstrap

26.08.2026 tarihinde Bronze 50, immutable paket/external geçmişi korunarak `REJECTED_INVALID_PACKAGE` durumuna alınmıştır. Önceki `IN_PROGRESS` ledger durumu rejection alt kaydında saklanır; paket yeniden yazılmaz ve trusted installed runtime sayılmaz. `de6c3c7b` preflight `000a93e7/2792` ile exact Git `f96cd181/2793` arasındaki tek tracked Python bytecode farkı `1490bb9f` reddedilmiş checkpointiyle korunur. Yalnız exact Bronze 51, rejected-parent provenance bundle'ını `REJECTED_PARENT_HISTORY_ANCHOR_ONLY` olarak kullanıp installed predecessor girdisini reddederek temiz program kökü, EXE ve uninstall yokluğuyla recovery fresh-install ve ayrı same-version maintenance çalıştırabilir. Sequence 52 ve sonrası normal exact continuation kuralına döner. Bütün exact test, kaynak bütünlüğü, preflight/postflight, installer-experience, UAT110, UAT111, sesli anlatım ve final kurulu uygulama UAT kapıları zorunludur. Bağlayıcı kayıt DEC-276 ve PR-241'dir.

İlk odaklı doğrulamada `4ec0820d` stale V29 preflight, `de8776f1` sabit 60 iş sözleşmesi, `6a401791` eksik work-step ortamı ve `cb7e8749` güvenli reddin dar test mesajıyla eşlenmesi olarak korunmuştur. Düzeltilen V29 preflight PASS; recovery odaklı ilk tur 5 dosyada 64/65 PASS vermiştir. Tek negatif assertion reddi gevşetmeden düzeltilmiş, retry beklemektedir.

`cb7e8749` önceki rejected checkpoint olarak korunur; ilk 64/65 ile düzeltme sonrasındaki 5 dosya/65 test ve 9 dosya/82 test ara geçmişi değişmez. Güncel kod auditinde normal sequence 52+ yolunun varsayımsal `fileVersion` bağı, mode union `false`/`null` ayrımı, installer başlamadan exact `packageVersion` ve görünür release bağı, alias-free fixture sözleşmesi, Bronze 51 `IN_PROGRESS` yaşam döngüsü fail-closed kapısı ve immutable parent bundle transaction öncesi canlı path/size/SHA/identity geri-okuması kapatılmıştır. Yeni PR-241 hash ve operation-check zinciriyle son odaklı tur 7 dosya/84 test PASS'tir. Exact filtresiz tam regresyon, build, kurulum ve UAT110/UAT111/final kurulu uygulama UAT pendingdir.

`b0615638` sonrasındaki taze odaklı kapanış, master yönetişim sözleşmesi dahil 8 dosya/85 test ve ticari temel 1.254 kontrol/87 dosya/61 iş/241 kural PASS vermiştir. Önceki “son/güncel” checkpoint anlatımları kendi tarihsel aşamalarına aittir; aktif son rejected checkpoint `b0615638` olur. Exact hedefli/tam regresyon, kaynak bütünlüğü, build, kurulum ve tüm kurulu uygulama UAT kapıları pendingdir.

Manifest üreticisinin desteklemediği `--help` kipinde dosyaları yazması, bu çağrı öncesinde ayrı mutation kural kontrolü bulunmadığı için fail-closed süreç reddidir ve `3eec5426` ile aktif son rejected checkpoint olarak korunur. Bu olay ürün kusuru veya PASS değildir. Manifest kapanışta ayrı kural kontrolünden sonra kanonik üreticiyle son kez yeniden oluşturulacak; exact kapılar aynı temiz committe yeniden çalıştırılacaktır.

Master makine doğrulaması PASS sonrasında Vitest 4.1.10'un desteklemediği `--minWorkers` seçeneği çağrıyı test başlamadan durdurmuştur. Bu gerçek araç çağrısı reddi `86602f7a` ile o aşamadaki son rejected checkpoint olarak korunur; ürün veya master belge kusuru ve PASS değildir. Doğru Vitest çağrısı desteklenen seçeneklerle yeniden çalıştırılacaktır.

Sonraki hızlı PowerShell yardımcı taraması `ConvertFrom-Json` boş anahtar sınırlaması ve karar değişiminden sonra kural kontrolü yenilenmeden başlatılması nedeniyle fail-closed durmuş, gerçek çağrı hatası `4c83cee8` ile son rejected checkpoint olarak kaydedilmiştir. Bu kayıt ürün JSON kusuru veya PASS değildir; önceki `cb7e8749` tarihsel zincirde kalır. Exact full regresyon, build, kurulum ve bütün UAT kapıları pendingdir.

İlk master belge üretim çağrısı PATH'te düz `python` bulunmadığı için içerik üretimine başlamadan fail-closed durmuş ve `aae819cc` güncel rejected checkpoint olarak kaydedilmiştir; `4c83cee8` önceki çağrı reddi olarak korunur. Doğrulanmış bundled Python retry üretimi tamamlamış, master DOCX 29/29 sayfada temas ve özgün çözünürlük görsel QA PASS vermiştir. Bu sonuç ürün testleri, paket veya kurulu uygulama UAT kapılarının yerine geçmez.

Güncel masterın final render denemesinde eski document-skill `scripts/render_docx.py` yolu `ac2c02c1`, ardından eski bundled LibreOffice çalıştırılabilir çözümlemesi `5c07258c` ile fail-closed korunmuştur. `5c07258c` son rejected checkpointtir; ikisi de belge içerik kusuru veya PASS değildir. Güncel skill scripti, doğrulanmış sistem LibreOffice ve bundled Poppler retry ile 29/29 sayfa üretmiş; bütün sayfalar temaslarda, 1, 7–8 ve 27–29 özgün çözünürlükte kusursuz bulunmuştur.

Bu görsel PASS sonrasındaki V5 makine kapısı, doğrulayıcıda kalan sabit `GUNCEL-2026-08-24-V5` işaretini reddetmiş ve gerçek test sonucu `b0615638` ile korunmuştur. Güncel `latestRejectedCheckpoint` budur; belge içerik kusuru veya PASS değildir. Active document set sürümü/asOf 26.08.2026 olarak eşlenmiş, doğrulayıcı markerı kanonik kayıttan dinamik okuyacak şekilde düzeltilmiştir; retry PASS olmadan kapanış yoktur.

Düzeltme retryı 241 kural, 185 karar, 106 ADR, 13 DOCX tablo ve 26 PDF sayfa makine kontrolünü; V5 governance sözleşmesi 1/1 testi PASS vermiştir. Son DOCX renderı 29/29 sayfa üretmiş; 2–7 önceki onaylı renderla SHA-256 özdeş, değişen 1 ve 8–29 özgün çözünürlükte tek tek kusursuz bulunmuştur. `b0615638` tarihsel ret olarak kalır; bu belge kapanışı ürünün full exact/paket/kurulu UAT kapılarının yerine geçmez.

Bu kapanış sonrasındaki exact etki değerlendirmesi, `tools/windows-packager/package-lock.json` yolunun fail-closed bağımlılık sicilinde eşlenmediğini saptamış ve gerçek kapı reddini `3e496f47` boş `wip(rejected)` checkpointiyle korumuştur. Aktif son rejected checkpoint budur; `86602f7a` ve önceki retler tarihsel zincirde kalır. Kök neden için `tools/` genel güvenlik ağına, izole Windows paketleyici manifesti ve lockfile'ı supply-chain, sürüm ve paket provenance hedefli testlerine bağlanır. Yeni exact etki, hedefli/tam regresyon, kaynak bütünlüğü, pre/postflight, paket ve kurulu UAT zinciri yeniden çalıştırılmadan teslim yoktur.

Yeni iki kuralın doğru eşleştiği odaklı sözleşme turu, assertionın kanonik alfabetik `ruleIds` sırasını ters beklemesi nedeniyle 13/14 kalmış ve `398de9c8` ile reddedilmiştir. Assertion düzeltmesi sonrasında 3 dosya/14 test PASS vermiştir. `398de9c8` o aşamadaki son, bugün tarihsel rejected checkpoint olarak korunur; bu odaklı PASS exact kapanış değildir.

Aktif V5 görsel QA sırasında güncel render aracının desteklemediği yol argümanları `75e4072c`, yanlış Poppler PATH retryı `0f98c7cc` ile gerçek çağrı retleri olarak korunmuştur. Workspace bağımlılık kaydından doğrulanan native Poppler ve sistem LibreOffice ile güncel V5 DOCX 29/29 sayfa yeniden üretilmiş ve bütün sayfalar özgün çözünürlükte tek tek taşma, örtüşme, kesilme, font ve bozuk karakter açısından temiz bulunmuştur. `0f98c7cc` o aşamadaki son, bugün tarihsel checkpointtir; belge QA sonucu exact ürün kapanışı değildir.

DEC-275/DEC-276 metni güncellenirken kullanıcı karar sicili SHA-256 bağının aynı atomik kümede henüz eşlenmemesi operation-rule-check tarafından reddedilmiş; FAIL sonrasında generator çağrısının başlaması `9a370e51` boş `wip(rejected)` checkpointiyle korunmuştur. Aktif son rejected checkpoint budur. Olay ürün veya belge kusuru ve PASS değildir; karar/hash eşliği onarılıp taze kural PASS alınmadan üretici yeniden çalıştırılmaz.

Temiz `9f16699d` kaynak kapanışı sonrasındaki kanal setup çağrısı mevcut worktree'leri fast-forward etmediği ve önce exact HEAD eşliği aradığı için Bronze `89c4332f` üzerinde fail-closed durmuş; gerçek süreç reddi `dbefb586` boş `wip(rejected)` checkpointiyle korunmuştur. Aktif son rejected checkpoint budur. Kanallar ayrı açık `ff-only` ile nihai commite taşınır ve setup/eşitlik kapısı yeniden çalıştırılır; bu ret ürün kusuru veya PASS değildir.

Ana kaynak ve Bronze/Silver/Gold `63c55074` commitinde exact temiz eşitlik PASS sonrasında etki zinciri 105 değişen yol/19 hedef test üretmiş; hedefli 19 dosya/188 test PASS olmuştur. Filtresiz turda 399 dosya/2.480 test PASS iken yalnız PPK-015 üretim kaynak ratchet hash eşliği düşmüştür. `.51` uygulama metası kaynak içeriğini değiştirdiği halde dosya/bölge, bulgu, adapter ve amaç sayıları değişmemiştir: 18/590/0/2/3. Gerçek FAIL `cc922201` ile korunur; ratchet canlı `debfeecf460834f50cf328bff58b2c19ad94ef229610c4c829a35c4331ef235a` özetine eşlenir ve bütün exact kapılar yeniden çalıştırılır.

V5 master belge düzenleme başlangıç makbuzunda tek format bekleyen yardımcıya birleşik `docx,pdf` değeri verilmesi içerik üretmeden reddedilmiş ve `df92cdba` ile korunmuştur. Aktif son rejected checkpoint budur; ürün/belge içerik kusuru veya PASS değildir. Desteklenen tek `docx` formatlı retry PASS sonrasında üretim ve zorunlu render/görsel QA devam eder.

`df92cdba` sonrasında kapanış renderındaki 15. sayfa Durum token bölünmesi `787c5570` ile korunmuştur. Üretici görünür karar durumlarındaki alt çizgileri izinli boşluklara dönüştürdükten sonra final DOCX yine 29 sayfa vermiş; 2–7 ve 9–13 byte-exact aynı, değişen 1, 8 ve 14–29 özgün çözünürlükte tek tek kusursuz bulunmuş ve 29/29 görsel QA PASS olmuştur. Exact ürün ve kurulu uygulama UAT kapıları ayrıca tamamlanacaktır.

`6f9139e1` exact Bronze zinciri assessment 107 yol/21 hedef test, hedefli 21 dosya/211 test ve filtresiz 399 dosya/2.481 test PASS vermiştir. Source-integrity, governed preflight sonrasında yenilenen yedi indeks/dizin dosyası ile ticari temel kanıtının manifestte eski SHA taşımasını reddetmiştir. Gerçek üretici sırası FAIL'i `2abcf853` ile korunur ve aktif son rejected checkpoint olur. Governed preflight writer tüm yazan alt üreticilerden önce tamamlanır; manifest/SHA256SUMS son üretici olur ve yeni exact committe bütün kapılar tekrarlanır.

30 sayfalık V5 renderın sayfa 12/14 ilk çoklu görüntü incelemesi önizleme kırpmasını dosya kırpılması sanmış ve `d1e0b803` ile korunmuştur. Ayrı özgün çözünürlük/piksel ölçümü her iki PNG'de dört sütun başlığını eksiksiz doğrulamış; nihai belge 30/30 görsel PASS olmuştur. Aynı denetim ADR dizininde ana karar sicilinin bağlayıcı saydığı `ADR-067` dosyasının bulunmadığını saptamıştır. Gerçek kaynak kayıt bütünlüğü FAIL'i `f1590772` ile korunur ve aktif son rejected checkpoint olur. ADR-067, DEC-084 ile Migrasyon 38'in mevcut claim rezervasyonu gerçeğinden geri kurulur; ana kural kontrolü ve V5 verifier ADR-001'den en yüksek ADR'ye kesintisiz kaynak dizisini zorunlu kılar.

Güncel master PDF 26 sayfa olarak Poppler ile özgün çözünürlükte render edilmiştir. Sayfa 1–3 ve 6–26 taşma, örtüşme, kırpılma, font/glyph, tablo başlığı, footer ve sayfa numarası açısından PASS; sayfa 4/5'te uzun `Yerel` makine durumları dar sütunda `FAIL_CLO`/`SED` benzeri token-ortası bölünmeler üretmiştir. Gerçek okunabilirlik FAIL'i `6d94ab9e` ile korunur ve aktif son rejected checkpoint olur. Görünür paket/doğrulama durumları kanonik makine değerini değiştirmeden alt çizgiler yerine anlamsal boşluklarla render edilir; DOCX/PDF yeniden üretilip tam görsel QA tekrarlanır.

`6d94ab9e` düzeltmesi sonrasında görünür Yerel durumlar yalnız anlamsal boşluklardan sarılmış; önceki `WIT/H`, `CLO/SED` ve `ACCEPTA/NCE` bölünmeleri tekrarlanmamıştır. Son render DOCX 30/30 ve PDF 26/26 olmak üzere toplam 56/56 sayfada özgün çözünürlük görsel QA PASS vermiştir. ADR-067 her iki dizinde ADR-066/068 arasında doğru sırada ve exact dosya yoluyla görünür; taşma, örtüşme, kırpılma, font/glyph, bozuk karakter, tablo başlığı/sınırı, footer veya sayfa numarası kusuru yoktur.

Final-freeze render aracının ilk çağrısı desteklenmeyen `--libreoffice_path` ve `--poppler_path` argümanları nedeniyle belge testi başlamadan reddedilmiş ve `764e856b` boş `wip(rejected)` checkpointiyle korunmuştur. Doğru retry araç yollarını `PATH` üzerinden sağlamış; final kaynaklardan DOCX 30 ve PDF 26 sayfayı eksiksiz render etmiştir. Bu çağrı reddi ürün veya belge kusuru değildir; aktif son rejected checkpoint olarak kayıt zincirinde kalır.

Final-freeze2 çoklu önizlemesinde DOCX 2/7 header-footer/marjı ile PDF çift sayfa footerlarının kırpıldığı sanılmış; tam sayfa özgün çözünürlük readback bu mobilyaların eksiksiz olduğunu ve kırpmanın yalnız önizleme kaynaklı yanlış pozitif olduğunu göstermiştir. Aynı turdaki PDF 9 karar dizininde uzun exact dosya yollarının karakter ortasından sarılması gerçek okunabilirlik FAIL'idir ve bütün çağrı `17ad92d0` ile korunur. PDF karar/ADR yol hücreleri exact metni değiştirmeyen U+200B ayraç-sonrası kırma noktalarıyla üretilir; yeni tam render/görsel QA zorunludur.

Final-freeze3 30 DOCX ve 27 PDF sayfa üretmiş, makine kapısı PASS vermiştir. Ancak U+200B görünmez boşlukları ReportLab'in `splitLongWords` davranışını durdurmamış; PDF karar/ADR yollarında `root-caus/e`, `boundar/ies` ve `.m/d` gibi token-ortası bölünmeler sürmüştür. Gerçek FAIL `a0d9df42` boş `wip(rejected)` checkpointiyle korunur. Yol üreticisi artık yolu `/`, `-`, `_` sonrasında parçalar, en çok 48 karakterlik deterministik satırlar kurar ve yalnız bu ayraç sınırlarında gerçek satır sonu üretir.

Deterministik ayraç satırlarıyla üretilen final-freeze4, DOCX 30/30 ve PDF 27/27 olmak üzere toplam 57/57 tam tek-sayfa özgün çözünürlük görsel QA PASS vermiştir. Karar/ADR yolları yalnız `/`, `-`, `_` sonrasında sarılır; token ortası bölünme yoktur. ADR-067 iki formatta 066/068 arasında exact başlık/yolla görünür. Taşma, örtüşme, kırpılma, font/glyph, tablo, header/footer, marj veya sayfa numarası kusuru bulunmamıştır.
...
`e71bd0d2` exact mutasyon assessment/analysis zinciri 121 değişen dosya ve 23 hedef test hesaplayarak PASS vermiştir. Hedefli kanıt yürürken başlatılan salt-okunur canlılık kontrolünün zorunlu operation-rule makbuzunu güncellemesi test sonu validation/toolchain değişmezlik korumasını düşürmüş; gerçek yürütme sırası reddi `62d65314` boş `wip(rejected)` checkpointiyle korunmuştur. Bu olay ürün assertion başarısızlığı veya PASS değildir. Yeni temiz exact committe korumalı hedefli/tam testler sırasında paralel kural kontrolü çalıştırılmadan seri retry zorunludur.
