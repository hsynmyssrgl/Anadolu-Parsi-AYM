...
if ($build -ne 219) { throw "Unified Bronze Windows security lifecycle requires Build219; current build=$build." }
...
if (-not $EvidencePath) { $EvidencePath = Join-Path $validationRoot "build219-bronze-security-windows-release-lifecycle.json" }
...
function Add-StepResult {
  param([string]$Id,[string]$Status,[int]$ExitCode,[string]$Error,[datetimeoffset]$StartedAt)
  $steps.Add([ordered]@{
    id = $Id
    status = $Status
    exitCode = $ExitCode
...
function Invoke-RecordedProcess {
  ...
  try {
    ...
    Add-StepResult -Id $Id -Status $status -ExitCode $process.ExitCode -Error $null -StartedAt $startedAt
