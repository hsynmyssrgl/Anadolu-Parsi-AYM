...
const definitions=[
  ['scope inventory and five-test matrix are exact',JSON.stringify(scope.requirements)===JSON.stringify(inventory.requirements)&&JSON.stringify(scope.validation.targetedTestFiles)===JSON.stringify(inventory.implementedTargetedTests)&&tests.length===5&&scope.validation.targetedTestRatchet===22],
  ['migration 100 source manifest and scope checksums are canonical',migration?.name==='places_travel_asset_pet_workflows'&&migration?.checksum===sha&&sha===scope.validation.migrationSha256&&sha==='1cbdd671c0e2d6eb4a58d2d56d84ed63413e5f6585af252eefdaebc5600d18c4'],
  ...
  ['repository enforces exact receipt and payload-free policy resolution',has(repository,["authorization.purpose!=='general'",'writeBinding(context,row)','findItemForPolicyResolution','LIMIT 1001'])],
  ...
  ['production runtime resolves exact owner and visibility privacy',has(runtime,["resourceType === 'places_travel_item'",'ownerPersonId: found.value.ownerPersonId','placesTravelVisibilityPrivacy'])],
  ...
  ['tests cover replay owner participants workflows rollback IPC and route',has(tests.join('\n'),['replays exact command','active same-family travel participants','atomically rolls back','rejects renderer authority','existing Life route'])],
