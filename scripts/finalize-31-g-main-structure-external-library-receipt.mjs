...
const step = '31-G';
const localPackageRoot = 'C:\\PPT\\AYM\\09_ARSIV\\KAYNAK_AGACI\\checkpoints\\31-G_Main_Structure_Core_Service_API_Foundation';
const libraryRoot = 'D:\\AYM_LIBRARY\\Panthera pardus tulliana\\Anadolu Parsı Aile Yaşam Merkezi\\Bronze 04.08.2026.29\\checkpoints\\31-G_Main_Structure_Core_Service_API_Foundation';
const truth = 'Bu teslim, yukarıdaki kanıtlarla sınırlıdır; çalıştırılmayan hiçbir kontrol PASS sayılmamıştır.';
const payloadPaths = Object.freeze([
  'package.json',
  'docs/decisions/DEC-168-main-structure-first-core-service-api-foundation.md',
  'docs/audit/31-G_MAIN_STRUCTURE_CORE_SERVICE_API_FOUNDATION.md',
  'docs/current/09_KULLANICI_KARARLARI_KAYDI.md',
  'config/user-decision-ledger.json',
  'config/31-g-main-structure-core-service-api-foundation-scope.json',
  'artifacts/authority/31-G_MAIN_STRUCTURE_PRIORITY_AUTHORITY.json',
  'artifacts/validation/31-G_MAIN_STRUCTURE_PRIORITY_VALIDATION.json',
  'artifacts/checkpoints/31-G_EXECUTION_RECORD.json',
  'artifacts/inventory/31-G_SCOPE_AND_STATUS_REPORT.json',
  'artifacts/validation/31-G_MAIN_STRUCTURE_CORE_SERVICE_API_CONTRACT.json',
  'artifacts/validation/31-G_ROOT_TYPESCRIPT.json',
  'artifacts/validation/31-G_TARGETED_VITEST.json',
  'artifacts/validation/31-G_CORE_SERVICE_RUNTIME.json',
  'artifacts/validation/31-G_FULL_VITEST_REGRESSION.json',
  'artifacts/validation/31-G_PRODUCTION_BUILD.json',
  ...
  'scripts/start-31-g-main-structure-core-service-api-foundation.mjs',
  'scripts/verify-31-g-main-structure-core-service-api-foundation-contract.mjs',
  'scripts/run-31-g-main-structure-local-validation.mjs',
  'scripts/finalize-31-g-main-structure-external-library-receipt.mjs',
  'scripts/verify-31-g-main-structure-completion-transition.mjs'
]);
const paths = {
  receipt: 'artifacts/checkpoints/31-G_LIBRARY_RECEIPT.json',
  readback: 'artifacts/validation/31-G_LIBRARY_READBACK_VERIFICATION.json',
  receiptReadback: 'artifacts/validation/31-G_RECEIPT_READBACK_VERIFICATION.json',
  persistence: 'artifacts/validation/31-G_RECEIPT_READBACK_PERSISTENCE_VERIFICATION.json',
  inventory: 'artifacts/validation/31-G_LIBRARY_FINAL_INVENTORY_VERIFICATION.json',
  completion: 'artifacts/checkpoints/31-G_COMPLETION_RECORD.json',
  transition: 'artifacts/validation/31-G_COMPLETION_TRANSITION_VALIDATION.json',
...
assert(stepBefore?.status === 'IN_PROGRESS' && stepBefore.validationStatus === 'PASS' && stepBefore.persistentReceiptStatus === 'PENDING', '31-G is not local PASS awaiting receipt');
assert(executionBefore.status === 'LOCAL_PASS_AWAITING_LIBRARY_RECEIPT', '31-G execution is not local PASS');
assert(validations.every((item) => item.status === 'PASS'), '31-G validation evidence is not clean PASS');
...
const manifest = {
  schemaVersion: 1, release, step, primaryRequirement: 'DHA-001', phase: 'MAIN_STRUCTURE_FOUNDATION_CHECKPOINT_PACKAGE', status: 'PASS',
...
const manifestName = '31-G_CHECKPOINT_MANIFEST.json';
...
assert(JSON.stringify(await listFiles(localPackageRoot)) === JSON.stringify(expectedBase), 'Local 31-G package file set is not exact');
...
assert(baseReadback.every((item) => item.status === 'PASS'), 'D: 31-G base package readback mismatch');
...
const receiptBinding = await writeLocalPair(paths.receipt, {
  schemaVersion: 1, release, step, primaryRequirement: 'DHA-001', phase: 'MAIN_STRUCTURE_CORE_SERVICE_API_FOUNDATION_EXTERNAL_LIBRARY_RECEIPT',
  ...
  nextOfficialStep: 'AUTO_PRIORITY_SELECTION_AFTER_31-G_PERSISTENT_RECEIPT',
...
assert(stageOne.every((item) => item.status === 'PASS'), 'D: 31-G receipt readback mismatch');
...
assert(stageTwo.every((item) => item.status === 'PASS'), 'D: 31-G receipt persistence mismatch');
...
assert(JSON.stringify(beforeInventoryActual) === JSON.stringify(beforeInventoryExpected), 'D: 31-G pre-inventory file set is not exact');
...
assert(JSON.stringify(finalActual) === JSON.stringify(finalExpected), 'D: 31-G final inventory is not exact');
...
const step31G = plan.steps.find((item) => item.id === step);
Object.assign(step31G, { status: 'COMPLETED', validationStatus: 'PASS', persistentReceiptStatus: 'PASS', persistentReceiptPath: paths.receipt, completionTransitionStatus: 'PASS' });
for (const path of [paths.receipt, paths.readback, paths.receiptReadback, paths.persistence, paths.inventory, paths.completion, paths.transition]) if (!step31G.localEvidence.includes(path)) step31G.localEvidence.push(path);
plan.updatedAt = completedAt;
plan.segmentationNote = '31-G main-structure Core Service API foundation is immutable COMPLETED/PASS with exact D: USB readback. Data ownership migrations and Windows service installation remain open; no requirement is COMPLETE and no new Build is issued.';
await writeJson(full(paths.plan), plan);
const ledger = await readJson(full(paths.ledger));
ledger.libraryUploadStatus = '31-G_COMPLETED_RECEIPT_PASS';
ledger.nextOfficialTask = 'AUTO_PRIORITY_SELECTION_AFTER_31-G_PERSISTENT_RECEIPT';
ledger.activeMicroStep = null;
ledger.externalLibraryAuthority31G = { step, status: 'PASS', storageBackend: 'EXTERNAL_USB_D_DRIVE', path: libraryRoot, receipt: paths.receipt, focusedCheckpointOnly: true };
...
const transitionChecks = [
  ...
  ['work plan', step31G.status === 'COMPLETED' && step31G.persistentReceiptStatus === 'PASS'],
  ['governance', ledger.libraryUploadStatus === '31-G_COMPLETED_RECEIPT_PASS' && ledger.activeMicroStep === null],
...
assert(transitionChecks.every((item) => item.status === 'PASS'), '31-G completion transition failed');
...
console.log(`31-G external USB Library receipt: PASS (${expectedBase.length}/${expectedBase.length} base files; ${finalActual.length}/${finalExpected.length} final files).`);
console.log(`Library: ${libraryRoot}`);
console.log('31-G official step status: COMPLETED; requirements: OPEN; new Build: false.');
