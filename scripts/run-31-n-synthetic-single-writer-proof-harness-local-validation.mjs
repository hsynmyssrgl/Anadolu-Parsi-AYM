...
const paths = {
  ...
  failures: 'artifacts/checkpoints/31-N_INITIAL_VALIDATION_FAILURES.json',
  scopeConfig: 'config/31-n-synthetic-single-writer-proof-harness-scope.json',
  contract: 'artifacts/validation/31-N_SYNTHETIC_SINGLE_WRITER_PROOF_HARNESS_CONTRACT.json',
  typecheck: 'artifacts/validation/31-N_ROOT_TYPESCRIPT.json',
  targeted: 'artifacts/validation/31-N_TARGETED_VITEST.json',
  runtime: 'artifacts/validation/31-N_CORE_SERVICE_RUNTIME.json',
  security: 'artifacts/validation/31-N_SECURITY_AND_PREDECESSOR_REGRESSION.json',
  regression: 'artifacts/validation/31-N_FULL_VITEST_REGRESSION.json',
  build: 'artifacts/validation/31-N_PRODUCTION_BUILD.json',
  platform: 'artifacts/validation/platform-policy-gate.json',
  scope: 'artifacts/inventory/31-N_SCOPE_AND_STATUS_REPORT.json',
...
if (!step || plan.currentStep !== '31-N' || step.status !== 'IN_PROGRESS') throw new Error('31-N is not active');
const validationPending = execution.status === 'IN_PROGRESS_VALIDATION_PENDING' && scopeConfig.status === 'IN_PROGRESS';
const cleanRerun = execution.status === 'LOCAL_PASS_AWAITING_LIBRARY_RECEIPT'
  && execution.validationStatus === 'PASS'
  && execution.persistentReceiptStatus === 'PENDING'
  && scopeConfig.status === 'IN_PROGRESS';
if (!validationPending && !cleanRerun) throw new Error('31-N is not awaiting validation or an idempotent clean rerun');
if (initialFailures.countsAsPass !== false || initialFailures.status !== 'SUPERSEDED_FAILED_ATTEMPTS_NOT_PASS') {
  throw new Error('31-N failed attempts are not truthfully classified');
}
if (!initialFailures.failures.every((failure) =>
  String(failure.rerunStatus).startsWith('PASS') || failure.rerunStatus === 'REMEDIATED_AWAITING_CLEAN_RERUN'
)) {
  throw new Error('31-N initial failures do not have clean rerun evidence');
}

const contractRun = run('31-N contract', ['scripts/verify-31-n-synthetic-single-writer-proof-harness-contract.mjs']);
pass(contractRun);
const contract = await readJson(paths.contract);
if (contract.status !== 'PASS' || contract.failed !== 0 || contract.passed !== contract.expected) throw new Error('31-N contract is not clean PASS');
...
await writeJson(paths.typecheck, {
  ...
  step: '31-N',
  phase: 'ROOT_TYPESCRIPT_NO_EMIT',
  ...publicResult(typecheck),
  diagnosticCount: typecheck.status === 'PASS' ? 0 : null,
  executedAt: new Date().toISOString()
});
pass(typecheck);

const buildProjects = [
  'packages/core-service-contracts/tsconfig.json',
...
await writeJson(paths.targeted, {
  ...
  release: plan.release,
  step: '31-N',
  phase: 'TARGETED_VITEST',
  ...publicResult(targeted),
  testFilePassCount: targetedFiles,
  testPassCount: targetedTests,
  testFileFailCount: targeted.status === 'PASS' ? 0 : null,
  testFailCount: targeted.status === 'PASS' ? 0 : null,
  executedAt: new Date().toISOString()
});
pass(targeted);
...
await writeJson(paths.runtime, {
  ...
  release: plan.release,
  step: '31-N',
  phase: 'CORE_SERVICE_DETACHED_SYNTHETIC_HARNESS_RUNTIME_GATES',
  status: 'PASS',
  expected: runtimeResults.length,
  executed: runtimeResults.length,
  passed: runtimeResults.length,
  failed: 0,
  checks: runtimeResults.map(publicResult),
  executedAt: new Date().toISOString()
});
const platform = await readJson(paths.platform);
if (platform.status !== 'PASS' || platform.newBypassCount !== 0 || platform.runtimeStatus !== 'PASS') {
  throw new Error('Platform Policy is not clean PASS');
}
...
const securityCommands = [
  ...
  ['31-N contract verifier syntax', '--check', 'scripts/verify-31-n-synthetic-single-writer-proof-harness-contract.mjs'],
  ['31-N local validator syntax', '--check', 'scripts/run-31-n-synthetic-single-writer-proof-harness-local-validation.mjs'],
  ['31-N finalizer syntax', '--check', 'scripts/finalize-31-n-synthetic-single-writer-proof-harness-external-library-receipt.mjs'],
  ['31-N completion verifier syntax', '--check', 'scripts/verify-31-n-synthetic-single-writer-proof-harness-completion-transition.mjs']
];
...
await writeJson(paths.security, {
  ...
  release: plan.release,
  step: '31-N',
  phase: 'SYNTHETIC_SINGLE_WRITER_SECURITY_AND_PREDECESSOR_REGRESSION',
  status: 'PASS',
  expected: securityResults.length,
  executed: securityResults.length,
  passed: securityResults.length,
  failed: 0,
  checks: securityResults.map(publicResult),
  executedAt: new Date().toISOString()
});
...
await writeJson(paths.regression, {
  ...
  release: plan.release,
  step: '31-N',
  phase: 'FULL_VITEST_REGRESSION',
  ...publicResult(regression),
  testFilePassCount: fullFiles,
  testPassCount: fullTests,
  testFileFailCount: regression.status === 'PASS' ? 0 : null,
  testFailCount: regression.status === 'PASS' ? 0 : null,
  executedAt: new Date().toISOString()
});
pass(regression);
...
await writeJson(paths.build, {
  ...
  status: 'PASS',
  affectedPackageBuilds: packageBuilds.map(publicResult),
  electronMain: 'PASS',
  renderer: 'PASS',
  commands: [publicResult(electron), publicResult(renderer)],
  executedAt: new Date().toISOString()
});
...
const evidence = [
  paths.contract, paths.typecheck, paths.targeted, paths.runtime, paths.security, paths.regression,
  paths.build, paths.platform, paths.failures, paths.scope, paths.audit
];
...
plan.segmentationNote = `31-N synthetic single-writer proof harness is LOCAL_PASS_AWAITING_LIBRARY_RECEIPT after ${contract.passed}/${contract.expected} contract, root TypeScript, ${packageBuilds.length}/${packageBuilds.length} affected builds, ${targetedTests}/${targetedTests} targeted tests, ${runtimeResults.length}/${runtimeResults.length} runtime gates, ${securityResults.length}/${securityResults.length} security/predecessor regressions, full Vitest ${fullTests}/${fullTests}, and production build PASS. The harness remains synthetic and non-authoritative; no production lease, real gate PASS, runtime wiring, real data, SQLite transfer, or cutover authority is attached.`;
await writeJson(paths.plan, plan);
ledger.libraryUploadStatus = '31-N_SYNTHETIC_SINGLE_WRITER_HARNESS_LOCAL_PASS_AWAITING_LIBRARY_RECEIPT';
...
await writeJson(paths.scope, {
  ...
  release: plan.release,
  step: '31-N',
  primaryRequirement: scopeConfig.primaryRequirement,
  requirements: scopeConfig.requirements,
  status: 'LOCAL_PASS_AWAITING_LIBRARY_RECEIPT',
  officialStepStatus: 'IN_PROGRESS_AWAITING_LIBRARY_RECEIPT',
  targetSliceStatus: 'PASS',
  validationStatus: 'PASS',
