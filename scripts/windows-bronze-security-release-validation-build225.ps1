...
if ($build -ne 225) { throw "Unified Bronze Windows security lifecycle requires Build225; current build=$build." }
...
if (-not $EvidencePath) { $EvidencePath = Join-Path $validationRoot "build225-bronze-security-windows-release-lifecycle.json" }
...
function Add-StepResult {
  param([string]$Id,[string]$Status,[int]$ExitCode,[string]$Error,[datetimeoffset]$StartedAt,[string]$StdoutTail="",[string]$StderrTail="",[string]$StdoutPath="",[string]$StderrPath="")
  $steps.Add([ordered]@{
    id = $Id
    status = $Status
    exitCode = $ExitCode
...
function Invoke-RecordedProcess {
  ...
  try {
    ...
    $stdoutPath = Join-Path $validationRoot "build225-$safeId-full-stdout.log"
    $stderrPath = Join-Path $validationRoot "build225-$safeId-full-stderr.log"
    ...
    Add-StepResult -Id $Id -Status $status -ExitCode $process.ExitCode -Error $null -StartedAt $startedAt -StdoutTail $stdoutTail -StderrTail $stderrTail -StdoutPath (Split-Path -Leaf $stdoutPath) -StderrPath (Split-Path -Leaf $stderrPath)
