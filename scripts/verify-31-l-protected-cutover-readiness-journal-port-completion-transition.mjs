...
const paths = {
  ...
  scopeReport: 'artifacts/inventory/31-L_SCOPE_AND_STATUS_REPORT.json',
...
const later = inspectAuthorizedSuccessorLifecycle({ plan: docs.plan, ledger: docs.ledger, predecessorId: '31-L' });
...
const step = docs.plan.steps.find((item) => item.id === '31-L');
...
check((docs.ledger.libraryUploadStatus === '31-L_COMPLETED_RECEIPT_PASS' && docs.ledger.activeMicroStep === null) || (later.planValid && later.ledgerValid && later.nextTaskValid), 'ledger complete or authorized successor active');
...
check(docs.receipt.openBoundaries.durableReadinessJournal === 'PORT_ONLY_NO_PRODUCTION_ADAPTER' && docs.receipt.openBoundaries.trustedAnchorPersistence === 'PORT_ONLY_NO_PRODUCTION_ADAPTER', 'journal and anchor remain port-only with no adapter');
check(docs.receipt.openBoundaries.crashConsistency === 'NOT_PROVEN' && docs.receipt.openBoundaries.shutdownPersistence === 'NOT_PROVEN', 'durability proofs remain open');
...
if (failed.length) {
  console.error(`31-L completion transition: FAIL (${failed.length}/${checks.length}).`);
  for (const item of failed) console.error(`- ${item.name}`);
  process.exit(1);
}
console.log(`31-L completion transition: PASS (${checks.length}/${checks.length}${external ? '; D: external hash/size readback included' : ''}).`);
