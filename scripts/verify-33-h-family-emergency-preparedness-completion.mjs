...
if (process.argv.slice(2).some((argument) => !allowedArguments.has(argument))) {
  throw new Error('Unsupported 33-H completion verifier argument');
}
...
const canonicalLibraryRoot = 'D:\\AYM_LIBRARY\\Panthera pardus tulliana\\Anadolu Parsı Aile Yaşam Merkezi\\Bronze 04.08.2026.29\\checkpoints\\33-H_Family_Emergency_Preparedness';
const canonicalLocalCheckpointRoot = 'C:\\PPT\\AYM\\09_ARSIV\\KAYNAK_AGACI\\checkpoints\\33-H_Family_Emergency_Preparedness';
...
const paths = Object.freeze({
  ...
  inventory: 'artifacts/validation/33-H_LIBRARY_FINAL_INVENTORY_VERIFICATION.json',
  ...
  predecessorReceipt: 'artifacts/checkpoints/33-G_LIBRARY_RECEIPT.json',
...
const step = docs.plan.steps.find((item) => item.id === '33-H');
const successor = docs.plan.steps.find((item) => item.id === '33-I');
...
check(
  ...
  'receipt and completion bind the canonical 33-H D: and local archive paths'
);
...
check(
  (step?.status === 'COMPLETED' && step.validationStatus === 'PASS'
    ...
    || (allowPlanPending && docs.plan.currentStep === '33-H' && step?.status === 'IN_PROGRESS'
      && step.validationStatus === 'PENDING' && step.persistentReceiptStatus === 'PENDING'
      && step.completionTransitionStatus === 'PENDING'),
  'work plan preserves 33-H completion or the exact ledger-sealed recovery state'
);
check(successor?.status === 'PENDING' || successor?.status === 'IN_PROGRESS'
  || successor?.status === 'COMPLETED', '33-I successor is governed');
check(
  (docs.plan.currentStep === '33-H'
    && docs.ledger.libraryUploadStatus === '33-H_COMPLETED_RECEIPT_PASS'
    && docs.ledger.nextOfficialTask === 'AUTO_PRIORITY_SELECTION_AFTER_33-H_PERSISTENT_RECEIPT'
    && docs.ledger.activeMicroStep === null)
    || (laterLifecycle.planValid && laterLifecycle.ledgerValid && laterLifecycle.nextTaskValid),
  'ledger preserves 33-H completion through the governed 33-I-or-later lifecycle'
);
check(
  docs.ledger.externalLibraryAuthority33H?.step === '33-H'
    && docs.ledger.externalLibraryAuthority33H?.status === 'PASS'
    && docs.ledger.externalLibraryAuthority33H?.storageBackend === 'EXTERNAL_USB_D_DRIVE'
    && docs.ledger.externalLibraryAuthority33H?.path === canonicalLibraryRoot
    && docs.ledger.externalLibraryAuthority33H?.localCheckpointPath === canonicalLocalCheckpointRoot
    && docs.ledger.externalLibraryAuthority33H?.receipt === paths.receipt
    && docs.ledger.externalLibraryAuthority33H?.focusedCheckpointOnly === true,
  'ledger retains exact 33-H external library authority'
);
...
'scope binds exact DEC-219 requirement and evidence package');
...
check(proofKeys.every((key) => exactTruth(docs[key])),
  'all proof documents make no barcode, expiry verification, notification, sensor, readiness, or network claim');
check(
  finalEvidence?.finalClosureEvidence === true
    ...
    && docs.migrationManifest?.migrationVersions?.find((item) => item.version === 86)?.name
      === 'b5_family_emergency_preparedness_ledger'
    && docs.migrationManifest?.migrationVersions?.find((item) => item.version === 86)?.checksum
      === expectedMigrationChecksum,
  'scope binds exact 52/15/11, 3/14, PPK 545/277/242, migration 86 and dynamic full/build evidence'
);
check(
...
check(
  docs.predecessorReceipt.step === '33-G'
  ...
  '33-G receipt and sidecar bind an ancestor sourceCommit base'
);
check(proofKeys.every((key) => docs[key].predecessorStep === '33-G'
  ...
  && docs[key].sourceCommitRange
    === `${docs.predecessorReceipt.sourceCommit}..${docs.receipt.sourceCommit}`
  && docs[key].migration86Checksum === expectedMigrationChecksum),
'all proof documents bind the 33-G base range and migration 86 checksum');
check(docs.receipt.nextOfficialStep === '33-I' && docs.completion.nextOfficialStep === '33-I',
  'proof documents bind the governed 33-I successor');
...
check(
  ...
  'all 33-H proof documents bind one source commit'
);
check(
  [...proofKeys.map((key) => docs[key])].every((item) => item.mandatoryTruthSentence === truth),
  'all 33-H proof documents retain the exact mandatory truth sentence'
);
...
try {
  ...
  check(manifest.sourceCommit === docs.receipt.sourceCommit
    ...
    && manifest.sourceCommitRange
      === `${docs.predecessorReceipt.sourceCommit}..${docs.receipt.sourceCommit}`
    && manifest.migration86Checksum === expectedMigrationChecksum
  ...
  'manifest binds 33-G base, 33-H source, migration, DEC-219, requirements, payload mode, and truth');
  ...
  check(requiredRestorePaths.every((path) => sourcePaths.includes(path)),
    'manifest contains the exact 33-H restore-critical source and evidence set');
} catch (error) {
...
const verifyCheckpoint = async (checkpointRoot, label) => {
  ...
  try {
    ...
    check(false, `${label}: 33-H checkpoint is readable: ${error instanceof Error ? error.message : String(error)}`);
  }
};
...
if (failures.length > 0) {
  console.error(`33-H completion verification: FAIL (${failures.length}/${checks.length}).`);
  for (const item of failures) console.error(`- ${item.name}`);
  process.exit(1);
}
console.log(`33-H completion verification: PASS (${checks.length}/${checks.length}; local archive readback included${external ? '; D: external readback included' : ''}).`);
