...
const reportPath = resolve(option('--report', 'artifacts/validation/build182-validation-boundary.json'));
...
const [preflight, integrity, contract, runtime, syntax, packageTypes, desktopTypes, request, response] = await Promise.all([
  readJson('artifacts/validation/build182-source-preflight-final.json'),
  readJson('artifacts/validation/build182-source-integrity-final.json'),
  readJson('artifacts/validation/build182-root-trust-verification-contract.json'),
  readJson('artifacts/validation/build182-root-trust-verification-runtime.json'),
  readJson('artifacts/validation/build182-root-trust-verification-syntax.json'),
...
if (preflight.status !== 'PASS' || preflight.configuredCheckCount !== 162 || preflight.executedCheckCount !== 162 || preflight.passedCheckCount !== 162 || preflight.failedCheckCount !== 0 || preflight.notRunCheckCount !== 0) throw new Error('Build 182 source preflight must be PASS 162/162.');
if (integrity.status !== 'PASS') throw new Error('Build 182 source integrity must be PASS.');
if (contract.status !== 'PASS' || contract.assertions !== 56 || contract.failures?.length !== 0) throw new Error('Build 182 contract must be PASS 56/56.');
if (runtime.status !== 'PASS' || runtime.checks !== 18) throw new Error('Build 182 runtime must be PASS 18/18.');
if (syntax.status !== 'PASS' || syntax.checks !== 3) throw new Error('Build 182 syntax must be PASS 3/3.');
...
const results = [
  { id: 'source-preflight', status: 'PASS', evidence: 'artifacts/validation/build182-source-preflight-final.json' },
  { id: 'source-integrity', status: 'PASS', evidence: 'artifacts/validation/build182-source-integrity-final.json' },
