...
if (policyServiceAvailabilityGate.status !== 0) failures.push(policyServiceAvailabilityGate.stderr || policyServiceAvailabilityGate.stdout || 'policy service availability gate failed');
const report = {
  schemaVersion: 4,
  ...
  policyServiceAvailabilityGateStatus: policyServiceAvailabilityGate.status === 0 ? 'PASS' : 'FAIL',
...
console.log(`Platform Policy Gate: PASS / authorization bypass 0 / presentation conditions ${presentationConditions.length} / AST gate PASS / capability manifest gate PASS / application security profile gate PASS / policy service availability gate PASS.\n${runtime.stdout.trim()}`);
