...
const paths = {
  ...
  scopeReport: 'artifacts/inventory/31-M_SCOPE_AND_STATUS_REPORT.json',
...
const later = inspectAuthorizedSuccessorLifecycle({ plan: docs.plan, ledger: docs.ledger, predecessorId: '31-M' });
...
const step = docs.plan.steps.find((item) => item.id === '31-M');
...
check((docs.ledger.libraryUploadStatus === '31-M_COMPLETED_RECEIPT_PASS' && docs.ledger.activeMicroStep === null) || (later.planValid && later.ledgerValid && later.nextTaskValid), 'ledger complete or authorized successor active');
...
check(docs.receipt.openBoundaries.privateSigningMaterial === 'FORBIDDEN_NOT_PRESENT' && docs.receipt.openBoundaries.keyRotationAndRevocation === 'NOT_IMPLEMENTED', 'private material remains absent and key lifecycle remains open');
check(docs.scopeReport.deliveredBoundary.publicKeyInput === 'PUBLIC_KEY_KEYOBJECT_ONLY_NO_PEM_OR_PRIVATE_KEY_INPUT' && docs.scopeReport.deliveredBoundary.runtimeIntegration === 'NOT_WIRED', 'public-key-only verifier remains detached from runtime');
...
if (failed.length) {
  console.error(`31-M completion transition: FAIL (${failed.length}/${checks.length}).`);
  for (const item of failed) console.error(`- ${item.name}`);
  process.exit(1);
}
console.log(`31-M completion transition: PASS (${checks.length}/${checks.length}${external ? '; D: external hash/size readback included' : ''}).`);
