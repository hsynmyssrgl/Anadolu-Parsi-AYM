...
check('PPK-021 successor AST ratchet remains green', astGate.status === 'PASS' && astGate.productionSourceZones === 18 && astGate.scannedFiles >= 379 && astGate.privilegedSurfaces === 543 && astGate.exactAllowlistEntries === 543 && astGate.findings.length === 0);
check('PPK-022 successor capability ratchet remains green', capabilityGate.status === 'PASS' && capabilityGate.productionSourceZones === 18 && capabilityGate.scannedFiles >= 379 && capabilityGate.capabilitySurfaces === 242 && capabilityGate.exactManifestSurfaces === 242 && capabilityGate.findings.length === 0);
