...
const applicationVersion = process.argv[2] ?? '02.08.2026.226';
const lifecyclePath = process.argv[3] ?? 'artifacts/validation/build226-bronze-security-windows-release-lifecycle.json';
...
const integrityPath = process.argv[8] ?? 'artifacts/validation/build226-bronze-security-source-integrity-windows.json';
const reportPath = process.argv[9] ?? 'artifacts/validation/build226-bronze-security-windows-closure-result.json';
const expectedBuild = Number(applicationVersion.split('.').at(-1));
if (!Number.isInteger(expectedBuild) || expectedBuild !== 226) throw new Error(`Build226 unified verifier received unexpected application version: ${applicationVersion}`);
...
add(shared, 'lifecycle-version', lifecycle.applicationVersion === applicationVersion && lifecycle.build === 226, { applicationVersion: lifecycle.applicationVersion, build: lifecycle.build });
...
const inspectOpen021 = async (label, path, mode) => {
  ...
  for (const [index, run] of (evidence.runs ?? []).entries()) {
    ...
    add(open021, `${label}-run${index + 1}-status`, security?.status === 'PASS' && security?.platform === 'win32' && security?.build === 226, security?.status);
...
const inspectOpen022 = async (label, path, mode) => {
  ...
  for (const [index, run] of (evidence.runs ?? []).entries()) {
    ...
    add(open022, `${label}-run${index + 1}-side-status`, side?.status === 'PASS' && side?.platform === 'win32' && side?.build === 226, side?.status);
...
const report = {
  ...
  limitations: [
    'READY_TO_CLOSE is valid only for evidence generated on a real Windows host from the exact Build226 source snapshot.',
...
console.log(`Build226 unified Windows closure result: ${status} — OPEN-021=${report.closureReadiness.open021}, OPEN-022=${report.closureReadiness.open022}.`);
