...
const reportPath = process.argv[2] ?? 'artifacts/validation/build221-workspace-dist-prerequisite.json';
...
await writeFile(reportPath,`${JSON.stringify({schemaVersion:1,product:'Anadolu Parsı Aile Yaşam Merkezi',applicationVersion:'02.08.2026.221',packageVersion:'2.8.2026-221',build:221,purpose:'Fail-closed workspace package dist readiness before Windows package:win',status,checks:results.length,passCount:results.filter(x=>x.status==='PASS').length,results,generatedAt:new Date().toISOString()},null,2)}\n`);
console.log(`Build221 workspace dist prerequisite: ${status} (${results.filter(x=>x.status==='PASS').length}/${results.length}).`);
