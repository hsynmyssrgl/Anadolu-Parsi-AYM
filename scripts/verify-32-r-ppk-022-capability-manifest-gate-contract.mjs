...
check('manifest contains exactly 360 unique surfaces', entries.length === 360 && new Set(keys).size === 360);
...
check('gate scans all current production sources', gate.productionSourceZones === 18 && gate.scannedFiles === 525);
check('gate and manifest cardinality are exact', gate.capabilitySurfaces === 360 && gate.exactManifestSurfaces === 360);
...
check('domain boundary is fixed and content free', includesAll(domain, ['PlatformCapabilityManifestGateBoundaryView', 'protectedCapabilityCount: 7', 'canonicalApplicationCount: 14', 'exactAstSurfaceCount: 360', 'sourcePathsExposedToClient: false', 'manifestHashesExposedToClient: false']));
...
check('AST tests cover exact production and drift denial', includesAll(astTest, ['inventoryPlatformCapabilityManifestSurfaces()', 'inventory.files).toBe(525)', 'toHaveLength(360)', 'UNDECLARED_CAPABILITY_SURFACE', 'CAPABILITY_SURFACE_ENTRY_INVALID', 'APPLICATION_CAPABILITY_BASELINE_MISMATCH']));
...
check('decision pins current Desktop OCR capability truth and local translation source ratchet', includesAll(decision, ['525 dosya / 360 exact capability yüzeyi', manifestSha256, '`ocr-worker` capability kümesi boştur', '`lowPrivilegeSandboxVerified=false`']));
...
check('master register contains DEC-203 and current exact capability ratchet', masterRegister.includes('## DEC-203') && masterRegister.includes('DEC-203-ppk-022-capability-manifest-build-runtime-gate.md') && masterRegister.includes('525 dosya / 360 exact yüzey') && masterRegister.includes(manifestSha256));
