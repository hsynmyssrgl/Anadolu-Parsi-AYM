...
check('gate scans all current production sources', gate.productionSourceZones === 18 && gate.scannedFiles === 532);
...
check('AST tests cover exact production and drift denial', includesAll(astTest, ['inventoryPlatformCapabilityManifestSurfaces()', 'inventory.files).toBe(532)', 'toHaveLength(375)', 'UNDECLARED_CAPABILITY_SURFACE', 'CAPABILITY_SURFACE_ENTRY_INVALID', 'APPLICATION_CAPABILITY_BASELINE_MISMATCH']));
...
check('decision pins current Desktop OCR capability truth and family meeting source ratchet', includesAll(decision, ['532 dosya / 375 exact capability yüzeyi', manifestSha256, '`ocr-worker` capability kümesi boştur', '`lowPrivilegeSandboxVerified=false`']));
...
check('master register contains DEC-203 and current exact capability ratchet', masterRegister.includes('## DEC-203') && masterRegister.includes('DEC-203-ppk-022-capability-manifest-build-runtime-gate.md') && masterRegister.includes('532 dosya / 375 exact yüzey') && masterRegister.includes(manifestSha256));
