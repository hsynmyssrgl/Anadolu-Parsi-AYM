...
check('manifest contains exactly 435 unique surfaces', entries.length === 435 && new Set(keys).size === 435);
...
check('gate scans all current production sources', gate.productionSourceZones === 18 && gate.scannedFiles === 569);
check('gate and manifest cardinality are exact', gate.capabilitySurfaces === 435 && gate.exactManifestSurfaces === 435);
...
check('domain boundary is fixed and content free', includesAll(domain, ['PlatformCapabilityManifestGateBoundaryView', 'protectedCapabilityCount: 7', 'canonicalApplicationCount: 14', 'exactAstSurfaceCount: 435', 'sourcePathsExposedToClient: false', 'manifestHashesExposedToClient: false']));
...
check('AST tests cover exact production dynamic execution and drift denial', includesAll(astTest, ['inventoryPlatformCapabilityManifestSurfaces()', 'inventory.files).toBe(569)', 'toHaveLength(435)', 'CAPABILITY_DYNAMIC_EXECUTION_UNRESOLVED', 'UNDECLARED_CAPABILITY_SURFACE', 'CAPABILITY_SURFACE_ENTRY_INVALID', 'APPLICATION_CAPABILITY_BASELINE_MISMATCH']));
...
check('historical decision preserves capability and sandbox truth without acting as current ratchet', includesAll(decision, ['`ocr-worker` capability kümesi boştur', '`communication-service` capability kümesi boştur', '`lowPrivilegeSandboxVerified=false`']));
...
check('master register contains the historical DEC-203 reference', masterRegister.includes('## DEC-203') && masterRegister.includes('DEC-203-ppk-022-capability-manifest-build-runtime-gate.md'));
