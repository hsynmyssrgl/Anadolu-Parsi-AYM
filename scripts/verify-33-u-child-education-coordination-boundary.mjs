...
const definitions=[
  ...
  ['application enforces age private space pickup budget and local truth',hasAll(application,["visibility !== 'adolescent_private'","input.kind === 'pickup_authority'","input.kind === 'allowance_budget'",'pickupCredentialIssuance','allowancePaymentExecution'])],
  ...
  ['IPC exposes exact four channels with bounded safe results',hasAll(ipc,["'childEducation:getCenter'","'childEducation:createItem'","'childEducation:updateItem'","'childEducation:deleteItem'",'childEducationReceiptResult'])],
  ['renderer extends Life route with age privacy and local-only boundaries',hasAll(panel,['Çocuk eğitim merkezi','adolescent_private','Kimlik Merkezi referansı','okul portalına bağlanmaz','servisi canlı izlemez'])],
