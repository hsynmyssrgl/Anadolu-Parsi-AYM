...
const reportPath=resolve(option('--report','artifacts/validation/build164-validation-boundary.json'));
...
const [sourcePreflight,sourceIntegrity,contract,runtime,syntax,packageSourceTypes,desktopMainTypes,requestCreation,responseStatus]=await Promise.all([
 readJson('artifacts/validation/build164-source-preflight-final.json'),readJson('artifacts/validation/build164-source-integrity-final.json'),readJson('artifacts/validation/build164-ipc-adaptive-resource-budget-contract.json'),readJson('artifacts/validation/build164-ipc-adaptive-resource-budget-runtime.json'),readJson('artifacts/validation/build164-ipc-adaptive-resource-budget-syntax.json'),readJson('artifacts/validation/package-source-typecheck.json'),readJson('artifacts/validation/desktop-main-source-typecheck.json'),readJson('artifacts/validation/build154-handoff-request-creation.json'),readJson('artifacts/validation/build154-handoff-response-status.json')
]);
if(sourcePreflight.status!=='PASS')throw new Error('Build 164 source preflight must be PASS.');
if(sourceIntegrity.status!=='PASS')throw new Error('Build 164 source integrity must be PASS.');
if(contract.status!=='PASS'||contract.assertions!==42)throw new Error('Build 164 adaptive budget contract must be PASS 42/42.');
if(runtime.status!=='PASS'||runtime.checks!==40)throw new Error('Build 164 adaptive budget runtime must be PASS 40/40.');
if(syntax.status!=='PASS'||syntax.fileCount!==7)throw new Error('Build 164 adaptive budget syntax must be PASS 7/7.');
...
const results=[
 {id:'source-preflight',status:'PASS',evidence:'artifacts/validation/build164-source-preflight-final.json'},
 {id:'source-integrity',status:'PASS',evidence:'artifacts/validation/build164-source-integrity-final.json'},
