...
const reportPath=option('--report','artifacts/validation/build160-ipc-request-lifecycle-syntax.json');
...
const files=[
  ...
  'apps/desktop/src/main/ipc-transport-context.ts',
...
const report={schemaVersion:1,product:'Anadolu Parsı Aile Yaşam Merkezi',featureBuild:160,stage:'Bronze RC2 Active Development',status:failures.length?'FAIL':'PASS',fileCount:files.length,files,failures,generatedAt:new Date().toISOString()};
await mkdir(dirname(reportPath),{recursive:true});await writeFile(reportPath,`${JSON.stringify(report,null,2)}\n`);
if(failures.length){console.error(`Build 160 IPC lifecycle syntax: FAIL (${files.length-failures.length}/${files.length})`);for(const failure of failures)console.error(`- ${failure}`);process.exitCode=1;}else console.log(`Build 160 IPC lifecycle syntax: PASS (${files.length}/${files.length}).`);
