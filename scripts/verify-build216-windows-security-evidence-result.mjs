...
const applicationVersion = process.argv[2] ?? '01.08.2026.216';
...
const reportPath = process.argv[5] ?? 'artifacts/validation/build216-windows-security-evidence-result.json';
const expectedBuild = Number(applicationVersion.split('.').at(-1));
if (!Number.isInteger(expectedBuild) || expectedBuild !== 216) throw new Error(`Build216 evidence verifier received unexpected application version: ${applicationVersion}`);
...
console.log(`Build216 Windows security evidence result: ${status} (${checks.filter((item) => item.status === 'PASS').length}/${checks.length}).`);
