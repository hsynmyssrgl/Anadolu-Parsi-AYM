...
const step = '31-O';
const localPackageRoot = 'C:\\PPT\\AYM\\09_ARSIV\\KAYNAK_AGACI\\checkpoints\\31-O_Synthetic_Key_Lifecycle_Proof_Harness';
const libraryRoot = 'D:\\AYM_LIBRARY\\Panthera pardus tulliana\\Anadolu Parsı Aile Yaşam Merkezi\\Bronze 04.08.2026.29\\checkpoints\\31-O_Synthetic_Key_Lifecycle_Proof_Harness';
const truth = 'Bu teslim yalnız 31-O sentetik ve gönderilemez anahtar yaşam döngüsü düzeneğini kanıtlar; gerçek KEY_LIFECYCLE_PROOF, korumalı sağlayıcı, runtime bağlantısı, gerçek anahtar/veri veya cutover PASS değildir.';
const payloadPaths = Object.freeze([
  ...
  'docs/audit/31-O_SYNTHETIC_KEY_LIFECYCLE_PROOF_HARNESS.md',
  ...
  'config/31-o-synthetic-key-lifecycle-proof-harness-scope.json',
  'artifacts/authority/31-O_SYNTHETIC_KEY_LIFECYCLE_PROOF_HARNESS_AUTHORITY.json',
  'artifacts/validation/31-O_PRIORITY_SELECTION_VALIDATION.json',
  'artifacts/checkpoints/31-O_EXECUTION_RECORD.json',
  'artifacts/checkpoints/31-O_INITIAL_VALIDATION_FAILURES.json',
  'artifacts/inventory/31-O_SCOPE_AND_STATUS_REPORT.json',
  'artifacts/validation/31-O_SYNTHETIC_KEY_LIFECYCLE_PROOF_HARNESS_CONTRACT.json',
  'artifacts/validation/31-O_ROOT_TYPESCRIPT.json',
  'artifacts/validation/31-O_TARGETED_VITEST.json',
  'artifacts/validation/31-O_CORE_SERVICE_RUNTIME.json',
  'artifacts/validation/31-O_SECURITY_AND_PREDECESSOR_REGRESSION.json',
  'artifacts/validation/31-O_FULL_VITEST_REGRESSION.json',
  'artifacts/validation/31-O_PRODUCTION_BUILD.json',
  ...
  'scripts/start-31-o-synthetic-key-lifecycle-proof-harness.mjs',
  'scripts/verify-31-o-synthetic-key-lifecycle-proof-harness-contract.mjs',
  'scripts/run-31-o-synthetic-key-lifecycle-proof-harness-local-validation.mjs',
  'scripts/finalize-31-o-synthetic-key-lifecycle-proof-harness-external-library-receipt.mjs',
  'scripts/verify-31-o-synthetic-key-lifecycle-proof-harness-completion-transition.mjs',
  'scripts/verify-31-n-synthetic-single-writer-proof-harness-contract.mjs',
  'scripts/verify-31-n-synthetic-single-writer-proof-harness-completion-transition.mjs'
]);
const paths = {
  receipt: 'artifacts/checkpoints/31-O_LIBRARY_RECEIPT.json',
  readback: 'artifacts/validation/31-O_LIBRARY_READBACK_VERIFICATION.json',
  receiptReadback: 'artifacts/validation/31-O_RECEIPT_READBACK_VERIFICATION.json',
  persistence: 'artifacts/validation/31-O_RECEIPT_READBACK_PERSISTENCE_VERIFICATION.json',
  inventory: 'artifacts/validation/31-O_LIBRARY_FINAL_INVENTORY_VERIFICATION.json',
  completion: 'artifacts/checkpoints/31-O_COMPLETION_RECORD.json',
  transition: 'artifacts/validation/31-O_COMPLETION_TRANSITION_VALIDATION.json',
...
const [planBefore, executionBefore, scopeBefore, ...validations] = await Promise.all([
  ...
  readJson(full(paths.scopeConfig)),
...
assert(localPass || recoverable, '31-O is neither local PASS awaiting receipt nor a recoverable finalization');
assert(validations.every((item) => item.status === 'PASS'), '31-O validation is not clean PASS');
...
const manifest = {
  schemaVersion: 1, release, step, primaryRequirement: 'DHA-001',
  phase: 'SYNTHETIC_KEY_LIFECYCLE_PROOF_HARNESS_CHECKPOINT_PACKAGE', status: 'PASS',
...
const manifestName = '31-O_CHECKPOINT_MANIFEST.json';
...
const receipt = await writePair(paths.receipt, {
  schemaVersion: 1, release, step, primaryRequirement: 'DHA-001', phase: 'SYNTHETIC_KEY_LIFECYCLE_PROOF_HARNESS_EXTERNAL_LIBRARY_RECEIPT',
  ...
  openBoundaries: scopeBefore.openBoundaries, nextOfficialStep: 'AUTO_PRIORITY_SELECTION_AFTER_31-O_PERSISTENT_RECEIPT',
...
const completion = {
  ...
  nextOfficialStep: 'AUTO_PRIORITY_SELECTION_AFTER_31-O_PERSISTENT_RECEIPT',
  currentAuthoritativeSourceExternalProtectionStatus: 'SEPARATE_GOV_005_REFRESH_REQUIRED_AFTER_SOURCE_CHANGE',
...
const step31O = plan.steps.find((item) => item.id === step);
Object.assign(step31O, { status: 'COMPLETED', validationStatus: 'PASS', persistentReceiptStatus: 'PASS', persistentReceiptPath: paths.receipt, completionTransitionStatus: 'PASS' });
for (const path of [paths.receipt, paths.readback, paths.receiptReadback, paths.persistence, paths.inventory, paths.completion, paths.transition]) {
  if (!step31O.localEvidence.includes(path)) step31O.localEvidence.push(path);
}
plan.updatedAt = completedAt;
plan.segmentationNote = '31-O synthetic key-lifecycle proof harness is immutable COMPLETED/PASS with exact D: USB readback. It remains non-submittable and detached; real KEY_LIFECYCLE_PROOF, protected provider, DEC-171 cutover, real key/data access, and SQLite ownership transfer remain blocked.';
await writeJson(full(paths.plan), plan);
const ledger = await readJson(full(paths.ledger));
ledger.libraryUploadStatus = '31-O_COMPLETED_RECEIPT_PASS';
ledger.nextOfficialTask = 'AUTO_PRIORITY_SELECTION_AFTER_31-O_PERSISTENT_RECEIPT';
ledger.activeMicroStep = null;
ledger.externalLibraryAuthority31O = { step, status: 'PASS', storageBackend: 'EXTERNAL_USB_D_DRIVE', path: libraryRoot, receipt: paths.receipt, focusedCheckpointOnly: true };
...
const checks = [
  ...
  ['work plan', step31O.status === 'COMPLETED' && step31O.persistentReceiptStatus === 'PASS'],
  ['governance', ledger.libraryUploadStatus === '31-O_COMPLETED_RECEIPT_PASS' && ledger.activeMicroStep === null],
  ['synthetic-only boundary', scopeConfig.targets.stateMachine === 'PURE_SYNTHETIC_KEY_LIFECYCLE_ONLY' && scopeConfig.targets.candidateClassification === 'MODELED_GATE_ONLY_NO_GATE_ID_NON_SUBMITTABLE'],
  ['runtime and provider detached', scopeConfig.targets.runtimeIntegration === 'NOT_WIRED' && scopeConfig.openBoundaries.productionProtectedProvider === 'NOT_ATTACHED'],
  ['real key gate blocked', scopeConfig.targets.realKeyMaterial === 'NOT_ACCESSED' && scopeConfig.targets.realKeyLifecycleGate === 'NOT_SATISFIED'],
...
assert(checks.every((item) => item.status === 'PASS'), '31-O transition failed');
...
console.log(`31-O external USB Library receipt: PASS (${expectedBase.length}/${expectedBase.length} base files; ${finalActual.length}/${finalExpected.length} final files).`);
console.log(`Library: ${libraryRoot}`);
console.log('31-O official step status: COMPLETED; synthetic-only lifecycle harness: DETACHED/NON-SUBMITTABLE; DEC-171 cutover: BLOCKED; new Build: false.');
