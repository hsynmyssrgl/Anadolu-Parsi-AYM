...
if ($build -ne 224) { throw "Unified Bronze Windows closure requires Build224; current build=$build." }
...
try {
  ...
  Invoke-RequiredStep -Id "exact-source-integrity" -Operation {
    & node.exe "scripts\verify-source-integrity.mjs" "--report" "artifacts\validation\build224-bronze-security-source-integrity-windows.json"
  }
  ...
  Invoke-RequiredStep -Id "workspace-package-dist-guard" -Operation {
    & node.exe "scripts\verify-build224-workspace-dist-prerequisite.mjs"
  }
  ...
  $lifecycleArgs=@("-NoProfile","-ExecutionPolicy","Bypass","-File","scripts\windows-bronze-security-release-validation-build224.ps1","-EvidencePath","artifacts\validation\build224-bronze-security-windows-release-lifecycle.json")
  ...
  & node.exe "scripts\verify-build224-bronze-security-windows-result.mjs" $applicationVersion `
    "artifacts\validation\build224-bronze-security-windows-release-lifecycle.json" `
    ...
    "artifacts\validation\build224-bronze-security-source-integrity-windows.json" `
    "artifacts\validation\build224-bronze-security-windows-closure-result.json"
...
$resultPath=Join-Path $validationRoot "build224-bronze-security-windows-closure-result.json"
$result=$null
...
$summaryPath=Join-Path $validationRoot "build224-bronze-security-windows-run-summary.json"
$summary=[ordered]@{
...
if((Test-Path -LiteralPath $sourceManifestPath -PathType Leaf)-and(Test-Path -LiteralPath $sourceShaSumsPath -PathType Leaf)){
  $evidencePaths=@(
    $summaryPath,
    (Join-Path $validationRoot "build224-bronze-security-source-integrity-windows.json"),
    (Join-Path $validationRoot "build224-bronze-security-windows-release-lifecycle.json"),
  ...
  $manifestPath=Join-Path $validationRoot "build224-bronze-security-windows-evidence-manifest.json"
  ...
  $bundlePath=Join-Path $validationRoot "Bronze_Guvenlik_Windows_Kanitlari_Build224_$runId.zip"
