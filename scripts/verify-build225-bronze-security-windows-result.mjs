...
const applicationVersion = process.argv[2] ?? '02.08.2026.225';
const lifecyclePath = process.argv[3] ?? 'artifacts/validation/build225-bronze-security-windows-release-lifecycle.json';
...
const integrityPath = process.argv[8] ?? 'artifacts/validation/build225-bronze-security-source-integrity-windows.json';
const reportPath = process.argv[9] ?? 'artifacts/validation/build225-bronze-security-windows-closure-result.json';
const expectedBuild = Number(applicationVersion.split('.').at(-1));
if (!Number.isInteger(expectedBuild) || expectedBuild !== 225) throw new Error(`Build225 unified verifier received unexpected application version: ${applicationVersion}`);
...
add(shared, 'lifecycle-version', lifecycle.applicationVersion === applicationVersion && lifecycle.build === 225, { applicationVersion: lifecycle.applicationVersion, build: lifecycle.build });
...
const inspectOpen021 = async (label, path, mode) => {
  ...
  for (const [index, run] of (evidence.runs ?? []).entries()) {
    ...
    add(open021, `${label}-run${index + 1}-status`, security?.status === 'PASS' && security?.platform === 'win32' && security?.build === 225, security?.status);
  ...
  add(open021, `${label}-full-process-logs-declared`, Array.isArray(evidence.fullProcessLogs) && evidence.fullProcessLogs.length === 4, evidence.fullProcessLogs);
  for (const log of evidence.fullProcessLogs ?? []) add(open021, `${label}-full-log-${log}`, await exists(join(dirname(path), log)), log);
};
...
const inspectOpen022 = async (label, path, mode) => {
  ...
  for (const [index, run] of (evidence.runs ?? []).entries()) {
    ...
    add(open022, `${label}-run${index + 1}-side-status`, side?.status === 'PASS' && side?.platform === 'win32' && side?.build === 225, side?.status);
    add(open022, `${label}-run${index + 1}-safe-storage-behavior`, side?.safeStorage?.protectionId === 'electron-safe-storage-v1' && side?.safeStorage?.encryptionAvailable === 'PASS' && side?.safeStorage?.encryptDecryptRoundTrip === 'PASS', side?.safeStorage);
    add(open022, `${label}-run${index + 1}-provider-basis`, side?.safeStorage?.provider === 'windows-dpapi' && side?.safeStorage?.providerBasis === 'electron-safe-storage-windows-platform-contract', side?.safeStorage);
  ...
  add(open022, `${label}-full-process-logs-declared`, Array.isArray(evidence.fullProcessLogs) && evidence.fullProcessLogs.length === 4, evidence.fullProcessLogs);
  for (const log of evidence.fullProcessLogs ?? []) add(open022, `${label}-full-log-${log}`, await exists(join(dirname(path), log)), log);
};
...
const report = {
  ...
  limitations: [
    'READY_TO_CLOSE is valid only for evidence generated on a real Windows host from the exact Build225 source snapshot.',
...
console.log(`Build225 unified Windows closure result: ${status} — OPEN-021=${report.closureReadiness.open021}, OPEN-022=${report.closureReadiness.open022}.`);
