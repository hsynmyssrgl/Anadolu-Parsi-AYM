...
if (process.argv.slice(2).some((argument) => !allowedArguments.has(argument))) {
  throw new Error('Unsupported 33-J finalizer argument');
}
const dryRun = process.argv.includes('--dry-run');
const release = 'Bronze 04.08.2026.29';
const stepId = '33-J';
const successorId = '33-K';
...
const predecessorReceiptPath = 'artifacts/checkpoints/33-I_LIBRARY_RECEIPT.json';
const predecessorReceiptSidecarPath = `${predecessorReceiptPath}.sha256`;
const localPackageRoot = 'C:\\PPT\\AYM\\09_ARSIV\\KAYNAK_AGACI\\checkpoints\\33-J_Family_Emergency_Card_Portability';
const libraryRoot = 'D:\\AYM_LIBRARY\\Panthera pardus tulliana\\Anadolu Pars\u0131 Aile Ya\u015fam Merkezi\\Bronze 04.08.2026.29\\checkpoints\\33-J_Family_Emergency_Card_Portability';
...
const supportingPayloadPaths = Object.freeze([
  ...
  'packages/domain/src/platform-capability-manifest-gate.ts',
  ...
  'packages/platform-policy/src/platform-capability-manifest-policy.ts',
...
const paths = Object.freeze({
  ...
  inventory: 'config/33-j-family-emergency-card-portability-inventory.json',
  ...
  finalInventory: 'artifacts/validation/33-J_LIBRARY_FINAL_INVENTORY_VERIFICATION.json',
...
const writeGovernanceJsonAtomic = async (path, value) => {
  ...
  const temporary = resolve(root, '.tmp', '33-j-governance-commit', `${basename(path)}.${process.pid}.tmp`);
...
const exactEvidence = (scope, boundary, contract, runtime, migrationManifest) => {
  ...
  return scope?.validation?.finalEvidence?.finalClosureEvidence === true
    ...
    && contract.migration88Checksum === expectedMigrationChecksum
...
if (dryRun) {
  ...
  const recovery = plan?.currentStep === stepId
    ...
    && ledger?.libraryUploadStatus === '33-J_COMPLETED_RECEIPT_PASS'
    && ledger?.nextOfficialTask === 'AUTO_PRIORITY_SELECTION_AFTER_33-J_PERSISTENT_RECEIPT'
    && ledger?.activeMicroStep === null
    && ledger?.externalLibraryAuthority33J?.status === 'PASS'
    && ledger?.externalLibraryAuthority33J?.path === libraryRoot
    && ledger?.externalLibraryAuthority33J?.localCheckpointPath === localPackageRoot
    && ledger?.externalLibraryAuthority33J?.receipt === paths.receipt;
  const activeReady = plan?.currentStep === stepId
    ...
    && ledger?.libraryUploadStatus === '33-J_LOCAL_PASS_AWAITING_LIBRARY_RECEIPT'
    && ledger?.nextOfficialTask
      === '33-J governed offline emergency card printable/PDF encrypted document pack and battery-conscious mode';
  ...
  if (completed || recovery) {
    const arguments_ = ['scripts/verify-33-j-family-emergency-card-portability-completion.mjs', '--external'];
    ...
    const predecessorAncestry = /^[0-9a-f]{40}$/u.test(predecessorReceipt?.sourceCommit ?? '')
      ...
      : { status: 1 };
    check('33-I receipt sourceCommit is an ancestor base of current HEAD',
      predecessorAncestry.status === 0, predecessorReceipt?.sourceCommit);
    check('33-I predecessor receipt sidecar binds exact bytes',
  ...
  const line = `33-J external receipt dry audit: ${failures.length === 0 ? 'PASS' : 'FAIL'} (${checks.length - failures.length}/${checks.length}; read-only, no files written).`;
...
assert(Object.values(documentResults).every((result) => result.ok),
  '33-J finalizer prerequisites are missing or unreadable');
...
if (stepBefore?.status === 'COMPLETED') {
  const verification = spawnSync(process.execPath, [
    'scripts/verify-33-j-family-emergency-card-portability-completion.mjs', '--external'
  ], { cwd: root, encoding: 'utf8', windowsHide: true });
  ...
  assert(verification.status === 0, '33-J is completed and verify-only readback failed');
  console.log('33-J is already completed; finalizer performed verify-only readback and made no changes.');
  process.exit(0);
}
...
const ledgerSealedPlanPending = planBefore.currentStep === stepId
  ...
  && ledgerBefore.libraryUploadStatus === '33-J_COMPLETED_RECEIPT_PASS'
  && ledgerBefore.nextOfficialTask === 'AUTO_PRIORITY_SELECTION_AFTER_33-J_PERSISTENT_RECEIPT'
  && ledgerBefore.activeMicroStep === null
  && ledgerBefore.externalLibraryAuthority33J?.status === 'PASS'
  && ledgerBefore.externalLibraryAuthority33J?.path === libraryRoot
  && ledgerBefore.externalLibraryAuthority33J?.localCheckpointPath === localPackageRoot
  && ledgerBefore.externalLibraryAuthority33J?.receipt === paths.receipt;
if (ledgerSealedPlanPending) {
  const head = gitRun(['rev-parse', 'HEAD']);
  assert(head.status === 0, 'Could not resolve source commit during 33-J governance recovery');
  const recoveryReceipt = await readJson(paths.receipt);
  assert(recoveryReceipt.sourceCommit === head.stdout.trim(),
    '33-J recovery receipt is not bound to current HEAD');
  const verification = spawnSync(process.execPath, [
    'scripts/verify-33-j-family-emergency-card-portability-completion.mjs',
  ...
  if (verification.stdout) process.stdout.write(verification.stdout);
  if (verification.stderr) process.stderr.write(verification.stderr);
  assert(verification.status === 0, '33-J ledger-sealed recovery verification failed');
  ...
  planBefore.segmentationNote = '33-J is immutable COMPLETED/PASS with exact local and D: USB hash/size readback. 33-K remains PENDING for governed successor selection.';
  ...
  assert(completedVerification.status === 0, '33-J recovered completion verification failed');
  console.log('33-J governance recovery: PASS (ledger-sealed plan transition completed).');
  process.exit(0);
}
...
const localReady = planBefore.currentStep === stepId
  ...
  && ledgerBefore.libraryUploadStatus === '33-J_LOCAL_PASS_AWAITING_LIBRARY_RECEIPT'
  && ledgerBefore.nextOfficialTask
    === '33-J governed offline emergency card printable/PDF encrypted document pack and battery-conscious mode';
assert(localReady, '33-J is not the sole active receipt-pending step; completed steps are verify-only');
assert(scope.status === 'COMPLETE' && inventory.status === 'COMPLETE',
  '33-J scope or inventory is not COMPLETE');
assert(scope.decision === decision && exactArray(scope.requirements, requirements)
  && exactArray(inventory.requirements, requirements)
  && scope.validation?.boundary === paths.boundary
  && scope.validation?.contract === paths.contract
  && scope.validation?.runtime === paths.runtime,
'33-J DEC-221 requirement/evidence package changed');
assert(exactTruth(scope.truth), '33-J manual offline no-external-service truth changed');
assert(exactEvidence(scope, boundary, contract, runtime, migrationManifest),
  '33-J boundary, contract, runtime, targeted, full-test, or build evidence changed');
assert(exactRegistry(registry),
  'B5-03/EXT-016 registry status, 13-link chain, or evidence changed');
...
assert(predecessorReceipt?.step === '33-I'
...
'33-I predecessor receipt is not an exact completed PASS source base');
assert(await sidecarBindsExactBytes(predecessorReceiptPath),
  '33-I predecessor receipt sidecar does not bind exact bytes');
...
assert(predecessorAncestry.status === 0,
  '33-I receipt sourceCommit is not an ancestor of the 33-J source commit');
const status = gitRun(['status', '--porcelain']);
assert(status.status === 0 && status.stdout.trim() === '',
  '33-J finalization requires a clean committed source tree');
const trackedSnapshot = gitRun(['ls-tree', '-r', 'HEAD']);
assert(trackedSnapshot.status === 0, 'Could not enumerate the exact 33-J tracked source snapshot');
const trackedEntries = trackedSnapshot.stdout.trim().split(/\r?\n/u).filter(Boolean).map((line) => {
  ...
  assert(match && (match[1] === '100644' || match[1] === '100755'),
    `33-J payload forbids non-regular tracked entries: ${line}`);
...
assert(trackedPaths.length > 0 && new Set(trackedPaths).size === trackedPaths.length,
  '33-J tracked source snapshot is empty or contains duplicate paths');
...
'33-J payload does not cover the exact tracked source snapshot and required evidence');
...
const manifest = {
  ...
  decision,
  requirements,
  phase: 'FAMILY_EMERGENCY_CARD_PORTABILITY_CHECKPOINT_PACKAGE',
  status: 'PASS',
  sourceCommit,
  predecessorStep: '33-I',
  predecessorReceiptPath,
...
assert(JSON.stringify(localBaseActual) === JSON.stringify(expectedBase),
  'Local 33-J checkpoint base set is not exact');
...
assert(baseReadback.every((item) => item.status === 'PASS'),
  'D: 33-J base package readback mismatch');
...
assert(receiptArtifacts.every((item) => item.status === 'PASS'),
  'D: 33-J receipt readback mismatch');
...
assert(persistenceArtifacts.every((item) => item.status === 'PASS'),
  'D: 33-J receipt persistence mismatch');
...
assert(JSON.stringify(actualBeforeInventory) === JSON.stringify(expectedBeforeInventory),
  'D: 33-J pre-inventory set is not exact');
...
assert(JSON.stringify(baseFinalActual) === JSON.stringify(baseFinalExpected),
  'D: 33-J base final inventory set is not exact');
...
assert(finalInventoryArtifacts.every((item) => item.status === 'PASS'),
  'D: 33-J final inventory pair readback mismatch');
...
const step = plan.steps.find((item) => item.id === stepId);
assert(step, '33-J work step is missing');
Object.assign(step, {
  status: 'COMPLETED',
  validationStatus: 'PASS',
  persistentReceiptStatus: 'PASS',
...
plan.segmentationNote = '33-J is immutable COMPLETED/PASS with exact local and D: USB hash/size readback. 33-K remains PENDING for governed successor selection.';

const ledger = await readJson(paths.ledger);
ledger.libraryUploadStatus = '33-J_COMPLETED_RECEIPT_PASS';
ledger.nextOfficialTask = 'AUTO_PRIORITY_SELECTION_AFTER_33-J_PERSISTENT_RECEIPT';
ledger.activeMicroStep = null;
ledger.externalLibraryAuthority33J = {
  step: stepId,
  status: 'PASS',
  storageBackend: 'EXTERNAL_USB_D_DRIVE',
  path: libraryRoot,
...
const transitionChecks = [
  ...
  ['ledger complete', ledger.libraryUploadStatus === '33-J_COMPLETED_RECEIPT_PASS'
    && ledger.activeMicroStep === null],
...
for (const failure of transitionFailures) console.error(`33-J transition check failed: ${failure.name}`);
assert(transitionChecks.every((item) => item.status === 'PASS'),
  '33-J completion transition failed');
...
assert(closureArtifacts.every((item) => item.status === 'PASS'),
  'D: 33-J completion/transition pair readback mismatch');
...
assert(JSON.stringify(actualBeforeClosureInventory) === JSON.stringify(expectedBeforeClosureInventory),
  'D: 33-J pre-closure inventory set is not exact');
...
assert(closureInventoryArtifacts.every((item) => item.status === 'PASS'),
  'D: 33-J closure inventory pair readback mismatch');
...
assert(JSON.stringify(closureFinalActual) === JSON.stringify(closureFinalExpected),
  'D: 33-J closure inventory set is not exact');
...
assert(JSON.stringify(localClosureFinalActual) === JSON.stringify(closureFinalExpected),
  'Local 33-J closure inventory set is not exact');
...
assert(localClosureReadback.every((item) => item.status === 'PASS'),
  'Local and D: 33-J closure packages differ');
...
console.log(`33-J external receipt finalized: PASS (${closureFinalExpected.length} exact local/D: files; source ${sourceCommit}).`);
