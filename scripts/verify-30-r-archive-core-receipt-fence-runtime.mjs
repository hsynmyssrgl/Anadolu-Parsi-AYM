...
const REPORT_PATH = 'artifacts/validation/30-R-archive-core-receipt-fence-runtime.json';
const VITEST_PATH = 'artifacts/validation/30-R-archive-core-receipt-fence-vitest.json';
...
check(vitest?.numTotalTests === 20 && vitest?.numPassedTests === 20 && vitest?.numFailedTests === 0, '30-R and production regressions are 20/20 PASS');
...
const report = {
  ...
  step: '30-R',
  requirement: 'PPK-002',
  phase: 'ARCHIVE_CORE_TABLE_RECEIPT_FENCE_CONTROLLED_RUNTIME',
  status,
  controlledChecks: {
    expected: checks.length + 20,
    actual: checks.filter((item) => item.status === 'PASS').length + (vitest?.numPassedTests ?? 0),
    processAssertions: checks.length,
    focusedVitestExpected: 20,
  ...
  evidenceBoundary: {
    ...
    archiveAccessoryTableUniversalFenceEnforcement: 'NOT_COMPLETE',
...
if (status !== 'PASS') {
  console.error(`30-R archive core receipt-fence runtime: FAIL (${failures.length}/${checks.length} process assertions).`);
  console.error(failures.join('\n'));
  console.error(TRUTH);
  process.exit(1);
}
console.log(`30-R archive core receipt-fence runtime: PASS (${report.controlledChecks.actual}/${report.controlledChecks.expected} controlled checks; PPK-002 remains PARTIAL).`);
console.log(TRUTH);
