...
const reportPath=resolve(option('--report','artifacts/validation/build162-validation-boundary.json'));
...
const [sourcePreflight,sourceIntegrity,contract,runtime,syntax,packageSourceTypes,desktopMainTypes,requestCreation,responseStatus]=await Promise.all([
 readJson('artifacts/validation/build162-source-preflight-final.json'),
 readJson('artifacts/validation/build162-source-integrity-final.json'),
 readJson('artifacts/validation/build162-ipc-read-sharing-contract.json'),
 readJson('artifacts/validation/build162-ipc-read-sharing-runtime.json'),
 readJson('artifacts/validation/build162-ipc-read-sharing-syntax.json'),
...
if(sourcePreflight.status!=='PASS')throw new Error('Build 162 source preflight must be PASS.');
if(sourceIntegrity.status!=='PASS')throw new Error('Build 162 source integrity must be PASS.');
if(contract.status!=='PASS'||contract.assertions!==45)throw new Error('Build 162 read sharing contract must be PASS 45/45.');
if(runtime.status!=='PASS'||runtime.checks!==33)throw new Error('Build 162 read sharing runtime must be PASS 33/33.');
if(syntax.status!=='PASS'||syntax.fileCount!==4)throw new Error('Build 162 read sharing syntax must be PASS 4/4.');
...
const results=[
 {id:'source-preflight',status:'PASS',evidence:'artifacts/validation/build162-source-preflight-final.json'},
 {id:'source-integrity',status:'PASS',evidence:'artifacts/validation/build162-source-integrity-final.json'},
