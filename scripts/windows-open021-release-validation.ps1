...
$installRoot = Join-Path $root ".tmp\open021-windows-release-validation\$runId"
...
if (-not $EvidencePath) { $EvidencePath = Join-Path $validationRoot "build$build-open021-windows-release-lifecycle.json" }
...
try {
  if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") { throw "OPEN-021 release validation can run only on Windows." }
  ...
  Invoke-Checked -Id "development-open021-launch" -FilePath "node.exe" `
  ...
  if (Test-Path -LiteralPath $currentUserUninstallKey) {
    ...
    if (-not $isStaleCurrentBuildEntry) { throw "An existing current-user installation is present; OPEN-021 validation refuses to replace it." }
  }
  ...
  Invoke-Checked -Id "installed-open021-launch" -FilePath "node.exe" `
...
Write-Host "OPEN-021 Windows release lifecycle verified. Evidence: $EvidencePath"
