...
if ($build -ne 227) { throw "Unified Bronze Windows closure requires Build227; current build=$build." }
$productName = ([System.IO.File]::ReadAllText((Join-Path $root "apps\desktop\package.json"), [System.Text.Encoding]::UTF8) | ConvertFrom-Json).build.productName
...
Push-Location $root
try {
  if ($PSVersionTable.PSEdition -eq "Core" -and -not $IsWindows) { throw "Unified Bronze closure requires a real Windows host." }
  ...
  Invoke-RequiredStep -Id "exact-source-integrity" -Operation {
    & node.exe "scripts\verify-source-integrity.mjs" "--report" "artifacts\validation\build227-bronze-security-source-integrity-windows.json"
  }
  Invoke-RequiredStep -Id "dependency-bootstrap-prerequisite" -Operation { & npm.cmd ci --no-audit --no-fund }
  Invoke-RequiredStep -Id "windows-packager-bootstrap-prerequisite" -Operation { & npm.cmd run windows-packager:install }
  ...
  Invoke-RequiredStep -Id "workspace-package-build-prerequisite" -Operation { & npm.cmd run build:packages }
  Invoke-RequiredStep -Id "workspace-package-dist-guard" -Operation { & node.exe "scripts\verify-build227-workspace-dist-prerequisite.mjs" }
  Invoke-RequiredStep -Id "license-rtf-sync-prerequisite" -Operation { & npm.cmd run verify:license-sync --workspace @ppt/desktop }

  $lifecycleArgs = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\windows-bronze-security-release-validation-build227.ps1", "-EvidencePath", "artifacts\validation\build227-bronze-security-windows-release-lifecycle.json")
  if ($KeepInstalled) { $lifecycleArgs += "-KeepInstalled" }
  & powershell.exe @lifecycleArgs
  $lifecycleExit = $LASTEXITCODE
  if ($null -eq $lifecycleExit) { $lifecycleExit = 1 }
  ...
  & node.exe "scripts\verify-build227-bronze-security-windows-result.mjs" $applicationVersion `
    "artifacts\validation\build227-bronze-security-windows-release-lifecycle.json" `
    ...
    "artifacts\validation\build227-bronze-security-source-integrity-windows.json" `
    "artifacts\validation\build227-bronze-security-windows-closure-result.json"
  $resultExitCode = $LASTEXITCODE
  if ($null -eq $resultExitCode) { $resultExitCode = 1 }
  $steps.Add([ordered]@{ id = "independent-closure-verifier"; status = if ($resultExitCode -eq 0) { "PASS" } elseif ($resultExitCode -eq 21 -or $resultExitCode -eq 22) { "PARTIAL" } else { "FAIL" }; exitCode = $resultExitCode; startedAt = $null; completedAt = [DateTimeOffset]::UtcNow.ToString("O") })
} catch {
...
$resultPath = Join-Path $validationRoot "build227-bronze-security-windows-closure-result.json"
$result = $null
...
$summaryPath = Join-Path $validationRoot "build227-bronze-security-windows-run-summary.json"
$summary = [ordered]@{
  schemaVersion = 1
  product = $productName
  ...
  host = [ordered]@{ identity = [System.Security.Principal.WindowsIdentity]::GetCurrent().Name; osVersion = [System.Environment]::OSVersion.VersionString; powershellVersion = $PSVersionTable.PSVersion.ToString() }
  steps = $steps
  error = $failure
}
[System.IO.File]::WriteAllText($summaryPath, "$($summary | ConvertTo-Json -Depth 12)`n", [System.Text.UTF8Encoding]::new($false))
...
if ((Test-Path -LiteralPath $sourceManifestPath -PathType Leaf) -and (Test-Path -LiteralPath $sourceShaSumsPath -PathType Leaf)) {
  $requiredPaths = @(
    $summaryPath,
    (Join-Path $validationRoot "build227-bronze-security-source-integrity-windows.json"),
    (Join-Path $validationRoot "build227-bronze-security-windows-release-lifecycle.json"),
  ...
  $diagnosticLogs = @(
    Get-ChildItem -LiteralPath $validationRoot -File -ErrorAction SilentlyContinue |
      Where-Object { $_.Name -like 'build227-*-full-*.log' -or $_.Name -like 'windows-open02*-full-*.log' -or $_.Name -like 'windows-open02*-early-startup.json' } |
      Select-Object -ExpandProperty FullName
  )
  $evidencePaths = @($requiredPaths) + @($diagnosticLogs)
  $files = @()
  foreach ($path in $evidencePaths) {
    $present = Test-Path -LiteralPath $path -PathType Leaf
    $files += [ordered]@{ relativePath = Split-Path -Leaf $path; required = $requiredPaths -contains $path; present = $present; sizeBytes = if ($present) { (Get-Item -LiteralPath $path).Length } else { $null }; sha256 = if ($present) { (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() } else { $null } }
  }
  $manifest = [ordered]@{
    schemaVersion = 1
    product = $productName
    ...
    applicationVersion = $applicationVersion
    platform = "win32"
    bundleIntegrityStatus = if (($files | Where-Object { $_.required -and -not $_.present }).Count -eq 0) { "PASS" } else { "FAIL" }
    independentClosureVerifier = if ($result) { $result.status } else { "NOT_RUN" }
    closureReadiness = [ordered]@{ open021 = $open021Readiness; open022 = $open022Readiness }
    source = [ordered]@{ manifestSha256 = (Get-FileHash -LiteralPath $sourceManifestPath -Algorithm SHA256).Hash.ToLowerInvariant(); sha256SumsSha256 = (Get-FileHash -LiteralPath $sourceShaSumsPath -Algorithm SHA256).Hash.ToLowerInvariant() }
    files = $files
    generatedAt = [DateTimeOffset]::UtcNow.ToString("O")
  }
  $manifestPath = Join-Path $validationRoot "build227-bronze-security-windows-evidence-manifest.json"
  ...
  $bundlePath = Join-Path $validationRoot "Bronze_Guvenlik_Windows_Kanitlari_Build227_$runId.zip"
