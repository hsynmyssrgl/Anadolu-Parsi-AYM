...
const [cmd,runner,lifecycle,result,open021,open022]=await Promise.all([
  read('BRONZE_WINDOWS_GUVENLIK_KAPAT_BUILD225.cmd'),read('scripts/run-build225-bronze-security-closure.ps1'),read('scripts/windows-bronze-security-release-validation-build225.ps1'),read('scripts/verify-build225-bronze-security-windows-result.mjs'),read('scripts/windows-open021-launch-test.mjs'),read('scripts/windows-open022-launch-test.mjs')
]);
const results=[];const check=(id,c)=>results.push({id,status:c?'PASS':'FAIL'});
check('one-click-build225-cmd',cmd.includes('run-build225-bronze-security-closure.ps1'));
check('exact-build225-guards',runner.includes('$build -ne 225')&&lifecycle.includes('$build -ne 225')&&result.includes('expectedBuild !== 225'));
check('source-integrity-prerequisite',runner.includes('build225-bronze-security-source-integrity-windows.json'));
...
const failures=results.filter(x=>x.status==='FAIL');const report={schemaVersion:1,build:225,status:failures.length?'FAIL':'PASS',checks:results.length,passed:results.length-failures.length,failed:failures.length,results,generatedAt:new Date().toISOString()};await mkdir('artifacts/validation',{recursive:true});await writeFile('artifacts/validation/build225-windows-retry-contract.json',`${JSON.stringify(report,null,2)}\n`);console.log(`Build225 Windows retry contract: ${report.status} (${report.passed}/${report.checks}).`);if(failures.length)process.exitCode=1;
