...
try {
  ...
  assert.deepEqual(migrationSummary.appliedVersions, Array.from({ length: 99 }, (_unused, index) => index + 1));
  assert.equal(migrationSummary.schemaAfter.tableCount, 149);
  ...
  try {
    ...
    assert.equal(migrations.length, 99, 'Migration kayıtları eksik.');
