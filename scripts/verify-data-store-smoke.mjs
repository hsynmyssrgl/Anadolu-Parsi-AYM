...
try {
  ...
  assert.deepEqual(migrationSummary.appliedVersions, Array.from({ length: 89 }, (_unused, index) => index + 1));
  assert.equal(migrationSummary.schemaAfter.tableCount, 107);
  ...
  try {
    ...
    assert.equal(migrations.length, 89, 'Migration kayıtları eksik.');
