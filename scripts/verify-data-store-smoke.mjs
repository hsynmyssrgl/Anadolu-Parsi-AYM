...
try {
  ...
  assert.deepEqual(migrationSummary.appliedVersions, Array.from({ length: 100 }, (_unused, index) => index + 1));
  assert.equal(migrationSummary.schemaAfter.tableCount, 151);
  ...
  try {
    ...
    assert.equal(migrations.length, 100, 'Migration kayıtları eksik.');
