...
try {
  ...
  assert.deepEqual(migrationSummary.appliedVersions, Array.from({ length: 90 }, (_unused, index) => index + 1));
  assert.equal(migrationSummary.schemaAfter.tableCount, 109);
  ...
  try {
    ...
    assert.equal(migrations.length, 90, 'Migration kayıtları eksik.');
