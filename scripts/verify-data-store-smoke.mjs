...
try {
  ...
  assert.deepEqual(migrationSummary.appliedVersions, Array.from({ length: 97 }, (_unused, index) => index + 1));
  assert.equal(migrationSummary.schemaAfter.tableCount, 144);
  ...
  try {
    ...
    assert.equal(migrations.length, 97, 'Migration kayıtları eksik.');
