...
try {
  ...
  assert.deepEqual(migrationSummary.appliedVersions, Array.from({ length: 81 }, (_unused, index) => index + 1));
  assert.equal(migrationSummary.schemaAfter.tableCount, 90);
  ...
  try {
    ...
    assert.equal(migrations.length, 81, 'Migration kayıtları eksik.');
