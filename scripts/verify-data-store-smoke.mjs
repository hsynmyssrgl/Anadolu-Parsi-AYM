...
try {
  ...
  assert.deepEqual(migrationSummary.appliedVersions, Array.from({ length: 82 }, (_unused, index) => index + 1));
  assert.equal(migrationSummary.schemaAfter.tableCount, 92);
  ...
  try {
    ...
    assert.equal(migrations.length, 82, 'Migration kayıtları eksik.');
