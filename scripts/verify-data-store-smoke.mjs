...
try {
  ...
  assert.deepEqual(migrationSummary.appliedVersions, Array.from({ length: 92 }, (_unused, index) => index + 1));
  assert.equal(migrationSummary.schemaAfter.tableCount, 122);
  ...
  try {
    ...
    assert.equal(migrations.length, 92, 'Migration kayıtları eksik.');
