...
try {
  ...
  assert.deepEqual(migrationSummary.appliedVersions, Array.from({ length: 93 }, (_unused, index) => index + 1));
  assert.equal(migrationSummary.schemaAfter.tableCount, 132);
  ...
  try {
    ...
    assert.equal(migrations.length, 93, 'Migration kayıtları eksik.');
