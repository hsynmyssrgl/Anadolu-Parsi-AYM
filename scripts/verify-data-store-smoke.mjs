...
try {
  ...
  assert.deepEqual(migrationSummary.appliedVersions, Array.from({ length: 91 }, (_unused, index) => index + 1));
  assert.equal(migrationSummary.schemaAfter.tableCount, 111);
  ...
  try {
    ...
    assert.equal(migrations.length, 91, 'Migration kayıtları eksik.');
