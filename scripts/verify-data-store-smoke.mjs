...
try {
  ...
  assert.deepEqual(migrationSummary.appliedVersions, Array.from({ length: 98 }, (_unused, index) => index + 1));
  assert.equal(migrationSummary.schemaAfter.tableCount, 147);
  ...
  try {
    ...
    assert.equal(migrations.length, 98, 'Migration kayıtları eksik.');
