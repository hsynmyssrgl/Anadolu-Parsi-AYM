...
const step = '31-D';
const localPackageRoot = 'C:\\PPT\\AYM\\09_ARSIV\\KAYNAK_AGACI\\checkpoints\\31-D_PPK-002_Family_Import_Reused_Location_Read_Receipt';
...
const payloadPaths = Object.freeze([
  'package.json',
  'docs/decisions/DEC-163-ppk-002-family-import-reused-location-read-receipt.md',
  'docs/audit/31-D_PPK-002_FAMILY_IMPORT_REUSED_LOCATION_READ_RECEIPT.md',
  'config/31-d-family-import-reused-location-read-receipt-scope.json',
  'artifacts/authority/31-D_AUTO_PRIORITY_SELECTION_AUTHORITY.json',
  'artifacts/validation/31-D_PRIORITY_SELECTION_VALIDATION.json',
  'artifacts/checkpoints/31-D_EXECUTION_RECORD.json',
  'artifacts/inventory/31-D_SCOPE_AND_STATUS_REPORT.json',
  'artifacts/validation/31-D_FAMILY_IMPORT_REUSED_LOCATION_READ_RECEIPT_CONTRACT.json',
  'artifacts/validation/31-D_DESKTOP_MAIN_TYPESCRIPT.json',
  'artifacts/validation/31-D_TARGETED_VITEST.json',
  'artifacts/validation/31-D_FULL_VITEST_REGRESSION.json',
  ...
  'scripts/start-31-d-family-import-reused-location-read-receipt.mjs',
  'scripts/verify-31-d-family-import-reused-location-read-receipt-contract.mjs',
  'scripts/record-31-d-local-validation.mjs',
  'scripts/finalize-31-d-external-library-receipt.mjs',
  'scripts/verify-31-d-completion-transition.mjs'
]);
const paths = {
...
const compareRoots = async (sourceRoot, targetRoot, names) => Promise.all(names.map(async (path) => {
  ...
  return {
    path, sourceSizeBytes: sourceBytes.length, librarySizeBytes: targetBytes.length,
...
const [planBefore, executionBefore, contract, typecheck, targeted, regression, platform] = await Promise.all([
  ...
  readJson(full('artifacts/validation/31-D_FAMILY_IMPORT_REUSED_LOCATION_READ_RECEIPT_CONTRACT.json')),
  readJson(full('artifacts/validation/31-D_DESKTOP_MAIN_TYPESCRIPT.json')),
  readJson(full('artifacts/validation/31-D_TARGETED_VITEST.json')),
  readJson(full('artifacts/validation/31-D_FULL_VITEST_REGRESSION.json')),
  readJson(full('artifacts/validation/platform-policy-gate.json'))
]);
const stepBefore = planBefore.steps.find((item) => item.id === step);
assert(stepBefore?.status === 'IN_PROGRESS' && stepBefore.validationStatus === 'PASS' && stepBefore.persistentReceiptStatus === 'PENDING', '31-D is not local PASS awaiting receipt');
assert(executionBefore.status === 'LOCAL_PASS_AWAITING_LIBRARY_RECEIPT', '31-D execution is not local PASS');
assert([contract, typecheck, targeted, regression, platform].every((item) => item.status === 'PASS'), '31-D validation evidence is not clean PASS');
...
const manifestName = '31-D_CHECKPOINT_MANIFEST.json';
...
assert(JSON.stringify(await listFiles(localPackageRoot)) === JSON.stringify(expectedBase), 'Local 31-D package file set is not exact');
...
assert(baseReadback.every((item) => item.status === 'PASS'), 'D: 31-D base package readback mismatch');
...
const receiptBinding = await writeLocalPair(paths.receipt, {
  schemaVersion: 1, release, step, requirement: 'PPK-002', phase: 'FAMILY_IMPORT_REUSED_LOCATION_READ_RECEIPT_EXTERNAL_LIBRARY_RECEIPT',
  ...
  openBoundaries: {
    newlyCreatedLocationLinkedEventImport: 'NEXT_SEPARATE_SLICE_FAIL_CLOSED',
    governedImportRollbackReceiptFence: 'NOT_COMPLETE', universalRepositoryEnforcement: 'NOT_COMPLETE', obligationExecution: 'NOT_RUN_NOT_PASS'
  },
  nextOfficialStep: 'AUTO_PRIORITY_SELECTION_AFTER_31-D_PERSISTENT_RECEIPT',
  currentAuthoritativeSourceExternalProtectionStatus: 'PENDING_SEPARATE_FROM_FOCUSED_31_D_CHECKPOINT',
...
assert(stageOne.every((item) => item.status === 'PASS'), 'D: 31-D receipt readback mismatch');
const receiptReadbackBinding = await writeLocalPair(paths.receiptReadback, {
  schemaVersion: 1, release, step, phase: 'EXTERNAL_USB_RECEIPT_READBACK', status: 'PASS', expected: 4, executed: 4,
  matched: 4, failed: 0, artifacts: stageOne, verifiedAt: new Date().toISOString(), mandatoryTruthSentence: truth
});
...
assert(stageTwo.every((item) => item.status === 'PASS'), 'D: 31-D receipt persistence mismatch');
...
assert(JSON.stringify(beforeInventoryActual) === JSON.stringify(beforeInventoryExpected), 'D: 31-D pre-inventory file set is not exact');
...
assert(JSON.stringify(finalActual) === JSON.stringify(finalExpected), 'D: 31-D final inventory is not exact');
...
const step31D = plan.steps.find((item) => item.id === step);
Object.assign(step31D, {
...
for (const path of [paths.receipt, paths.readback, paths.receiptReadback, paths.persistence, paths.inventory, paths.completion, paths.transition]) {
  if (!step31D.localEvidence.includes(path)) step31D.localEvidence.push(path);
}
plan.updatedAt = completedAt;
plan.segmentationNote = '31-C and 31-D are immutable COMPLETED/PASS receipt chains. 31-D reused-location exact read receipt chain has an exact focused package and D: external USB readback. PPK-002 remains PARTIAL; newly-created-location-linked event import and governed rollback remain open; no new Build is issued.';
...
ledger.externalLibraryAuthority31D = { step, status: 'PASS', storageBackend: 'EXTERNAL_USB_D_DRIVE', path: libraryRoot, receipt: paths.receipt, focusedCheckpointOnly: true };
...
const transitionChecks = [
  ...
  ['work plan', step31D.status === 'COMPLETED' && step31D.persistentReceiptStatus === 'PASS'],
  ['governance', ledger.libraryUploadStatus === '31-D_COMPLETED_RECEIPT_PASS' && ledger.activeMicroStep === null],
...
assert(transitionChecks.every((item) => item.status === 'PASS'), '31-D completion transition failed');
...
console.log(`31-D external USB Library receipt: PASS (${expectedBase.length}/${expectedBase.length} base files; ${finalActual.length}/${finalExpected.length} final files).`);
console.log(`Library: ${libraryRoot}`);
console.log('31-D official step status: COMPLETED; PPK-002: PARTIAL; new Build: false.');
