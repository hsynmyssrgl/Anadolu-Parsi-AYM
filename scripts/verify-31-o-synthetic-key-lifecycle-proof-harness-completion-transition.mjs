...
const paths = {
  ...
  scopeReport: 'artifacts/inventory/31-O_SCOPE_AND_STATUS_REPORT.json',
...
const later = inspectAuthorizedSuccessorLifecycle({ plan: docs.plan, ledger: docs.ledger, predecessorId: '31-O' });
...
const step = docs.plan.steps.find((item) => item.id === '31-O');
...
check((docs.ledger.libraryUploadStatus === '31-O_COMPLETED_RECEIPT_PASS' && docs.ledger.activeMicroStep === null) || (later.planValid && later.ledgerValid && later.nextTaskValid), 'ledger complete or authorized successor active');
...
check(docs.receipt.openBoundaries.productionProtectedProvider === 'NOT_ATTACHED', 'production protected provider remains detached');
...
check(docs.receipt.openBoundaries.processCrashEvidence === 'NOT_PROVEN' && docs.receipt.openBoundaries.processRestartEvidence === 'NOT_PROVEN' && docs.receipt.openBoundaries.memoryClearingEvidence === 'NOT_PROVEN', 'process and memory-clearing proof remain open');
...
check(docs.scopeReport.deliveredBoundary.stateMachine === 'PURE_SYNTHETIC_KEY_LIFECYCLE_ONLY' && docs.scopeReport.deliveredBoundary.candidateClassification === 'MODELED_GATE_ONLY_NO_GATE_ID_NON_SUBMITTABLE', 'harness remains synthetic and non-submittable');
check(docs.scopeReport.deliveredBoundary.runtimeIntegration === 'NOT_WIRED' && docs.scopeReport.deliveredBoundary.realKeyMaterial === 'NOT_ACCESSED' && docs.scopeReport.deliveredBoundary.realKeyLifecycleGate === 'NOT_SATISFIED', 'runtime, real key material, and real gate remain detached');
...
if (failed.length) {
  console.error(`31-O completion transition: FAIL (${failed.length}/${checks.length}).`);
  for (const item of failed) console.error(`- ${item.name}`);
  process.exit(1);
}
console.log(`31-O completion transition: PASS (${checks.length}/${checks.length}${external ? '; D: external hash/size readback included' : ''}).`);
