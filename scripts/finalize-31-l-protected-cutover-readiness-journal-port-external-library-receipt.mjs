...
const step = '31-L';
const localPackageRoot = 'C:\\PPT\\AYM\\09_ARSIV\\KAYNAK_AGACI\\checkpoints\\31-L_Protected_Cutover_Readiness_Journal_Port';
const libraryRoot = 'D:\\AYM_LIBRARY\\Panthera pardus tulliana\\Anadolu Parsı Aile Yaşam Merkezi\\Bronze 04.08.2026.29\\checkpoints\\31-L_Protected_Cutover_Readiness_Journal_Port';
const truth = 'Bu teslim yalnız 31-L port ve detached varsayılan-ret sınırını kanıtlar; üretim adaptörü, kalıcılık, gerçek veri veya cutover PASS değildir.';
const payloadPaths = Object.freeze([
  ...
  'docs/audit/31-L_PROTECTED_CUTOVER_READINESS_JOURNAL_PORT.md',
  ...
  'config/31-l-protected-cutover-readiness-journal-port-scope.json',
  'artifacts/authority/31-L_PROTECTED_READINESS_JOURNAL_PORT_AUTHORITY.json',
  'artifacts/validation/31-L_PRIORITY_SELECTION_VALIDATION.json',
  'artifacts/checkpoints/31-L_EXECUTION_RECORD.json',
  'artifacts/checkpoints/31-L_INITIAL_VALIDATION_FAILURES.json',
  'artifacts/inventory/31-L_SCOPE_AND_STATUS_REPORT.json',
  'artifacts/validation/31-L_PROTECTED_CUTOVER_READINESS_JOURNAL_PORT_CONTRACT.json',
  'artifacts/validation/31-L_ROOT_TYPESCRIPT.json',
  'artifacts/validation/31-L_TARGETED_VITEST.json',
  'artifacts/validation/31-L_CORE_SERVICE_RUNTIME.json',
  'artifacts/validation/31-L_SECURITY_AND_PREDECESSOR_REGRESSION.json',
  'artifacts/validation/31-L_FULL_VITEST_REGRESSION.json',
  'artifacts/validation/31-L_PRODUCTION_BUILD.json',
  ...
  'scripts/start-31-l-protected-cutover-readiness-journal-port.mjs',
  'scripts/verify-31-l-protected-cutover-readiness-journal-port-contract.mjs',
  'scripts/run-31-l-protected-cutover-readiness-journal-port-local-validation.mjs',
  'scripts/finalize-31-l-protected-cutover-readiness-journal-port-external-library-receipt.mjs',
...
const paths = {
  receipt: 'artifacts/checkpoints/31-L_LIBRARY_RECEIPT.json',
  readback: 'artifacts/validation/31-L_LIBRARY_READBACK_VERIFICATION.json',
  receiptReadback: 'artifacts/validation/31-L_RECEIPT_READBACK_VERIFICATION.json',
  persistence: 'artifacts/validation/31-L_RECEIPT_READBACK_PERSISTENCE_VERIFICATION.json',
  inventory: 'artifacts/validation/31-L_LIBRARY_FINAL_INVENTORY_VERIFICATION.json',
  completion: 'artifacts/checkpoints/31-L_COMPLETION_RECORD.json',
  transition: 'artifacts/validation/31-L_COMPLETION_TRANSITION_VALIDATION.json',
...
assert(localPass || recoverable, '31-L is neither local PASS awaiting receipt nor a recoverable finalization');
assert(validations.every((item) => item.status === 'PASS'), '31-L validation is not clean PASS');
...
const manifest = {
  ...
  phase: 'PROTECTED_CUTOVER_READINESS_JOURNAL_PORT_CHECKPOINT_PACKAGE',
  status: 'PASS',
...
const manifestName = '31-L_CHECKPOINT_MANIFEST.json';
...
const receipt = await writePair(paths.receipt, {
  schemaVersion: 1, release, step, primaryRequirement: 'DHA-001', phase: 'PROTECTED_CUTOVER_READINESS_JOURNAL_PORT_EXTERNAL_LIBRARY_RECEIPT',
  ...
  openBoundaries: scopeConfigBefore.openBoundaries, nextOfficialStep: 'AUTO_PRIORITY_SELECTION_AFTER_31-L_PERSISTENT_RECEIPT',
...
const completion = {
  ...
  nextOfficialStep: 'AUTO_PRIORITY_SELECTION_AFTER_31-L_PERSISTENT_RECEIPT',
  currentAuthoritativeSourceExternalProtectionStatus: 'SEPARATE_GOV_005_REFRESH_REQUIRED_AFTER_SOURCE_CHANGE',
...
const step31L = plan.steps.find((item) => item.id === step);
Object.assign(step31L, {
...
for (const path of [paths.receipt, paths.readback, paths.receiptReadback, paths.persistence, paths.inventory, paths.completion, paths.transition]) {
  if (!step31L.localEvidence.includes(path)) step31L.localEvidence.push(path);
}
plan.updatedAt = completedAt;
plan.segmentationNote = '31-L protected readiness-journal port is immutable COMPLETED/PASS with exact D: USB readback. The production adapter remains detached; DEC-171 cutover, real family-data transfer, and SQLite ownership transfer remain blocked.';
await writeJson(full(paths.plan), plan);
const ledger = await readJson(full(paths.ledger));
ledger.libraryUploadStatus = '31-L_COMPLETED_RECEIPT_PASS';
ledger.nextOfficialTask = 'AUTO_PRIORITY_SELECTION_AFTER_31-L_PERSISTENT_RECEIPT';
ledger.activeMicroStep = null;
ledger.externalLibraryAuthority31L = {
...
const checks = [
  ...
  ['work plan', step31L.status === 'COMPLETED' && step31L.persistentReceiptStatus === 'PASS'],
  ['governance', ledger.libraryUploadStatus === '31-L_COMPLETED_RECEIPT_PASS' && ledger.activeMicroStep === null],
  ['port-only boundary', scopeConfig.openBoundaries.durableReadinessJournal === 'PORT_ONLY_NO_PRODUCTION_ADAPTER'],
...
assert(checks.every((item) => item.status === 'PASS'), '31-L transition failed');
...
console.log(`31-L external USB Library receipt: PASS (${expectedBase.length}/${expectedBase.length} base files; ${finalActual.length}/${finalExpected.length} final files).`);
console.log(`Library: ${libraryRoot}`);
console.log('31-L official step status: COMPLETED; production journal adapter: DETACHED; DEC-171 cutover: BLOCKED; new Build: false.');
