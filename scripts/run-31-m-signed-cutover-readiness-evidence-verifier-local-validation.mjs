...
const paths = {
  ...
  failures: 'artifacts/checkpoints/31-M_INITIAL_VALIDATION_FAILURES.json',
  scopeConfig: 'config/31-m-signed-cutover-readiness-evidence-verifier-boundary-scope.json',
  contract: 'artifacts/validation/31-M_SIGNED_CUTOVER_READINESS_EVIDENCE_VERIFIER_CONTRACT.json',
  typecheck: 'artifacts/validation/31-M_ROOT_TYPESCRIPT.json',
  targeted: 'artifacts/validation/31-M_TARGETED_VITEST.json',
  runtime: 'artifacts/validation/31-M_CORE_SERVICE_RUNTIME.json',
  security: 'artifacts/validation/31-M_SECURITY_AND_PREDECESSOR_REGRESSION.json',
  regression: 'artifacts/validation/31-M_FULL_VITEST_REGRESSION.json',
  build: 'artifacts/validation/31-M_PRODUCTION_BUILD.json',
...
if (!step || plan.currentStep !== '31-M' || step.status !== 'IN_PROGRESS') throw new Error('31-M is not active');
...
if (!validationPending && !cleanRerun) throw new Error('31-M is not awaiting validation or an idempotent clean rerun');
if (initialFailures.countsAsPass !== false || initialFailures.status !== 'SUPERSEDED_FAILED_ATTEMPTS_NOT_PASS') {
  throw new Error('31-M failed attempts are not truthfully classified');
}
if (!initialFailures.failures.every((failure) =>
  ...
  throw new Error('31-M initial failures do not have clean rerun evidence');
}

const contractRun = run('31-M contract', ['scripts/verify-31-m-signed-cutover-readiness-evidence-verifier-boundary-contract.mjs']);
pass(contractRun);
const contract = await readJson(paths.contract);
if (contract.status !== 'PASS' || contract.failed !== 0 || contract.passed !== contract.expected) throw new Error('31-M contract is not clean PASS');
...
await writeJson(paths.runtime, {
  ...
  release: plan.release,
  step: '31-M',
  phase: 'CORE_SERVICE_DETACHED_SIGNED_VERIFIER_RUNTIME_GATES',
  status: 'PASS',
...
const securityCommands = [
  ...
  ['31-M contract verifier syntax', '--check', 'scripts/verify-31-m-signed-cutover-readiness-evidence-verifier-boundary-contract.mjs'],
  ['31-M local validator syntax', '--check', 'scripts/run-31-m-signed-cutover-readiness-evidence-verifier-local-validation.mjs'],
  ['31-M finalizer syntax', '--check', 'scripts/finalize-31-m-signed-cutover-readiness-evidence-verifier-boundary-external-library-receipt.mjs'],
  ['31-M completion verifier syntax', '--check', 'scripts/verify-31-m-signed-cutover-readiness-evidence-verifier-boundary-completion-transition.mjs']
];
...
await writeJson(paths.security, {
  ...
  release: plan.release,
  step: '31-M',
  phase: 'SIGNED_VERIFIER_SECURITY_AND_PREDECESSOR_REGRESSION',
  status: 'PASS',
...
plan.segmentationNote = `31-M signed readiness-evidence verifier is LOCAL_PASS_AWAITING_LIBRARY_RECEIPT after ${contract.passed}/${contract.expected} contract, root TypeScript, ${packageBuilds.length}/${packageBuilds.length} affected builds, ${targetedTests}/${targetedTests} targeted tests, ${runtimeResults.length}/${runtimeResults.length} runtime gates, ${securityResults.length}/${securityResults.length} security/predecessor regressions, full Vitest ${fullTests}/${fullTests}, and production build PASS. Only an Ed25519 public-KeyObject verifier boundary is delivered; no production key authority, signer, runtime wiring, real data, SQLite transfer, or cutover authority is attached.`;
await writeJson(paths.plan, plan);
ledger.libraryUploadStatus = '31-M_SIGNED_EVIDENCE_VERIFIER_LOCAL_PASS_AWAITING_LIBRARY_RECEIPT';
...
console.log(`31-M local validation: PASS (${contract.passed}/${contract.expected} contract; ${targetedTests}/${targetedTests} targeted; ${runtimeResults.length}/${runtimeResults.length} runtime; ${securityResults.length}/${securityResults.length} security/predecessor; ${fullTests}/${fullTests} full); awaiting D: Library receipt.`);
