...
const reportPath=option('--report','artifacts/validation/build167-ipc-adaptive-budget-maintenance-session-syntax.json');
...
const report={schemaVersion:1,product:'Anadolu Parsı Aile Yaşam Merkezi',featureBuild:167,stage:'Bronze RC2 Active Development',status:failures.length?'FAIL':'PASS',fileCount:files.length,files,failures,generatedAt:new Date().toISOString()};
await mkdir(dirname(reportPath),{recursive:true});await writeFile(reportPath,`${JSON.stringify(report,null,2)}\n`);
if(failures.length){console.error(`Build 167 IPC adaptive budget maintenance session syntax: FAIL (${files.length-failures.length}/${files.length})`);for(const failure of failures)console.error(`- ${failure}`);process.exitCode=1;}else console.log(`Build 167 IPC adaptive budget maintenance session syntax: PASS (${files.length}/${files.length}).`);
