...
const paths = {
  plan: 'config/work-segmentation-plan.json', ledger: 'config/active-governance-ledger.json', execution: 'artifacts/checkpoints/31-K_EXECUTION_RECORD.json',
  failures: 'artifacts/checkpoints/31-K_INITIAL_VALIDATION_FAILURES.json', scopeConfig: 'config/31-k-monotonic-cutover-readiness-evidence-scope.json',
  contract: 'artifacts/validation/31-K_MONOTONIC_CUTOVER_READINESS_EVIDENCE_CONTRACT.json', typecheck: 'artifacts/validation/31-K_ROOT_TYPESCRIPT.json',
  targeted: 'artifacts/validation/31-K_TARGETED_VITEST.json', runtime: 'artifacts/validation/31-K_CORE_SERVICE_DESKTOP_RUNTIME.json',
  security: 'artifacts/validation/31-K_SECURITY_AND_PREDECESSOR_REGRESSION.json', regression: 'artifacts/validation/31-K_FULL_VITEST_REGRESSION.json',
  build: 'artifacts/validation/31-K_PRODUCTION_BUILD.json', platform: 'artifacts/validation/platform-policy-gate.json',
  scope: 'artifacts/inventory/31-K_SCOPE_AND_STATUS_REPORT.json', audit: 'docs/audit/31-K_MONOTONIC_CUTOVER_READINESS_EVIDENCE.md'
};
...
const run = (name, args, cwd = root) => {
  ...
  const result = spawnSync(process.execPath, args, { cwd, encoding: 'utf8', env: { ...process.env, NO_COLOR: '1', FORCE_COLOR: '0' }, maxBuffer: 24 * 1024 * 1024 });
...
const step = plan.steps.find((item) => item.id === '31-K');
if (!step || plan.currentStep !== '31-K' || step.status !== 'IN_PROGRESS') throw new Error('31-K is not active');
...
if (!validationPending && !cleanRerun) throw new Error('31-K is not awaiting validation or an idempotent clean rerun');
if (initialFailures.countsAsPass !== false || initialFailures.status !== 'SUPERSEDED_FAILED_ATTEMPTS_NOT_PASS') throw new Error('31-K failed attempts are not truthfully classified');
await readFile(full(paths.audit), 'utf8');

const contractRun = run('31-K contract', ['scripts/verify-31-k-monotonic-cutover-readiness-evidence-contract.mjs']); pass(contractRun);
const contract = await readJson(paths.contract);
if (contract.status !== 'PASS' || contract.failed !== 0 || contract.passed !== contract.expected) throw new Error('31-K contract is not clean PASS');

const typecheck = run('Root TypeScript', ['node_modules/typescript/bin/tsc', '--noEmit']);
await writeJson(paths.typecheck, { schemaVersion: 1, release: plan.release, step: '31-K', phase: 'ROOT_TYPESCRIPT_NO_EMIT', ...publicResult(typecheck), diagnosticCount: typecheck.status === 'PASS' ? 0 : null, executedAt: new Date().toISOString() }); pass(typecheck);
...
await writeJson(paths.security, { schemaVersion: 1, release: plan.release, step: '31-K', phase: 'READINESS_SECURITY_AND_PREDECESSOR_REGRESSION', status: 'PASS', expected: securityResults.length, executed: securityResults.length, passed: securityResults.length, failed: 0, checks: securityResults.map(publicResult), executedAt: new Date().toISOString() });
...
await writeJson(paths.build, { schemaVersion: 1, release: plan.release, step: '31-K', phase: 'PRODUCTION_BUILD', status: 'PASS', affectedPackageBuilds: packageBuilds.map(publicResult), electronMain: 'PASS', renderer: 'PASS', commands: [publicResult(electron), publicResult(renderer)], executedAt: new Date().toISOString() });
...
const validation = {
  ...
  targetedVitest: `PASS_${targetedTests}_OF_${targetedTests}_IN_${targetedFiles}_FILES`, runtimeGates: `PASS_${runtimeResults.length}_OF_${runtimeResults.length}`,
  securityAndPredecessorRegression: `PASS_${securityResults.length}_OF_${securityResults.length}`, fullVitest: `PASS_${fullTests}_OF_${fullTests}_IN_${fullFiles}_FILES`,
...
plan.segmentationNote = `31-K monotonic readiness evidence is LOCAL_PASS_AWAITING_LIBRARY_RECEIPT after ${contract.passed}/${contract.expected} contract, root TypeScript, ${packageBuilds.length}/${packageBuilds.length} package builds, ${targetedTests}/${targetedTests} targeted tests, ${runtimeResults.length}/${runtimeResults.length} runtime gates, ${securityResults.length}/${securityResults.length} security/predecessor regressions, full Vitest ${fullTests}/${fullTests}, and production build PASS. DEC-171 and Desktop data authority remain unchanged.`;
await writeJson(paths.plan, plan); ledger.libraryUploadStatus = '31-K_MONOTONIC_READINESS_LOCAL_PASS_AWAITING_LIBRARY_RECEIPT'; ledger.updatedAt = validatedAt; await writeJson(paths.ledger, ledger);
...
await writeJson(paths.scope, { schemaVersion: 1, release: plan.release, step: '31-K', primaryRequirement: scopeConfig.primaryRequirement, requirements: scopeConfig.requirements, status: 'LOCAL_PASS_AWAITING_LIBRARY_RECEIPT', officialStepStatus: 'IN_PROGRESS_AWAITING_LIBRARY_RECEIPT', targetSliceStatus: 'PASS', validationStatus: 'PASS', persistentReceiptStatus: 'PENDING', officialCompletionClaimed: false, requirementCompletionClaimed: false, cleanEvidence: validation, deliveredFoundation: scopeConfig.targets, acceptanceRules: scopeConfig.acceptanceRules, requiredGates: scopeConfig.requiredGates, openBoundaries: scopeConfig.openBoundaries, requirementStatuses: Object.fromEntries(scopeConfig.requirements.map((id) => [id, 'OPEN_FOUNDATION_ONLY'])), newBuildIssued: false, generatedAt: validatedAt });
console.log(`31-K local validation: PASS (${contract.passed}/${contract.expected} contract; ${targetedTests}/${targetedTests} targeted; ${runtimeResults.length}/${runtimeResults.length} runtime; ${securityResults.length}/${securityResults.length} security/predecessor; ${fullTests}/${fullTests} full); awaiting D: Library receipt.`);
