...
const reportPath = process.argv[2] ?? 'artifacts/validation/build222-workspace-dist-prerequisite.json';
...
await writeFile(reportPath,`${JSON.stringify({schemaVersion:1,product:'Anadolu Parsı Aile Yaşam Merkezi',applicationVersion:'02.08.2026.222',packageVersion:'2.8.2026-222',build:222,purpose:'Fail-closed workspace package dist readiness before Windows package:win',status,checks:results.length,passCount:results.filter(x=>x.status==='PASS').length,results,generatedAt:new Date().toISOString()},null,2)}\n`);
console.log(`Build222 workspace dist prerequisite: ${status} (${results.filter(x=>x.status==='PASS').length}/${results.length}).`);
