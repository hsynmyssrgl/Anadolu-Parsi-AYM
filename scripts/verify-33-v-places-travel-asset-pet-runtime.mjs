import { mkdir,readFile,writeFile } from 'node:fs/promises';import { resolve } from 'node:path';import { spawnSync } from 'node:child_process';
...
const [scope,inventory,registry,roadmap,plan,ledger]=await Promise.all([json('config/33-v-places-travel-asset-pet-scope.json'),json('config/33-v-places-travel-asset-pet-inventory.json'),json('config/accepted-scope-registry.json'),json('config/remaining-scope-package-roadmap.json'),json('config/work-segmentation-plan.json'),json('config/active-governance-ledger.json')]);
const testFiles=Object.freeze(scope.validation.targetedTestFiles);const execute=(args,timeout=300000)=>spawnSync(process.execPath,args,{cwd:root,encoding:'utf8',windowsHide:true,timeout,maxBuffer:64*1024*1024,env:process.env});
const clean=(value)=>String(value??'').replace(/\u001b\[[0-?]*[ -/]*[@-~]/gu,'');const output=(result)=>clean(`${result.stdout??''}\n${result.stderr??''}`);
...
types.electron=execute(['node_modules/typescript/bin/tsc','-p','apps/desktop/tsconfig.electron.json','--noEmit']);types.renderer=execute(['node_modules/typescript/bin/tsc','-p','apps/desktop/tsconfig.renderer.json','--noEmit']);
const requirements=scope.requirements;const roadmapItem=roadmap.packages?.find((item)=>item.step==='33-V');const registryItems=requirements.map((id)=>registry.requirements?.find((item)=>item.id===id));
...
const definitions=[
  ...
  ['local test result meets exact 5/27 ratchet',files===5&&tests===27&&scope.validation.targetedTestFileRatchet===5&&scope.validation.targetedTestRatchet===27&&inventory.validation.targetedTestRatchet===27],
  ...
  ['33-V remains planned behind 33-R and 33-S while 33-P stays active',roadmapItem?.status==='PLANNED'&&JSON.stringify(roadmapItem.dependsOn)===JSON.stringify(['33-R','33-S'])&&plan.currentStep==='33-P'&&ledger.activeMicroStep==='33-P'&&registryItems.every((item)=>item&&item.status!=='COMPLETE'&&item.chain?.evidence===false)],
  ['external no-claim truth remains closed',scope.truth.localOnly===true&&scope.truth.externalBookingPerformed===false&&scope.truth.liveTransportTrackingPerformed===false&&scope.truth.paymentExecutionPerformed===false&&scope.truth.documentVerificationPerformed===false&&scope.truth.aiProcessingAllowed===false&&scope.truth.externalSharingAllowed===false],
...
const report={schemaVersion:1,step:'33-V',decision:'DEC-233',status:failures.length?'FAIL':'PASS',governanceState:'PLANNED',countsAsRequirementPass:false,targetedTestFilesPassed:files,targetedTestsPassed:tests,migration100Sha256:m100?.checksum??null,ppk021:p21,ppk022:p22,checkCount:checks.length,passed:checks.length-failures.length,failed:failures.length,checks,generatedAt:new Date().toISOString()};
if(!noWrite){await mkdir(resolve(root,'artifacts/validation'),{recursive:true});await writeFile(resolve(root,'artifacts/validation/33-V-places-travel-asset-pet-runtime.json'),`${JSON.stringify(report,null,2)}\n`);}
if(failures.length){console.error(`33-V runtime: FAIL (${failures.length}/${checks.length}).`);for(const item of failures)console.error(item.name);process.exit(1);}console.log(`33-V runtime: PASS (${checks.length}/${checks.length}); ${files}/${files} files, ${tests}/${tests} tests.`);
