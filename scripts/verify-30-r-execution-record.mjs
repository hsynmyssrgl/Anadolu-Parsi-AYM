...
const RECORD_PATH = 'artifacts/checkpoints/30-R_EXECUTION_RECORD.json';
const REPORT_PATH = 'artifacts/validation/30-R-execution-record-contract.json';

const readJson = async (path) => JSON.parse(await readFile(path, 'utf8'));
const sha256 = async (path) => createHash('sha256').update(await readFile(path)).digest('hex');
const record = await readJson(RECORD_PATH);
const contract = await readJson('artifacts/validation/30-R-archive-core-receipt-fence-contract.json');
const runtime = await readJson('artifacts/validation/30-R-archive-core-receipt-fence-runtime.json');
const focused = await readJson('artifacts/validation/30-R-archive-core-receipt-fence-vitest.json');
const full = await readJson('artifacts/validation/30-R-full-vitest-final.json');
const finalValidation = await readJson('artifacts/validation/30-R_FINAL_VALIDATION_PASS.json');
const security = await readJson('artifacts/validation/30-R_SECURITY_REVIEW.json');
const diagnostic = await readJson('artifacts/validation/30-R_DIAGNOSTIC_IMPLEMENTATION_ATTEMPTS.json');
const scopeReport = await readJson('artifacts/inventory/30-R_SCOPE_AND_STATUS_REPORT.json');
...
check('contract count exact', record.validation?.contractChecks === 77 && record.validation?.contractPassed === 77 && contract.status === 'PASS' && contract.checkCount === 77 && contract.failed === 0);
check('runtime count exact', record.validation?.controlledRuntimeChecks === 33 && record.validation?.controlledRuntimePassed === 33 && runtime.status === 'PASS' && runtime.controlledChecks?.actual === 33);
check('focused and regression tests exact', record.validation?.focusedAndProductionRegressionTests === 20 && record.validation?.focusedAndProductionRegressionPassed === 20 && focused.success === true && focused.numTotalTests === 20 && focused.numPassedTests === 20 && focused.numFailedTests === 0);
check('full tests exact', record.validation?.fullVitestTests === 84 && record.validation?.fullVitestPassed === 84 && full.success === true && full.numTotalTests === 84 && full.numPassedTests === 84 && full.numFailedTests === 0);
...
check('diagnostic accounting exact', diagnostic.status === 'DIAGNOSTIC_ONLY_NOT_PASS' && diagnostic.attemptCount === 5 && diagnostic.preservedFailedAttempts === 5 && diagnostic.failedAttemptsCountedAsPass === 0 && diagnostic.attempts?.length === 5);
...
check('core table target is exact PASS', record.evidenceBoundary?.archiveCoreTableDirectSqlFence === 'TARGETED_PASS');
check('accessory table enforcement remains open', record.evidenceBoundary?.archiveAccessoryTableUniversalFenceEnforcement === 'NOT_COMPLETE');
...
check('30-R remains sole active work', plan.currentStep === STEP && plan.steps?.filter((item) => item.status === 'IN_PROGRESS').length === 1);
const workItem = plan.steps?.find((item) => item.id === STEP);
check('30-R work state remains pending receipt', workItem?.status === 'IN_PROGRESS' && workItem?.validationStatus === 'PENDING' && workItem?.persistentReceiptStatus === 'PENDING');
check('active ledger retains exact 30-R task', ledger.nextOfficialTask === '30-R PPK-002 database-enforced receipt binding for direct writes to core archive business tables');
check('active ledger is awaiting 30-R receipt', ledger.libraryUploadStatus === '30-R_LOCAL_PASS_AWAITING_LIBRARY_RECEIPT');
...
console.log(`30-R execution record contract: ${report.status} (${report.passed}/${report.expected}).`);
console.log(TRUTH);
