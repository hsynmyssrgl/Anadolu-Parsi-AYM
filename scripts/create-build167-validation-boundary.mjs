...
const reportPath=resolve(option('--report','artifacts/validation/build167-validation-boundary.json'));
...
const [sourcePreflight,sourceIntegrity,contract,runtime,syntax,packageSourceTypes,desktopMainTypes,requestCreation,responseStatus]=await Promise.all([
 readJson('artifacts/validation/build167-source-preflight-final.json'),readJson('artifacts/validation/build167-source-integrity-final.json'),readJson('artifacts/validation/build167-ipc-adaptive-budget-maintenance-session-contract.json'),readJson('artifacts/validation/build167-ipc-adaptive-budget-maintenance-session-runtime.json'),readJson('artifacts/validation/build167-ipc-adaptive-budget-maintenance-session-syntax.json'),readJson('artifacts/validation/package-source-typecheck.json'),readJson('artifacts/validation/desktop-main-source-typecheck.json'),readJson('artifacts/validation/build154-handoff-request-creation.json'),readJson('artifacts/validation/build154-handoff-response-status.json')
]);
if(sourcePreflight.status!=='PASS')throw new Error('Build 167 source preflight must be PASS.');
if(sourceIntegrity.status!=='PASS')throw new Error('Build 167 source integrity must be PASS.');
if(contract.status!=='PASS'||contract.assertions!==50)throw new Error('Build 167 maintenance session contract must be PASS 50/50.');
if(runtime.status!=='PASS'||runtime.checks!==29)throw new Error('Build 167 maintenance session runtime must be PASS 29/29.');
if(syntax.status!=='PASS'||syntax.fileCount!==10)throw new Error('Build 167 maintenance session syntax must be PASS 10/10.');
...
const results=[
 {id:'source-preflight',status:'PASS',evidence:'artifacts/validation/build167-source-preflight-final.json'},
 {id:'source-integrity',status:'PASS',evidence:'artifacts/validation/build167-source-integrity-final.json'},
