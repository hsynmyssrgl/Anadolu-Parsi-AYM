...
const paths = {
  ...
  failures: 'artifacts/checkpoints/31-L_INITIAL_VALIDATION_FAILURES.json',
  scopeConfig: 'config/31-l-protected-cutover-readiness-journal-port-scope.json',
  contract: 'artifacts/validation/31-L_PROTECTED_CUTOVER_READINESS_JOURNAL_PORT_CONTRACT.json',
  typecheck: 'artifacts/validation/31-L_ROOT_TYPESCRIPT.json',
  targeted: 'artifacts/validation/31-L_TARGETED_VITEST.json',
  runtime: 'artifacts/validation/31-L_CORE_SERVICE_RUNTIME.json',
  security: 'artifacts/validation/31-L_SECURITY_AND_PREDECESSOR_REGRESSION.json',
  regression: 'artifacts/validation/31-L_FULL_VITEST_REGRESSION.json',
  build: 'artifacts/validation/31-L_PRODUCTION_BUILD.json',
  platform: 'artifacts/validation/platform-policy-gate.json',
  scope: 'artifacts/inventory/31-L_SCOPE_AND_STATUS_REPORT.json',
...
if (!step || plan.currentStep !== '31-L' || step.status !== 'IN_PROGRESS') throw new Error('31-L is not active');
...
if (!validationPending && !cleanRerun) throw new Error('31-L is not awaiting validation or an idempotent clean rerun');
if (initialFailures.countsAsPass !== false || initialFailures.status !== 'SUPERSEDED_FAILED_ATTEMPTS_NOT_PASS') {
  throw new Error('31-L failed attempts are not truthfully classified');
}
if (!initialFailures.failures.every((failure) =>
  ...
  throw new Error('31-L initial failures do not have clean rerun evidence');
}

const contractRun = run('31-L contract', ['scripts/verify-31-l-protected-cutover-readiness-journal-port-contract.mjs']);
pass(contractRun);
const contract = await readJson(paths.contract);
if (contract.status !== 'PASS' || contract.failed !== 0 || contract.passed !== contract.expected) throw new Error('31-L contract is not clean PASS');
...
await writeJson(paths.runtime, {
  ...
  release: plan.release,
  step: '31-L',
  phase: 'CORE_SERVICE_DETACHED_JOURNAL_RUNTIME_GATES',
  status: 'PASS',
...
const securityCommands = [
  ...
  ['31-L contract verifier syntax', '--check', 'scripts/verify-31-l-protected-cutover-readiness-journal-port-contract.mjs'],
  ['31-L local validator syntax', '--check', 'scripts/run-31-l-protected-cutover-readiness-journal-port-local-validation.mjs'],
  ['31-L finalizer syntax', '--check', 'scripts/finalize-31-l-protected-cutover-readiness-journal-port-external-library-receipt.mjs'],
  ['31-L completion verifier syntax', '--check', 'scripts/verify-31-l-protected-cutover-readiness-journal-port-completion-transition.mjs']
];
...
await writeJson(paths.security, {
  ...
  release: plan.release,
  step: '31-L',
  phase: 'PROTECTED_JOURNAL_SECURITY_AND_PREDECESSOR_REGRESSION',
  status: 'PASS',
...
plan.segmentationNote = `31-L protected readiness-journal port is LOCAL_PASS_AWAITING_LIBRARY_RECEIPT after ${contract.passed}/${contract.expected} contract, root TypeScript, ${packageBuilds.length}/${packageBuilds.length} affected builds, ${targetedTests}/${targetedTests} targeted tests, ${runtimeResults.length}/${runtimeResults.length} runtime gates, ${securityResults.length}/${securityResults.length} security/predecessor regressions, full Vitest ${fullTests}/${fullTests}, and production build PASS. No adapter, path, key, real data, SQLite transfer, or cutover authority is attached.`;
await writeJson(paths.plan, plan);
ledger.libraryUploadStatus = '31-L_PROTECTED_READINESS_JOURNAL_PORT_LOCAL_PASS_AWAITING_LIBRARY_RECEIPT';
...
console.log(`31-L local validation: PASS (${contract.passed}/${contract.expected} contract; ${targetedTests}/${targetedTests} targeted; ${runtimeResults.length}/${runtimeResults.length} runtime; ${securityResults.length}/${securityResults.length} security/predecessor; ${fullTests}/${fullTests} full); awaiting D: Library receipt.`);
