...
if ($build -ne 222) { throw "Unified Bronze Windows closure requires Build222; current build=$build." }
...
try {
  ...
  Invoke-RequiredStep -Id "exact-source-integrity" -Operation {
    & node.exe "scripts\verify-source-integrity.mjs" "--report" "artifacts\validation\build222-bronze-security-source-integrity-windows.json"
  }
  ...
  Invoke-RequiredStep -Id "workspace-package-dist-guard" -Operation {
    & node.exe "scripts\verify-build222-workspace-dist-prerequisite.mjs"
  }

  $lifecycleArgs=@("-NoProfile","-ExecutionPolicy","Bypass","-File","scripts\windows-bronze-security-release-validation-build222.ps1","-EvidencePath","artifacts\validation\build222-bronze-security-windows-release-lifecycle.json")
  ...
  & node.exe "scripts\verify-build222-bronze-security-windows-result.mjs" $applicationVersion `
    "artifacts\validation\build222-bronze-security-windows-release-lifecycle.json" `
    ...
    "artifacts\validation\build222-bronze-security-source-integrity-windows.json" `
    "artifacts\validation\build222-bronze-security-windows-closure-result.json"
...
$resultPath=Join-Path $validationRoot "build222-bronze-security-windows-closure-result.json"
$result=$null
...
$summaryPath=Join-Path $validationRoot "build222-bronze-security-windows-run-summary.json"
$summary=[ordered]@{
...
if((Test-Path -LiteralPath $sourceManifestPath -PathType Leaf)-and(Test-Path -LiteralPath $sourceShaSumsPath -PathType Leaf)){
  $evidencePaths=@(
    $summaryPath,
    (Join-Path $validationRoot "build222-bronze-security-source-integrity-windows.json"),
    (Join-Path $validationRoot "build222-bronze-security-windows-release-lifecycle.json"),
  ...
  $manifestPath=Join-Path $validationRoot "build222-bronze-security-windows-evidence-manifest.json"
  ...
  $bundlePath=Join-Path $validationRoot "Bronze_Guvenlik_Windows_Kanitlari_Build222_$runId.zip"
