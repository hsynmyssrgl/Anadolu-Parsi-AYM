...
const reportPath=resolve(option('--report','artifacts/validation/build161-validation-boundary.json'));
...
const [sourcePreflight,sourceIntegrity,contract,runtime,syntax,packageSourceTypes,desktopMainTypes,requestCreation,responseStatus]=await Promise.all([
 readJson('artifacts/validation/build161-source-preflight-final.json'),
 readJson('artifacts/validation/build161-source-integrity-final.json'),
 readJson('artifacts/validation/build161-ipc-backpressure-contract.json'),
 readJson('artifacts/validation/build161-ipc-backpressure-runtime.json'),
 readJson('artifacts/validation/build161-ipc-backpressure-syntax.json'),
...
if(sourcePreflight.status!=='PASS')throw new Error('Build 161 source preflight must be PASS.');
if(sourceIntegrity.status!=='PASS')throw new Error('Build 161 source integrity must be PASS.');
if(contract.status!=='PASS'||contract.assertions!==34)throw new Error('Build 161 backpressure contract must be PASS 34/34.');
if(runtime.status!=='PASS'||runtime.checks!==22)throw new Error('Build 161 backpressure runtime must be PASS 22/22.');
if(syntax.status!=='PASS'||syntax.fileCount!==2)throw new Error('Build 161 backpressure syntax must be PASS 2/2.');
if(packageSourceTypes.status!=='PASS'||desktopMainTypes.status!=='PASS')throw new Error('Build 161 controlled TypeScript checks must be PASS.');
...
const results=[
 {id:'source-preflight',status:'PASS',evidence:'artifacts/validation/build161-source-preflight-final.json'},
 {id:'source-integrity',status:'PASS',evidence:'artifacts/validation/build161-source-integrity-final.json'},
