...
check('existing central PEP and UoW runner are reused', has('adapter', 'RepositoryBackedTimelinePolicyTransactionRunner', 'authorization.occurredAt') && has('productionPolicy', "'accessibility_preferences'", 'accessibilityPreferencesRepository.findForPolicyResolution'), `${sources.adapter};${sources.productionPolicy}`);
