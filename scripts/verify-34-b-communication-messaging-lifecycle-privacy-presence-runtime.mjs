...
results.push(run('targeted 5 files 30 tests',node,npmArgs(['run','verify:34-b:targeted'])));
...
const report={schemaVersion:1,step:'34-B',decision:'DEC-239',status:failures.length?'FAIL':'PASS',governanceState:'PLANNED',
  countsAsRequirementPass:false,targetedTestFiles:5,targetedTests:30,checkCount:results.length,
...
console.log(`34-B runtime: PASS (${results.length}/${results.length}; 5 files/30 tests; requirement PASS=false; write=${!noWrite}).`);
