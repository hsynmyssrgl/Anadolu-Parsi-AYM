...
const paths = {
  receipt: 'artifacts/checkpoints/31-I_LIBRARY_RECEIPT.json', readback: 'artifacts/validation/31-I_LIBRARY_READBACK_VERIFICATION.json',
  receiptReadback: 'artifacts/validation/31-I_RECEIPT_READBACK_VERIFICATION.json', persistence: 'artifacts/validation/31-I_RECEIPT_READBACK_PERSISTENCE_VERIFICATION.json',
  inventory: 'artifacts/validation/31-I_LIBRARY_FINAL_INVENTORY_VERIFICATION.json', completion: 'artifacts/checkpoints/31-I_COMPLETION_RECORD.json',
  transition: 'artifacts/validation/31-I_COMPLETION_TRANSITION_VALIDATION.json', execution: 'artifacts/checkpoints/31-I_EXECUTION_RECORD.json',
  scopeReport: 'artifacts/inventory/31-I_SCOPE_AND_STATUS_REPORT.json', scope: 'config/31-i-headless-device-secret-protection-boundary-scope.json',
...
const later = inspectAuthorizedSuccessorLifecycle({ plan: docs.plan, ledger: docs.ledger, predecessorId: '31-I' });
...
const step = docs.plan.steps.find((item) => item.id === '31-I');
...
check((docs.ledger.libraryUploadStatus === '31-I_COMPLETED_RECEIPT_PASS' && docs.ledger.activeMicroStep === null) || (later.planValid && later.ledgerValid && later.nextTaskValid), 'ledger complete or authorized successor active');
...
if (failed.length) { console.error(`31-I completion transition: FAIL (${failed.length}/${checks.length}).`); for (const item of failed) console.error(`- ${item.name}`); process.exit(1); }
console.log(`31-I completion transition: PASS (${checks.length}/${checks.length}${external ? '; D: external hash/size readback included' : ''}).`);
