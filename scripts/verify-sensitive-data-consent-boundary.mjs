...
export const verifySensitiveDataConsentBoundary = async () => {
  ...
  check('PPK-021 exact ratchet accepts the reviewed compositions and zero direct role bypass',
    astGate.status === 'PASS' && astGate.privilegedSurfaces === 590 && astGate.exactAllowlistEntries === 590
  ...
  check('PPK-022 successor capability ratchet stays exact and green',
    capabilityGate.status === 'PASS' && capabilityGate.capabilitySurfaces === 254
    && capabilityGate.exactManifestSurfaces === 254 && capabilityGate.findings.length === 0);
