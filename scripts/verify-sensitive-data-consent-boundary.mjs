...
export const verifySensitiveDataConsentBoundary = async () => {
  ...
  check('PPK-021 exact ratchet accepts the reviewed compositions and zero direct role bypass',
    astGate.status === 'PASS' && astGate.privilegedSurfaces === 874 && astGate.exactAllowlistEntries === 874
  ...
  check('PPK-022 successor capability ratchet stays exact and green',
    capabilityGate.status === 'PASS' && capabilityGate.capabilitySurfaces === 392
    && capabilityGate.exactManifestSurfaces === 392 && capabilityGate.findings.length === 0);
