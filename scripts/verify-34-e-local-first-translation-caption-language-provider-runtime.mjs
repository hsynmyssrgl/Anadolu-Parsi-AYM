...
const noWrite=process.argv.includes('--no-write');const root=assertGovernedSourceRoot({allowReleaseChannel:noWrite});const node=process.execPath;
...
const results=[];
results.push(run('boundary',node,['scripts/verify-34-e-local-first-translation-caption-language-provider-boundary.mjs','--no-write']));
results.push(run('contract',node,['scripts/verify-34-e-local-first-translation-caption-language-provider-contract.mjs','--no-write']));
results.push(run('targeted 5 files 23 tests',node,npmArgs(['run','verify:34-e:targeted'])));
...
if(!noWrite){await mkdir(resolve(root,'artifacts/validation'),{recursive:true});await writeFile(resolve(root,
  'artifacts/validation/34-E-local-first-translation-caption-language-provider-runtime.json'),`${JSON.stringify(report,null,2)}\n`,'utf8');}
if(failures.length){console.error(`34-E runtime: FAIL (${failures.length}/${results.length}).`);
  for(const item of failures)console.error(`${item.name}: exit ${item.exitCode}`);process.exit(1);}
console.log(`34-E runtime: PASS (${results.length}/${results.length}; 5 files/23 tests; requirement PASS=false; write=${!noWrite}).`);
