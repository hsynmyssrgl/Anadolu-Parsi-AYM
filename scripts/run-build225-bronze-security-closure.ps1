...
if ($build -ne 225) { throw "Unified Bronze Windows closure requires Build225; current build=$build." }
...
try {
  ...
  Invoke-RequiredStep -Id "exact-source-integrity" -Operation {
    & node.exe "scripts\verify-source-integrity.mjs" "--report" "artifacts\validation\build225-bronze-security-source-integrity-windows.json"
  }
  ...
  Invoke-RequiredStep -Id "workspace-package-dist-guard" -Operation {
    & node.exe "scripts\verify-build225-workspace-dist-prerequisite.mjs"
  }
  ...
  $lifecycleArgs=@("-NoProfile","-ExecutionPolicy","Bypass","-File","scripts\windows-bronze-security-release-validation-build225.ps1","-EvidencePath","artifacts\validation\build225-bronze-security-windows-release-lifecycle.json")
  ...
  & node.exe "scripts\verify-build225-bronze-security-windows-result.mjs" $applicationVersion `
    "artifacts\validation\build225-bronze-security-windows-release-lifecycle.json" `
    ...
    "artifacts\validation\build225-bronze-security-source-integrity-windows.json" `
    "artifacts\validation\build225-bronze-security-windows-closure-result.json"
...
$resultPath=Join-Path $validationRoot "build225-bronze-security-windows-closure-result.json"
$result=$null
...
$summaryPath=Join-Path $validationRoot "build225-bronze-security-windows-run-summary.json"
$summary=[ordered]@{
...
if((Test-Path -LiteralPath $sourceManifestPath -PathType Leaf)-and(Test-Path -LiteralPath $sourceShaSumsPath -PathType Leaf)){
  $evidencePaths=@(
    $summaryPath,
    (Join-Path $validationRoot "build225-bronze-security-source-integrity-windows.json"),
    (Join-Path $validationRoot "build225-bronze-security-windows-release-lifecycle.json"),
  ...
  $fullDiagnosticLogs = @(
    Get-ChildItem -LiteralPath $validationRoot -File -ErrorAction SilentlyContinue |
      Where-Object { $_.Name -like 'build225-*-full-*.log' -or $_.Name -like 'windows-open02*-full-*.log' -or $_.Name -like 'windows-open02*-early-startup.json' } |
  ...
  $manifestPath=Join-Path $validationRoot "build225-bronze-security-windows-evidence-manifest.json"
  ...
  $bundlePath=Join-Path $validationRoot "Bronze_Guvenlik_Windows_Kanitlari_Build225_$runId.zip"
