...
const definitions=[
  ...
  ['data store smoke includes migration 101',smoke.status===0&&smokeReport?.status==='passed'&&smokeReport?.migrationVersions?.at(-1)===101],
  ...
  ['provider inference network cloud and downstream action truth remain closed',scope.truth.localFirst===true&&scope.truth.providerConfigured===false&&scope.truth.networkUsed===false&&scope.truth.cloudUsed===false&&scope.truth.modelInferencePerformed===false&&scope.truth.durableActionPerformed==='not_performed'&&scope.truth.confirmationExecutesDownstreamAction===false],
