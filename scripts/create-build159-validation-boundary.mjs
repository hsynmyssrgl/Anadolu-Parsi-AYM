...
const reportPath=resolve(option('--report','artifacts/validation/build159-validation-boundary.json'));
...
const [sourcePreflight,sourceIntegrity,contract,runtime,syntax,packageSourceTypes,desktopMainTypes,requestCreation,responseStatus]=await Promise.all([
 readJson('artifacts/validation/build159-source-preflight-final.json'),
 readJson('artifacts/validation/build159-source-integrity-final.json'),
 readJson('artifacts/validation/build159-ipc-transport-contract.json'),
 readJson('artifacts/validation/build159-ipc-transport-runtime.json'),
 readJson('artifacts/validation/build159-ipc-transport-syntax.json'),
...
if(sourcePreflight.status!=='PASS')throw new Error('Build 159 source preflight must be PASS.');
if(sourceIntegrity.status!=='PASS')throw new Error('Build 159 source integrity must be PASS.');
if(contract.status!=='PASS'||contract.assertions!==37)throw new Error('Build 159 IPC transport contract must be PASS 37/37.');
if(runtime.status!=='PASS'||runtime.checks!==19)throw new Error('Build 159 IPC transport runtime must be PASS 19/19.');
if(syntax.status!=='PASS'||syntax.fileCount!==7||syntax.channelParity?.status!=='PASS')throw new Error('Build 159 IPC transport syntax/parity must be PASS.');
if(packageSourceTypes.status!=='PASS'||desktopMainTypes.status!=='PASS')throw new Error('Build 159 controlled TypeScript checks must be PASS.');
...
const results=[
 {id:'source-preflight',status:'PASS',evidence:'artifacts/validation/build159-source-preflight-final.json'},
 {id:'source-integrity',status:'PASS',evidence:'artifacts/validation/build159-source-integrity-final.json'},
