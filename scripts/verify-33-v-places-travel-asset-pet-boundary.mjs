...
const definitions=[
  ...
  ['application enforces private owner participants and no claims',has(application,["visibility==='private'","participants.includes(ownerPersonId)",'externalBookingPerformed','petHealthAdviceProvided'])],
  ['repository uses exact PEP receipt and bounded results',has(repository,["authorization.purpose!=='general'",'writeBinding(context,row)','LIMIT 1001','findItemForPolicyResolution'])],
  ...
  ['IPC exposes four bounded channels',has(ipc,["'placesTravel:getCenter'","'placesTravel:createItem'","'placesTravel:updateItem'","'placesTravel:deleteItem'",'placesTravelReceiptResult'])],
  ['renderer reuses Life route and states local truth',has(panel,['Yer ve seyahat merkezi','Harita, rezervasyon, ödeme','OCR kimliği sadece öneri referansıdır','Evcil hayvan kaydı sağlık tavsiyesi değildir'])],
