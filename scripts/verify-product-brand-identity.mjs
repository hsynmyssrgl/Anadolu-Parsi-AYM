...
check(desktopPackage.build?.nsis?.shortcutName === 'ParsYuva Aile Yaşam Merkezi', 'Kısayol adı tam değil.');
check(/^ParsYuva-Aile-Yasam-Merkezi-(?:Bronze|Silver|Gold)-/u.test(desktopPackage.build?.win?.artifactName ?? ''), 'Kurulum dosyası Türkçe anlamlı ASCII ürün adı ve sürüm kanalını taşımıyor.');
...
check(installer.includes('$INSTDIR\\ParsYuva Aile Yaşam Merkezi.exe'), 'Kaldırıcı yeni çalıştırılabilir dosya adını kullanmıyor.');
