...
check(meta.includes("CURRENT_PRODUCT_NAME = 'ParsYuva AYM'"), 'Görünür ürün ParsYuva AYM değil.');
...
check(desktopPackage.build?.productName === 'ParsYuva AYM', 'Paket görünen ürün adı ParsYuva AYM değil.');
check(desktopPackage.build?.nsis?.shortcutName === 'ParsYuva AYM', 'Kısayol adı ParsYuva AYM değil.');
check(/^ParsYuva-AYM-(?:Bronze|Silver|Gold)-/u.test(desktopPackage.build?.win?.artifactName ?? ''), 'Kurulum dosyası yeni marka ve sürüm kanalını taşımıyor.');
...
check(installer.includes('LangString AymFinishTitle ${AYM_LANG_TURKISH} "ParsYuva AYM kullanıma hazır"'), 'Kurulum bitiş başlığı ParsYuva AYM değil.');
check(!installer.includes('ParsYuva AYM Aile Yaşam Merkezi'), 'Kurulum başlığında AYM uzun adla tekrarlanıyor.');
check(installer.includes('$INSTDIR\\ParsYuva AYM.exe'), 'Kaldırıcı yeni çalıştırılabilir dosya adını kullanmıyor.');
...
check(artifactStore.includes('product: CURRENT_PRODUCT_NAME'), 'Yeni korumalı artifact ParsYuva AYM adıyla yazılmıyor.');
...
console.log(`Product Brand Identity: PASS (${checks} checks / ParsYuva AYM / legacy data compatible).`);
