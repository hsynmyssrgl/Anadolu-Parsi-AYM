...
check('vault session guard defers recoverable locks before PEP and seals real authorization failures', sources.main.includes("if (guardAction === 'defer_locked') return;") && sources.main.includes("channel: 'system:captureVaultSessionCheckpoint'") && sources.main.includes("event: 'vault.session_guard.authorization_failed'") && sources.main.includes('sealUserDataSession();'));
