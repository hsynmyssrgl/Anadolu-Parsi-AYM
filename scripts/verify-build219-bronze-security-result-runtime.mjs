...
const reportPath=process.argv[2]??'artifacts/validation/build219-bronze-security-result-runtime.json';
const root=process.cwd();const tempRoot=await mkdtemp(join(tmpdir(),'ppt-build219-unified-'));const ev=join(tempRoot,'evidence');await mkdir(ev,{recursive:true});
const writeJson=(n,v)=>writeFile(join(ev,n),`${JSON.stringify(v,null,2)}\n`);
const open021Security={status:'PASS',platform:'win32',build:219,applicationVersion:'01.08.2026.219',efs:{status:'PASS',protectionStatus:'windows-efs',activeDatabase:'memory-only',directoryEncryptedAttribute:'PASS',snapshotEncryptedAttribute:'PASS',snapshotSqliteRoundTrip:'PASS',stagingCleanup:'PASS'}};
const open021Run={status:'PASS',applicationVersion:'01.08.2026.219',windowsOpen021EfsEvidence:open021Security};
const open021Launch=(mode)=>({schemaVersion:1,applicationVersion:'01.08.2026.219',mode,status:'PASS',platform:'win32',official:true,windowsOpen021EfsRuntime:'PASS',rendererSandboxPolicy:'PASS',runs:[open021Run,open021Run]});
const side={schemaVersion:1,build:219,status:'PASS',platform:'win32',applicationVersion:'01.08.2026.219',safeStorage:{status:'PASS',selectedBackend:'dpapi',protectionId:'electron-safe-storage-v1',encryptionAvailable:'PASS'},keyEnvelope:{status:'PASS',deviceWrapped:'PASS',noPlainDataKey:'PASS'},containers:{pplog:'PASS',pptdiag:'PASS',pptreport:'PASS',ciphertextHidesPlaintext:'PASS',decryptRoundTrip:'PASS'},startupEvidence:{status:'PASS',encryptedAtRest:'PASS',decryptRoundTrip:'PASS',protectionProvider:'windows-dpapi'},volatilePaths:{status:'PASS',sessionDataUnderVolatileRoot:'PASS',crashDumpsUnderVolatileRoot:'PASS'}};
const startup={status:'PASS',protectionProvider:'windows-dpapi',encryptionRoundTrip:'PASS',diagnosticOnly:false};
const open022Run={status:'PASS',applicationVersion:'01.08.2026.219',startupSecurity:startup,windowsOpen022SideArtifactEvidence:side};
const open022Launch=(mode)=>({schemaVersion:1,applicationVersion:'01.08.2026.219',mode,status:'PASS',platform:'win32',official:true,dpapiCrossProcessPersistence:'PASS',windowsSafeStorageDpapiRuntime:'PASS',protectedSideArtifactWindowsRuntime:'PASS',volatileBrowserCrashRuntime:'PASS',rendererSandboxPolicy:'PASS',runs:[open022Run,open022Run]});
const lifecycle={schemaVersion:1,applicationVersion:'01.08.2026.219',packageVersion:'1.8.2026-219',build:219,status:'PASS',official:true,diagnosticOnly:false,installer:{sha256:'c'.repeat(64)},steps:['windows-installer-build','development-open021-launch','development-open022-launch','silent-install','installed-open021-launch','installed-open022-launch','silent-uninstall'].map(id=>({id,status:'PASS',exitCode:0}))};
...
const verifier=resolve(root,'scripts/verify-build219-bronze-security-windows-result.mjs');
const invoke=(out)=>spawnSync(process.execPath,[verifier,'01.08.2026.219',join(ev,'lifecycle.json'),join(ev,'021dev.json'),join(ev,'021pkg.json'),join(ev,'022dev.json'),join(ev,'022pkg.json'),join(ev,'integrity.json'),join(ev,out)],{cwd:root,encoding:'utf8'});
