...
const noWriteArgs = noWrite ? ['--no-write'] : [];
const contractArgs = ['scripts/verify-32-r-ppk-022-capability-manifest-gate-contract.mjs', ...(candidateMode ? ['--candidate'] : []), ...noWriteArgs];

const commands = [
  { id: 'ppk-022-contract', args: contractArgs, expectOutput: `PPK-022${candidateMode ? ' candidate' : ''} contract: PASS` },
  { id: 'ppk-022-capability-production-gate', args: ['scripts/verify-platform-capability-manifest-gate.mjs', ...noWriteArgs], expectOutput: '"status": "PASS"' },
  { id: 'ppk-021-ast-ratchet-regression', args: ['scripts/verify-platform-policy-ast-gate.mjs', ...noWriteArgs], expectOutput: '"status": "PASS"' },
  { id: 'combined-platform-policy-gate', args: ['scripts/verify-platform-policy-gate.mjs', ...noWriteArgs], expectOutput: 'capability manifest gate PASS' },
  ...
  { id: 'data-store-smoke', before: [['node_modules/typescript/bin/tsc', '-p', 'tests/smoke/tsconfig.data-store.json']], args: ['scripts/verify-data-store-smoke.mjs', ...noWriteArgs], expectOutput: '"checks": 14' },
  { id: 'migration-77-no-ppk022-schema-change', before: [['node_modules/typescript/bin/tsc', '-p', 'packages/database/tsconfig.json'], ['node_modules/typescript/bin/tsc', '-p', 'tests/smoke/tsconfig.data-store.json']], args: ['scripts/verify-database-migrations.mjs', ...noWriteArgs], expectOutput: '"version": 77' },
...
const report = {
  ...
  scannedProductionFiles: 566,
  exactCapabilitySurfaces: 428,
  exactManifestEntries: 428,
  pinnedBootstrapSurfaces: 26,
  maliciousAstSelfTests: 35,
