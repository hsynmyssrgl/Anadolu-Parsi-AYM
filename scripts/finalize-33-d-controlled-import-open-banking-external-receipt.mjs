...
const stepId = '33-D';
const localPackageRoot = 'C:\\PPT\\AYM\\09_ARSIV\\KAYNAK_AGACI\\checkpoints\\33-D_B4_Controlled_Import_Open_Banking';
const libraryRoot = 'D:\\AYM_LIBRARY\\Panthera pardus tulliana\\Anadolu Parsı Aile Yaşam Merkezi\\Bronze 04.08.2026.29\\checkpoints\\33-D_B4_Controlled_Import_Open_Banking';
const truth = 'Bu makbuz yalnız 33-D B4-13/B4-14 kontrollü içe aktarma ve ağsız sentetik OHVPS sınırını kapatır; canlı banka bağlantısı, banka kimlik bilgisi, dış rıza veya ödeme yürütme iddiası değildir.';
...
const payloadPaths = Object.freeze([
  ...
  'apps/desktop/src/renderer/FinancePlanningPanel.tsx',
...
const paths = Object.freeze({
  ...
  inventory: 'config/33-d-b4-controlled-import-open-banking-inventory.json',
  boundary: 'artifacts/validation/33-D-b4-controlled-import-open-banking-boundary.json',
  contract: 'artifacts/validation/33-D-b4-controlled-import-open-banking-contract.json',
  runtime: 'artifacts/validation/33-D-b4-controlled-import-open-banking-runtime.json',
  ...
  finalInventory: 'artifacts/validation/33-D_LIBRARY_FINAL_INVENTORY_VERIFICATION.json',
...
const writeGovernanceJsonAtomic = async (path, value) => {
  ...
  const temporary = resolve(root, '.tmp', '33-d-governance-commit', `${basename(path)}.${process.pid}.tmp`);
...
const successorBefore = planBefore.steps.find((item) => item.id === '33-E');
if (stepBefore?.status === 'COMPLETED') {
  const verification = spawnSync(process.execPath, ['scripts/verify-33-d-controlled-import-open-banking-completion.mjs', '--external'], {
  ...
  assert(verification.status === 0, '33-D is completed and verify-only readback failed');
  console.log('33-D is already completed; finalizer performed verify-only readback and made no changes.');
  process.exit(0);
}
const ledgerSealedPlanPending = planBefore.currentStep === stepId
  ...
  && ledgerBefore.libraryUploadStatus === '33-D_COMPLETED_RECEIPT_PASS'
  && ledgerBefore.nextOfficialTask === 'AUTO_PRIORITY_SELECTION_AFTER_33-D_PERSISTENT_RECEIPT'
  && ledgerBefore.activeMicroStep === null
  && ledgerBefore.externalLibraryAuthority33D?.status === 'PASS'
  && ledgerBefore.externalLibraryAuthority33D?.path === libraryRoot
  && ledgerBefore.externalLibraryAuthority33D?.receipt === paths.receipt;
if (ledgerSealedPlanPending) {
  ...
  assert(head.status === 0, 'Could not resolve source commit during 33-D governance recovery');
  const recoveryReceipt = await readJson(paths.receipt);
  assert(recoveryReceipt.sourceCommit === head.stdout.trim(), '33-D recovery receipt is not bound to current HEAD');
  const verification = spawnSync(process.execPath, ['scripts/verify-33-d-controlled-import-open-banking-completion.mjs', '--external', '--allow-plan-pending'], {
    cwd: root, encoding: 'utf8', windowsHide: true
  });
  if (verification.stdout) process.stdout.write(verification.stdout);
  if (verification.stderr) process.stderr.write(verification.stderr);
  assert(verification.status === 0, '33-D ledger-sealed recovery verification failed');
  ...
  planBefore.segmentationNote = '33-D is immutable COMPLETED/PASS with exact D: USB hash/size readback. 33-E remains PENDING until the governed step transition is applied.';
  await writeGovernanceJsonAtomic(paths.plan, planBefore);
  const completedVerification = spawnSync(process.execPath, ['scripts/verify-33-d-controlled-import-open-banking-completion.mjs', '--external'], {
    cwd: root, encoding: 'utf8', windowsHide: true
  });
  ...
  assert(completedVerification.status === 0, '33-D recovered completion verification failed');
  console.log('33-D governance recovery: PASS (ledger-sealed plan transition completed).');
  process.exit(0);
}
const localReady = planBefore.currentStep === stepId
  ...
  && ledgerBefore.libraryUploadStatus === '33-D_LOCAL_PASS_AWAITING_LIBRARY_RECEIPT'
  && ledgerBefore.nextOfficialTask === '33-D B4 controlled finance import and network-free OHVPS adapter closure';
assert(localReady, '33-D is not the sole active receipt-pending step; completed steps are verify-only');
assert(scope.status === 'COMPLETE' && inventory.status === 'COMPLETE', '33-D scope or inventory is not COMPLETE');
assert([boundary, contract, runtime].every((item) => item.status === 'PASS'), '33-D validation evidence is not clean PASS');
assert(boundary.checksFailed === 0 && contract.checksFailed === 0 && runtime.checksFailed === 0, '33-D validation contains failures');
assert(scope.openBanking.liveBankConnection === 'not_implemented' && scope.openBanking.networkAccess === 'not_performed', '33-D no-live truth changed');
const finalEvidence = scope.validation?.finalEvidence;
assert(
  finalEvidence?.boundaryChecksPassed === 40
    && finalEvidence.contractChecksPassed === 14
    && finalEvidence.runtimeChecksPassed === 11
    && finalEvidence.targetedTestsPassed === 17
    && finalEvidence.fullVitestTestsPassed === 931
    && finalEvidence.productionWorkspaceBuildsPassed === 18
    && finalEvidence.finalClosureEvidence === true,
  '33-D declared final evidence counts changed'
);
const registryRequirements = ['B4-13', 'B4-14'].map((id) => registry.requirements.find((item) => item.id === id));
assert(registryRequirements.every((item) => item?.status === 'COMPLETE'), 'B4-13/B4-14 registry status is not COMPLETE');
assert(registryRequirements.every((item) => Object.keys(item.chain ?? {}).length === 13 && Object.values(item.chain).every((value) => value === true)), 'B4-13/B4-14 registry chain is not exact 13/13');
...
assert(status.status === 0 && status.stdout.trim() === '', '33-D finalization requires a clean committed source tree');
...
const manifestName = '33-D_CHECKPOINT_MANIFEST.json';
const manifest = {
  schemaVersion: 1,
  release,
  step: stepId,
  requirements: ['B4-13', 'B4-14'],
  phase: 'CONTROLLED_IMPORT_OPEN_BANKING_CHECKPOINT_PACKAGE',
  status: 'PASS',
  sourceCommit,
  payloadCount: payload.length,
  payload,
  validation: { boundary: '40/40', contract: '14/14', runtime: '11/11', targetedTests: '17/17', fullTests: '931/931', builds: '18/18' },
  noLiveBoundary: true,
...
assert(JSON.stringify(await listFiles(localPackageRoot)) === JSON.stringify(expectedBase), 'Local 33-D checkpoint set is not exact');
...
assert(baseReadback.every((item) => item.status === 'PASS'), 'D: 33-D base package readback mismatch');
...
const receipt = await writePair(paths.receipt, {
  schemaVersion: 1, release, step: stepId, requirements: ['B4-13', 'B4-14'], status: 'PASS',
  ...
  libraryReadbackVerification: readback, liveBankConnectionImplemented: false, networkAccessPerformed: false,
  credentialsCollected: false, externalConsentPerformed: false, nextOfficialStep: '33-E', newBuildIssued: false,
  currentAuthoritativeSourceExternalProtectionStatus: 'SEPARATE_GOV_005_REFRESH_REQUIRED_AFTER_SOURCE_CHANGE',
...
assert(receiptArtifacts.every((item) => item.status === 'PASS'), 'D: 33-D receipt readback mismatch');
...
assert(persistenceArtifacts.every((item) => item.status === 'PASS'), 'D: 33-D receipt persistence mismatch');
...
assert(JSON.stringify(actualBeforeInventory) === JSON.stringify(expectedBeforeInventory), 'D: 33-D pre-inventory set is not exact');
...
assert(JSON.stringify(baseFinalActual) === JSON.stringify(baseFinalExpected), 'D: 33-D base final inventory set is not exact');
const finalInventoryArtifacts = await compare(root, libraryRoot, [paths.finalInventory, `${paths.finalInventory}.sha256`]);
assert(finalInventoryArtifacts.every((item) => item.status === 'PASS'), 'D: 33-D final inventory pair readback mismatch');
...
const completion = await writePair(paths.completion, {
  ...
  officialCompletionClaimed: true, requirementCompletionClaimed: true,
  persistentReceiptPath: paths.receipt, libraryPath: libraryRoot, storageBackend: 'EXTERNAL_USB_D_DRIVE', sourceCommit,
  externalInventory: {
  ...
  evidence: [...supplementPairs, finalInventory], liveBankConnectionImplemented: false, networkAccessPerformed: false,
  credentialsCollected: false, externalConsentPerformed: false, nextOfficialStep: '33-E', newBuildIssued: false,
  currentAuthoritativeSourceExternalProtectionStatus: 'SEPARATE_GOV_005_REFRESH_REQUIRED_AFTER_SOURCE_CHANGE',
...
assert(step, '33-D work step is missing');
Object.assign(step, {
  status: 'COMPLETED', validationStatus: 'PASS', persistentReceiptStatus: 'PASS',
  persistentReceiptPath: paths.receipt, completionTransitionStatus: 'PASS'
});
...
plan.segmentationNote = '33-D is immutable COMPLETED/PASS with exact D: USB hash/size readback. 33-E remains PENDING until the governed step transition is applied.';

const ledger = await readJson(paths.ledger);
ledger.libraryUploadStatus = '33-D_COMPLETED_RECEIPT_PASS';
ledger.nextOfficialTask = 'AUTO_PRIORITY_SELECTION_AFTER_33-D_PERSISTENT_RECEIPT';
ledger.activeMicroStep = null;
ledger.externalLibraryAuthority33D = { step: stepId, status: 'PASS', storageBackend: 'EXTERNAL_USB_D_DRIVE', path: libraryRoot, receipt: paths.receipt, focusedCheckpointOnly: true };
...
const transitionChecks = [
  ...
  ['boundary PASS', boundary.status === 'PASS' && boundary.checksPassed === 40],
  ['contract PASS', contract.status === 'PASS' && contract.checksPassed === 14],
  ...
  ['ledger complete', ledger.libraryUploadStatus === '33-D_COMPLETED_RECEIPT_PASS' && ledger.activeMicroStep === null],
  ['no-live truth', receipt && scope.openBanking.liveBankConnection === 'not_implemented'],
  ['next step pending', plan.steps.find((item) => item.id === '33-E')?.status === 'PENDING']
].map(([name, passed]) => ({ name, status: passed ? 'PASS' : 'FAIL' }));
assert(transitionChecks.every((item) => item.status === 'PASS'), '33-D completion transition failed');
...
assert(closureArtifacts.every((item) => item.status === 'PASS'), 'D: 33-D completion/transition pair readback mismatch');
...
assert(JSON.stringify(actualBeforeClosureInventory) === JSON.stringify(expectedBeforeClosureInventory), 'D: 33-D pre-closure inventory set is not exact');
...
assert(closureInventoryArtifacts.every((item) => item.status === 'PASS'), 'D: 33-D closure inventory pair readback mismatch');
...
assert(JSON.stringify(closureFinalActual) === JSON.stringify(closureFinalExpected), 'D: 33-D closure inventory set is not exact');

await writeGovernanceJsonAtomic(paths.ledger, ledger);
await writeGovernanceJsonAtomic(paths.plan, plan);

console.log(`33-D external receipt finalized: PASS (${closureFinalExpected.length} exact D: files; source ${sourceCommit}).`);
