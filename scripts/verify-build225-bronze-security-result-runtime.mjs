...
const reportPath=process.argv[2]??'artifacts/validation/build225-bronze-security-result-runtime.json';
const root=process.cwd();const tempRoot=await mkdtemp(join(tmpdir(),'ppt-build225-unified-'));const ev=join(tempRoot,'evidence');await mkdir(ev,{recursive:true});
...
const open021Security={status:'PASS',platform:'win32',build:225,applicationVersion:'02.08.2026.225',efs:{status:'PASS',protectionStatus:'windows-efs',activeDatabase:'memory-only',directoryEncryptedAttribute:'PASS',snapshotEncryptedAttribute:'PASS',snapshotSqliteRoundTrip:'PASS',stagingCleanup:'PASS'}};
const open021Run={status:'PASS',applicationVersion:'02.08.2026.225',windowsOpen021EfsEvidence:open021Security};
const open021Launch=(mode)=>({schemaVersion:1,applicationVersion:'02.08.2026.225',mode,status:'PASS',platform:'win32',official:true,windowsOpen021EfsRuntime:'PASS',rendererSandboxPolicy:'PASS',fullProcessLogs:fullLogs(`windows-open021-${mode}`),runs:[open021Run,open021Run]});
const side={schemaVersion:1,build:225,status:'PASS',platform:'win32',applicationVersion:'02.08.2026.225',safeStorage:{status:'PASS',selectedBackend:'unknown',runtimeBackendReported:false,provider:'windows-dpapi',providerBasis:'electron-safe-storage-windows-platform-contract',protectionId:'electron-safe-storage-v1',encryptionAvailable:'PASS',encryptDecryptRoundTrip:'PASS'},keyEnvelope:{status:'PASS',deviceWrapped:'PASS',noPlainDataKey:'PASS'},containers:{pplog:'PASS',pptdiag:'PASS',pptreport:'PASS',ciphertextHidesPlaintext:'PASS',decryptRoundTrip:'PASS'},startupEvidence:{status:'PASS',encryptedAtRest:'PASS',decryptRoundTrip:'PASS',protectionProvider:'windows-dpapi'},volatilePaths:{status:'PASS',sessionDataUnderVolatileRoot:'PASS',crashDumpsUnderVolatileRoot:'PASS'}};
const startup={status:'PASS',protectionProvider:'windows-dpapi',encryptionRoundTrip:'PASS',diagnosticOnly:false};
const open022Run={status:'PASS',applicationVersion:'02.08.2026.225',startupSecurity:startup,windowsOpen022SideArtifactEvidence:side};
const open022Launch=(mode)=>({schemaVersion:1,applicationVersion:'02.08.2026.225',mode,status:'PASS',platform:'win32',official:true,dpapiCrossProcessPersistence:'PASS',windowsSafeStorageDpapiRuntime:'PASS',protectedSideArtifactWindowsRuntime:'PASS',volatileBrowserCrashRuntime:'PASS',rendererSandboxPolicy:'PASS',fullProcessLogs:fullLogs(`windows-open022-${mode}`),runs:[open022Run,open022Run]});
const lifecycle={schemaVersion:1,applicationVersion:'02.08.2026.225',packageVersion:'2.8.2026-225',build:225,status:'PASS',official:true,diagnosticOnly:false,installer:{sha256:'c'.repeat(64)},steps:['windows-installer-build','development-open021-launch','development-open022-launch','silent-install','installed-open021-launch','installed-open022-launch','silent-uninstall'].map(id=>({id,status:'PASS',exitCode:0}))};
...
const verifier=resolve(root,'scripts/verify-build225-bronze-security-windows-result.mjs');
const invoke=(out)=>spawnSync(process.execPath,[verifier,'02.08.2026.225',join(ev,'lifecycle.json'),join(ev,'021dev.json'),join(ev,'021pkg.json'),join(ev,'022dev.json'),join(ev,'022pkg.json'),join(ev,'integrity.json'),join(ev,out)],{cwd:root,encoding:'utf8'});
...
const status=checks.every(x=>x.status==='PASS')?'PASS':'FAIL';await mkdir('artifacts/validation',{recursive:true});await writeFile(reportPath,`${JSON.stringify({schemaVersion:1,product:'Anadolu Parsı Aile Yaşam Merkezi',applicationVersion:'02.08.2026.225',build:225,status,checks:checks.length,passCount:checks.filter(x=>x.status==='PASS').length,results:checks,generatedAt:new Date().toISOString()},null,2)}\n`);await rm(tempRoot,{recursive:true,force:true});console.log(`Build225 unified result runtime: ${status} (${checks.filter(x=>x.status==='PASS').length}/${checks.length}).`);if(status!=='PASS')process.exitCode=1;
