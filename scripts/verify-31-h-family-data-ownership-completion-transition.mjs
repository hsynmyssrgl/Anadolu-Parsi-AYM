...
const paths = {
  ...
  scopeReport: 'artifacts/inventory/31-H_SCOPE_AND_STATUS_REPORT.json',
  scope: 'config/31-h-core-service-family-data-session-ownership-control-plane-scope.json',
...
const laterSuccessor = inspectAuthorizedSuccessorLifecycle({ plan: docs.plan, ledger: docs.ledger, predecessorId: '31-H' });
...
const step = docs.plan.steps.find((item) => item.id === '31-H');
...
check((docs.ledger.libraryUploadStatus === '31-H_COMPLETED_RECEIPT_PASS' && docs.ledger.activeMicroStep === null) || (laterSuccessor.planValid && laterSuccessor.ledgerValid && laterSuccessor.nextTaskValid), 'ledger complete or authorized successor active');
...
if (failed.length) {
  console.error(`31-H completion transition: FAIL (${failed.length}/${checks.length}).`);
  for (const item of failed) console.error(`- ${item.name}`);
  process.exit(1);
}
console.log(`31-H completion transition: PASS (${checks.length}/${checks.length}${external ? '; D: external hash/size readback included' : ''}).`);
