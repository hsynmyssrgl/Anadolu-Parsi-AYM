...
const paths = {
  ...
  scopeReport: 'artifacts/inventory/31-G_SCOPE_AND_STATUS_REPORT.json',
...
const laterSuccessor = inspectAuthorizedSuccessorLifecycle({ plan: docs.plan, ledger: docs.ledger, predecessorId: '31-G' });
...
const step = docs.plan.steps.find((item) => item.id === '31-G');
...
check((docs.ledger.libraryUploadStatus === '31-G_COMPLETED_RECEIPT_PASS' && docs.ledger.activeMicroStep === null) || (laterSuccessor.planValid && laterSuccessor.ledgerValid && laterSuccessor.nextTaskValid), 'ledger complete or authorized successor active');
...
if (failed.length) {
  console.error(`31-G completion transition: FAIL (${failed.length}/${checks.length}).`);
  for (const item of failed) console.error(`- ${item.name}`);
  process.exit(1);
}
console.log(`31-G completion transition: PASS (${checks.length}/${checks.length}${external ? '; D: external hash/size readback included' : ''}).`);
