...
const definitions=[
  ['scope inventory and five-test matrix are exact',JSON.stringify(scope.requirements)===JSON.stringify(inventory.requirements)&&JSON.stringify(scope.validation.targetedTestFiles)===JSON.stringify(inventory.implementedTargetedTests)&&tests.length===5&&scope.validation.targetedTestRatchet===20],
  ['migration 101 source manifest and scope checksums are canonical',migration?.name==='consent_bound_family_ai_assistant'&&migration?.checksum===sha&&sha===scope.validation.migrationSha256&&sha==='3758e4c795b59e09b7701dbf6dd3b89c6717506be8e4ee9cc13b8ba20cdf3715'],
  ...
  ['repository enforces exact receipt and payload-free policy resolution',has(repository,['writeBinding(context,row)','findSuggestionForPolicyResolution','LIMIT 501'])],
  ...
  ['tests cover replay mismatch consent revocation rollback no-action IPC and route',has(tests.join('\n'),['replayed:true',"query:'toplant", "status:'revoked'",'controlled 33-W outbox failure','durableActionPerformed','familyAiAssistant:future','family-ai-assistant-title'])],
  ...
  ['PPK ratchets are exact PASS while requirement remains open',p21.status==='PASS'&&p21.scannedProductionFiles===474&&p21.exactPrivilegedSurfaceCount===730&&p21.exactAllowlistSha256==='70db1305706956168b12fff86ad42cd9140227b0bfa0b5bc5e90c976c54ef971'&&p22.status==='PASS'&&p22.scannedProductionFiles===474&&p22.exactCapabilitySurfaceCount===345&&p22.exactCapabilityManifestSha256==='1b8625264023eb79d3f36a3c25ca19480569bea6aa1f4589841b1b4d14d5ec3e'&&scope.validation.countsAsRequirementPass===false]
];
