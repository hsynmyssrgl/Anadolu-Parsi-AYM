...
const canonicalLibraryRoot = 'D:\\AYM_LIBRARY\\Panthera pardus tulliana\\Anadolu Parsı Aile Yaşam Merkezi\\Bronze 04.08.2026.29\\checkpoints\\33-E_B5_Category_Life_Home_Vehicle';
const canonicalLocalCheckpointRoot = 'C:\\PPT\\AYM\\09_ARSIV\\KAYNAK_AGACI\\checkpoints\\33-E_B5_Category_Life_Home_Vehicle';
const paths = Object.freeze({
  ...
  inventory: 'artifacts/validation/33-E_LIBRARY_FINAL_INVENTORY_VERIFICATION.json',
...
const step = docs.plan.steps.find((item) => item.id === '33-E');
const successor = docs.plan.steps.find((item) => item.id === '33-F');
const laterLifecycle = inspectAuthorizedSuccessorLifecycle({ plan: docs.plan, ledger: docs.ledger, predecessorId: '33-E' });
...
check(
  ...
  'receipt and completion bind the canonical 33-E D: and local archive paths'
);
...
check(
  (step?.status === 'COMPLETED' && step.validationStatus === 'PASS' && step.persistentReceiptStatus === 'PASS')
    || (allowPlanPending && docs.plan.currentStep === '33-E' && step?.status === 'IN_PROGRESS' && step.validationStatus === 'PENDING' && step.persistentReceiptStatus === 'PENDING'),
  'work plan preserves 33-E completion or the exact ledger-sealed recovery state'
);
check(successor?.status === 'PENDING' || successor?.status === 'IN_PROGRESS' || successor?.status === 'COMPLETED', '33-F successor is governed');
check(
  (docs.plan.currentStep === '33-E' && docs.ledger.libraryUploadStatus === '33-E_COMPLETED_RECEIPT_PASS' && docs.ledger.activeMicroStep === null)
    || (laterLifecycle.planValid && laterLifecycle.ledgerValid && laterLifecycle.nextTaskValid),
  'ledger preserves 33-E completion through the governed successor lifecycle'
);
check(
  docs.ledger.externalLibraryAuthority33E?.status === 'PASS'
    && docs.ledger.externalLibraryAuthority33E?.storageBackend === 'EXTERNAL_USB_D_DRIVE'
    && docs.ledger.externalLibraryAuthority33E?.path === canonicalLibraryRoot
    && docs.ledger.externalLibraryAuthority33E?.localCheckpointPath === canonicalLocalCheckpointRoot
    && docs.ledger.externalLibraryAuthority33E?.receipt === paths.receipt,
  'ledger retains exact 33-E external library authority'
);
check(
  docs.scope.truth.dataSource === 'manual'
    && docs.scope.truth.externalRegistryLookup === 'not_performed'
  ...
  'manual-only managed-life truth is preserved'
);
check(
  docs.receipt.dataSource === 'manual'
    && docs.receipt.externalRegistryLookup === 'not_performed'
  ...
  'receipt makes no external registry, provider, payment, document-content, or network claim'
);
...
check(
  ...
  'all 33-E proof documents bind one source commit'
);
...
const verifyCheckpoint = async (checkpointRoot, label) => {
  ...
  try {
    ...
    check(false, `${label}: 33-E checkpoint is readable: ${error instanceof Error ? error.message : String(error)}`);
  }
};
...
if (failures.length > 0) {
  console.error(`33-E completion verification: FAIL (${failures.length}/${checks.length}).`);
  for (const item of failures) console.error(`- ${item.name}`);
  process.exit(1);
}
console.log(`33-E completion verification: PASS (${checks.length}/${checks.length}; local archive readback included${external ? '; D: external readback included' : ''}).`);
