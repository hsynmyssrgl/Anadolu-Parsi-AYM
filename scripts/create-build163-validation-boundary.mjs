...
const reportPath=resolve(option('--report','artifacts/validation/build163-validation-boundary.json'));
...
const [sourcePreflight,sourceIntegrity,contract,runtime,syntax,packageSourceTypes,desktopMainTypes,requestCreation,responseStatus]=await Promise.all([
 readJson('artifacts/validation/build163-source-preflight-final.json'),
 readJson('artifacts/validation/build163-source-integrity-final.json'),
 readJson('artifacts/validation/build163-ipc-performance-telemetry-contract.json'),
 readJson('artifacts/validation/build163-ipc-performance-telemetry-runtime.json'),
 readJson('artifacts/validation/build163-ipc-performance-telemetry-syntax.json'),
...
if(sourcePreflight.status!=='PASS')throw new Error('Build 163 source preflight must be PASS.');
if(sourceIntegrity.status!=='PASS')throw new Error('Build 163 source integrity must be PASS.');
if(contract.status!=='PASS'||contract.assertions!==40)throw new Error('Build 163 telemetry contract must be PASS 40/40.');
if(runtime.status!=='PASS'||runtime.checks!==29)throw new Error('Build 163 telemetry runtime must be PASS 29/29.');
if(syntax.status!=='PASS'||syntax.fileCount!==6)throw new Error('Build 163 telemetry syntax must be PASS 6/6.');
if(packageSourceTypes.status!=='PASS'||desktopMainTypes.status!=='PASS')throw new Error('Build 162 controlled TypeScript checks must be PASS.');
...
const results=[
 {id:'source-preflight',status:'PASS',evidence:'artifacts/validation/build163-source-preflight-final.json'},
 {id:'source-integrity',status:'PASS',evidence:'artifacts/validation/build163-source-integrity-final.json'},
