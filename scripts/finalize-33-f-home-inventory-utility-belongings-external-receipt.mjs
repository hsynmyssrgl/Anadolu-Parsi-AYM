...
const paths = Object.freeze({
  ...
  inventory: 'config/33-f-home-inventory-utility-belongings-inventory.json',
  boundary: 'artifacts/validation/33-F-home-inventory-utility-belongings-boundary.json',
  contract: 'artifacts/validation/33-F-home-inventory-utility-belongings-contract.json',
  runtime: 'artifacts/validation/33-F-home-inventory-utility-belongings-runtime.json',
  ...
  finalInventory: 'artifacts/validation/33-F_LIBRARY_FINAL_INVENTORY_VERIFICATION.json',
...
const successorBefore = planBefore.steps.find((item) => item.id === '33-G');
if (stepBefore?.status === 'COMPLETED') {
  const verification = spawnSync(process.execPath, ['scripts/verify-33-f-home-inventory-utility-belongings-completion.mjs', '--external'], {
  ...
  assert(verification.status === 0, '33-F is completed and verify-only readback failed');
  console.log('33-F is already completed; finalizer performed verify-only readback and made no changes.');
  process.exit(0);
}
const ledgerSealedPlanPending = planBefore.currentStep === stepId
  ...
  && ledgerBefore.libraryUploadStatus === '33-F_COMPLETED_RECEIPT_PASS'
  && ledgerBefore.nextOfficialTask === 'AUTO_PRIORITY_SELECTION_AFTER_33-F_PERSISTENT_RECEIPT'
  && ledgerBefore.activeMicroStep === null
  && ledgerBefore.externalLibraryAuthority33F?.status === 'PASS'
  && ledgerBefore.externalLibraryAuthority33F?.path === libraryRoot
  && ledgerBefore.externalLibraryAuthority33F?.localCheckpointPath === localPackageRoot
  && ledgerBefore.externalLibraryAuthority33F?.receipt === paths.receipt;
if (ledgerSealedPlanPending) {
  ...
  assert(head.status === 0, 'Could not resolve source commit during 33-F governance recovery');
  const recoveryReceipt = await readJson(paths.receipt);
  assert(recoveryReceipt.sourceCommit === head.stdout.trim(), '33-F recovery receipt is not bound to current HEAD');
  const verification = spawnSync(process.execPath, ['scripts/verify-33-f-home-inventory-utility-belongings-completion.mjs', '--external', '--allow-plan-pending'], {
    cwd: root, encoding: 'utf8', windowsHide: true
  });
  if (verification.stdout) process.stdout.write(verification.stdout);
  if (verification.stderr) process.stderr.write(verification.stderr);
  assert(verification.status === 0, '33-F ledger-sealed recovery verification failed');
  ...
  planBefore.segmentationNote = '33-F is immutable COMPLETED/PASS with exact D: USB hash/size readback. 33-G remains PENDING until the next governed multi-requirement slice is selected.';
  await writeGovernanceJsonAtomic(paths.plan, planBefore);
  const completedVerification = spawnSync(process.execPath, ['scripts/verify-33-f-home-inventory-utility-belongings-completion.mjs', '--external'], {
    cwd: root, encoding: 'utf8', windowsHide: true
  });
  ...
  assert(completedVerification.status === 0, '33-F recovered completion verification failed');
  console.log('33-F governance recovery: PASS (ledger-sealed plan transition completed).');
  process.exit(0);
}
const localReady = planBefore.currentStep === stepId
  ...
  && ledgerBefore.libraryUploadStatus === '33-F_LOCAL_PASS_AWAITING_LIBRARY_RECEIPT'
  && ledgerBefore.nextOfficialTask === '33-F governed home spaces, utility consumption, belongings warranty and service workflow';
assert(localReady, '33-F is not the sole active receipt-pending step; completed steps are verify-only');
assert(scope.status === 'COMPLETE' && inventory.status === 'COMPLETE', '33-F scope or inventory is not COMPLETE');
assert([boundary, contract, runtime].every((item) => item.status === 'PASS'), '33-F validation evidence is not clean PASS');
assert(boundary.checksFailed === 0 && contract.checksFailed === 0 && runtime.checksFailed === 0, '33-F validation contains failures');
assert(
  ...
  '33-F manual-only home-inventory truth changed'
);
...
assert(
  finalEvidence?.boundaryChecksPassed === 51
    ...
    && finalEvidence.targetedTestsPassed === 37
    && finalEvidence.fullVitestTestsPassed === 968
    && finalEvidence.productionWorkspaceBuildsPassed === 18
    && finalEvidence.finalClosureEvidence === true,
  '33-F declared final evidence counts changed'
);
const registryRequirements = ['EXT-030', 'EXT-032'].map((id) => registry.requirements.find((item) => item.id === id));
assert(registryRequirements.every((item) => item?.status === 'COMPLETE'), 'EXT-030/EXT-032 registry status is not COMPLETE');
assert(registryRequirements.every((item) => Object.keys(item.chain ?? {}).length === 13 && Object.values(item.chain).every((value) => value === true)), 'EXT-030/EXT-032 registry chain is not exact 13/13');
...
assert(status.status === 0 && status.stdout.trim() === '', '33-F finalization requires a clean committed source tree');
const trackedSnapshot = spawnSync('git', [
  '-c', 'safe.directory=C:/PPT/AYM/06_KOD/app', 'ls-tree', '-r', '--name-only', 'HEAD'
], { cwd: root, encoding: 'utf8', windowsHide: true });
assert(trackedSnapshot.status === 0, 'Could not enumerate the exact 33-F tracked source snapshot');
const trackedPaths = trackedSnapshot.stdout.trim().split(/\r?\n/u).filter(Boolean).sort();
assert(trackedPaths.length > 0 && new Set(trackedPaths).size === trackedPaths.length, '33-F tracked source snapshot is empty or contains duplicate paths');
...
assert(
  ...
  '33-F payload does not cover the exact tracked source snapshot and required evidence'
);
...
const manifestName = '33-F_CHECKPOINT_MANIFEST.json';
const manifest = {
  schemaVersion: 1,
  release,
  step: stepId,
  requirements: ['EXT-030', 'EXT-032'],
  phase: 'HOME_INVENTORY_UTILITY_BELONGINGS_CHECKPOINT_PACKAGE',
  status: 'PASS',
  sourceCommit,
  ...
  payload,
  validation: { boundary: '51/51', contract: '15/15', runtime: '11/11', targetedTests: '37/37', fullTests: '968/968', builds: '18/18' },
...
assert(JSON.stringify(localBaseActual) === JSON.stringify(expectedBase), 'Local 33-F checkpoint base set is not exact');
...
assert(baseReadback.every((item) => item.status === 'PASS'), 'D: 33-F base package readback mismatch');
...
const receipt = await writePair(paths.receipt, {
  schemaVersion: 1, release, step: stepId, requirements: ['EXT-030', 'EXT-032'], status: 'PASS',
  ...
  libraryReadbackVerification: readback, dataSource: 'manual', smartMeterLookup: 'not_performed',
  providerContact: 'not_performed', warrantyLookup: 'not_performed', ocr: 'not_performed',
  paymentExecution: 'not_performed', documentContentExposure: 'not_performed',
  networkEgressAdded: false, nextOfficialStep: '33-G', newBuildIssued: false,
  currentAuthoritativeSourceExternalProtectionStatus: 'SEPARATE_GOV_005_REFRESH_REQUIRED_AFTER_SOURCE_CHANGE',
...
assert(receiptArtifacts.every((item) => item.status === 'PASS'), 'D: 33-F receipt readback mismatch');
...
assert(persistenceArtifacts.every((item) => item.status === 'PASS'), 'D: 33-F receipt persistence mismatch');
...
assert(JSON.stringify(actualBeforeInventory) === JSON.stringify(expectedBeforeInventory), 'D: 33-F pre-inventory set is not exact');
...
assert(JSON.stringify(baseFinalActual) === JSON.stringify(baseFinalExpected), 'D: 33-F base final inventory set is not exact');
const finalInventoryArtifacts = await compare(root, libraryRoot, [paths.finalInventory, `${paths.finalInventory}.sha256`]);
assert(finalInventoryArtifacts.every((item) => item.status === 'PASS'), 'D: 33-F final inventory pair readback mismatch');
...
const completion = await writePair(paths.completion, {
  ...
  evidence: [...supplementPairs, finalInventory], dataSource: 'manual', smartMeterLookup: 'not_performed',
  providerContact: 'not_performed', warrantyLookup: 'not_performed', ocr: 'not_performed',
  paymentExecution: 'not_performed', documentContentExposure: 'not_performed',
  networkEgressAdded: false, nextOfficialStep: '33-G', newBuildIssued: false,
  currentAuthoritativeSourceExternalProtectionStatus: 'SEPARATE_GOV_005_REFRESH_REQUIRED_AFTER_SOURCE_CHANGE',
...
assert(step, '33-F work step is missing');
Object.assign(step, {
  status: 'COMPLETED', validationStatus: 'PASS', persistentReceiptStatus: 'PASS',
  persistentReceiptPath: paths.receipt, completionTransitionStatus: 'PASS'
});
...
plan.segmentationNote = '33-F is immutable COMPLETED/PASS with exact D: USB hash/size readback. 33-G remains PENDING until the next governed multi-requirement slice is selected.';

const ledger = await readJson(paths.ledger);
ledger.libraryUploadStatus = '33-F_COMPLETED_RECEIPT_PASS';
ledger.nextOfficialTask = 'AUTO_PRIORITY_SELECTION_AFTER_33-F_PERSISTENT_RECEIPT';
ledger.activeMicroStep = null;
ledger.externalLibraryAuthority33F = {
...
const transitionChecks = [
  ...
  ['ledger complete', ledger.libraryUploadStatus === '33-F_COMPLETED_RECEIPT_PASS' && ledger.activeMicroStep === null],
  ['manual-only truth', receipt && scope.truth.dataSource === 'manual' && scope.truth.networkEgressAdded === false],
  ['next step pending', plan.steps.find((item) => item.id === '33-G')?.status === 'PENDING']
].map(([name, passed]) => ({ name, status: passed ? 'PASS' : 'FAIL' }));
assert(transitionChecks.every((item) => item.status === 'PASS'), '33-F completion transition failed');
...
assert(closureArtifacts.every((item) => item.status === 'PASS'), 'D: 33-F completion/transition pair readback mismatch');
...
assert(JSON.stringify(actualBeforeClosureInventory) === JSON.stringify(expectedBeforeClosureInventory), 'D: 33-F pre-closure inventory set is not exact');
...
assert(closureInventoryArtifacts.every((item) => item.status === 'PASS'), 'D: 33-F closure inventory pair readback mismatch');
...
assert(JSON.stringify(closureFinalActual) === JSON.stringify(closureFinalExpected), 'D: 33-F closure inventory set is not exact');
const localClosureFinalActual = await listFiles(localPackageRoot);
assert(JSON.stringify(localClosureFinalActual) === JSON.stringify(closureFinalExpected), 'Local 33-F closure inventory set is not exact');
const localClosureReadback = await compare(localPackageRoot, libraryRoot, closureFinalExpected);
assert(localClosureReadback.every((item) => item.status === 'PASS'), 'Local and D: 33-F closure packages differ');

await writeGovernanceJsonAtomic(paths.ledger, ledger);
await writeGovernanceJsonAtomic(paths.plan, plan);

console.log(`33-F external receipt finalized: PASS (${closureFinalExpected.length} exact D: files; source ${sourceCommit}).`);
