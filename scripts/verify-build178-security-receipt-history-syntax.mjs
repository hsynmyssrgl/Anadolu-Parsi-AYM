...
const out = process.argv[2] ?? 'artifacts/validation/build178-security-receipt-history-syntax.json';
const checks = [];
...
for (const [name, path, kind] of [
  ['receipt archive TypeScript syntax', 'apps/desktop/src/main/security-event-receipt-store.ts', ts.ScriptKind.TS],
  ['receipt signer TypeScript syntax', 'apps/desktop/src/main/security-event-receipt.ts', ts.ScriptKind.TS],
  ['data store TypeScript syntax', 'apps/desktop/src/main/data-store.ts', ts.ScriptKind.TS],
  ['preload TypeScript syntax', 'apps/desktop/src/main/preload.ts', ts.ScriptKind.TS],
  ['renderer TSX syntax', 'apps/desktop/src/renderer/App.tsx', ts.ScriptKind.TSX],
  ['renderer declaration syntax', 'apps/desktop/src/renderer/global.d.ts', ts.ScriptKind.TS]
]) {
...
const store = await readFile('apps/desktop/src/main/security-event-receipt-store.ts', 'utf8');
check('receipt archive has no renderer dependency', !store.includes('/renderer/') && !store.includes('window.'));
check('receipt archive has no Electron dependency', !store.includes("from 'electron'"));
check('receipt archive contains no credential fields', !/(password|totpSecret|recoveryCode|sessionToken)/i.test(store));
...
console.log(`Build 178 security receipt history syntax: PASS (${checks.length}/${checks.length})`);
