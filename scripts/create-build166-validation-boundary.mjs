...
const reportPath=resolve(option('--report','artifacts/validation/build166-validation-boundary.json'));
...
const [sourcePreflight,sourceIntegrity,contract,runtime,syntax,packageSourceTypes,desktopMainTypes,requestCreation,responseStatus]=await Promise.all([
 readJson('artifacts/validation/build166-source-preflight-final.json'),readJson('artifacts/validation/build166-source-integrity-final.json'),readJson('artifacts/validation/build166-ipc-adaptive-budget-operator-contract.json'),readJson('artifacts/validation/build166-ipc-adaptive-budget-operator-runtime.json'),readJson('artifacts/validation/build166-ipc-adaptive-budget-operator-syntax.json'),readJson('artifacts/validation/package-source-typecheck.json'),readJson('artifacts/validation/desktop-main-source-typecheck.json'),readJson('artifacts/validation/build154-handoff-request-creation.json'),readJson('artifacts/validation/build154-handoff-response-status.json')
]);
if(sourcePreflight.status!=='PASS')throw new Error('Build 166 source preflight must be PASS.');
if(sourceIntegrity.status!=='PASS')throw new Error('Build 166 source integrity must be PASS.');
if(contract.status!=='PASS'||contract.assertions!==46)throw new Error('Build 166 operator contract must be PASS 46/46.');
if(runtime.status!=='PASS'||runtime.checks!==26)throw new Error('Build 166 operator runtime must be PASS 26/26.');
if(syntax.status!=='PASS'||syntax.fileCount!==8)throw new Error('Build 166 operator syntax must be PASS 8/8.');
...
const results=[
 {id:'source-preflight',status:'PASS',evidence:'artifacts/validation/build166-source-preflight-final.json'},
 {id:'source-integrity',status:'PASS',evidence:'artifacts/validation/build166-source-integrity-final.json'},
