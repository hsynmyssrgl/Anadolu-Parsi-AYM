...
if ($build -ne 226) { throw "Unified Bronze Windows security lifecycle requires Build226; current build=$build." }
...
if (-not $EvidencePath) { $EvidencePath = Join-Path $validationRoot "build226-bronze-security-windows-release-lifecycle.json" }
...
function Invoke-RecordedProcess {
  ...
  try {
    ...
    $stdoutPath = Join-Path $validationRoot "build226-$safeId-full-stdout.log"
    $stderrPath = Join-Path $validationRoot "build226-$safeId-full-stderr.log"
