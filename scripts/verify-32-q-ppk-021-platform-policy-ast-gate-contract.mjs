...
check('manifest contains exactly 531 surfaces', keys.length === 531 && uniqueKeys.size === 531);
...
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 531 && gate.exactAllowlistEntries === 531);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 531']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 531', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(531)']));
...
check('scope records six rules and 531 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.exactPrivilegedSurfaceCount === 531);
