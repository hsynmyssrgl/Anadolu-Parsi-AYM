...
check('manifest contains exactly 730 surfaces', keys.length === 730 && uniqueKeys.size === 730);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 474);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 730 && gate.exactAllowlistEntries === 730);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 730']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 730', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(730)', 'inventory.files).toBe(474)']));
...
check('scope records six rules and 730 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 474 && scope.boundaries?.exactPrivilegedSurfaceCount === 730);
...
check('decision pins the current family-AI-expanded source ratchet without rewriting historical closure', includesAll(decision, ['474 dosya / 730 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('474 dosya / 730 exact yüzey') && masterRegister.includes(manifestSha256));
