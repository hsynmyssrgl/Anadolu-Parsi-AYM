...
check('manifest contains exactly 851 surfaces', keys.length === 851 && uniqueKeys.size === 851);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 532);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 851 && gate.exactAllowlistEntries === 851);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 851']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 851', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(851)', 'inventory.files).toBe(532)']));
...
check('scope records six rules and 851 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 532 && scope.boundaries?.exactPrivilegedSurfaceCount === 851);
...
check('decision pins the current family-meeting-expanded source ratchet without rewriting historical closure', includesAll(decision, ['532 dosya / 851 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('532 dosya / 851 exact yüzey') && masterRegister.includes(manifestSha256));
