...
check('manifest contains exactly 707 surfaces', keys.length === 707 && uniqueKeys.size === 707);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 456);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 707 && gate.exactAllowlistEntries === 707);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 707']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 707', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(707)', 'inventory.files).toBe(456)']));
...
check('scope records six rules and 707 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 456 && scope.boundaries?.exactPrivilegedSurfaceCount === 707);
...
check('decision pins the current household-operations-expanded source ratchet without rewriting historical closure', includesAll(decision, ['456 dosya / 707 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('456 dosya / 707 exact yüzey') && masterRegister.includes(manifestSha256));
