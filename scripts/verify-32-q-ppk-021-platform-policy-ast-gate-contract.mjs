...
check('manifest contains exactly 723 surfaces', keys.length === 723 && uniqueKeys.size === 723);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 468);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 723 && gate.exactAllowlistEntries === 723);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 723']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 723', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(723)', 'inventory.files).toBe(468)']));
...
check('scope records six rules and 723 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 468 && scope.boundaries?.exactPrivilegedSurfaceCount === 723);
...
check('decision pins the current places-travel-expanded source ratchet without rewriting historical closure', includesAll(decision, ['468 dosya / 723 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('468 dosya / 723 exact yüzey') && masterRegister.includes(manifestSha256));
