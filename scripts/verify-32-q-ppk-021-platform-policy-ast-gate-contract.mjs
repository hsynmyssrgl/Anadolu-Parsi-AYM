...
check('manifest contains exactly 715 surfaces', keys.length === 715 && uniqueKeys.size === 715);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 462);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 715 && gate.exactAllowlistEntries === 715);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 715']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 715', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(715)', 'inventory.files).toBe(462)']));
...
check('scope records six rules and 715 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 462 && scope.boundaries?.exactPrivilegedSurfaceCount === 715);
...
check('decision pins the current child-education-expanded source ratchet without rewriting historical closure', includesAll(decision, ['462 dosya / 715 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('462 dosya / 715 exact yüzey') && masterRegister.includes(manifestSha256));
