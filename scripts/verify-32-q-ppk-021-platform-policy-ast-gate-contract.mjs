...
check('manifest contains exactly 692 surfaces', keys.length === 692 && uniqueKeys.size === 692);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 447);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 692 && gate.exactAllowlistEntries === 692);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 692']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 692', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(692)', 'inventory.files).toBe(447)']));
...
check('scope records six rules and 692 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 447 && scope.boundaries?.exactPrivilegedSurfaceCount === 692);
...
check('decision pins the current archive-evidence-media-search-expanded source ratchet without rewriting historical closure', includesAll(decision, ['447 dosya / 692 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('447 dosya / 692 exact yüzey') && masterRegister.includes(manifestSha256));
