...
check('manifest contains exactly 876 surfaces', keys.length === 876 && uniqueKeys.size === 876);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 556);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 876 && gate.exactAllowlistEntries === 876);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 876']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 876', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(876)', 'inventory.files).toBe(556)']));
...
check('scope records six rules and 876 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 556 && scope.boundaries?.exactPrivilegedSurfaceCount === 876);
...
check('decision pins the current local media preflight source ratchet without rewriting historical closure', includesAll(decision, ['556 dosya / 876 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('556 dosya / 876 exact yüzey') && masterRegister.includes(manifestSha256));
