...
check('manifest contains exactly 805 surfaces', keys.length === 805 && uniqueKeys.size === 805);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 513);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 805 && gate.exactAllowlistEntries === 805);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 805']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 805', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(805)', 'inventory.files).toBe(513)']));
...
check('scope records six rules and 805 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 513 && scope.boundaries?.exactPrivilegedSurfaceCount === 805);
...
check('decision pins the current realtime-calling-expanded source ratchet without rewriting historical closure', includesAll(decision, ['513 dosya / 805 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('513 dosya / 805 exact yüzey') && masterRegister.includes(manifestSha256));
