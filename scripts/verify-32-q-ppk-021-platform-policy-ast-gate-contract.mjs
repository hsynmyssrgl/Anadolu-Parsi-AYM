...
check('manifest contains exactly 795 surfaces', keys.length === 795 && uniqueKeys.size === 795);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 507);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 795 && gate.exactAllowlistEntries === 795);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 795']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 795', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(795)', 'inventory.files).toBe(507)']));
...
check('scope records six rules and 795 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 507 && scope.boundaries?.exactPrivilegedSurfaceCount === 795);
...
check('decision pins the current messaging-lifecycle-expanded source ratchet without rewriting historical closure', includesAll(decision, ['507 dosya / 795 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('507 dosya / 795 exact yüzey') && masterRegister.includes(manifestSha256));
