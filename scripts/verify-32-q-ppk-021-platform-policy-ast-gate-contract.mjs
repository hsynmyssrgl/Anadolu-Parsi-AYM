...
check('manifest contains exactly 858 surfaces', keys.length === 858 && uniqueKeys.size === 858);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 552);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 858 && gate.exactAllowlistEntries === 858);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 858']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 858', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(858)', 'inventory.files).toBe(552)']));
...
check('scope records six rules and 858 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 552 && scope.boundaries?.exactPrivilegedSurfaceCount === 858);
...
check('decision pins the current remaining-package-expanded source ratchet without rewriting historical closure', includesAll(decision, ['552 dosya / 858 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('552 dosya / 858 exact yüzey') && masterRegister.includes(manifestSha256));
