...
check('manifest contains exactly 886 surfaces', keys.length === 886 && uniqueKeys.size === 886);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 563);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 886 && gate.exactAllowlistEntries === 886);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 886']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 886', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(886)', 'inventory.files).toBe(563)']));
...
check('scope records six rules and 886 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 563 && scope.boundaries?.exactPrivilegedSurfaceCount === 886);
...
check('decision pins the current local source ratchet without rewriting historical closure', includesAll(decision, ['563 dosya / 886 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('563 dosya / 886 exact yüzey') && masterRegister.includes(manifestSha256));
