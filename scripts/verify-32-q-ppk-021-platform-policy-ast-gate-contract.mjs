...
check('manifest contains exactly 763 surfaces', keys.length === 763 && uniqueKeys.size === 763);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 493);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 763 && gate.exactAllowlistEntries === 763);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 763']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 763', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(763)', 'inventory.files).toBe(493)']));
...
check('scope records six rules and 763 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 493 && scope.boundaries?.exactPrivilegedSurfaceCount === 763);
...
check('decision pins the current signed-plugin-expanded source ratchet without rewriting historical closure', includesAll(decision, ['493 dosya / 763 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('493 dosya / 763 exact yüzey') && masterRegister.includes(manifestSha256));
