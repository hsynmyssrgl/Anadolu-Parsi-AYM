...
check('manifest contains exactly 699 surfaces', keys.length === 699 && uniqueKeys.size === 699);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 450);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 699 && gate.exactAllowlistEntries === 699);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 699']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 699', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(699)', 'inventory.files).toBe(450)']));
...
check('scope records six rules and 699 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 450 && scope.boundaries?.exactPrivilegedSurfaceCount === 699);
...
check('decision pins the current health-care-coordination-expanded source ratchet without rewriting historical closure', includesAll(decision, ['450 dosya / 699 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('450 dosya / 699 exact yüzey') && masterRegister.includes(manifestSha256));
