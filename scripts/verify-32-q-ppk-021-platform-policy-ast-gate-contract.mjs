...
check('manifest contains exactly 779 surfaces', keys.length === 779 && uniqueKeys.size === 779);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 500);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 779 && gate.exactAllowlistEntries === 779);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 779']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 779', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(779)', 'inventory.files).toBe(500)']));
...
check('scope records six rules and 779 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 500 && scope.boundaries?.exactPrivilegedSurfaceCount === 779);
...
check('decision pins the current communication-foundation-expanded source ratchet without rewriting historical closure', includesAll(decision, ['500 dosya / 779 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('500 dosya / 779 exact yüzey') && masterRegister.includes(manifestSha256));
