...
check('manifest contains exactly 685 surfaces', keys.length === 685 && uniqueKeys.size === 685);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 442);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 685 && gate.exactAllowlistEntries === 685);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 685']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 685', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(685)', 'inventory.files).toBe(442)']));
...
check('scope records six rules and 685 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 442 && scope.boundaries?.exactPrivilegedSurfaceCount === 685);
...
check('decision pins the current OCR-search-expanded source ratchet without rewriting historical closure', includesAll(decision, ['442 dosya / 685 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('442 dosya / 685 exact yüzey') && masterRegister.includes(manifestSha256));
