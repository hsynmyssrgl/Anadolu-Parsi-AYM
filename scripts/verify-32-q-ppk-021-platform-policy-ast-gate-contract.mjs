...
check('manifest contains exactly 682 surfaces', keys.length === 682 && uniqueKeys.size === 682);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 441);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 682 && gate.exactAllowlistEntries === 682);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 682']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 682', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(682)', 'inventory.files).toBe(441)']));
...
check('scope records six rules and 682 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 441 && scope.boundaries?.exactPrivilegedSurfaceCount === 682);
...
check('decision pins the current OCR-expanded source ratchet without rewriting historical closure', includesAll(decision, ['441 dosya / 682 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('441 dosya / 682 exact yüzey') && masterRegister.includes(manifestSha256));
