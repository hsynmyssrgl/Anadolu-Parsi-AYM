...
check('manifest contains exactly 874 surfaces', keys.length === 874 && uniqueKeys.size === 874);
...
check('gate scans the exact current production snapshot', gate.productionSourceZones === 18 && gate.scannedFiles === 555);
check('gate exact surface and allowance cardinality match', gate.privilegedSurfaces === 874 && gate.exactAllowlistEntries === 874);
...
check('policy snapshot is exact fail closed', includesAll(policy, ["syntaxModel: 'TYPESCRIPT_AST'", "enforcement: 'fail-closed'", "defaultDecision: 'DENY'", 'exactAllowlistEntries: 874']));
...
check('domain boundary is content free and fixed', includesAll(domain, ['PlatformPolicyAstGateBoundaryView', 'protectedRuleCount: 6', 'exactAllowlistEntries: 874', 'sourcePathsExposedToClient: false', 'allowlistHashExposedToClient: false']));
...
check('AST test verifies full production manifest exactness', includesAll(astTest, ['inventoryPlatformPolicyAstSurfaces()', 'result.findings).toEqual([])', 'result.allowedCount).toBe(874)', 'inventory.files).toBe(555)']));
...
check('scope records six rules and 874 exact surfaces', scope.boundaries?.protectedRuleCount === 6 && scope.protectedRules?.length === 6 && scope.boundaries?.scannedProductionFiles === 555 && scope.boundaries?.exactPrivilegedSurfaceCount === 874);
...
check('decision pins the current messaging maintenance source ratchet without rewriting historical closure', includesAll(decision, ['555 dosya / 874 exact yüzey', manifestSha256, 'tarihsel kapanış kanıtını yeniden yazmadan']));
...
check('master register contains DEC-202 and current exact source ratchet', masterRegister.includes('## DEC-202') && masterRegister.includes('DEC-202-ppk-021-platform-policy-ast-fail-gate.md') && masterRegister.includes('555 dosya / 874 exact yüzey') && masterRegister.includes(manifestSha256));
