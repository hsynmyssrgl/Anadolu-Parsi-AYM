...
const paths = {
  ...
  platformPolicyTransactionRepository: 'packages/repositories/src/platform-policy-transaction-repository.ts',
...
check('policy classifies two semantic owners one current OCR metadata owner three ledgers and two content-free control tables', [
...
check('data lifecycle schema inspection registers exact OCR metadata classes without registering semantic payload fields', ['SOURCE_DELETION_REGISTERED_SEMANTIC_OWNERS', 'SOURCE_DELETION_CURRENT_METADATA_OWNERS', 'SOURCE_DELETION_METADATA_ONLY_MUTATION_LEDGERS', 'SOURCE_DELETION_CONTENT_FREE_METADATA_TABLES'].every((marker) => files.repository.includes(marker)) && files.repository.includes('REGISTERED_DERIVED_PAYLOAD_METADATA_TABLES') && files.migrations.includes('CREATE TABLE local_governed_ocr_source_deletion_recovery_intents') && files.migrations.includes('CREATE TRIGGER trg_33q_source_deletion_recovery_update') && files.migrations.includes('CREATE TRIGGER trg_33q_source_deletion_recovery_delete') && files.platformPolicyTransactionRepository.includes('INSERT INTO local_governed_ocr_source_deletion_recovery_intents') && files.platformPolicyTransactionRepository.includes('listRecoverableArchiveSecureDestroyOperations'));
