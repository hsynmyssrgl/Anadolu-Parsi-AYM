...
const step = '31-P';
const localPackageRoot = 'C:\\PPT\\AYM\\09_ARSIV\\KAYNAK_AGACI\\checkpoints\\31-P_Synthetic_Rollback_Recovery_Drill';
const libraryRoot = 'D:\\AYM_LIBRARY\\Panthera pardus tulliana\\Anadolu Parsı Aile Yaşam Merkezi\\Bronze 04.08.2026.29\\checkpoints\\31-P_Synthetic_Rollback_Recovery_Drill';
const truth = 'Bu teslim yalnız 31-P sentetik ve gönderilemez geri alma/kurtarma tatbikatını kanıtlar; gerçek ROLLBACK_DRILL, süreç çökmesi, yedek geri yükleme, runtime bağlantısı, gerçek veri veya cutover PASS değildir.';
const payloadPaths = Object.freeze([
  'package.json', 'docs/10_MASTER_DECISION_REGISTER.md', 'docs/current/09_KULLANICI_KARARLARI_KAYDI.md',
  ...
  'docs/decisions/DEC-175-synthetic-single-writer-proof-harness-boundary.md',
  'docs/decisions/DEC-176-synthetic-key-lifecycle-proof-harness-boundary.md',
  'docs/decisions/DEC-177-synthetic-rollback-recovery-drill-boundary.md',
  'docs/audit/31-P_SYNTHETIC_ROLLBACK_RECOVERY_DRILL.md',
  'config/work-segmentation-plan.json', 'config/active-governance-ledger.json', 'config/accepted-scope-registry.json',
  'config/user-decision-ledger.json', 'config/31-p-synthetic-rollback-recovery-drill-scope.json',
  'artifacts/authority/31-P_SYNTHETIC_ROLLBACK_RECOVERY_DRILL_AUTHORITY.json',
  'artifacts/validation/31-P_PRIORITY_SELECTION_VALIDATION.json',
  'artifacts/checkpoints/31-P_EXECUTION_RECORD.json', 'artifacts/checkpoints/31-P_INITIAL_VALIDATION_FAILURES.json',
  'artifacts/inventory/31-P_SCOPE_AND_STATUS_REPORT.json',
  'artifacts/validation/31-P_SYNTHETIC_ROLLBACK_RECOVERY_DRILL_CONTRACT.json',
  'artifacts/validation/31-P_ROOT_TYPESCRIPT.json', 'artifacts/validation/31-P_TARGETED_VITEST.json',
  'artifacts/validation/31-P_CORE_SERVICE_RUNTIME.json', 'artifacts/validation/31-P_SECURITY_AND_PREDECESSOR_REGRESSION.json',
  'artifacts/validation/31-P_FULL_VITEST_REGRESSION.json', 'artifacts/validation/31-P_PRODUCTION_BUILD.json',
  'artifacts/validation/platform-policy-gate.json',
  'apps/core-service/src/synthetic-rollback-recovery-drill.ts',
  'apps/core-service/src/synthetic-key-lifecycle-proof-harness.ts',
  'apps/core-service/src/synthetic-single-writer-proof-harness.ts',
  'apps/core-service/src/family-data-cutover-readiness-ledger.ts', 'apps/core-service/src/core-service-runtime.ts',
  ...
  'packages/core-service-contracts/src/index.ts', 'scripts/lib/authorized-successor-lifecycle.mjs',
  'scripts/start-31-p-synthetic-rollback-recovery-drill.mjs',
  'scripts/verify-31-p-synthetic-rollback-recovery-drill-contract.mjs',
  'scripts/run-31-p-synthetic-rollback-recovery-drill-local-validation.mjs',
  'scripts/finalize-31-p-synthetic-rollback-recovery-drill-external-library-receipt.mjs',
  'scripts/verify-31-p-synthetic-rollback-recovery-drill-completion-transition.mjs',
  'scripts/verify-31-o-synthetic-key-lifecycle-proof-harness-contract.mjs',
  'scripts/verify-31-o-synthetic-key-lifecycle-proof-harness-completion-transition.mjs'
]);
const paths = {
  receipt: 'artifacts/checkpoints/31-P_LIBRARY_RECEIPT.json', readback: 'artifacts/validation/31-P_LIBRARY_READBACK_VERIFICATION.json',
  receiptReadback: 'artifacts/validation/31-P_RECEIPT_READBACK_VERIFICATION.json', persistence: 'artifacts/validation/31-P_RECEIPT_READBACK_PERSISTENCE_VERIFICATION.json',
  inventory: 'artifacts/validation/31-P_LIBRARY_FINAL_INVENTORY_VERIFICATION.json', completion: 'artifacts/checkpoints/31-P_COMPLETION_RECORD.json',
  transition: 'artifacts/validation/31-P_COMPLETION_TRANSITION_VALIDATION.json', execution: 'artifacts/checkpoints/31-P_EXECUTION_RECORD.json',
  scope: 'artifacts/inventory/31-P_SCOPE_AND_STATUS_REPORT.json', scopeConfig: 'config/31-p-synthetic-rollback-recovery-drill-scope.json',
  plan: 'config/work-segmentation-plan.json', ledger: 'config/active-governance-ledger.json'
};
const validationPaths = [
  'artifacts/validation/31-P_PRIORITY_SELECTION_VALIDATION.json', 'artifacts/validation/31-P_SYNTHETIC_ROLLBACK_RECOVERY_DRILL_CONTRACT.json',
  'artifacts/validation/31-P_ROOT_TYPESCRIPT.json', 'artifacts/validation/31-P_TARGETED_VITEST.json',
  'artifacts/validation/31-P_CORE_SERVICE_RUNTIME.json', 'artifacts/validation/31-P_SECURITY_AND_PREDECESSOR_REGRESSION.json',
  'artifacts/validation/31-P_FULL_VITEST_REGRESSION.json', 'artifacts/validation/31-P_PRODUCTION_BUILD.json',
  'artifacts/validation/platform-policy-gate.json'
...
const writePair = async (path, value) => {
  ...
  await writeBytes(full(path), bytes); await writeBytes(full(`${path}.sha256`), Buffer.from(`${digest}  ${basename(path)}\n`, 'ascii'));
...
const [planBefore, executionBefore, scopeBefore, ...validations] = await Promise.all([
  readJson(full(paths.plan)), readJson(full(paths.execution)), readJson(full(paths.scopeConfig)),
...
assert(localPass || recoverable, '31-P is neither local PASS awaiting receipt nor a recoverable finalization');
assert(validations.every((item) => item.status === 'PASS'), '31-P validation is not clean PASS');
...
const manifest = {
  schemaVersion: 1, release, step, primaryRequirement: 'DHA-001', phase: 'SYNTHETIC_ROLLBACK_RECOVERY_DRILL_CHECKPOINT_PACKAGE',
  status: 'PASS', payloadCount: payload.length, payload, validation: executionBefore.validation, requirements: executionBefore.requirements,
  requirementCompletionClaimed: false, persistentReceiptStatus: 'PENDING', officialCompletionClaimed: false,
  newBuildIssued: false, createdAt: new Date().toISOString(), mandatoryTruthSentence: truth
};
const manifestName = '31-P_CHECKPOINT_MANIFEST.json';
...
const receipt = await writePair(paths.receipt, {
  schemaVersion: 1, release, step, primaryRequirement: 'DHA-001', phase: 'SYNTHETIC_ROLLBACK_RECOVERY_DRILL_EXTERNAL_LIBRARY_RECEIPT',
  ...
  openBoundaries: scopeBefore.openBoundaries, nextOfficialStep: 'AUTO_PRIORITY_SELECTION_AFTER_31-P_PERSISTENT_RECEIPT',
...
const completion = {
  ...
  nextOfficialStep: 'AUTO_PRIORITY_SELECTION_AFTER_31-P_PERSISTENT_RECEIPT',
  currentAuthoritativeSourceExternalProtectionStatus: 'SEPARATE_GOV_005_REFRESH_REQUIRED_AFTER_SOURCE_CHANGE',
  newBuildIssued: false, completedAt, mandatoryTruthSentence: truth
};
await writeJson(full(paths.completion), completion);
const plan = await readJson(full(paths.plan)); const step31P = plan.steps.find((item) => item.id === step);
Object.assign(step31P, { status: 'COMPLETED', validationStatus: 'PASS', persistentReceiptStatus: 'PASS', persistentReceiptPath: paths.receipt, completionTransitionStatus: 'PASS' });
for (const path of [paths.receipt, paths.readback, paths.receiptReadback, paths.persistence, paths.inventory, paths.completion, paths.transition]) if (!step31P.localEvidence.includes(path)) step31P.localEvidence.push(path);
plan.updatedAt = completedAt;
plan.segmentationNote = '31-P synthetic rollback/recovery drill is immutable COMPLETED/PASS with exact D: USB readback. It remains non-submittable and detached; real ROLLBACK_DRILL, process crash, backup restore, DEC-171 cutover, real data access, and SQLite ownership transfer remain blocked.';
await writeJson(full(paths.plan), plan);
const ledger = await readJson(full(paths.ledger));
ledger.libraryUploadStatus = '31-P_COMPLETED_RECEIPT_PASS'; ledger.nextOfficialTask = 'AUTO_PRIORITY_SELECTION_AFTER_31-P_PERSISTENT_RECEIPT'; ledger.activeMicroStep = null;
ledger.externalLibraryAuthority31P = { step, status: 'PASS', storageBackend: 'EXTERNAL_USB_D_DRIVE', path: libraryRoot, receipt: paths.receipt, focusedCheckpointOnly: true };
ledger.updatedAt = completedAt; await writeJson(full(paths.ledger), ledger);
...
const checks = [
  ...
  ['work plan', step31P.status === 'COMPLETED' && step31P.persistentReceiptStatus === 'PASS'],
  ['governance', ledger.libraryUploadStatus === '31-P_COMPLETED_RECEIPT_PASS' && ledger.activeMicroStep === null],
  ['synthetic-only boundary', scopeConfig.targets.stateMachine === 'PURE_SYNTHETIC_ROLLBACK_RECOVERY_ONLY' && scopeConfig.targets.candidateClassification === 'MODELED_GATE_ONLY_NO_GATE_ID_NON_SUBMITTABLE'],
  ['Desktop writer boundary', scopeConfig.targets.writerInvariant === 'DESKTOP_ONLY_CORE_SERVICE_NEVER_WRITABLE'],
  ['real crash and restore excluded', scopeConfig.targets.failureMode === 'SYNTHETIC_EVENT_ONLY_NO_PROCESS_CRASH' && scopeConfig.targets.recoveryMode === 'MODELED_ONLY_NO_BACKUP_RESTORE'],
  ['runtime and real gate detached', scopeConfig.targets.runtimeIntegration === 'NOT_WIRED' && scopeConfig.targets.realRollbackDrillGate === 'NOT_SATISFIED'],
  ['DEC-171 and SQLite boundary', scopeConfig.openBoundaries.realVaultTransfer === 'NOT_PERFORMED_BLOCKED' && scopeConfig.openBoundaries.sqliteOwnershipTransfer === 'NOT_PERFORMED_BLOCKED'],
  ['Build boundary', completion.newBuildIssued === false]
].map(([name, passed]) => ({ name, status: passed ? 'PASS' : 'FAIL' }));
assert(checks.every((item) => item.status === 'PASS'), '31-P transition failed');
...
console.log(`31-P external USB Library receipt: PASS (${expectedBase.length}/${expectedBase.length} base files; ${finalActual.length}/${finalExpected.length} final files).`);
console.log(`Library: ${libraryRoot}`);
console.log('31-P official step status: COMPLETED; synthetic-only rollback drill: DETACHED/NON-SUBMITTABLE; DEC-171 cutover: BLOCKED; new Build: false.');
