...
if ($build -ne 223) { throw "Unified Bronze Windows closure requires Build223; current build=$build." }
...
try {
  ...
  Invoke-RequiredStep -Id "exact-source-integrity" -Operation {
    & node.exe "scripts\verify-source-integrity.mjs" "--report" "artifacts\validation\build223-bronze-security-source-integrity-windows.json"
  }
  ...
  Invoke-RequiredStep -Id "workspace-package-dist-guard" -Operation {
    & node.exe "scripts\verify-build223-workspace-dist-prerequisite.mjs"
  }

  $lifecycleArgs=@("-NoProfile","-ExecutionPolicy","Bypass","-File","scripts\windows-bronze-security-release-validation-build223.ps1","-EvidencePath","artifacts\validation\build223-bronze-security-windows-release-lifecycle.json")
  ...
  & node.exe "scripts\verify-build223-bronze-security-windows-result.mjs" $applicationVersion `
    "artifacts\validation\build223-bronze-security-windows-release-lifecycle.json" `
    ...
    "artifacts\validation\build223-bronze-security-source-integrity-windows.json" `
    "artifacts\validation\build223-bronze-security-windows-closure-result.json"
...
$resultPath=Join-Path $validationRoot "build223-bronze-security-windows-closure-result.json"
$result=$null
...
$summaryPath=Join-Path $validationRoot "build223-bronze-security-windows-run-summary.json"
$summary=[ordered]@{
...
if((Test-Path -LiteralPath $sourceManifestPath -PathType Leaf)-and(Test-Path -LiteralPath $sourceShaSumsPath -PathType Leaf)){
  $evidencePaths=@(
    $summaryPath,
    (Join-Path $validationRoot "build223-bronze-security-source-integrity-windows.json"),
    (Join-Path $validationRoot "build223-bronze-security-windows-release-lifecycle.json"),
  ...
  $manifestPath=Join-Path $validationRoot "build223-bronze-security-windows-evidence-manifest.json"
  ...
  $bundlePath=Join-Path $validationRoot "Bronze_Guvenlik_Windows_Kanitlari_Build223_$runId.zip"
