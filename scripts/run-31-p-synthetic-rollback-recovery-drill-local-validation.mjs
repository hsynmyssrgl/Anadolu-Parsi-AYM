...
const paths = {
  ...
  execution: 'artifacts/checkpoints/31-P_EXECUTION_RECORD.json', failures: 'artifacts/checkpoints/31-P_INITIAL_VALIDATION_FAILURES.json',
  scopeConfig: 'config/31-p-synthetic-rollback-recovery-drill-scope.json',
  contract: 'artifacts/validation/31-P_SYNTHETIC_ROLLBACK_RECOVERY_DRILL_CONTRACT.json',
  typecheck: 'artifacts/validation/31-P_ROOT_TYPESCRIPT.json', targeted: 'artifacts/validation/31-P_TARGETED_VITEST.json',
  runtime: 'artifacts/validation/31-P_CORE_SERVICE_RUNTIME.json', security: 'artifacts/validation/31-P_SECURITY_AND_PREDECESSOR_REGRESSION.json',
  regression: 'artifacts/validation/31-P_FULL_VITEST_REGRESSION.json', build: 'artifacts/validation/31-P_PRODUCTION_BUILD.json',
  platform: 'artifacts/validation/platform-policy-gate.json', scope: 'artifacts/inventory/31-P_SCOPE_AND_STATUS_REPORT.json',
...
const run = (name, args, cwd = root) => {
  ...
  const result = spawnSync(process.execPath, args, { cwd, encoding: 'utf8', env: { ...process.env, NO_COLOR: '1', FORCE_COLOR: '0' }, windowsHide: true, maxBuffer: 24 * 1024 * 1024 });
...
if (!step || plan.currentStep !== '31-P' || step.status !== 'IN_PROGRESS') throw new Error('31-P is not active');
...
if (!validationPending && !cleanRerun) throw new Error('31-P is not awaiting validation or an idempotent clean rerun');
if (initialFailures.countsAsPass !== false || !['NO_FAILED_ATTEMPTS', 'FAILED_ATTEMPTS_RETAINED_NOT_COUNTED_AS_PASS'].includes(initialFailures.status)) throw new Error('31-P failed attempts are not truthfully classified');
if (!initialFailures.failures.every((failure) => String(failure.rerunStatus).startsWith('PASS') || failure.rerunStatus === 'REMEDIATED_AWAITING_CLEAN_RERUN')) throw new Error('31-P initial failures do not have clean rerun evidence');

const contractRun = run('31-P contract', ['scripts/verify-31-p-synthetic-rollback-recovery-drill-contract.mjs']); pass(contractRun);
const contract = await readJson(paths.contract);
if (contract.status !== 'PASS' || contract.failed !== 0 || contract.passed !== contract.expected) throw new Error('31-P contract is not clean PASS');
const typecheck = run('Root TypeScript', ['node_modules/typescript/bin/tsc', '--noEmit']);
await writeJson(paths.typecheck, { schemaVersion: 1, release: plan.release, step: '31-P', phase: 'ROOT_TYPESCRIPT_NO_EMIT', ...publicResult(typecheck), diagnosticCount: typecheck.status === 'PASS' ? 0 : null, executedAt: new Date().toISOString() });
pass(typecheck);
...
await writeJson(paths.targeted, { schemaVersion: 1, release: plan.release, step: '31-P', phase: 'TARGETED_VITEST', ...publicResult(targeted), testFilePassCount: targetedFiles, testPassCount: targetedTests, testFileFailCount: targeted.status === 'PASS' ? 0 : null, testFailCount: targeted.status === 'PASS' ? 0 : null, executedAt: new Date().toISOString() });
pass(targeted);
...
await writeJson(paths.runtime, { schemaVersion: 1, release: plan.release, step: '31-P', phase: 'CORE_SERVICE_DETACHED_SYNTHETIC_ROLLBACK_RECOVERY_RUNTIME_GATES', status: 'PASS', expected: runtimeResults.length, executed: runtimeResults.length, passed: runtimeResults.length, failed: 0, checks: runtimeResults.map(publicResult), executedAt: new Date().toISOString() });
...
const securityCommands = [
  ...
  ['31-O key-lifecycle contract successor regression', 'scripts/verify-31-o-synthetic-key-lifecycle-proof-harness-contract.mjs', '--successor-regression'],
  ...
  ['31-O completion regression', 'scripts/verify-31-o-synthetic-key-lifecycle-proof-harness-completion-transition.mjs'],
  ...
  ['31-P contract verifier syntax', '--check', 'scripts/verify-31-p-synthetic-rollback-recovery-drill-contract.mjs'],
  ['31-P local validator syntax', '--check', 'scripts/run-31-p-synthetic-rollback-recovery-drill-local-validation.mjs'],
  ['31-P finalizer syntax', '--check', 'scripts/finalize-31-p-synthetic-rollback-recovery-drill-external-library-receipt.mjs'],
  ['31-P completion verifier syntax', '--check', 'scripts/verify-31-p-synthetic-rollback-recovery-drill-completion-transition.mjs']
];
const securityResults = securityCommands.map(([name, ...args]) => run(name, args)); securityResults.forEach(pass);
await writeJson(paths.security, { schemaVersion: 1, release: plan.release, step: '31-P', phase: 'SYNTHETIC_ROLLBACK_RECOVERY_SECURITY_AND_PREDECESSOR_REGRESSION', status: 'PASS', expected: securityResults.length, executed: securityResults.length, passed: securityResults.length, failed: 0, checks: securityResults.map(publicResult), executedAt: new Date().toISOString() });
...
await writeJson(paths.regression, { schemaVersion: 1, release: plan.release, step: '31-P', phase: 'FULL_VITEST_REGRESSION', ...publicResult(regression), testFilePassCount: fullFiles, testPassCount: fullTests, testFileFailCount: regression.status === 'PASS' ? 0 : null, testFailCount: regression.status === 'PASS' ? 0 : null, executedAt: new Date().toISOString() });
pass(regression);
...
plan.segmentationNote = `31-P synthetic rollback/recovery drill is LOCAL_PASS_AWAITING_LIBRARY_RECEIPT after ${contract.passed}/${contract.expected} contract, root TypeScript, ${packageBuilds.length}/${packageBuilds.length} affected builds, ${targetedTests}/${targetedTests} targeted tests, ${runtimeResults.length}/${runtimeResults.length} runtime gates, ${securityResults.length}/${securityResults.length} security/predecessor regressions, full Vitest ${fullTests}/${fullTests}, and production build PASS. The drill remains synthetic and non-submittable; no process crash, backup restore, real gate PASS, runtime wiring, real data, SQLite transfer, or cutover authority is attached.`;
await writeJson(paths.plan, plan);
ledger.libraryUploadStatus = '31-P_SYNTHETIC_ROLLBACK_RECOVERY_DRILL_LOCAL_PASS_AWAITING_LIBRARY_RECEIPT'; ledger.updatedAt = validatedAt; await writeJson(paths.ledger, ledger);
Object.assign(execution, { status: 'LOCAL_PASS_AWAITING_LIBRARY_RECEIPT', officialStepStatus: 'IN_PROGRESS_AWAITING_LIBRARY_RECEIPT', targetSliceStatus: 'PASS', validationStatus: 'PASS', persistentReceiptStatus: 'PENDING', officialCompletionClaimed: false, validation, evidence, requirementCompletionClaimed: false, newBuildIssued: false, validatedAt });
