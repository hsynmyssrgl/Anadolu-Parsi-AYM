...
const reportPath=resolve(option('--report','artifacts/validation/build211-validation-boundary.json'));
const readJson=async p=>JSON.parse(await readFile(p,'utf8'));
const preflight=await readJson('artifacts/validation/build211-source-preflight-final.json');
const integrity=await readJson('artifacts/validation/build211-source-integrity.json');
let clean={status:'NOT_RUN',classification:'NOT_RUN'};try{clean=await readJson('artifacts/validation/build211-clean-npm-ci.json')}catch{}
const results=[
...
const report={schemaVersion:1,product:'Anadolu Parsı Aile Yaşam Merkezi',build:211,stage:'Bronze RC2 Active Development',overallStatus:'INCOMPLETE',passCount,failCount,notRunCount,results,generatedAt:new Date().toISOString()};
await mkdir(dirname(reportPath),{recursive:true});await writeFile(reportPath,`${JSON.stringify(report,null,2)}\n`);console.log(`Build 211 validation boundary: INCOMPLETE — PASS ${passCount} / FAIL ${failCount} / NOT_RUN ${notRunCount}`);
