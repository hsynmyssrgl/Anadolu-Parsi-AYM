...
const step = '31-N';
const localPackageRoot = 'C:\\PPT\\AYM\\09_ARSIV\\KAYNAK_AGACI\\checkpoints\\31-N_Synthetic_Single_Writer_Proof_Harness';
const libraryRoot = 'D:\\AYM_LIBRARY\\Panthera pardus tulliana\\Anadolu Parsı Aile Yaşam Merkezi\\Bronze 04.08.2026.29\\checkpoints\\31-N_Synthetic_Single_Writer_Proof_Harness';
const truth = 'Bu teslim yalnız 31-N sentetik ve yetkisiz tek-yazıcı düzeneğini kanıtlar; gerçek SINGLE_WRITER_PROOF, üretim lease, runtime bağlantısı, gerçek veri veya cutover PASS değildir.';
const payloadPaths = Object.freeze([
  ...
  'docs/audit/31-N_SYNTHETIC_SINGLE_WRITER_PROOF_HARNESS.md',
  ...
  'config/31-n-synthetic-single-writer-proof-harness-scope.json',
  'artifacts/authority/31-N_SYNTHETIC_SINGLE_WRITER_PROOF_HARNESS_AUTHORITY.json',
  'artifacts/validation/31-N_PRIORITY_SELECTION_VALIDATION.json',
  'artifacts/checkpoints/31-N_EXECUTION_RECORD.json',
  'artifacts/checkpoints/31-N_INITIAL_VALIDATION_FAILURES.json',
  'artifacts/inventory/31-N_SCOPE_AND_STATUS_REPORT.json',
  'artifacts/validation/31-N_SYNTHETIC_SINGLE_WRITER_PROOF_HARNESS_CONTRACT.json',
  'artifacts/validation/31-N_ROOT_TYPESCRIPT.json',
  'artifacts/validation/31-N_TARGETED_VITEST.json',
  'artifacts/validation/31-N_CORE_SERVICE_RUNTIME.json',
  'artifacts/validation/31-N_SECURITY_AND_PREDECESSOR_REGRESSION.json',
  'artifacts/validation/31-N_FULL_VITEST_REGRESSION.json',
  'artifacts/validation/31-N_PRODUCTION_BUILD.json',
  ...
  'scripts/start-31-n-synthetic-single-writer-proof-harness.mjs',
  'scripts/verify-31-n-synthetic-single-writer-proof-harness-contract.mjs',
  'scripts/run-31-n-synthetic-single-writer-proof-harness-local-validation.mjs',
  'scripts/finalize-31-n-synthetic-single-writer-proof-harness-external-library-receipt.mjs',
...
assert(localPass || recoverable, '31-N is neither local PASS awaiting receipt nor a recoverable finalization');
assert(validations.every((item) => item.status === 'PASS'), '31-N validation is not clean PASS');
...
const manifest = {
  ...
  phase: 'SYNTHETIC_SINGLE_WRITER_PROOF_HARNESS_CHECKPOINT_PACKAGE',
  status: 'PASS',
...
const manifestName = '31-N_CHECKPOINT_MANIFEST.json';
...
const receipt = await writePair(paths.receipt, {
  schemaVersion: 1, release, step, primaryRequirement: 'DHA-001', phase: 'SYNTHETIC_SINGLE_WRITER_PROOF_HARNESS_EXTERNAL_LIBRARY_RECEIPT',
  ...
  openBoundaries: scopeConfigBefore.openBoundaries, nextOfficialStep: 'AUTO_PRIORITY_SELECTION_AFTER_31-N_PERSISTENT_RECEIPT',
...
const completion = {
  ...
  nextOfficialStep: 'AUTO_PRIORITY_SELECTION_AFTER_31-N_PERSISTENT_RECEIPT',
  currentAuthoritativeSourceExternalProtectionStatus: 'SEPARATE_GOV_005_REFRESH_REQUIRED_AFTER_SOURCE_CHANGE',
...
const step31N = plan.steps.find((item) => item.id === step);
Object.assign(step31N, {
...
for (const path of [paths.receipt, paths.readback, paths.receiptReadback, paths.persistence, paths.inventory, paths.completion, paths.transition]) {
  if (!step31N.localEvidence.includes(path)) step31N.localEvidence.push(path);
}
plan.updatedAt = completedAt;
plan.segmentationNote = '31-N synthetic single-writer proof harness is immutable COMPLETED/PASS with exact D: USB readback. It remains non-authoritative and detached; real SINGLE_WRITER_PROOF, production lease, DEC-171 cutover, real family-data transfer, and SQLite ownership transfer remain blocked.';
await writeJson(full(paths.plan), plan);
const ledger = await readJson(full(paths.ledger));
ledger.libraryUploadStatus = '31-N_COMPLETED_RECEIPT_PASS';
ledger.nextOfficialTask = 'AUTO_PRIORITY_SELECTION_AFTER_31-N_PERSISTENT_RECEIPT';
ledger.activeMicroStep = null;
ledger.externalLibraryAuthority31N = {
...
const checks = [
  ...
  ['work plan', step31N.status === 'COMPLETED' && step31N.persistentReceiptStatus === 'PASS'],
  ['governance', ledger.libraryUploadStatus === '31-N_COMPLETED_RECEIPT_PASS' && ledger.activeMicroStep === null],
  ['synthetic-only boundary', scopeConfig.targets.stateMachine === 'PURE_SYNTHETIC_SINGLE_WRITER_ONLY' && scopeConfig.targets.evidenceClassification === 'SYNTHETIC_NON_AUTHORITATIVE_REAL_GATE_FALSE'],
  ['production runtime and lease detached', scopeConfig.targets.runtimeIntegration === 'NOT_WIRED' && scopeConfig.openBoundaries.productionWriterLease === 'NOT_ATTACHED'],
...
assert(checks.every((item) => item.status === 'PASS'), '31-N transition failed');
...
console.log(`31-N external USB Library receipt: PASS (${expectedBase.length}/${expectedBase.length} base files; ${finalActual.length}/${finalExpected.length} final files).`);
console.log(`Library: ${libraryRoot}`);
console.log('31-N official step status: COMPLETED; synthetic-only harness: DETACHED/NON-AUTHORITATIVE; DEC-171 cutover: BLOCKED; new Build: false.');
