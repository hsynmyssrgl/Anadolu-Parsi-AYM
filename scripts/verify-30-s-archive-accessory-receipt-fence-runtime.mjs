...
const REPORT_PATH = 'artifacts/validation/30-S-archive-accessory-receipt-fence-runtime.json';
const VITEST_PATH = 'artifacts/validation/30-S-archive-accessory-receipt-fence-vitest.json';
...
const commands = [
  ...
  ['focused-and-production-regression-vitest', [
    'node_modules/vitest/vitest.mjs',
    'run',
    'apps/desktop/tests/archive-accessory-receipt-fence.test.ts',
...
check(vitest?.numTotalTests === 21 && vitest?.numPassedTests === 21 && vitest?.numFailedTests === 0, '30-S, 30-R and production regressions are 21/21 PASS');
...
const report = {
  ...
  step: '30-S',
  requirement: 'PPK-002',
  phase: 'ARCHIVE_ACCESSORY_AND_EVENT_ATTACHMENT_RECEIPT_FENCE_CONTROLLED_RUNTIME',
  status,
  controlledChecks: {
    expected: checks.length + 21,
    actual: checks.filter((item) => item.status === 'PASS').length + (vitest?.numPassedTests ?? 0),
    processAssertions: checks.length,
    focusedVitestExpected: 21,
  ...
  evidenceBoundary: {
    PPK002: 'PARTIAL',
    archiveCoreTableDirectSqlFence: 'TARGETED_PASS',
    archiveAccessoryTableReceiptFence: 'TARGETED_PASS',
...
if (status !== 'PASS') {
  console.error(`30-S archive accessory receipt-fence runtime: FAIL (${failures.length}/${checks.length} process assertions).`);
  console.error(failures.join('\n'));
  console.error(TRUTH);
  process.exit(1);
}
console.log(`30-S archive accessory receipt-fence runtime: PASS (${report.controlledChecks.actual}/${report.controlledChecks.expected} controlled checks; PPK-002 remains PARTIAL).`);
console.log(TRUTH);
