...
const definitions=[
  ...
  ['migration 99 source manifest and scope checksums are canonical',migration99?.name==='child_education_coordination'&&migration99?.checksum===migrationSha256&&migrationSha256===scope.validation?.migrationSha256&&migrationSha256==='38ecd03c71e734d1700f2ee44ece10e47935277e003e7e4ce21da7b6034db98e'],
  ['migration owns exact current item and immutable mutation tables',hasAll(migrations,['child_education_mutations','child_education_items','trg_33u_child_education_item_delete','trg_33u_child_education_mutation_delete'])],
  ...
  ['application tests cover replay private-space age fields update and delete',hasAll(appTest,['replays exact input','teen-owned private','under-13 child','enforces allowance','content-minimizes deletion'])],
  ['repository tests cover forged scope immutable ledger and bounded policy metadata',hasAll(repositoryTest,['rejects forged owner','guardian forging an adolescent-private','immutable physical ledgers','payload-free metadata'])],
  ['IPC tests reject renderer authority and unsafe result shapes',hasAll(ipcTest,['rejects renderer authority','rejects authority, AI, sharing','unknown channels','serializes durable writes'])],
  ['DataStore tests prove PEP fail closed explicit grant privacy and rollback',hasAll(dataStoreTest,['fails closed before child reads','upsertPermission','adolescent_private','atomically rolls back','event_outbox'])],
  ['UI test pins existing route stable retry age privacy and local truth',hasAll(uiTest,['existing Life route','stable retry identities','age-aware privacy','local-only boundaries'])],
  ...
  ['PPK ratchets are exact while requirement remains open',scope.validation?.ppk021?.scannedProductionFiles===555&&scope.validation?.ppk021?.exactPrivilegedSurfaceCount===873&&scope.validation?.ppk021?.exactAllowlistSha256==='843cb93dce2402bbaeb3d44b5538b88a3a55f4832436ad23aaf61937bc8c99dc'&&scope.validation?.ppk022?.scannedProductionFiles===555&&scope.validation?.ppk022?.exactCapabilitySurfaceCount===392&&scope.validation?.ppk022?.exactCapabilityManifestSha256==='cb879c739cb8ef3a2e92d1f0e451cd21ba7e9d4b0fcd519f343cddd725c9745c'&&scope.validation?.countsAsRequirementPass===false&&inventory.validation?.countsAsRequirementPass===false]
];
