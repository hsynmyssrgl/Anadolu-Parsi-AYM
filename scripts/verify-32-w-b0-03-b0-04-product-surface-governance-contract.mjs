...
export const verifyB0ProductSurfaceGovernanceContract = async (root = process.cwd()) => {
  ...
  check('renderer derives menu and consumes governance state', includesAll(app, ['PRODUCT_NAVIGATION_ROUTES.map', 'PRODUCT_NAVIGATION_GROUPS.map', 'getProductSurfaceGovernance().then(setProductSurfaceGovernance)', 'B0-03 / B0-04 · ürün yüzeyi gerçeklik kapısı']));
  check('renderer has no duplicate literal route or menu registry', !/type ScreenId\s*=\s*\n\s*\|/u.test(app) && !/const navItems[^=]*=\s*\[/u.test(app));
  check('targeted tests cover success and fail-closed mutation paths', includesAll(applicationTest, ['17 product + 5 governance = 22', 'fails closed when route', 'fails closed when an unused renderer API']) && includesAll(integrationTest, ['shared domain navigation contract', 'zero-argument governance IPC', 'historical 14 API set']));
