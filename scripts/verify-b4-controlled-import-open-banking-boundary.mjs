...
export const verifyB4ControlledImportOpenBankingBoundary = async () => {
  ...
  check('PPK-021 preserves the historical shared composition under the current exact 699 and 337 successor ratchet',
    astKeys.has('USE_CASE_COMPOSITION|apps/desktop/src/main/data-store.ts|CommitFinanceImportBatchUseCase')
    && astGate.status === 'PASS' && astGate.privilegedSurfaces === 699
    && astGate.exactAllowlistEntries === 699 && astGate.surfaceCounts?.USE_CASE_COMPOSITION === 337
