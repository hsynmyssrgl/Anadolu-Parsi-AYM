...
if (cliArguments.length > 1 || cliArguments.some((argument) => argument !== '--no-write')) {
  throw new Error('Unsupported B4 controlled import open banking boundary argument.');
}
...
export const verifyB4ControlledImportOpenBankingBoundary = async () => {
  ...
  check('PPK-021 preserves the historical shared composition under the current exact 897 and 435 successor ratchet',
    astKeys.has('USE_CASE_COMPOSITION|apps/desktop/src/main/data-store.ts|CommitFinanceImportBatchUseCase')
    && astGate.status === 'PASS' && astGate.privilegedSurfaces === 897
    && astGate.exactAllowlistEntries === 897 && astGate.surfaceCounts?.USE_CASE_COMPOSITION === 435
    && astGate.directRoleAuthorizationBypasses === 0 && astGate.findings.length === 0);
  check('PPK-022 preserves historical bounded-read surfaces under the current exact successor ratchet', capabilityGate.status === 'PASS'
    && capabilityGate.capabilitySurfaces > 0 && capabilityGate.capabilitySurfaces === capabilityGate.exactManifestSurfaces
