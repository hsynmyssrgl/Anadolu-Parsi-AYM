...
const successor31DCompleted = plan.currentStep === '31-D' && active.length === 0 && step31D?.status === 'COMPLETED' && step31D.persistentReceiptStatus === 'PASS';
...
check(receipt.status === 'PASS' && receipt.officialStepStatus === 'COMPLETED' && receipt.persistentReceiptStatus === 'PASS', '31-C receipt is official PASS');
check(receipt.libraryPath === libraryRoot && receipt.storageBackend === 'EXTERNAL_USB_D_DRIVE', '31-C receipt binds D: USB Library');
check(receipt.PPK002 === 'PARTIAL' && receipt.requirementCompletionClaimed === false && receipt.newBuildIssued === false, '31-C receipt preserves scope and Build boundaries');
check(receipt.openBoundaries.importedEventLocationReadReceiptChain === 'NEXT_SEPARATE_SLICE_FAIL_CLOSED', 'location-linked imported events remain next fail-closed slice');
check(receipt.openBoundaries.governedImportRollbackReceiptFence === 'NOT_COMPLETE', 'governed import rollback remains open');
check(readback.status === 'PASS' && readback.failed === 0 && readback.matched === readback.expected, '31-C base package readback is PASS');
check(receiptReadback.status === 'PASS' && receiptReadback.matched === 4 && receiptReadback.failed === 0, '31-C receipt readback is 4/4 PASS');
check(persistence.status === 'PASS' && persistence.matched === 2 && persistence.failed === 0, '31-C receipt persistence is 2/2 PASS');
check(inventory.status === 'PASS' && inventory.actualFilesBeforeInventory === inventory.expectedFilesBeforeInventory, '31-C final inventory is exact');
check(completion.status === 'PASS' && completion.officialStepStatus === 'COMPLETED' && completion.persistentReceiptStatus === 'PASS', '31-C completion record is PASS');
check(transition.status === 'PASS' && transition.failed === 0 && transition.passed === transition.expected, '31-C completion transition is PASS');
check(execution.officialStepStatus === 'COMPLETED' && execution.officialCompletionClaimed === true, '31-C execution record is completed');
check(scope.officialStepStatus === 'COMPLETED' && scope.PPK002 === 'PARTIAL', '31-C scope report is completed with PPK-002 partial');
check(step31C?.status === 'COMPLETED' && step31C.persistentReceiptStatus === 'PASS' && (active.length === 0 || laterSuccessor.planValid), 'work plan preserves completed 31-C through an authorized active successor');
check((['31-C_COMPLETED_RECEIPT_PASS', '31-D_COMPLETED_RECEIPT_PASS', '31-E_COMPLETED_RECEIPT_PASS', '31-F_COMPLETED_RECEIPT_PASS'].includes(ledger.libraryUploadStatus) && ledger.activeMicroStep === null) || laterSuccessor.ledgerValid, 'ledger records completed 31-C receipt chain');
check(laterSuccessor.nextTaskValid || ledger.nextOfficialTask === (successor31FCompleted ? 'AUTO_PRIORITY_SELECTION_AFTER_31-F_PERSISTENT_RECEIPT' : successor31ECompleted ? 'AUTO_PRIORITY_SELECTION_AFTER_31-E_PERSISTENT_RECEIPT' : successor31DCompleted ? 'AUTO_PRIORITY_SELECTION_AFTER_31-D_PERSISTENT_RECEIPT' : 'AUTO_PRIORITY_SELECTION_AFTER_31-C_PERSISTENT_RECEIPT'), 'ledger requires next priority selection');
...
if (verifyExternal) {
  ...
  try {
    ...
    check(false, 'D: 31-C Library is readable', String(error));
  }
}
const failures = checks.filter((item) => item.status === 'FAIL');
if (failures.length) {
  console.error(`31-C completion transition: FAIL (${failures.length}/${checks.length}).`);
  for (const item of failures) console.error(`- ${item.name}${item.detail ? `: ${item.detail}` : ''}`);
  process.exit(1);
}
console.log(`31-C completion transition: PASS (${checks.length}/${checks.length}${verifyExternal ? '; D: external readback included' : '; durable local evidence'}).`);
console.log('31-C: COMPLETED; persistent receipt: PASS; PPK-002: PARTIAL; new Build: false.');
